...
namespace WebVideoService.WebVideo
{
    ...
    public class WebVideoPlayer
    {
        public string HtmlPage { get; set; } =
            "Unknown player type or WebVideoPlayer received invalid parameters.";

        public string[][] HeadersToSet { get; set; } = [];

        public WebVideoPlayer(NameValueCollection Parameters, string VideoUrl)
        {
            foreach (var Par in Parameters.AllKeys)
            {
                if (!string.IsNullOrEmpty(Par) && Par != "type")
                    VideoUrl += Par + "=" + HttpUtility.UrlEncode(Parameters[Par]) + "&";
            }
            ...
            var SampleUrl = Parameters["url"] ?? "https://www.youtube.com/watch?v=XXXXXXX";
            var PreferPage = "intro";
            if (Parameters["gui"] == "1")
            {
                Parameters["type"] = null;
                if (Parameters["prefer"] != null)
                    PreferPage = Parameters["prefer"] + "&url=" + SampleUrl;
            }
            ...
            switch (Parameters["type"])
            {
                ...
                case "":
                    HtmlPage =
                        ...
                        + "                <option value='vorbis -strict -2'>Ogg Vorbis</option>"
                        ...
                        + "</form>"
                        + ""
                        + "<iframe name='player_frame' src='/!player/?type="
                        + PreferPage
                        + "' "
                ...
                case "embed":
                    ...
                    HtmlPage =
                        ...
                        + "src='"
                        + VideoUrl
                        + "' autostart='true' style='width: 100%; height: 100%;' />";
                    break;
                case "embedwm":
                ...
                case "embedvlc":
                    ...
                    HtmlPage =
                        "<embed id='MediaPlayer' type='application/x-vlc-plugin'"
                ...
                case "objectns":
                    ...
                    HtmlPage =
                        "<center><object ID='MediaPlayer' style='width: 100%; height: 100%;' "
                ...
                case "objectwm":
                    ...
                    HtmlPage =
                        "<object ID='MediaPlayer' style='width: 100%; height: 100%;' "
                ...
                case "html5":
                    // HTML5 VIDEO tag
                    HtmlPage =
                        "<center><video id='MediaPlayer' src='"
                        + VideoUrl
                        + "' controls='yes' autoplay='yes' style='width: 100%; height: 100%;'>"
                ...
                case "dynimg":
                    ...
                    HtmlPage =
                        "<center><IMG DYNSRC='"
                        + VideoUrl
                        + "' CONTROLS START='FILEOPEN' SRC='"
                        + "http://www.linuxtopia.org/HowToGuides/HTML_tutorials/graphics/moonflag.gif"
                        + "'></center>";
                    break;
                case "link":
                    // Link only
                    HtmlPage =
                        "<center><big><a href='"
                        + VideoUrl
                        + "'>Download the video</a></big></center>";
                    if (Parameters["f"] == null)
                        HtmlPage +=
                            "<center><p>Or select format, codecs and convert the video online.</p></center>";
                    break;
                case "file":
                    // Redirect to file only
                    HtmlPage =
                        "<center>Please wait up to 30 sec.<br>If nothing appear, <a href='"
                        + VideoUrl
                        + "'>click here</a> to download manually.</center>";
                    HeadersToSet =
                    [
                        ["Refresh", "0; url=" + VideoUrl],
                    ];
                    break;
            }
        }

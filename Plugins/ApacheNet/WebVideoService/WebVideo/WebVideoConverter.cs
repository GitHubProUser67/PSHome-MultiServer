...
namespace WebVideoService.WebVideo
{
    /// <summary>
    /// Web video dowloader & converter
    /// (Windows only)
    /// </summary>
    public partial class WebVideoConverter
    {
        /// <summary>
        /// Download and convert an online video (from hostings like YouTube, VK, etc)
        /// </summary>
        public static WebVideo ConvertVideo(
        ...
        )
        {
            ...
            {
                var UseFFmpeg = true;
                var GetYoutubeJson = false;
                var YoutubeDlArgs = string.Empty;
                var FFmpegArgs = string.Empty;

                // Check options
                if (!Arguments.TryGetValue("url", out var url))
                    throw new InvalidOperationException(
                        "[WebVideoConverter] - Internet video address is missing."
                    );

                // Configure output file type
                string PreferredMIME = "application/octet-stream",
                    PreferredName = "video.avi";
                if (Arguments.TryGetValue("f", out var argument)) // (ffmpeg output format)
                {
                    switch (argument)
                    {
                        case "avi":
                ...
                {
                    if (
                        (Arg.Key.StartsWith("vf") && Arguments["vcodec"] != "copy")
                    ...
                    switch (Arg.Key.ToLowerInvariant())
                    {
                        case "enable":
                            if (!ToBoolean(Arg.Value))
                                throw new Exception(
                                    "[WebVideoConverter] - This feature is disabled by administrator."
                                );
                            continue;
                        case "url":
                        case "content-type":
                        ...
                        default:
                            LoggerAccessor.LogWarn(
                                "[WebVideoConverter] - Unsupported argument: {0}",
                                Arg.Key
                            );
                            continue;
                    }
                ...
                var YoutubeDlStart = new ProcessStartInfo()
                {
                    FileName =
                        $"{Path.Combine(ConverterDir, "yt-dlp")}/yt-dlp{(RuntimeInformation.OSArchitecture == Architecture.Arm64 ? "_arm64" : string.Empty)}",
                ...
                {
                    FileName = $"{Path.Combine(ConverterDir, "ffmpeg")}/ffmpeg",
                ...
                {
                    LoggerAccessor.LogWarn(
                        "[WebVideoConverter] - Video convert: {0} {1} | {2} {3}",
                        YoutubeDlStart.FileName,
                        YoutubeDlStart.Arguments,
                        FFmpegStart.FileName,
                ...
                {
                    LoggerAccessor.LogWarn(
                        "[WebVideoConverter] - Video convert: {0} {1}",
                        YoutubeDlStart.FileName,
                        YoutubeDlStart.Arguments
                    );
                    YoutubeDl = Process.Start(YoutubeDlStart);
                    ...
                    YoutubeDl.ErrorDataReceived += (o, e) =>
                    {
                        if (e.Data != null)
                        {
                            ...
                            if (e.Data.StartsWith("ERROR:"))
                            {
                                ...
                                video.ErrorMessage = string.Concat(
                                    "Online video failed to download: ",
                                    e.Data.AsSpan(7)
                                );
                                LoggerAccessor.LogError(
                                    "[WebVideoConverter] - yt-dlp: {0}",
                                    e.Data
                                );
                            }
                            if (e.Data.StartsWith("WARNING:"))
                                LoggerAccessor.LogWarn("[WebVideoConverter] - yt-dlp: {0}", e.Data);
                            if (MyRegex().IsMatch(e.Data))
                            {
                                var match = MyRegex().Match(e.Data);
                                //assuming, it's succcessfull & have 2 groups
                                EndTime = DateTime.Now.Add(
                                    TimeSpan.Parse(
                                        Regex.IsMatch(match.Groups[1].Value, @"\d\d:\d\d:\d\d")
                                            ? match.Groups[1].Value
                                            : "00:" + match.Groups[1].Value
                                    )
                                );
                            }
                        }
                ...
                if (!GetYoutubeJson)
                {
                    if (UseFFmpeg && YoutubeDl != null && FFmpeg != null)
                    {
                        ...
                        new Task(() =>
                        {
                            YoutubeDl.StandardOutput.BaseStream.CopyTo(
                ...
                new Task(() =>
                {
                    ...
                    {
                        Thread.Sleep(1000);
                        ProcessUtils.PreventProcessIdle(ref YoutubeDl, ref YoutubeDlCpuLoad);
                    }
                }).Start();
                if (UseFFmpeg)
                    new Task(() =>
                    {
                        ...
                        {
                            Thread.Sleep(1000);
                            ProcessUtils.PreventProcessIdle(ref FFmpeg, ref FFmpegCpuLoad);
                        }
                    }).Start();
            ...
            catch (Exception VidCvtError)
            {
                ...
                LoggerAccessor.LogError(
                    "[WebVideoConverter] - Cannot convert video: {0} - {1}",
                    VidCvtError.GetType(),
                    VidCvtError.Message
                );
            }
        ...
        private static bool ToBoolean(string s)
        {
            ...
            throw new InvalidCastException(
                "[WebVideoConverter] - Only the following are supported for converting strings to boolean: "
                    + string.Join(",", trueStrings)
                    + " and "
                    + string.Join(",", falseStrings).Replace(",,", string.Empty)
            ); //hide empty & null
        }

        [GeneratedRegex(@"\[download\].*ETA (\d\d:\d\d:\d\d|\d\d:\d\d)")]

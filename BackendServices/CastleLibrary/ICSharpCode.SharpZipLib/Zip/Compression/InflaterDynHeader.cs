...
namespace CastleLibrary.ICSharpCode.SharpZipLib.Zip.Compression
{
    internal class InflaterDynHeader
    {
        ...
        private IEnumerable<bool> CreateStateMachine()
        {
            ...
            while (index < dataCodeCount)
            {
                ...
                else
                {
                    ...
                    {
                        if (index == 0)
                            throw new StreamDecodingException(
                                "[InflaterDynHeader] - Cannot repeat previous code length when no other code length has been read"
                            );
                    ...
                    if (index + repeatCount > dataCodeCount)
                        throw new StreamDecodingException(
                            "[InflaterDynHeader] - Cannot repeat code lengths past total number of data code lengths"
                        );
            ...
            if (codeLengths[256] == 0)
                throw new StreamDecodingException(
                    "[InflaterDynHeader] - Inflater dynamic header end-of-block code missing"
                );

            litLenTree = new InflaterHuffmanTree(
                new ArraySegment<byte>(codeLengths, 0, litLenCodeCount)
            );
            distTree = new InflaterHuffmanTree(
                new ArraySegment<byte>(codeLengths, litLenCodeCount, distanceCodeCount)
            );
        ...
        public InflaterHuffmanTree LiteralLengthTree =>
            litLenTree
            ?? throw new StreamDecodingException(
                "[InflaterDynHeader] - Header properties were accessed before header had been successfully read"
            );

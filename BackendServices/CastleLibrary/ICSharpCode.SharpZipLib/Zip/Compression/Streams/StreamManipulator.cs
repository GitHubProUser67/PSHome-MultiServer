namespace CastleLibrary.ICSharpCode.SharpZipLib.Zip.Compression.Streams
{
    /// <summary>
    ...
    public class StreamManipulator
    {
        ...
        public int PeekBits(int bitCount)
        {
            if (bitsInBuffer_ < bitCount)
            {
                if (windowStart_ == windowEnd_)
                    return -1; // ok
                buffer_ |= (uint)(
                    (window_[windowStart_++] & 0xff | (window_[windowStart_++] & 0xff) << 8)
                    << bitsInBuffer_
                );
                bitsInBuffer_ += 16;
            }
        ...
        public int CopyBytes(byte[] output, int offset, int length)
        {
            ...
            if ((bitsInBuffer_ & 7) != 0)
                ...
                throw new InvalidOperationException("[StreamManipulator] - Bit buffer is not byte aligned!");
        ...
        public void SetInput(byte[] buffer, int offset, int count)
        {
            ...
            if (offset < 0)
                throw new ArgumentOutOfRangeException(nameof(offset), "[StreamManipulator] - Cannot be negative");

            else if (count < 0)
                throw new ArgumentOutOfRangeException(nameof(count), "[StreamManipulator] - Cannot be negative");

            else if (windowStart_ < windowEnd_)
                throw new InvalidOperationException("[StreamManipulator] - Old input was not completely processed");

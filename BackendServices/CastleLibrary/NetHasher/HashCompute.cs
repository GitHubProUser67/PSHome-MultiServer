    ...
    public class HashCompute
    {
        ...
        public static byte[] ComputeObject(object inData, string hashName, byte[] HMACKey = null)
        {
            if (inData is byte[] bytes)
                ...
                return ComputeHash([b], hashName, HMACKey);
            else if (inData is short s)
                return ComputeHash(
                    BitConverter.GetBytes(
                        !EndianAwareConverter.isLittleEndianSystem ? EndianUtils.ReverseShort(s) : s
                    ),
                    hashName,
                    HMACKey
                );
                ...
                return ComputeHash(
                    BitConverter.GetBytes(
                        !EndianAwareConverter.isLittleEndianSystem ? EndianUtils.ReverseChar(c) : c
                    ),
                    hashName,
                    HMACKey
                );
                ...
                return ComputeHash(
                    BitConverter.GetBytes(
                        !EndianAwareConverter.isLittleEndianSystem ? EndianUtils.ReverseInt(i) : i
                    ),
                    hashName,
                    HMACKey
                );
                ...
                return ComputeHash(
                    BitConverter.GetBytes(
                        !EndianAwareConverter.isLittleEndianSystem ? EndianUtils.ReverseLong(l) : l
                    ),
                    hashName,
                    HMACKey
                );
                ...
                return ComputeHash(
                    BitConverter.GetBytes(
                        !EndianAwareConverter.isLittleEndianSystem ? EndianUtils.ReverseFloat(f) : f
                    ),
                    hashName,
                    HMACKey
                );
            ...
            throw new ArgumentException(
                $"[HashCompute] - ComputeObject - Unsupported data type: {inData.GetType()}",
                nameof(inData)
            );
        }
                ...
                else
                {
                    HMAC hmac = hashName switch
                    {
                        ...
                        _ => throw new ArgumentException(
                            $"[HashCompute] - ComputeHash - Unknown HMAC algorithm: {hashName}"
                        ),
                    };
        ...
        private static byte[] ComputeWithHashAlgorithm(object data, string hashName)
        {
            using (var algorithm = HashAlgorithm.Create(hashName))
            {
                if (algorithm == null)
                    throw new ArgumentException(
                        $"[HashCompute] - ComputeWithHashAlgorithm - Unknown hash algorithm: {hashName}"
                    );
        ...
        private static byte[] ComputeSha1Hash(byte[] data)
        {
            try
            {
                ...
                return tpm.Hash(data, TpmAlgId.Sha1, TpmRh.Owner, out _);
            }
            catch
            {
        ...
        private static byte[] ComputeSha224Hash(byte[] data)
        {
            ...
            var hashBuf = new byte[digest.GetDigestSize()];
        ...
        private static byte[] ComputeSha224Hash(Stream data)
        {
            var digest = new Sha224Digest();
            int bytesRead;
            var buffer = new byte[Sha224StreamBufferSize];
        ...
        {
            var hmac = new HMac(new Sha224Digest());
            hmac.Init(new KeyParameter(key));
            hmac.BlockUpdate(data, 0, data.Length);
            var hashBuf = new byte[hmac.GetMacSize()];
        ...
        private static byte[] ComputeSha256Hash(byte[] data)
        {
            try
            {
                ...
                return tpm.Hash(data, TpmAlgId.Sha256, TpmRh.Owner, out _);
            }
            catch
            {
        ...
        private static byte[] ComputeSha384Hash(byte[] data)
        {
            try
            {
                ...
                return tpm.Hash(data, TpmAlgId.Sha384, TpmRh.Owner, out _);
            }
            catch
            {
        ...
        private static byte[] ComputeSha512Hash(byte[] data)
        {
            try
            {
                ...
                return tpm.Hash(data, TpmAlgId.Sha512, TpmRh.Owner, out _);
            }
            catch
            {
        ...
        {
            var result = new byte[values.Length * sizeof(long)];
            Buffer.BlockCopy(values, 0, result, 0, result.Length);
            return result;
        }

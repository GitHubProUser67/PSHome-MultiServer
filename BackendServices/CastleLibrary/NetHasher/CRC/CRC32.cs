...
namespace CastleLibrary.NetHasher.CRC
{
    ...
    {
        private const ushort _crcArraySize = 256;

        public static uint[] TABLE { get; } = GenerateHldcCRCTableSliced();

        private static readonly uint[] _castagnoli_hash = new uint[_crcArraySize]
        {
            0x00000000,
            0xF26B8303,
            0xE13B70F7,
        ...
        private static uint[] GenerateHldcCRCTableSliced()
        {
            uint[] final_table = new uint[_crcArraySize * 4],
                iso_hldc_hash =
                    new uint[_crcArraySize]
                    {
                        0x00000000,
                        0x77073096,
                        0xEE0E612C,
            ...
            Array.Copy(iso_hldc_hash, 0, final_table, 0, iso_hldc_hash.Length);

            for (uint i = _crcArraySize; i < final_table.Length; i++)
            {
                var r = final_table[i - _crcArraySize];
                final_table[i] = final_table[r & byte.MaxValue] ^ (r >> 8);
            }
        ...
        public static uint Create(byte[] data, int offset, int length)
        {
            ...
            for (var i = offset; i < length; i++)
                crc = (crc >> 8) ^ TABLE[(crc ^ data[i]) & byte.MaxValue];
        ...
        public static uint CreateCastagnoli(byte[] data, int offset, int length)
        {
            ...
            for (var i = offset; length > 0; i++, length--)
                crc = (crc >> 8) ^ _castagnoli_hash[(byte)crc ^ data[i]];

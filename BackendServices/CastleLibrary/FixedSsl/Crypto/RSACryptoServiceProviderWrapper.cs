...
namespace CastleLibrary.FixedSsl.Crypto
{
    ...
    public class RSACryptoServiceProviderWrapper
    {
        ...
        public byte[] Encrypt(byte[] data, bool useOaepPadding)
        {
            ...
            return _rsa.Encrypt(
                data,
                useOaepPadding ? RSAEncryptionPadding.OaepSHA1 : RSAEncryptionPadding.Pkcs1
            );
        }
        ...
        {
            return _rsaCsp != null
                ? _rsaCsp.Decrypt(data, useOaepPadding)
                : _rsa.Decrypt(
                    data,
                    useOaepPadding ? RSAEncryptionPadding.OaepSHA1 : RSAEncryptionPadding.Pkcs1
                );
        ...
        {
            return padding != RSASignaturePadding.Pkcs1
                    ? throw new NotSupportedException(
                        "[RSACryptoServiceProviderWrapper] - RSACryptoServiceProvider only supports PKCS#1 padding."
                    )
                : _rsaCsp != null ? _rsaCsp.SignData(data, hashAlgorithm.Name)
                : _rsa.SignData(data, hashAlgorithm, padding);
        }
        ...
        )
        {
            return padding != RSASignaturePadding.Pkcs1
                ...
                : _rsaCsp != null ? _rsaCsp.VerifyData(data, hashAlgorithm.Name, signature)
                : _rsa.VerifyData(data, signature, hashAlgorithm, padding);
        }
    }
}

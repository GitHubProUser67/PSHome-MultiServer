using CastleLibrary.SharpCompress.LZMA.LZ;
using CastleLibrary.SharpCompress.LZMA.RangeCoder;

namespace CastleLibrary.SharpCompress.LZMA
{
    internal class Encoder : ICoder, ISetCoderProperties, IWriteCoderProperties
    {
            ...
            public void SetPrices(uint posState, uint numSymbols, uint[] prices, uint st)
            {
                ...
                for (; i < Base.K_NUM_LOW_LEN_SYMBOLS + Base.K_NUM_MID_LEN_SYMBOLS; i++)
                {
                    if (i >= numSymbols)
                        return;
                    prices[st + i] =
                        b0 + _midCoder[posState].GetPrice(i - Base.K_NUM_LOW_LEN_SYMBOLS);
                }
                for (; i < numSymbols; i++)
                    prices[st + i] =
                        b1
        ...
        {
            return _repMatchLenEncoder.GetPrice(len - Base.K_MATCH_MIN_LEN, posState)
        ...
        private uint GetOptimum(uint position, out uint backRes)
        {
            ...
            var len = lenEnd;
            do _optimum[len--]._price = K_IFINITY_PRICE;
            while (len >= 2);
            ...
            while (true)
            {
                ...
                var curAnd1Price =
                    curPrice
                    + _isMatch[(state._index << Base.K_NUM_POS_STATES_BITS_MAX) + posState]
                        .GetPrice0()
                    + _literalEncoder
                ...
                {
                    var lenTest = _matchFinder.GetMatchLen(
                        0 - 1,
                        _reps[repIndex],
                        numAvailableBytes
                    );
        ...
        )
        {
            ...
            try
            {
                ...
                {
                    CodeOneBlock(
                        out var processedInSize,
                        out var processedOutSize,
                        out var finished
                    );
                    if (finished)
                        return;
        ...
        private void FillDistancesPrices()
        {
            ...
            for (
                uint lenToPosState = 0;
                lenToPosState < Base.K_NUM_LEN_TO_POS_STATES;
                lenToPosState++
            )
            {
                uint posSlot;
            ...
            for (var i = 0; i < properties.Length; i++)
            {
                ...
                switch (propIDs[i])
                {
                    ...
                    case CoderPropId.DictionarySize:
                    {
                        ...
                        for (
                            dicLogSize = 0;
                            dicLogSize < (uint)kDicLogSizeMaxCompress;
                            dicLogSize++
                        )

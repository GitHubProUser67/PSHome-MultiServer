...
namespace CastleLibrary.SharpCompress.LZMA.LZ
{
    internal partial class OutWindow : IAsyncDisposable
    {
        ...
        )
        {
            ...
            var fastSize = Math.Min(
                Math.Min(Math.Min(_windowSize - _pos, _windowSize - pos), Math.Abs(pos - _pos)),
                HasSpace ? (int)Math.Min(rem, _limit - _total) : 0
            );
            if (fastSize >= 2)
            {

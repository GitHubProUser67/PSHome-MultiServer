...
namespace CastleLibrary.S0ny.SSFW
{
    public static class GuidGenerator
    {
        private static readonly byte[] _authIV = new byte[]
        {
            0x30,
            0x57,
            0xB5,
        ...
        public static string SSFWGenerateGuid(string input1, string input2, string key = null)
        {
            ...
            var input1Bytes = Encoding.UTF8.GetBytes(input1 + "**H0mEIsG3reAT!!!!!!!!!!!!!!");
            var input2Bytes = Encoding.UTF8.GetBytes("C0MeBaCKHOm3*!*!*!*!*!*!*!*!" + input2);
            ...
            if (!string.IsNullOrEmpty(key))
            {
                ...
                if (Convert.TryFromBase64String(key, buffer, out var bytesWritten))
                {
                    var keyBytes = buffer[..bytesWritten].ToArray();
                    md5hash = DotNetHasher.ComputeMD5String(
                        InitiateCBCEncryptBufferTobase64String(
                            JsonSerializer.Serialize(input1Bytes),
                            keyBytes,
                            _authIV
                        )
                    );
                    sha512hash = DotNetHasher.ComputeSHA512String(
                        InitiateCBCEncryptBufferTobase64String(
                            JsonSerializer.Serialize(input2Bytes),
                            keyBytes,
                            _authIV
                        )
                    );
            ...
            return (
                md5hash.Substring(1, 8)
                + "-"
                + sha512hash.Substring(2, 4)
                + "-"
                + md5hash.Substring(10, 4)
                + "-"
                + sha512hash.Substring(16, 4)
                + "-"
                + sha512hash.Substring(19, 12)
            ).ToLower();
        }

        private static byte[] InitiateCBCEncryptBufferTobase64String(
            string FileString,
            byte[] KeyBytes,
            byte[] m_iv
        )
        {
            ...
            CustomLogger.LoggerAccessor.LogError(
                "[GuidGenerator] - InitiateCBCEncryptBufferTobase64String - Invalid KeyBytes or IV!"
            );
        ...
        )
        {
            ...
            CustomLogger.LoggerAccessor.LogError(
                "[GuidGenerator] - InitiateCBCEncryptBuffer - Invalid KeyBytes or IV!"
            );

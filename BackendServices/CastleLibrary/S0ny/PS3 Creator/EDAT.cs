using System.Numerics;
using CastleLibrary.Utils;
using CustomLogger;
using EndianTools;

namespace CastleLibrary.S0ny.PS3_Creator
{
    public class EDAT
    {
        private const sbyte STATUS_ERROR_OUPUTFILE_IO = -101;
        private const sbyte STATUS_ERROR_INPUTFILE_IO = -100;
        private const sbyte STATUS_ERROR_HASHTITLEIDNAME = -1;
        private const sbyte STATUS_ERROR_HASHDEVKLIC = -2;
        private const sbyte STATUS_ERROR_MISSINGKEY = -3;
        private const sbyte STATUS_ERROR_HEADERCHECK = -4;
        private const sbyte STATUS_ERROR_DECRYPTING = -5;
        private const sbyte STATUS_ERROR_INCORRECT_FLAGS = -6;
        private const sbyte STATUS_ERROR_INCORRECT_VERSION = -7;
        private const byte STATUS_OK = 0;
        private const long FLAG_COMPRESSED = 0x00000001L;
        private const long FLAG_0x02 = 0x00000002L;
        private const long FLAG_KEYENCRYPTED = 0x00000008L;
        private const long FLAG_0x10 = 0x00000010L;
        private const long FLAG_0x20 = 0x00000020L;
        private const long FLAG_SDAT = 0x01000000L;
        private const long FLAG_DEBUG = 0x80000000L;

        private const int HEADER_MAX_BLOCKSIZE = 0x3C00;

        // From: https://github.com/ErikPshat/data_makenp/blob/master/Windows/src/make_npdata.cpp
        private static readonly byte[] edat_footer_v1 = new byte[0x10]
        {
            0x45,
            0x44,
            0x41,
            0x54,
            0x41,
            0x20,
            0x70,
            0x61,
            0x63,
            0x6B,
            0x61,
            0x67,
            0x65,
            0x72,
            0x00,
            0x00,
        };
        private static readonly byte[] edat_footer_v2 = new byte[0x10]
        {
            0x45,
            0x44,
            0x41,
            0x54,
            0x41,
            0x20,
            0x32,
            0x2E,
            0x34,
            0x2E,
            0x30,
            0x2E,
            0x57,
            0x00,
            0x00,
            0x00,
        };
        private static readonly byte[] edat_footer_v3 = new byte[0x10]
        {
            0x45,
            0x44,
            0x41,
            0x54,
            0x41,
            0x20,
            0x33,
            0x2E,
            0x33,
            0x2E,
            0x30,
            0x2E,
            0x57,
            0x00,
            0x00,
            0x00,
        };
        private static readonly byte[] edat_footer_v4 = new byte[0x10]
        {
            0x45,
            0x44,
            0x41,
            0x54,
            0x41,
            0x20,
            0x34,
            0x2E,
            0x30,
            0x2E,
            0x30,
            0x2E,
            0x57,
            0x00,
            0x00,
            0x00,
        };
        private static readonly byte[] sdat_footer_v1 = new byte[0x10]
        {
            0x53,
            0x44,
            0x41,
            0x54,
            0x41,
            0x20,
            0x70,
            0x61,
            0x63,
            0x6B,
            0x61,
            0x67,
            0x65,
            0x72,
            0x00,
            0x00,
        };
        private static readonly byte[] sdat_footer_v2 = new byte[0x10]
        {
            0x53,
            0x44,
            0x41,
            0x54,
            0x41,
            0x20,
            0x32,
            0x2E,
            0x34,
            0x2E,
            0x30,
            0x2E,
            0x57,
            0x00,
            0x00,
            0x00,
        };
        private static readonly byte[] sdat_footer_v3 = new byte[0x10]
        {
            0x53,
            0x44,
            0x41,
            0x54,
            0x41,
            0x20,
            0x33,
            0x2E,
            0x33,
            0x2E,
            0x30,
            0x2E,
            0x57,
            0x00,
            0x00,
            0x00,
        };
        private static readonly byte[] sdat_footer_v4 = new byte[0x10]
        {
            0x53,
            0x44,
            0x41,
            0x54,
            0x41,
            0x20,
            0x34,
            0x2E,
            0x30,
            0x2E,
            0x30,
            0x2E,
            0x57,
            0x00,
            0x00,
            0x00,
        };

        /* KDSBEST START */
        public int EncryptFile(
            String inFile,
            String outFile,
            byte[] devKLic,
            byte[] keyFromRif,
            byte[] contentID,
            byte[] flags,
            byte[] type,
            byte[] version
        )
        {
            if (string.IsNullOrEmpty(inFile))
                return STATUS_ERROR_INPUTFILE_IO;
            if (string.IsNullOrEmpty(outFile))
                return STATUS_ERROR_OUPUTFILE_IO;

            var fin = File.Open(inFile, FileMode.Open);
            //  MemoryMappedFile fin1 = MemoryMappedFile.CreateFromFile(inFile, FileMode.Open);
            var ptr = new NPD[1]; //Ptr to Ptr
            var o = File.Open(outFile, FileMode.Create);
            var fn = o.Name.Split('\\');
            var npd = WriteValidNPD(
                fn[fn.Length - 1],
                devKLic,
                ptr,
                fin,
                contentID,
                flags,
                version,
                type
            );
            o.Write(npd, 0, npd.Length);
            // FLAGS
            var buffer = new byte[4] { 1, 0, 0, 60 };
            o.Write(buffer, 0, 4);

            // blocksize 0x00004000
            buffer[0] = 0;
            buffer[1] = 0;
            buffer[2] = 64;
            buffer[3] = 0;
            o.Write(buffer, 0, 4);
            var finlen = fin.Length;
            var lenBuf = BitConverter.GetBytes(finlen);
            if (!EndianTools.EndianAwareConverter.isLittleEndianSystem)
                Array.Reverse(lenBuf);
            var rLenBuf = new byte[8];
            for (var i = 0; i < 8; i++)
                rLenBuf[i] = 0x00;
            for (var i = 0; i < lenBuf.Length; i++)
                rLenBuf[7 - i] = lenBuf[i];
            o.Write(rLenBuf, 0, 8);

            // Fill the rest 0x10 bytes with dummy we generate the metasection hash later!
            // the bytes till 0x100 are unknown
            buffer[0] = 0x00;
            while (o.Length < 0x100)
                o.Write(buffer, 0, 1);

            var data = new EDATData
            {
                flags = 0x0100003C,
                blockSize = 0x00004000,
                fileLen = new BigInteger(finlen),
            };
            var rifkey = GetKey(ptr[0], data, devKLic, keyFromRif); //Obtain the key for decryption (result of sc471 or sdatkey)
            var hashFlag = 0x10000002;

            EncryptData(fin, o, ptr[0], data, rifkey);

            o.Seek(0x90, SeekOrigin.Begin);
            var aa = new AppLoader();
            aa.DoInit(
                hashFlag,
                version[0] == 4,
                0x00000001,
                new byte[0x10],
                new byte[0x10],
                rifkey
            );

            var startoffset = (data.GetFlags() & FLAG_SDAT) != 0L ? 32 : 16;
            var numBlocks = (int)(
                (data.GetFileLen() + data.GetBlockSize() - 1) / data.GetBlockSize()
            );
            long totallength = 0;
            var baseOffset = 0x100;
            int len;
            for (long remaining = startoffset * numBlocks; remaining > 0L; remaining -= len)
            {
                len = HEADER_MAX_BLOCKSIZE > remaining ? (int)remaining : HEADER_MAX_BLOCKSIZE;
                o.Seek(baseOffset + totallength, 0);
                var content = new byte[len];
                var ooo = new byte[len];
                o.Read(content, 0, content.Length);
                aa.DoUpdate(content, 0, ooo, 0, len);
                totallength += len;
            }
            var headerHash = new byte[16];
            aa.DoFinalButGetHash(headerHash);
            o.Seek(144L, 0);
            o.Write(headerHash, 0, headerHash.Length);
            o.Seek(0L, 0);
            var header = new byte[160];
            var headerODummy = new byte[160];
            o.Read(header, 0, header.Length);
            var a = new AppLoaderReverse();
            var generatedHash = new byte[16];
            a.DoAll(
                hashFlag,
                version[0] == 4,
                1,
                header,
                0,
                headerODummy,
                0,
                header.Length,
                new byte[16],
                new byte[16],
                rifkey,
                generatedHash,
                0
            );
            o.Seek(160L, 0);
            o.Write(generatedHash, 0, generatedHash.Length);
            while (o.Length < 256L)
                o.Write(buffer, 0, 1);
            o.Close();
            fin.Close();
            return STATUS_OK;
        }

        private static byte[] CreateNPDHash1(String filename, byte[] npd)
        {
            var fileBytes = ConversionUtils.CharsToByte(filename.ToCharArray());
            var data1 = new byte[0x30 + fileBytes.Length];
            ConversionUtils.Arraycopy(npd, 0x10, data1, 0, 0x30);
            ConversionUtils.Arraycopy(fileBytes, 0x00, data1, 0x30, fileBytes.Length);
            var hash1 = CreatorUtils.CMAC128(EDATKeys.npdrm_omac_key3, data1, 0, data1.Length);
            ConversionUtils.Arraycopy(hash1, 0, npd, 0x50, 0x10);
            var result1 = CreatorUtils.CompareBytes(hash1, 0, npd, 0x50, 0x10);
            return result1 ? hash1 : null;
        }

        private static byte[] CreateNPDHash2(byte[] klicensee, byte[] npd)
        {
            var xoredKey = new byte[0x10];
            CreatorUtils.XOR(xoredKey, klicensee, EDATKeys.npdrm_omac_key2);
            var calculated = CreatorUtils.CMAC128(xoredKey, npd, 0, 0x60);
            ConversionUtils.Arraycopy(calculated, 0, npd, 0x60, 0x10);
            var result2 = CreatorUtils.CompareBytes(calculated, 0, npd, 0x60, 0x10);
            return result2 ? calculated : null;
        }

        private byte[] WriteValidNPD(
            String filename,
            byte[] devKLic,
            NPD[] npdPtr,
            FileStream fin,
            byte[] contentID,
            byte[] flags,
            byte[] version,
            byte[] type
        )
        {
            var npd = new byte[0x80];
            //NPD Magic
            //ConversionUtils.arraycopy(npd, 0, result.magic, 0, 4);
            npd[0] = 0x4E;
            npd[1] = 0x50;
            npd[2] = 0x44;
            npd[3] = 0x00;
            //Version 3
            //result.version = ConversionUtils.be32(npd, 4);
            npd[4] = 0x00;
            npd[5] = 0x00;
            npd[6] = 0x00;
            npd[7] = version[0];
            //License 2 ref 3 klic /* 1 network, 2 local, 3 free */
            //result.license = ConversionUtils.be32(npd, 8);
            npd[8] = 0x00;
            npd[9] = 0x00;
            npd[10] = 0x00;
            npd[11] = 0x03;

            //Type /* 1 exec, 21 update */
            //result.type = ConversionUtils.be32(npd, 0xC);
            npd[12] = 0x00;
            npd[13] = 0x00;
            npd[14] = 0x00;
            npd[15] = type[0];

            //No Idea where I get the content_id
            //ConversionUtils.arraycopy(npd, 0x10, result.content_id, 0, 0x30
            for (var i = 0; i < 0x30; i++)
                npd[0x10 + i] = contentID[i];

            //Used to create IV
            //ConversionUtils.arraycopy(npd, 0x40, result.digest, 0, 0x10);
            var iv = ConversionUtils.CharsToByte("FixedLicenseEDAT".ToCharArray());
            ConversionUtils.Arraycopy(iv, 0, npd, 0x40, 0x10);

            //I guess it's a full file hash
            //ConversionUtils.arraycopy(npd, 0x50, result.titleHash, 0, 0x10);
            var hash = CreateNPDHash1(filename, npd);
            ConversionUtils.Arraycopy(hash, 0x00, npd, 0x50, 0x10);

            //Used to create Blockkey
            //ConversionUtils.arraycopy(npd, 0x60, result.devHash, 0, 0x10);
            var devHash = CreateNPDHash2(devKLic, npd);
            ConversionUtils.Arraycopy(devHash, 0, npd, 0x60, 0x10);

            //NPD EOF?!?!?!
            //result.unknown3 = ConversionUtils.be64(npd, 0x70);
            //result.unknown4 = ConversionUtils.be64(npd, 0x78);
            for (var i = 0; i < 16; i++)
                npd[0x70 + i] = 0x00;

            npdPtr[0] = NPD.CreateNPD(npd);
            return npd;
        }

        /* KDSBEST END */

        public int DecryptFile(String inFile, String outFile, byte[] devKLic, byte[] keyFromRif)
        {
            var fin = File.Open(inFile, FileMode.Open);
            var fn = fin.Name.Split('\\');

            var ptr = new NPD[1]; //Ptr to Ptr
            var result = validateNPD(fn[fn.Length - 1], devKLic, ptr, fin); //Validate NPD hashes
            if (result < 0)
            {
                fin.Close();
                return result;
            }
            var npd = ptr[0];
            var data = GetEDATData(fin); //Get flags, blocksize and file len
            var rifkey = GetKey(npd, data, devKLic, keyFromRif); //Obtain the key for decryption (result of sc471 or sdatkey)
            if (rifkey == null)
            {
                LoggerAccessor.LogError(
                    "[PS3 Creator] - EDAT - ERROR: Key for decryption is missing"
                );
                fin.Close();
                return STATUS_ERROR_MISSINGKEY;
            }
            else
            {
#if DEBUG
                LoggerAccessor.LogInfo(
                    "[PS3 Creator] - EDAT - DECRYPTION KEY: " + rifkey.BytesToHexStr()
                );
#endif
            }
            result = CheckHeader(rifkey, data, npd, fin);
            if (result < 0)
            {
                fin.Close();
                return result;
            }
            var o = File.Open(outFile, FileMode.Create);
            result = DecryptData(fin, o, npd, data, rifkey);
            if (result < 0)
            {
                fin.Close();
                return result;
            }
            fin.Close();
            o.Close();
            LoggerAccessor.LogInfo("[PS3 Creator] - EDAT - COMPLETE: File Written to disk");
            return STATUS_OK;
        }

        private static int CheckHeader(byte[] rifKey, EDATData data, NPD npd, FileStream i)
        {
            i.Seek(0, SeekOrigin.Begin);
            var header = new byte[0xA0];
            var o = new byte[0xA0];
            var expectedHash = new byte[0x10];
            //Version check
#if DEBUG
            LoggerAccessor.LogInfo(
                "[PS3 Creator] - EDAT - checking NPD Version:" + npd.GetVersion()
            );
#endif
            if ((npd.GetVersion() == 0) || (npd.GetVersion() == 1))
            {
                if ((data.GetFlags() & 0x7FFFFFFE) != 0)
                {
                    LoggerAccessor.LogError("[PS3 Creator] - EDAT - ERROR: Incorrect Header Flags");
                    return STATUS_ERROR_INCORRECT_FLAGS;
                }
            }
            else if (npd.GetVersion() == 2)
            {
                if ((data.GetFlags() & 0x7EFFFFE0) != 0)
                {
                    LoggerAccessor.LogError("[PS3 Creator] - EDAT - ERROR: Incorrect Header Flags");
                    return STATUS_ERROR_INCORRECT_FLAGS;
                }
            }
            else if (npd.GetVersion() == 3 || (npd.GetVersion() == 4))
            {
                if ((data.GetFlags() & 0x7EFFFFC0) != 0)
                {
                    LoggerAccessor.LogError("[PS3 Creator] - EDAT - ERROR: Incorrect Header Flags");
                    return STATUS_ERROR_INCORRECT_FLAGS;
                }
            }
            else
            {
                LoggerAccessor.LogError(
                    "[PS3 Creator] - EDAT - ERROR: Unsupported EDAT version (need keys)"
                );
                return STATUS_ERROR_INCORRECT_VERSION;
            }

            i.Read(header, 0, header.Length);
            i.Read(expectedHash, 0, expectedHash.Length);
#if DEBUG
            LoggerAccessor.LogInfo("[PS3 Creator] - EDAT - Checking header hash:");
#endif
            var a = new AppLoader();
            var hashFlag = ((data.GetFlags() & FLAG_KEYENCRYPTED) == 0) ? 0x00000002 : 0x10000002;
            if ((data.GetFlags() & FLAG_DEBUG) != 0)
                hashFlag |= 0x01000000;

            //Veryfing header
            var result = a.DoAll(
                hashFlag,
                npd.GetVersion() == 4,
                0x00000001,
                header,
                0,
                o,
                0,
                header.Length,
                new byte[0x10],
                new byte[0x10],
                rifKey,
                expectedHash,
                0
            );
            if (!result)
            {
                LoggerAccessor.LogError(
                    "[PS3 Creator] - EDAT - Error verifying header. Is rifKey valid?."
                );
                return STATUS_ERROR_HEADERCHECK;
            }
            if ((data.GetFlags() & FLAG_0x20) == 0L)
            {
#if DEBUG
                LoggerAccessor.LogInfo("[PS3 Creator] - EDAT - Checking metadata hash:");
#endif
                a = new AppLoader();
                a.DoInit(
                    hashFlag,
                    npd.GetVersion() == 4,
                    0x00000001,
                    new byte[0x10],
                    new byte[0x10],
                    rifKey
                );

                var sectionSize = ((data.GetFlags() & FLAG_COMPRESSED) != 0) ? 0x20 : 0x010;
                var numBlocks = (int)(
                    (data.GetFileLen() + data.GetBlockSize() - 11) / data.GetBlockSize()
                ); //Determine the metadatasection total len

                var readed = 0;
                var baseOffset = 0x100;
                //baseOffset +=  modifier; //There is an unknown offset to add to the metadatasection... value seen 0
                long remaining = sectionSize * numBlocks;
                while (remaining > 0)
                {
                    var lenToRead =
                        (HEADER_MAX_BLOCKSIZE > remaining) ? (int)remaining : HEADER_MAX_BLOCKSIZE;
                    i.Seek(baseOffset + readed, SeekOrigin.Begin);
                    var content = new byte[lenToRead];
                    o = new byte[lenToRead];
                    i.Read(content, 0, content.Length);
                    a.DoUpdate(content, 0, o, 0, lenToRead);
                    readed += lenToRead;
                    remaining -= lenToRead;
                }
                result = a.DoFinal(header, 0x90);

                if (!result)
                {
                    LoggerAccessor.LogError(
                        "[PS3 Creator] - EDAT - Error verifying metadatasection. Data tampered"
                    );
                    return STATUS_ERROR_HEADERCHECK;
                }
            }

            return STATUS_OK;
        }

        private static byte[] DecryptMetadataSection(byte[] metadata)
        {
            var result = new byte[0x10];
            result[0x00] = (byte)(metadata[0xC] ^ metadata[0x8] ^ metadata[0x10]);
            result[0x01] = (byte)(metadata[0xD] ^ metadata[0x9] ^ metadata[0x11]);
            result[0x02] = (byte)(metadata[0xE] ^ metadata[0xA] ^ metadata[0x12]);
            result[0x03] = (byte)(metadata[0xF] ^ metadata[0xB] ^ metadata[0x13]);
            result[0x04] = (byte)(metadata[0x4] ^ metadata[0x8] ^ metadata[0x14]);
            result[0x05] = (byte)(metadata[0x5] ^ metadata[0x9] ^ metadata[0x15]);
            result[0x06] = (byte)(metadata[0x6] ^ metadata[0xA] ^ metadata[0x16]);
            result[0x07] = (byte)(metadata[0x7] ^ metadata[0xB] ^ metadata[0x17]);
            result[0x08] = (byte)(metadata[0xC] ^ metadata[0x0] ^ metadata[0x18]);
            result[0x09] = (byte)(metadata[0xD] ^ metadata[0x1] ^ metadata[0x19]);
            result[0x0A] = (byte)(metadata[0xE] ^ metadata[0x2] ^ metadata[0x1A]);
            result[0x0B] = (byte)(metadata[0xF] ^ metadata[0x3] ^ metadata[0x1B]);
            result[0x0C] = (byte)(metadata[0x4] ^ metadata[0x0] ^ metadata[0x1C]);
            result[0x0D] = (byte)(metadata[0x5] ^ metadata[0x1] ^ metadata[0x1D]);
            result[0x0E] = (byte)(metadata[0x6] ^ metadata[0x2] ^ metadata[0x1E]);
            result[0x0F] = (byte)(metadata[0x7] ^ metadata[0x3] ^ metadata[0x1F]);
            return result;
        }

        private static EDATData GetEDATData(FileStream i)
        {
            i.Seek(0x80, SeekOrigin.Begin);
            var data = new byte[0x10];
            i.Read(data, 0, data.Length);
            return EDATData.CreateEDATData(data);
        }

        private int validateNPD(String filename, byte[] devKLic, NPD[] npdPtr, FileStream i)
        {
            i.Seek(0, SeekOrigin.Begin);
            var npd = new byte[0x80];
            i.Read(npd, 0, npd.Length);
            var extraData = new byte[0x04];
            i.Read(extraData, 0, extraData.Length);
            var flag = ConversionUtils.Be32(extraData, 0);
            if ((flag & FLAG_SDAT) != 0)
                LoggerAccessor.LogInfo(
                    "[PS3 Creator] - EDAT - SDAT detected. NPD header is not validated"
                );
            else if (!CheckNPDHash1(filename, npd))
            {
                LoggerAccessor.LogError("[PS3 Creator] - EDAT - ERROR: Hashing Title ID Name");
                return STATUS_ERROR_HASHTITLEIDNAME;
            }
            else if (devKLic == null)
                LoggerAccessor.LogWarn(
                    "[PS3 Creator] - EDAT - WARNING: Can not validate devklic header"
                );
            else if (!checkNPDHash2(devKLic, npd))
            {
                LoggerAccessor.LogError("[PS3 Creator] - EDAT - ERROR: Hashing devklic");
                return STATUS_ERROR_HASHDEVKLIC;
            }
            npdPtr[0] = NPD.CreateNPD(npd);
            return STATUS_OK;
        }

        private static bool CheckNPDHash1(String filename, byte[] npd)
        {
            var fileBytes = ConversionUtils.CharsToByte(filename.ToCharArray());
            var data1 = new byte[0x30 + fileBytes.Length];
            ConversionUtils.Arraycopy(npd, 0x10, data1, 0, 0x30);
            ConversionUtils.Arraycopy(fileBytes, 0x00, data1, 0x30, fileBytes.Length);
            var hash1 = CreatorUtils.CMAC128(EDATKeys.npdrm_omac_key3, data1, 0, data1.Length);
            var result1 = CreatorUtils.CompareBytes(hash1, 0, npd, 0x50, 0x10);
            if (result1)
            {
#if DEBUG
                LoggerAccessor.LogInfo(
                    "[PS3 Creator] - EDAT - NPD hash 1 is valid (" + hash1.BytesToHexStr() + ")"
                );
#endif
            }
            return result1;
        }

        private static bool checkNPDHash2(byte[] klicensee, byte[] npd)
        {
            var xoredKey = new byte[0x10];
            CreatorUtils.XOR(xoredKey, klicensee, EDATKeys.npdrm_omac_key2);
            var calculated = CreatorUtils.CMAC128(xoredKey, npd, 0, 0x60);
            var result2 = CreatorUtils.CompareBytes(calculated, 0, npd, 0x60, 0x10);
            if (result2)
            {
#if DEBUG
                LoggerAccessor.LogInfo(
                    "[PS3 Creator] - EDAT - NPD hash 2 is valid ("
                        + calculated.BytesToHexStr()
                        + ")"
                );
#endif
            }
            return result2;
        }

        private static byte[] GetKey(NPD npd, EDATData data, byte[] devKLic, byte[] keyFromRif)
        {
            byte[] result = null;
            if ((data.GetFlags() & FLAG_SDAT) != 0)
            {
                //Case SDAT
                result = new byte[0x10];
                CreatorUtils.XOR(result, npd.GetDevHash(), EDATKeys.SDATKEY);
            }
            else
            {
                //Case EDAT
                if (npd.GetLicense() == 0x03)
                    result = devKLic;
                else if (npd.GetLicense() == 0x02)
                    result = keyFromRif;
            }
            return result;
        }

        private int EncryptData(FileStream ii, FileStream o, NPD npd, EDATData data, byte[] rifkey)
        {
            var numBlocks = (int)(
                (data.GetFileLen() + (BigInteger)data.GetBlockSize() - (BigInteger)1)
                / (BigInteger)data.GetBlockSize()
            );
            var metadataSectionSize =
                (data.GetFlags() & FLAG_COMPRESSED) != 0L || (data.GetFlags() & FLAG_0x20) != 0L
                    ? 32
                    : 16;
            var baseOffset = 0x100;
            for (var i = 0; i < numBlocks; ++i)
            {
                if (i > 4043309056L / data.GetBlockSize())
                    i = i + 1 - 1;
                var offset = i * data.GetBlockSize();
                ii.Seek(offset, 0);
                var len = (int)data.GetBlockSize();
                if (i == numBlocks - 1)
                {
                    len = (int)(data.GetFileLen() % new BigInteger(data.GetBlockSize()));
                    if (len == 0)
                        len = (int)data.GetBlockSize();
                }
                var realLen = len;
                len = (len + 15) & -16;
                var encryptedData = new byte[len];
                var decryptedData = new byte[len];
                var toRead = realLen;
                while (toRead > 0)
                    toRead -= ii.Read(decryptedData, realLen - toRead, toRead);
                for (var ai = realLen; ai < len; ++ai)
                    decryptedData[ai] = 0;
                var key = new byte[16];
                var hash = new byte[16];
                var blockKey = CalculateBlockKey(i, npd);
                CreatorUtils.AesecbEncrypt(rifkey, blockKey, 0, key, 0, blockKey.Length);
                if ((data.GetFlags() & FLAG_0x10) != 0L)
                    CreatorUtils.AesecbEncrypt(rifkey, key, 0, hash, 0, key.Length);
                else
                    ConversionUtils.Arraycopy(key, 0, hash, 0L, key.Length);
                var cryptoFlag = 0x10000002;
                var hashFlag = 0x10000001;
                var a = new AppLoaderReverse();
                var iv = npd.GetDigest();
                var generatedHash = new byte[20];
                a.DoAll(
                    hashFlag,
                    npd.GetVersion() == 4,
                    cryptoFlag,
                    decryptedData,
                    0,
                    encryptedData,
                    0,
                    decryptedData.Length,
                    key,
                    iv,
                    hash,
                    generatedHash,
                    0
                );
                o.Seek(baseOffset + (i * (metadataSectionSize + data.GetBlockSize())), 0);
                var encryptedDataForFile = "555555555555555555555555".HexStrToBytes();
                var expectedKeyForFile = new byte[16];
                var expectedHashForFile = new byte[16];
                ConversionUtils.Arraycopy(generatedHash, 16, expectedHashForFile, 0L, 4);
                ConversionUtils.Arraycopy(
                    encryptedDataForFile,
                    0,
                    expectedHashForFile,
                    4L,
                    encryptedDataForFile.Length
                );
                CreatorUtils.XOR(expectedKeyForFile, expectedHashForFile, generatedHash);
                o.Write(expectedKeyForFile, 0, expectedKeyForFile.Length);
                o.Write(expectedHashForFile, 0, expectedHashForFile.Length);
                o.Write(encryptedData, 0, encryptedData.Length);
            }
            switch (npd.GetVersion())
            {
                case 0:
                case 1:
                    if ((data.GetFlags() & FLAG_SDAT) != 0)
                        o.Write(sdat_footer_v1, 0, sdat_footer_v1.Length);
                    else
                        o.Write(edat_footer_v1, 0, edat_footer_v1.Length);
                    break;
                case 2:
                    if ((data.GetFlags() & FLAG_SDAT) != 0)
                        o.Write(sdat_footer_v2, 0, sdat_footer_v2.Length);
                    else
                        o.Write(edat_footer_v2, 0, edat_footer_v2.Length);
                    break;
                case 3:
                    if ((data.GetFlags() & FLAG_SDAT) != 0)
                        o.Write(sdat_footer_v3, 0, sdat_footer_v3.Length);
                    else
                        o.Write(edat_footer_v3, 0, edat_footer_v3.Length);
                    break;
                case 4:
                    if ((data.GetFlags() & FLAG_SDAT) != 0)
                        o.Write(sdat_footer_v4, 0, sdat_footer_v4.Length);
                    else
                        o.Write(edat_footer_v4, 0, edat_footer_v4.Length);
                    break;
            }
            return STATUS_OK;
        }

        private int DecryptData(FileStream ii, FileStream o, NPD npd, EDATData data, byte[] rifkey)
        {
            var fileLen = data.GetFileLen();
            var numBlocks = (int)((fileLen + data.GetBlockSize() - 1) / data.GetBlockSize());
            var metadataSectionSize =
                ((data.GetFlags() & FLAG_COMPRESSED) != 0 || (data.GetFlags() & FLAG_0x20) != 0)
                    ? 0x20
                    : 0x10;
            var baseOffset = 0x100;
            for (var i = 0; i < numBlocks; i++)
            {
                ii.Seek(baseOffset + (i * metadataSectionSize), SeekOrigin.Begin);
                var expectedHash = new byte[20];
                long offset;
                int len;
                var compressionEnd = false;
                if ((data.GetFlags() & FLAG_COMPRESSED) != 0)
                {
                    var result = new byte[0x20];
                    var metadata = new byte[0x20];
                    ii.Read(metadata, 0, metadata.Length);
                    if (npd.GetVersion() <= 1L)
                        Array.Copy(metadata, result, metadata.Length);
                    else
                        result = DecryptMetadataSection(metadata);
                    offset = (int)ConversionUtils.Be64(result, 0);
                    len = (int)ConversionUtils.Be32(result, 8);
                    compressionEnd = (int)ConversionUtils.Be32(result, 0xC) != 0;
                    ConversionUtils.Arraycopy(metadata, 0, expectedHash, 0, 0x10);
                }
                else if ((data.GetFlags() & FLAG_0x20) != 0)
                {
                    var metadata = new byte[0x20];
                    ii.Seek(baseOffset + (i * (metadataSectionSize + data.GetBlockSize())), 0);
                    ii.Read(metadata, 0, metadata.Length);
                    for (var j = 0; j < 0x10; j++)
                        expectedHash[j] = (byte)(metadata[j] ^ metadata[j + 0x10]);
                    ConversionUtils.Arraycopy(metadata, 16, expectedHash, 16L, 4);
                    offset =
                        baseOffset
                        + (i * (metadataSectionSize + data.GetBlockSize()))
                        + metadataSectionSize;
                    len = (int)data.GetBlockSize();
                    if (i == numBlocks - 1)
                    {
                        var calclen = (int)(fileLen % new BigInteger(data.GetBlockSize()));
                        len = calclen > 0 ? calclen : len;
                    }
                }
                else
                {
                    ii.Read(expectedHash, 0, expectedHash.Length);
                    offset =
                        baseOffset + (i * data.GetBlockSize()) + (numBlocks * metadataSectionSize);
                    len = (int)data.GetBlockSize();
                    if (i == numBlocks - 1)
                        len = (int)(fileLen % new BigInteger(data.GetBlockSize()));
                }
                var realLen = (len + 15) & -16;
                LoggerAccessor.LogDebug(
                    "[PS3 Creator] - EDAT - Offset: %016X, len: %08X, realLen: %08X, Compressed: %d\r\n",
                    offset,
                    len,
                    realLen,
                    compressionEnd
                );
                ii.Seek(offset, SeekOrigin.Begin);
                var encryptedData = new byte[realLen];
                var decryptedData = new byte[realLen];
                ii.Read(encryptedData, 0, encryptedData.Length);
                var key = new byte[0x10];
                var hash = new byte[0x10];
                var blockKey = CalculateBlockKey(i, npd);
                CreatorUtils.AesecbEncrypt(rifkey, blockKey, 0, key, 0, blockKey.Length);
                if ((data.GetFlags() & FLAG_0x10) != 0)
                    CreatorUtils.AesecbEncrypt(rifkey, key, 0, hash, 0, key.Length);
                else
                    ConversionUtils.Arraycopy(key, 0, hash, 0, key.Length);
                var cryptoFlag = ((data.GetFlags() & FLAG_0x02) == 0) ? 0x2 : 0x1;
                var hashFlag =
                    (data.GetFlags() & FLAG_0x10) != 0L
                        ? (data.GetFlags() & FLAG_0x20) != 0L
                            ? 1
                            : 4
                        : 2;
                if ((data.GetFlags() & FLAG_KEYENCRYPTED) != 0)
                {
                    cryptoFlag |= 0x10000000;
                    hashFlag |= 0x10000000;
                }
                if ((data.GetFlags() & FLAG_DEBUG) != 0)
                {
                    cryptoFlag |= 0x01000000;
                    hashFlag |= 0x01000000;

                    if ((data.GetFlags() & FLAG_COMPRESSED) != 0L && compressionEnd)
                    {
                        var fileLenUint = (uint)fileLen;
                        var output = new byte[fileLenUint];
                        var res = LZ.Decompress(output, encryptedData, fileLenUint);
#if DEBUG
                        LoggerAccessor.LogInfo("[EDAT] - Compressed block size: " + fileLen);
                        LoggerAccessor.LogInfo("[EDAT] - Decompressed block size: " + res);
#endif
                        fileLenUint -= (uint)res;

                        if (fileLenUint == 0)
                        {
                            if (res < 0)
                            {
                                LoggerAccessor.LogError("[EDAT] - EDAT/SDAT decompression failed!");
                                return STATUS_ERROR_DECRYPTING;
                            }
                            else
                                LoggerAccessor.LogInfo(
                                    "[EDAT] - EDAT/SDAT successfully decompressed!"
                                );
                        }

                        o.Write(output, 0, output.Length);
                    }
                    else
                        o.Write(encryptedData, 0, realLen);
                }
                else
                {
                    new AppLoader().DoAll(
                        hashFlag,
                        npd.GetVersion() == 4,
                        cryptoFlag,
                        encryptedData,
                        0,
                        decryptedData,
                        0,
                        encryptedData.Length,
                        key,
                        (npd.GetVersion() <= 1) ? (new byte[0x10]) : npd.GetDigest(),
                        hash,
                        expectedHash,
                        0
                    );

                    if ((data.GetFlags() & FLAG_COMPRESSED) != 0 && compressionEnd)
                    {
                        var fileLenUint = (uint)fileLen;
                        var output = new byte[fileLenUint];
                        var res = LZ.Decompress(output, decryptedData, fileLenUint);
#if DEBUG
                        LoggerAccessor.LogInfo("[EDAT] - Compressed block size: " + fileLen);
                        LoggerAccessor.LogInfo("[EDAT] - Decompressed block size: " + res);
#endif
                        fileLenUint -= (uint)res;

                        if (fileLenUint == 0)
                        {
                            if (res < 0)
                            {
                                LoggerAccessor.LogError("[EDAT] - EDAT/SDAT decompression failed!");
                                return STATUS_ERROR_DECRYPTING;
                            }
                            else
                                LoggerAccessor.LogInfo(
                                    "[EDAT] - EDAT/SDAT successfully decompressed!"
                                );
                        }

                        o.Write(output, 0, output.Length);
                    }
                    else
                        o.Write(decryptedData, 0, realLen);
                }
            }
            return STATUS_OK;
        }

        private static byte[] CalculateBlockKey(int blk, NPD npd)
        {
            var baseKey = (npd.GetVersion() <= 1) ? (new byte[0x10]) : npd.GetDevHash();
            var result = new byte[0x10];
            ConversionUtils.Arraycopy(baseKey, 0, result, 0, 0xC);
            EndianAwareConverter.WriteInt32(result, Endianness.BigEndian, 0xC, blk);
            return result;
        }
    }
}

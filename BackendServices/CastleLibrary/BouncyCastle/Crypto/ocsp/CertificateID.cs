    ...
    public class CertificateID : IEquatable<CertificateID>
    {
        ...
        )
        {
            AlgorithmIdentifier digestAlgorithm = new AlgorithmIdentifier(
                new DerObjectIdentifier(hashAlgorithm),
                DerNull.Instance
            );
        ...
        public bool MatchesIssuer(IDigestFactory digestFactory, X509Certificate issuerCert)
        {
            if (!m_id.HashAlgorithm.Equals(digestFactory.AlgorithmDetails))
                throw new ArgumentException(
                    "digest factory does not match required digest algorithm"
                );
        ...
        )
        {
            ...
            return new CertificateID(
                new CertID(
                    originalID.HashAlgorithm,
                    originalID.IssuerNameHash,
                    originalID.IssuerKeyHash,
        ...
        )
        {
            try
            {
                ...
                return new CertID(
                    digestAlgorithm,
                    new DerOctetString(issuerNameHash),
                    new DerOctetString(issuerKeyHash),
                    serialNumber
                );
            }
            catch (Exception e)
        ...
        )
        {
            try
            {
                ...
                byte[] issuerKey = issuerCert.SubjectPublicKeyInfo.PublicKey.GetBytes();
                byte[] issuerKeyHash = X509Utilities.CalculateDigest(
                    digestFactory,
                    issuerKey,
                    0,
                    issuerKey.Length
                ...
                return new CertID(
                    (AlgorithmIdentifier)digestFactory.AlgorithmDetails,
                    new DerOctetString(issuerNameHash),
                    new DerOctetString(issuerKeyHash),
                    serialNumber
                );

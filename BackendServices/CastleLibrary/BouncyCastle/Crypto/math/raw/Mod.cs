...
namespace Org.BouncyCastle.Math.Raw
{
    ...
    internal static class Mod
    {
        ...
        public static void CheckedModOddInverse(
            ReadOnlySpan<uint> m,
            ReadOnlySpan<uint> x,
            Span<uint> z
        )
        ...
        public static void CheckedModOddInverseVar(
            ReadOnlySpan<uint> m,
            ReadOnlySpan<uint> x,
            Span<uint> z
        )
        ...
        public static uint ModOddInverse(ReadOnlySpan<uint> m, ReadOnlySpan<uint> x, Span<uint> z)
        {
            ...
            Span<int> alloc =
                (allocSize * Integers.NumBytes <= MaxStackAlloc)
                    ? stackalloc int[allocSize]
                    : new int[allocSize];

            Span<int> t = stackalloc int[4];
            Span<int> D = alloc[..len30];
            alloc = alloc[len30..];
            Span<int> E = alloc[..len30];
            alloc = alloc[len30..];
            Span<int> F = alloc[..len30];
            alloc = alloc[len30..];
            Span<int> G = alloc[..len30];
            alloc = alloc[len30..];
        ...
        public static bool ModOddInverseVar(
            ReadOnlySpan<uint> m,
            ReadOnlySpan<uint> x,
            Span<uint> z
        )
        ...
        public static void Random(SecureRandom random, ReadOnlySpan<uint> p, Span<uint> z)
        {
            ...
            Span<byte> bytes =
                allocSize <= MaxStackAlloc ? stackalloc byte[allocSize] : new byte[allocSize];
        ...
        private static void CNormalize30(
            int len30,
            int condNegate,
            Span<int> D,
            ReadOnlySpan<int> M
        ...
        private static void UpdateDE30(
            int len30,
            Span<int> D,
            Span<int> E,
            ReadOnlySpan<int> t,

...
namespace Org.BouncyCastle.Math.EC.Custom.Sec
{
    internal class SecP384R1Curve : AbstractFpCurve
    {
        ...
        public override ECLookupTable CreateCacheSafeLookupTable(ECPoint[] points, int off, int len)
        {
            uint[] table = new uint[len * SECP384R1_FE_INTS * 2];
            {
                ...
                for (int i = 0; i < len; ++i)
                {
                    ...
                    Nat.Copy(
                        SECP384R1_FE_INTS,
                        ((SecP384R1FieldElement)p.RawXCoord).x,
                        0,
                        table,
                        pos
                    );
                    ...
                    Nat.Copy(
                        SECP384R1_FE_INTS,
                        ((SecP384R1FieldElement)p.RawYCoord).x,
                        0,
                        table,
                        pos
                    );
        ...
        private class SecP384R1LookupTable : AbstractECLookupTable
        {
            ...
            {
                uint[] x = Nat.Create(SECP384R1_FE_INTS),
                    y = Nat.Create(SECP384R1_FE_INTS);
                int pos = 0;

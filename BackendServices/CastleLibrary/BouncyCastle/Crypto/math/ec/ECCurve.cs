...
namespace Org.BouncyCastle.Math.EC
{
    ...
    public abstract class ECCurve
    {
        ...
        internal virtual PreCompInfo Precompute(string name, IPreCompCallback callback)
        {
            ...
            lock (table)
            {
                PreCompInfo existing = table.TryGetValue(name, out var preCompInfo)
                    ? preCompInfo
                    : null;
        ...
        public virtual ECLookupTable CreateCacheSafeLookupTable(ECPoint[] points, int off, int len)
        {
            ...
            {
                ECPoint p = points[off + i];
                p.RawXCoord.EncodeTo(table, pos);
                pos += FE_BYTES;
                p.RawYCoord.EncodeTo(table, pos);
                pos += FE_BYTES;
            }
    ...
    public abstract class AbstractF2mCurve : ECCurve
    {
        ...
        private static IFiniteField BuildField(int m, int k1, int k2, int k3)
        {
            ...
            int[] exponents =
                (k2 | k3) == 0 ? new int[] { 0, k1, m } : new int[] { 0, k1, k2, k3, m };
        ...
        internal ECFieldElement SolveQuadraticEquation(ECFieldElement beta)
        {
            ...
            ECFieldElement gamma,
                z,
                zeroElement = FromBigInteger(BigInteger.Zero);
    ...
    public class F2mCurve : AbstractF2mCurve
    {
        ...
        public override ECFieldElement FromBigInteger(BigInteger x)
        {
            ...
            int[] ks = (k2 | k3) == 0 ? new int[] { k1 } : new int[] { k1, k2, k3 };
        ...
        private class DefaultF2mLookupTable : AbstractECLookupTable
        {
            ...
            public override ECPoint Lookup(int index)
            {
                ...
                ulong[] x = new ulong[FE_LONGS],
                    y = new ulong[FE_LONGS];
                int pos = 0;

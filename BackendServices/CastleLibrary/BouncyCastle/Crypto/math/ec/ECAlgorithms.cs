    ...
    public class ECAlgorithms
    {
        ...
        public static bool IsF2mField(IFiniteField field)
        {
            return field.Dimension > 1
                && field.Characteristic.Equals(BigInteger.Two)
        ...
        */
        ...
        internal static ECPoint ImplShamirsTrickJsf(
            ECPoint P,
            BigInteger k,
            ECPoint Q,
            BigInteger l
        ...
        )
        {
            ...
            // When P, Q are 'promoted' (i.e. reused several times), switch to fixed-point algorithm
            {
                ...
                if (
                    !negK
                    && !negL
                    && k.BitLength <= combSize
                    && l.BitLength <= combSize
                    && infoP.IsPromoted
                    && infoQ.IsPromoted
                )
                {
        ...
        )
        {
            ...
            int minWidth = WNafUtilities.GetWindowSize(
                System.Math.Max(k.BitLength, l.BitLength),
                8
            );

            WNafPreCompInfo infoP = WNafUtilities.Precompute(P, minWidth, true);
            ECPoint Q = EndoUtilities.MapPoint(endomorphism, P);
            WNafPreCompInfo infoQ = WNafUtilities.PrecomputeWithPointMap(
                Q,
                endomorphism.PointMap,
                infoP,
                true
            );
        ...
        )
        {
            ...
            for (int i = 0; i < halfCount; ++i)
            {
                ...
                int minWidth = WNafUtilities.GetWindowSize(
                    System.Math.Max(kj0.BitLength, kj1.BitLength),
                    8
                );
        ...
        )
        {
            ...
            Span<uint> K = len <= 64 ? stackalloc uint[len] : new uint[len];
            Nat.FromBigInteger(fullComb, k, K);
            Span<uint> L = len <= 64 ? stackalloc uint[len] : new uint[len];

...
namespace Org.BouncyCastle.Math.EC.Rfc8032
{
    internal static class Scalar25519
    {
        ...
        internal static void Multiply128Var(
            ReadOnlySpan<uint> x,
            ReadOnlySpan<uint> y128,
            Span<uint> z
        )
        {
        ...
        internal static bool ReduceBasisVar(ReadOnlySpan<uint> k, Span<uint> z0, Span<uint> z1)
        {
            ...
            Span<uint> Nu = stackalloc uint[16];
            LSq.CopyTo(Nu);
            Span<uint> Nv = stackalloc uint[16];
            Nat256.Square(k, Nv);
            ++Nv[0];
            Span<uint> p = stackalloc uint[16];
            Nat256.Mul(L, k, p);
        ...
        internal static void ToSignedDigits(int bits, Span<uint> z)
        {
            ...
            uint c1 = Nat.CAddTo(Size, ~(int)z[0] & 1, L, z);
            Debug.Assert(c1 == 0U);
            uint c2 = Nat.ShiftDownBit(Size, z, 1U);

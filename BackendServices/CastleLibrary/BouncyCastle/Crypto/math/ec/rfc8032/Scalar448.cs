...
namespace Org.BouncyCastle.Math.EC.Rfc8032
{
    internal static class Scalar448
    {
        ...
        private static readonly uint[] LSq =
        {
            0x1BA1FEA9U,
            0xC1ADFBB8U,
            0x49E0A8B2U,
        ...
        internal static void Multiply225Var(
            ReadOnlySpan<uint> x,
            ReadOnlySpan<uint> y225,
            Span<uint> z
        )
        {
        ...
        internal static bool ReduceBasisVar(ReadOnlySpan<uint> k, Span<uint> z0, Span<uint> z1)
        {
            ...
            Span<uint> Nu = stackalloc uint[28];
            LSq.CopyTo(Nu);
            Span<uint> Nv = stackalloc uint[28];
            Nat448.Square(k, Nv);
            ++Nv[0];
            Span<uint> p = stackalloc uint[28];
            Nat448.Mul(L, k, p);
            Span<uint> t = stackalloc uint[28];
            Span<uint> u0 = stackalloc uint[8];
            u0.CopyFrom(L);
        ...
        internal static void ToSignedDigits(int bits, ReadOnlySpan<uint> x, Span<uint> z)
        {
            ...
            z[Size] = (1U << (bits - 448)) + Nat.CAdd(Size, ~(int)x[0] & 1, x, L, z);

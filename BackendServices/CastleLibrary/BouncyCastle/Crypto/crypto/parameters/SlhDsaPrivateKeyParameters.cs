...
namespace Org.BouncyCastle.Crypto.Parameters
{
    ...
    {
        public static SlhDsaPrivateKeyParameters FromEncoding(
        ...
        )
        {
            ...
            SK sk = new SK(
                Arrays.CopyOfRange(encoding, 0, n),
                Arrays.CopyOfRange(encoding, n, 2 * n)
            );
            PK pk = new PK(
                Arrays.CopyOfRange(encoding, 2 * n, 3 * n),
                Arrays.CopyOfRange(encoding, 3 * n, 4 * n)
            );
        ...
        public byte[] GetEncoded() =>
            Arrays.ConcatenateAll(m_sk.Seed, m_sk.Prf, m_pk.Seed, m_pk.Root);

        public SlhDsaPublicKeyParameters GetPublicKey() =>
            new SlhDsaPublicKeyParameters(Parameters, m_pk);
        ...
        internal byte[] SignInternal(byte[] optRand, byte[] msg, int msgOff, int msgLen)
        {
            ...
            IndexedDigest idxDigest = engine.HMsg(
                signature,
                0,
                PK.Seed,
                PK.Root,

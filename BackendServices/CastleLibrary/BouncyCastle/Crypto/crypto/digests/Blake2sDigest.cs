...
namespace Org.BouncyCastle.Crypto.Digests
{
    ...
    public sealed class Blake2sDigest : IDigest
    {
        ...
        private void Init()
        {
            ...
            chainValue[0] =
                IV[0] ^ (uint)(digestLength | (keyLength << 8) | ((fanout << 16) | (depth << 24)));
        ...
        public int DoFinal(Span<byte> output)
        {
            ...
            int full = digestLength >> 2,
                partial = digestLength & 3;
            Pack.UInt32_To_LE(chainValue.AsSpan(0, full), output);
            if (partial > 0)
            {
                Pack.UInt32_To_LE_Low(
                    chainValue[full],
                    output.Slice(digestLength - partial, partial)
                );
            }

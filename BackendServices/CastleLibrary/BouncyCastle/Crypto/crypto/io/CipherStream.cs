    ...
    public sealed class CipherStream : Stream
    {
        ...
        public override int Read(Span<byte> buffer)
        {
            ...
            while (num < buffer.Length)
            {
                ...
                int numToCopy = System.Math.Min(
                    buffer.Length - num,
                    m_readBuf.Length - m_readBufPos
                );
        ...
        public override void Write(ReadOnlySpan<byte> buffer)
        {
            ...
            if (!buffer.IsEmpty)
            {
                ...
                Span<byte> output =
                    outputSize <= Streams.DefaultBufferSize
        ...
        )
        {
            ...
            if (!buffer.IsEmpty)
            {
                ...
                if (length > 0)
                {
                    var writeTask = m_stream.WriteAsync(
        ...
        protected override void Dispose(bool disposing)
        {
            if (disposing)
            {
                if (m_writeCipher != null)
                {
                    ...
                    Span<byte> output =
                        outputSize <= 256 ? stackalloc byte[outputSize] : new byte[outputSize];

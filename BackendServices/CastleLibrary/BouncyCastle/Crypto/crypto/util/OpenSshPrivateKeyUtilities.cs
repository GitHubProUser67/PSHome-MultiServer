    ...
    public static class OpenSshPrivateKeyUtilities
    {
        ...
        public static byte[] EncodePrivateKey(AsymmetricKeyParameter parameters)
        {
            ...
            else if (parameters is DsaPrivateKeyParameters dsaPrivateKey)
            {
                ...
                catch (Exception ex)
                {
                    throw new InvalidOperationException(
                        "unable to encode DSAPrivateKeyParameters " + ex.Message
                    );
                }
            }
            else if (parameters is Ed25519PrivateKeyParameters ed25519PrivateKey)
            {
                Ed25519PublicKeyParameters publicKeyParameters =
                    ed25519PrivateKey.GeneratePublicKey();
                ...
                {
                    byte[] pkEncoded = OpenSshPublicKeyUtilities.EncodePublicKey(
                        publicKeyParameters
                    );
...

                {
                    ...
                    pkBuild.WriteBlock(
                        Arrays.Concatenate(ed25519PrivateKey.GetEncoded(), pubKeyEncoded)
                    );
            ...
            throw new ArgumentException(
                "unable to convert " + Platform.GetTypeName(parameters) + " to openssh private key"
            );
        }
        ...
        public static AsymmetricKeyParameter ParsePrivateKeyBlob(byte[] blob)
        {
            ...
            if (blob[0] == 0x30)
            {
                ...
                {
                    if (
                        AllIntegers(sequence)
                        && ((DerInteger)sequence[0]).PositiveValue.Equals(BigIntegers.Zero)
                    )
                    {
            ...
            else
            {
                ...
                if (check1 != check2)
                    throw new InvalidOperationException(
                        "private key check values are not the same"
                    );
                ...
                if ("ssh-ed25519".Equals(keyType))
                {
                    ...
                    if (
                        edPrivateKey.Length
                        != Ed25519PrivateKeyParameters.KeySize + Ed25519PublicKeyParameters.KeySize
                    )
                        throw new InvalidOperationException("private key value of wrong length");

                    result = new Ed25519PrivateKeyParameters(
                        edPrivateKey[..Ed25519PrivateKeyParameters.KeySize]
                    );
                }

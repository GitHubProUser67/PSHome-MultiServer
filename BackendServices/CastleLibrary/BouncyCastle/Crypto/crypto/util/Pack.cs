...
namespace Org.BouncyCastle.Crypto.Utilities
{
    internal static class Pack
    {
        ...
        {
            return (uint)bs[0] << 16 | (uint)bs[1] << 8 | bs[2];
        }

        internal static uint BE_To_UInt24(byte[] bs, int off)
        {
            return (uint)bs[off] << 16 | (uint)bs[off + 1] << 8 | bs[off + 2];
        }
        ...
        {
            return bs[off] | (uint)bs[off + 1] << 8 | (uint)bs[off + 2] << 16;
        }
        ...
        {
            return bs[0] | (uint)bs[1] << 8 | (uint)bs[2] << 16;
        }

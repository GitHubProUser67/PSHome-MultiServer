...
namespace Org.BouncyCastle.Crypto.Kems.MLKem
{
    internal class IndCpa
    {
        ...
        private int GenerateMatrixNBlocks =>
            (
                (12 * MLKemEngine.N / 8 * (1 << 12) / MLKemEngine.Q + m_symmetric.XofBlockBytes)
                / m_symmetric.XofBlockBytes
            );
        ...
        private void GenerateMatrix(PolyVec[] a, ReadOnlySpan<byte> seed, bool transposed)
        {
            ...
            for (int i = 0; i < K; i++)
            {
                for (int j = 0; j < K; j++)
                {
                    ...
                    m_symmetric.XofSqueezeBlocks(
                        buf.AsSpan(0, GenerateMatrixNBlocks * m_symmetric.XofBlockBytes)
                    );
                    int buflen = GenerateMatrixNBlocks * m_symmetric.XofBlockBytes;
                    int ctr = RejectionSampling(
                        a[i].m_vec[j].m_coeffs,
                        0,
                        MLKemEngine.N,
                        buf,
                        buflen
                    ...
                    while (ctr < MLKemEngine.N)
                    {
                        ...
                        m_symmetric.XofSqueezeBlocks(
                            buf.AsSpan(off, m_symmetric.XofBlockBytes * 2)
                        );
                        buflen = off + m_symmetric.XofBlockBytes;
                        ctr += RejectionSampling(
                            a[i].m_vec[j].m_coeffs,
                            ctr,
                            MLKemEngine.N - ctr,
                            buf,
        ...
        private int RejectionSampling(short[] r, int off, int len, byte[] buf, int buflen)
        {
            ...
            {
                ushort val0 = (ushort)(
                    (((ushort)(buf[pos + 0] & 0xFF) >> 0) | ((ushort)(buf[pos + 1] & 0xFF) << 8))
                    & 0xFFF
                );
                ushort val1 = (ushort)(
                    (((ushort)(buf[pos + 1] & 0xFF) >> 4) | ((ushort)(buf[pos + 2] & 0xFF) << 4))
                    & 0xFFF
                );
                pos += 3;
        ...
        internal void GenerateKeyPair(byte[] d, out byte[] pk, out byte[] sk)
        {
            ...
            PolyVec e = new PolyVec(m_engine),
                pkpv = new PolyVec(m_engine),
                skpv = new PolyVec(m_engine);
        ...
        )
        {
            ...
            byte nonce = 0;
            PolyVec sp = new PolyVec(m_engine),
                pkpv = new PolyVec(m_engine),
                ep = new PolyVec(m_engine),
                bp = new PolyVec(m_engine);
            PolyVec[] MatrixTransposed = new PolyVec[K];
            Poly v = new Poly(m_engine),
                k = new Poly(m_engine),
                epp = new Poly(m_engine);
        ...
        {
            PolyVec bp = new PolyVec(m_engine),
                skpv = new PolyVec(m_engine);
            Poly v = new Poly(m_engine),
                mp = new Poly(m_engine);

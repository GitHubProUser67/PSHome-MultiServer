...
namespace Org.BouncyCastle.Crypto.Agreement
{
    ...
    /// </summary>
    public sealed class ECVkoAgreement : IRawAgreement
    {
        ...
        public void Init(ICipherParameters parameters)
        {
            if (!(parameters is ParametersWithUkm paramsWithUkm))
                throw new ArgumentException(
                    $"{nameof(ECVkoAgreement)} expects {nameof(ParametersWithUkm)}"
                );

            if (!(paramsWithUkm.Parameters is ECPrivateKeyParameters ecParams))
                throw new ArgumentException(
                    $"{nameof(ECVkoAgreement)} expects {nameof(ECPrivateKeyParameters)}"
                );
        ...
        private void ImplUpdateDigest(ICipherParameters publicKey)
        {
            ...
            if (p.IsInfinity)
                throw new InvalidOperationException(
                    "Infinity is not a valid agreement value for ECVKO"
                );

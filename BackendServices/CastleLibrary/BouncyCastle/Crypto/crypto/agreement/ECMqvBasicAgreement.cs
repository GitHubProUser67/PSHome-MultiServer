...
namespace Org.BouncyCastle.Crypto.Agreement
{
    ...
    public class ECMqvBasicAgreement : IBasicAgreement
    {
        ...
        public virtual int GetFieldSize() =>
            privParams.StaticPrivateKey.Parameters.Curve.FieldElementEncodingLength;
        ...
        public virtual BigInteger CalculateAgreement(ICipherParameters pubKey)
        {
            ...
            if (!parameters.Equals(pubParams.StaticPublicKey.Parameters))
                throw new InvalidOperationException(
                    "ECMQV public key components have wrong domain parameters"
                );

            ECPoint agreement = CalculateMqvAgreement(
                    parameters,
                    staticPrivateKey,
                    privParams.EphemeralPrivateKey,
                    privParams.EphemeralPublicKey,
                    pubParams.StaticPublicKey,
                    pubParams.EphemeralPublicKey
                )
                .Normalize();

            if (agreement.IsInfinity)
                throw new InvalidOperationException(
                    "Infinity is not a valid agreement value for MQV"
                );

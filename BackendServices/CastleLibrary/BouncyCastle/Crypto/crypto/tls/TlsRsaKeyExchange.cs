...
namespace Org.BouncyCastle.Crypto.Tls
{
    public static class TlsRsaKeyExchange
    {
        ...
        public static byte[] DecryptPreMasterSecret(
            byte[] buf,
            int off,
            int len,
        ...
        )
        {
            ...
            try
            {
                ...
                int badEncodingMask = CheckPkcs1Encoding2(
                    encoding,
                    pkcs1Length,
                    PreMasterSecretLength
                );
                int badVersionMask =
                    -(Pack.BE_To_UInt16(encoding, plainTextOffset) ^ protocolVersion) >> 31;
                int fallbackMask = badEncodingMask | badVersionMask;

                for (int i = 0; i < PreMasterSecretLength; ++i)
                {
                    result[i] = (byte)(
                        (result[i] & fallbackMask) | (encoding[plainTextOffset + i] & ~fallbackMask)
                    );
                }
        ...
        private static byte[] RsaBlinded(
            RsaKeyParameters privateKey,
            BigInteger input,
            SecureRandom secureRandom
        )
        {
            ...
            BigInteger offsetResult = unblind
                .Add(BigInteger.One)
                .ModMultiply(blindedResult, modulus);

...
namespace Org.BouncyCastle.Crypto.Modes
{
    ...
    public sealed class AsconAead128 : IAeadCipher
    {
        ...
        public void Init(bool forEncryption, ICipherParameters parameters)
        {
            ...
            if (npub.Length != CryptoABytes)
                throw new ArgumentException(
                    $"{AlgorithmName} requires exactly {CryptoABytes} bytes of IV"
                );
            ...
            if (k.Length != CryptoKeyBytes)
                throw new ArgumentException(
                    $"{AlgorithmName} key must be {CryptoABytes} bytes long"
                );
            ...
            switch (m_state)
            {
                ...
                case State.EncAad:
                {
                    ...
                    if (m_bufPos >= 8)
                    {
                        ...
                        S1 ^=
                            Pack.LE_To_UInt64(m_buf, 8)
                            & (ulong.MaxValue >> (56 - ((m_bufPos - 8) << 3)));
                    }
                    else
                    {
                        S0 ^=
                            Pack.LE_To_UInt64(m_buf, 0)
        ...
        private static void ProcessFinalDecrypt64(
        ...
        private static void ProcessFinalEncrypt64(
            ReadOnlySpan<byte> input,
            Span<byte> output,
            ref ulong s
        )

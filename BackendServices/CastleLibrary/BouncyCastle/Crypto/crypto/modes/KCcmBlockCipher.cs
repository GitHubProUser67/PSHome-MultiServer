    ...
    public class KCcmBlockCipher : IAeadBlockCipher
    {
        ...
        */
        ...
        private void SetNb(int Nb)
        {
            ...
            else
            {
                throw new ArgumentException(
                    "Nb = 4 is recommended by DSTU7624 but can be changed to only 6 or 8 in this implementation"
                );
            }
        }
        ...
        public virtual void Init(bool forEncryption, ICipherParameters parameters)
        {
            ...
            {
                if (
                    param.MacSize > MAX_MAC_BIT_LENGTH
                    || param.MacSize < MIN_MAC_BIT_LENGTH
                    || param.MacSize % 8 != 0
                )
        ...
        public int ProcessPacket(ReadOnlySpan<byte> input, Span<byte> output)
        {
            ...
            else
            {
                ...
                if (len > index)
                {
                    ...
                    for (int byteIndex = 0; byteIndex < macSize; byteIndex++)
                    {
                        output[index + byteIndex] = (byte)(
                            buffer[byteIndex] ^ input[index + byteIndex]
                        );
                    }
                    index += macSize;
                }
                ...
                Span<byte> calculatedMac =
                    macSize <= 64 ? stackalloc byte[macSize] : new byte[macSize];

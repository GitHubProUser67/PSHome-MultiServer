...
namespace Org.BouncyCastle.Crypto.Prng.Drbg
{
    ...
    {
        private static readonly byte[] ONE = { 0x01 };

        private static readonly long RESEED_MAX = 1L << (48 - 1);
        private static readonly int MAX_BITS_REQUEST = 1 << (19 - 1);

        private static readonly IDictionary<string, int> SeedLens = new Dictionary<string, int>(
        ...
        public int Generate(Span<byte> output, bool predictionResistant)
        {
            ...
            if (numberOfBits > MAX_BITS_REQUEST)
                throw new ArgumentException(
                    "Number of bits per request limited to " + MAX_BITS_REQUEST,
                    "output"
                );
        ...
        )
        {
            ...
            else
            {
                ...
                Span<byte> w =
                    digestSize <= 128 ? stackalloc byte[digestSize] : new byte[digestSize];
                mDigest.DoFinal(w);
        ...
        private int ImplGenerate(Span<byte> output)
        {
            ...
            int digestSize = mDigest.GetDigestSize();
            Span<byte> H = digestSize <= 128 ? stackalloc byte[digestSize] : new byte[digestSize];
            mDigest.DoFinal(H);
        ...
        public void Reseed(ReadOnlySpan<byte> additionalInput)
        {
            ...
            Span<byte> seedMaterial =
                seedMaterialLength <= 256
            ...
            Span<byte> subV =
                subVLength <= 128 ? stackalloc byte[subVLength] : new byte[subVLength];
            subV[0] = 0x00;
            mV.CopyTo(subV[1..]);
        ...
        private void Hashgen(ReadOnlySpan<byte> input, Span<byte> output)
        {
            ...
            Span<byte> data =
                dataSize <= 256 ? stackalloc byte[input.Length] : new byte[input.Length];
            input.CopyTo(data);

            Span<byte> dig = digestSize <= 128 ? stackalloc byte[digestSize] : new byte[digestSize];

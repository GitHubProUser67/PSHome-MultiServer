...
namespace Org.BouncyCastle.Crypto.Prng.Drbg
{
    ...
    public sealed class CtrSP800Drbg : ISP80090Drbg
    {
        ...
        private void CTR_DRBG_Update(ReadOnlySpan<byte> seed, Span<byte> key, Span<byte> v)
        {
            ...
            Span<byte> temp =
                seedLength <= 256 ? stackalloc byte[seedLength] : new byte[seedLength];

            int blockSize = mEngine.GetBlockSize();
            Span<byte> block = blockSize <= 64 ? stackalloc byte[blockSize] : new byte[blockSize];
        ...
        private void CTR_DRBG_Reseed_algorithm(ReadOnlySpan<byte> additionalInput)
        {
            ...
            Span<byte> input =
                inputLength <= 256 ? stackalloc byte[inputLength] : new byte[inputLength];
        ...
        private byte[] BlockCipherDF(ReadOnlySpan<byte> input, int N)
        {
            ...
            Span<byte> S = blockLen <= 256 ? stackalloc byte[blockLen] : new byte[blockLen];
            ...
            Span<byte> temp = tempSize <= 128 ? stackalloc byte[tempSize] : new byte[tempSize];

            Span<byte> bccOut = blockSize <= 64 ? stackalloc byte[blockSize] : new byte[blockSize];

            Span<byte> IV = blockSize <= 64 ? stackalloc byte[blockSize] : new byte[blockSize];
        ...
        */
        private void BCC(Span<byte> bccOut, ReadOnlySpan<byte> iV, ReadOnlySpan<byte> data)
        {
            ...
            Span<byte> chainingValue =
                blockSize <= 64 ? stackalloc byte[blockSize] : new byte[blockSize];
            Span<byte> inputBlock =
                blockSize <= 64 ? stackalloc byte[blockSize] : new byte[blockSize];
        ...
        public int Generate(Span<byte> output, bool predictionResistant)
        {
            ...
            if (mIsTdea)
            {
                ...
                if (outputLen > TDEA_MAX_BITS_REQUEST / 8)
                    throw new ArgumentException(
                        "Number of bits per request limited to " + TDEA_MAX_BITS_REQUEST,
                        "output"
                    );
            }
            else
            {
                ...
                if (outputLen > AES_MAX_BITS_REQUEST / 8)
                    throw new ArgumentException(
                        "Number of bits per request limited to " + AES_MAX_BITS_REQUEST,
                        "output"
                    );
            }
        ...
        public int GenerateWithInput(
            Span<byte> output,
            ReadOnlySpan<byte> additionalInput,
            bool predictionResistant
        )
        {
            int outputLen = output.Length;
        ...
        private int ImplGenerate(ReadOnlySpan<byte> seed, Span<byte> output)
        {
            ...
            Span<byte> tmp = mV.Length <= 64 ? stackalloc byte[mV.Length] : new byte[mV.Length];

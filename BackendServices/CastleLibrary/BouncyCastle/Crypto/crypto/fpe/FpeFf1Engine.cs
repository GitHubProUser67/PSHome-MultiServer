...
namespace Org.BouncyCastle.Crypto.Fpe
{
    public class FpeFf1Engine : FpeEngine
    {
        ...
        public FpeFf1Engine(IBlockCipher baseCipher)
            : base(baseCipher)
        {
            if (IsOverrideSet(SP80038G.FPE_DISABLED) || IsOverrideSet(SP80038G.FF1_DISABLED))
            {
            ...
            if (fpeParameters.Radix > 256)
            {
                if ((length & 1) != 0)
                    throw new ArgumentException(
                        "input must be an even number of bytes for a wide radix"
                    );

                ushort[] u16In = Pack.BE_To_UInt16(inBuf, inOff, length / 2);
                ushort[] u16Out = SP80038G.EncryptFF1w(
                    baseCipher,
                    fpeParameters.Radix,
                    fpeParameters.GetTweak(),
                    u16In,
                    0,
                    u16In.Length
                );
        ...
        )
        {
            ...
            if (fpeParameters.Radix > 256)
            {
                ...
                ushort[] u16In = Pack.BE_To_UInt16(inBuf, inOff, length / 2);
                ushort[] u16Out = SP80038G.DecryptFF1w(
                    baseCipher,
                    fpeParameters.Radix,
                    fpeParameters.GetTweak(),
                    u16In,

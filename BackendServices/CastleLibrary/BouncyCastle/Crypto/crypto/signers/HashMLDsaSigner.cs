    ...
    public sealed class HashMLDsaSigner : ISigner
    {
        ...
        public void Init(bool forSigning, ICipherParameters parameters)
        {
            ...
            // Cache the prefix
            {
                ...
                byte[] tr =
                    m_privateKey != null ? m_privateKey.m_tr : m_publicKey.GetPublicKeyHash();
        ...
        public byte[] GenerateSignature()
        {
            if (m_privateKey == null)
                throw new InvalidOperationException(
                    "HashMLDsaSigner not initialised for signature generation."
                );

            ShakeDigest msgRepDigest = FinishPreHash();

            byte[] sig = new byte[m_engine.CryptoBytes];
            m_engine.MsgRepEndSign(
                msgRepDigest,
                sig,
                sig.Length,
                m_privateKey.m_rho,
        ...
        public bool VerifySignature(byte[] signature)
        {
            if (m_publicKey == null)
                throw new InvalidOperationException(
                    "HashMLDsaSigner not initialised for verification"
                );

            ShakeDigest msgRepDigest = FinishPreHash();

            return m_engine.MsgRepEndVerifyInternal(
                msgRepDigest,
                signature,
                signature.Length,
                m_publicKey.m_rho,

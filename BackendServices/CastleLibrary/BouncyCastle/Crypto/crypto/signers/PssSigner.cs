...
namespace Org.BouncyCastle.Crypto.Signers
{
    ...
    public class PssSigner : ISigner
    {
        ...
        {
            return new PssSigner(
                cipher,
                Prehash.ForDigest(digest),
                digest,
                digest,
        ...
        )
        {
            return new PssSigner(
                cipher,
                Prehash.ForDigest(contentDigest),
                contentDigest,
                mgfDigest,
                saltLen,
                null,
        ...
        public virtual bool VerifySignature(byte[] signature)
        {
            ...
            byte[] dbMask = MaskGeneratorFunction(
                block,
                block.Length - hLen - 1,
                hLen,
                block.Length - hLen - 1
            ...
            for (
                int i = block.Length - hLen - 1, j = mDash.Length - hLen;
                j != mDash.Length;
                i++, j++
            )
            {

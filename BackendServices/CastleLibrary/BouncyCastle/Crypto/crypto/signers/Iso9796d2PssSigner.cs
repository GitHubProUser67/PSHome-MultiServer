...
namespace Org.BouncyCastle.Crypto.Signers
{
    ...
    /// </summary>
    public class Iso9796d2PssSigner : ISignerWithRecovery
    {
        /// <summary>
            ...
            else if (parameters is ParametersWithSalt withSalt)
            {
                if (!forSigning)
                    throw new ArgumentException(
                        "ParametersWithSalt only valid for signing",
                        nameof(parameters)
                    );
        ...
        {
            return messageLength == b.Length && Arrays.FixedTimeEquals(messageLength, a, 0, b, 0);
        }
        ...
        public virtual bool VerifySignature(byte[] signature)
        {
            ...
            {
                throw new InvalidOperationException(
                    "UpdateWithRecoveredMessage called on different signature"
                );
            }
            ...
            bool isOkay = Arrays.FixedTimeEquals(
                hLen,
                hash,
                0,
                block,
                block.Length - tLength - hLen
            );

...
namespace Org.BouncyCastle.Crypto.Signers
{
    public class GenericSigner : ISigner
    {
        ...
        public virtual string AlgorithmName =>
            $"Generic({m_engine.AlgorithmName}/{m_digest.AlgorithmName})";
        ...
        public virtual void Init(bool forSigning, ICipherParameters parameters)
        {
            ...
            AsymmetricKeyParameter key = (AsymmetricKeyParameter)
                ParameterUtilities.IgnoreRandom(parameters);
        ...
        public virtual byte[] GenerateSignature()
        {
            if (!m_forSigning)
                throw new InvalidOperationException(
                    "GenericSigner not initialised for signature generation."
                );
        ...
        public virtual bool VerifySignature(byte[] signature)
        {
            if (m_forSigning)
                throw new InvalidOperationException(
                    "GenericSigner not initialised for verification"
                );

...
namespace Org.BouncyCastle.Crypto.Signers
{
    public class Gost3410DigestSigner : ISigner
    {
        ...
        public virtual string AlgorithmName =>
            m_digest.AlgorithmName + "with" + m_dsa.AlgorithmName;
        ...
        public virtual byte[] GenerateSignature()
        {
            if (!m_forSigning)
                throw new InvalidOperationException(
                    "GOST3410DigestSigner not initialised for signature generation."
                );
        ...
        public virtual bool VerifySignature(byte[] signature)
        {
            if (m_forSigning)
                throw new InvalidOperationException(
                    "GOST3410DigestSigner not initialised for verification"
                );

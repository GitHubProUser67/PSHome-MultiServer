...
namespace Org.BouncyCastle.Crypto.Signers.SlhDsa
{
    internal abstract class SlhDsaEngine
    {
        ...
        public abstract IndexedDigest HMsg(
            byte[] prf,
            int prfOff,
            byte[] pkSeed,
            byte[] pkRoot,
        ...
        internal sealed class Sha2Engine : SlhDsaEngine
        {
            ...
            public override IndexedDigest HMsg(
                byte[] prf,
                int prfOff,
                byte[] pkSeed,
                byte[] pkRoot,
            ...
            )
            {
                ...
                ulong treeIndex =
                    Pack.BE_To_UInt64_Low(output, forsMsgBytes, treeBytes)
                    & ulong.MaxValue >> (64 - treeBits);

                uint leafIndex =
                    Pack.BE_To_UInt32_Low(output, forsMsgBytes + treeBytes, leafBytes)
        ...
        internal sealed class Shake256Engine : SlhDsaEngine
        {

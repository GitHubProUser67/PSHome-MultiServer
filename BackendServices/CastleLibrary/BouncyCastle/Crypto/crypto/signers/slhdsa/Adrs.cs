...
namespace Org.BouncyCastle.Crypto.Signers.SlhDsa
{
    internal sealed class Adrs
    {
        ...
        internal void SetTreeAddress(ulong tree) =>
            Pack.UInt64_To_BE(tree, m_value, OffsetTree + 4);

        internal void SetTreeHeight(uint height) =>
            Pack.UInt32_To_BE(height, m_value, OffsetTreeHgt);

        internal void SetTreeIndex(uint index) =>
            Pack.UInt32_To_BE(index, m_value, OffsetTreeIndex);
        ...
        internal void SetKeyPairAddress(uint keyPairAddr) =>
            Pack.UInt32_To_BE(keyPairAddr, m_value, OffsetKPAddr);

        internal uint GetKeyPairAddress() => Pack.BE_To_UInt32(m_value, OffsetKPAddr);

        internal void SetHashAddress(uint hashAddr) =>
            Pack.UInt32_To_BE(hashAddr, m_value, OffsetHashAddr);

        internal void SetChainAddress(uint chainAddr) =>
            Pack.UInt32_To_BE(chainAddr, m_value, OffsetChainAddr);

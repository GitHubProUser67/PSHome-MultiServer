...
namespace Org.BouncyCastle.Crypto.Signers
{
    public class RsaDigestSigner : ISigner
    {
        ...
        private static readonly IDictionary<string, DerObjectIdentifier> OidMap = new Dictionary<
            string,
            DerObjectIdentifier
        ...
        private static byte[] DerEncode(AlgorithmIdentifier digestAlgID, byte[] hash) =>
            new DigestInfo(digestAlgID, DerOctetString.WithContents(hash)).GetEncoded(
                Asn1Encodable.Der
            );

        private static bool TryGetAltAlgID(
            AlgorithmIdentifier algID,
            out AlgorithmIdentifier altAlgID
        )
        {

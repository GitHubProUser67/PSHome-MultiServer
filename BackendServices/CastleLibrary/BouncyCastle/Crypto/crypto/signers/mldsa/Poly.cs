...
namespace Org.BouncyCastle.Crypto.Signers.MLDsa
{
    internal class Poly
    {
        ...
        public Poly(MLDsaEngine engine)
        {
            ...
            m_polyUniformNBlocks =
                (768 + m_symmetric.Stream128BlockBytes - 1) / m_symmetric.Stream128BlockBytes;
        }
        ...
        public void UniformEta(byte[] seed, ushort nonce)
        {
            ...
            if (eta == 2)
            {
                PolyUniformEtaNBlocks =
                    (136 + m_symmetric.Stream256BlockBytes - 1) / m_symmetric.Stream256BlockBytes;
            }
            else if (eta == 4)
            {
                PolyUniformEtaNBlocks =
                    (227 + m_symmetric.Stream256BlockBytes - 1) / m_symmetric.Stream256BlockBytes;
            }
            else
            {
        ...
        public void PointwiseMontgomery(Poly v, Poly w)
        {
            for (int i = 0; i < N; ++i)
            {
                m_coeffs[i] = Reduce.MontgomeryReduce(
                    (long)((long)v.m_coeffs[i] * (long)w.m_coeffs[i])
                );
            }
        }
        ...
        {
            byte[] buf = new byte[
                m_engine.PolyUniformGamma1NBytes * m_symmetric.Stream256BlockBytes
            ];
            m_symmetric.Stream256Init(seed, nonce);
        ...
        public void Decompose(Poly a) =>
            Rounding.DecomposeAll(a.m_coeffs, m_coeffs, m_engine.Gamma2);
        ...
        internal void PackW1(byte[] r, int off)
        {
            ...
            if (gamma2 == (MLDsaEngine.Q - 1) / 88)
            {
                ...
                {
                    r[off + 3 * i + 0] = (byte)(
                        ((byte)m_coeffs[4 * i + 0]) | (m_coeffs[4 * i + 1] << 6)
                    );
                    r[off + 3 * i + 1] = (byte)(
                        (byte)(m_coeffs[4 * i + 1] >> 2) | (m_coeffs[4 * i + 2] << 4)
                    );
                    r[off + 3 * i + 2] = (byte)(
                        (byte)(m_coeffs[4 * i + 2] >> 4) | (m_coeffs[4 * i + 3] << 2)
                    );
                }
            }

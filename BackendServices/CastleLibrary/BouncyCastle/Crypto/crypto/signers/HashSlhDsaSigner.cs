...
namespace Org.BouncyCastle.Crypto.Signers
{
    public sealed class HashSlhDsaSigner : ISigner
    {
        ...
        {
            parameters = ParameterUtilities.GetContext(
                parameters,
                minLen: 0,
                maxLen: 255,
                out var providedContext
            );
            ...
            if (forSigning)
            {
                ...
                m_random = m_deterministic
                    ? null
                    : CryptoServicesRegistrar.GetSecureRandom(providedRandom);
        ...
        public byte[] GenerateSignature()
        {
            if (m_privateKey == null)
                throw new InvalidOperationException(
                    "HashSlhDsaSigner not initialised for signature generation."
                );
        ...
        public bool VerifySignature(byte[] signature)
        {
            if (m_publicKey == null)
                throw new InvalidOperationException(
                    "HashSlhDsaSigner not initialised for verification"
                );
        ...
        private sealed class Buffer : MemoryStream
        {
            ...
            internal byte[] GenerateSignature(
            ...
            )
            {
                lock (this)
                {
                    ...
                    byte[] addrnd =
                        random == null ? null : SecureRandom.GetNextBytes(random, engine.N);
            ...
            {
                lock (this)
                    TruncateAndClear(newLength: m_prefixLength);
            }

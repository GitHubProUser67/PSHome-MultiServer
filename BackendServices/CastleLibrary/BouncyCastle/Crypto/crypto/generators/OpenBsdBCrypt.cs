    ...
    public class OpenBsdBCrypt
    {
        ...
        )
        {
            if (!AllowedVersions.Contains(version))
                throw new ArgumentException(
                    "Version " + version + " is not accepted by this implementation.",
                    "version"
                );
        ...
        public static bool CheckPassword(string bcryptString, char[] password)
        {
            ...
            if (bcryptString.Length != 60)
                throw new DataLengthException(
                    "Bcrypt String length: " + bcryptString.Length + ", 60 required."
                );
            ...
            if (!AllowedVersions.Contains(version))
                throw new ArgumentException(
                    "Bcrypt version '" + version + "' is not supported by this implementation",
                    "bcryptString"
                );

            if (!int.TryParse(bcryptString.AsSpan(4, 2), out int cost))
                throw new ArgumentException(
                    "Invalid cost factor: " + bcryptString.Substring(4, 2),
                    nameof(bcryptString)
                );

            if (cost < 4 || cost > 31)
                throw new ArgumentException(
                    "Invalid cost factor: " + cost + ", 4 < cost < 31 expected."
                );
        ...
        private static void EncodeData(StringBuilder sb, byte[] data)
        {
            if (data.Length != 24 && data.Length != 16) // 192 bit key or 128 bit salt expected
                throw new DataLengthException(
                    "Invalid length: " + data.Length + ", 24 for key or 16 for salt expected"
                );
        ...
        private static byte[] DecodeSaltString(string saltString)
        {
            if (saltString.Length != 22) // bcrypt salt must be 22 (16 bytes)
                throw new DataLengthException(
                    "Invalid base64 salt length: " + saltString.Length + " , 22 required."
                );
            ...
            for (int i = 0; i < 22; i++)
            {
                ...
                if (value > 122 || value < 46 || (value > 57 && value < 65))
                    throw new ArgumentException(
                        "Salt string contains invalid character: " + value,
                        "saltString"
                    );
            }

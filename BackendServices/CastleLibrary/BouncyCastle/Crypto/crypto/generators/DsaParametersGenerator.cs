    ...
    public class DsaParametersGenerator
    {
        ...
        public virtual void Init(int size, int certainty, SecureRandom random)
        {
            if (!IsValidDsaStrength(size))
                throw new ArgumentException(
                    "size must be from 512 - 1024 and a multiple of 64",
                    "size"
                );
        ...
        public virtual void Init(DsaParameterGenerationParameters parameters)
        {
            ...
            if ((L < 1024 || L > 3072) || L % 1024 != 0)
                throw new ArgumentException(
                    "Values must be between 1024 and 3072 and a multiple of 1024",
                    "L"
                );
            if (L == 1024 && N != 160)
                        ...
                        if (usageIndex >= 0)
                        {
                            ...
                            if (g != null)
                                return new DsaParameters(
                                    ...
                                    q,
                                    g,
                                    new DsaValidationParameters(seed, counter, usageIndex)
                                );
                        }

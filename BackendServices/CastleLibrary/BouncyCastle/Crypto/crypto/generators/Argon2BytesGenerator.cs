...
namespace Org.BouncyCastle.Crypto.Generators
{
    public sealed class Argon2BytesGenerator
    {
        ...
        {
            if (
                parameters.Version != Argon2Parameters.Version10
            ...
            if (
                parameters.Type != Argon2Parameters.Argon2d
            ...
            if (parameters.Parallelism < MinParallelism)
                throw new InvalidOperationException(
                    "parallelism must be at least " + MinParallelism
                );
            if (parameters.Parallelism > MaxParallelism)
                throw new InvalidOperationException(
                    "parallelism must be at most " + MaxParallelism
                );
            ...
            int memoryBlocks = System.Math.Max(
                parameters.Memory,
                2 * Argon2SyncPoints * parameters.Parallelism
            );
        ...
        public int GenerateBytes(byte[] password, byte[] output) =>
            GenerateBytes(password, output, 0, output.Length);
        ...
        private int GetRefColumn(Position position, int index, ulong pseudoRandom, bool sameLane)
        {
            ...
            if (position.pass == 0)
            {
                ...
                else
                {
                    ...
                    referenceAreaSize = (ulong)(
                        position.slice * segmentLength + ((index == 0) ? (-1) : 0)
                    );
                }
            }
            else
        ...
        private void FillFirstBlocks(byte[] tmpBlockBytes, byte[] initialHashWithZeros)
        {
            ...
            for (int i = 0; i < parameters.Parallelism; i++)
            {
                ...
                if (EndianTools.EndianAwareConverter.isLittleEndianSystem)
                {
                    Span<byte> memorySpanZero = MemoryMarshal.AsBytes(
                        memory[i * laneLength + 0].v.AsSpan()
                    );
                    Span<byte> memorySpanOne = MemoryMarshal.AsBytes(
                        memory[i * laneLength + 1].v.AsSpan()
                    );

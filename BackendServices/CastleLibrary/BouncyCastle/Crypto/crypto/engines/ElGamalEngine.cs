    ...
    */
    public class ElGamalEngine : IAsymmetricBlockCipher
    {
        ...
        */
        ...
        {
            this.key = (ElGamalKeyParameters)
                ParameterUtilities.GetRandom(parameters, out var providedRandom);
            this.random = forEncryption
            ...
            if (forEncryption)
            {
                if (!(key is ElGamalPublicKeyParameters))
                    throw new ArgumentException(
                        "ElGamalPublicKeyParameters are required for encryption."
                    );
            }
            else
            {
                if (!(key is ElGamalPrivateKeyParameters))
                    throw new ArgumentException(
                        "ElGamalPrivateKeyParameters are required for decryption."
                    );
            }
        }
        ...
        */
        public virtual byte[] ProcessBlock(byte[] input, int inOff, int length)
        {
            ...
            if (key is ElGamalPrivateKeyParameters) // decryption
            {
                ...
                BigInteger m = gamma
                    .ModPow(p.Subtract(BigInteger.One).Subtract(priv.X), p)
                    .Multiply(phi)
                    .Mod(p);

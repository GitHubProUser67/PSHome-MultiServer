...
namespace Org.BouncyCastle.Crypto.Engines
{
    ...
    public class SM2Engine
    {
        ...
        private byte[] Encrypt(ReadOnlySpan<byte> input)
        {
            ...
            Span<byte> c3 = digestSize <= 128 ? stackalloc byte[digestSize] : new byte[digestSize];
            mDigest.DoFinal(c3);
        ...
        private void Kdf(IDigest digest, ECPoint c1, byte[] encData)
        {
            ...
            Span<byte> buf = bufSize <= 128 ? stackalloc byte[bufSize] : new byte[bufSize];
            int off = 0;
        ...
        private void AddFieldElement(IDigest digest, ECFieldElement v)
        {
            ...
            Span<byte> p =
                encodedLength <= 128 ? stackalloc byte[encodedLength] : new byte[encodedLength];
            v.EncodeTo(p);

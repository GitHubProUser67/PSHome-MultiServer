...
namespace Org.BouncyCastle.Crypto.Engines
{
    ...
    public class Salsa20Engine : IStreamCipher
    {
        ...
        private static readonly uint[] TAU_SIGMA = Pack.LE_To_UInt32(
            Strings.ToAsciiByteArray("expand 16-byte k" + "expand 32-byte k"),
            0,
            8
        );
        ...
        public virtual void Init(bool forEncryption, ICipherParameters parameters)
        {
            ...
            if (!(parameters is ParametersWithIV ivParams))
                throw new ArgumentException(
                    AlgorithmName + " Init requires an IV",
                    nameof(parameters)
                );
            ...
            if (iv == null || iv.Length != NonceSize)
                throw new ArgumentException(
                    AlgorithmName + " requires exactly " + NonceSize + " bytes of IV"
                );
        ...
        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        private static void QuarterRound_Sse2(

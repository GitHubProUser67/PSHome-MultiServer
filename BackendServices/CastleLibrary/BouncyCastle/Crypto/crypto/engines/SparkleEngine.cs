...
namespace Org.BouncyCastle.Crypto.Engines
{
    ...
    public sealed class SparkleEngine : IAeadCipher
    {
        ...
        public void Init(bool forEncryption, ICipherParameters parameters)
        {
            ...
            if (expectedKeyLength != key.KeyLength)
                throw new ArgumentException(
                    algorithmName + " requires exactly " + expectedKeyLength + " bytes of key"
                );

            int expectedIVLength = RATE_WORDS * 4;
            if (expectedIVLength != iv.Length)
                throw new ArgumentException(
                    algorithmName + " requires exactly " + expectedIVLength + " bytes of IV"
                );
        ...
        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        private static void ArxBox(
            Vector128<uint> rc,
            ref Vector128<uint> s00,
            ref Vector128<uint> s01
        )
        {

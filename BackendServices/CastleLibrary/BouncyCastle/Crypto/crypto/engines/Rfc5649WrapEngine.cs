    ...
    public class Rfc5649WrapEngine : IWrapper
    {
        ...
        public virtual byte[] Wrap(byte[] input, int inOff, int length)
        {
            ...
            if (paddedPlaintext.Length == 8)
            {
                ...
                for (
                    int i = 0, blockSize = m_engine.GetBlockSize();
                    i < paddedPlainTextWithIV.Length;
                    i += blockSize
                )
                {
        ...
        public virtual byte[] Unwrap(byte[] input, int inOff, int length)
        {
            ...
            Array.Copy(
                paddedPlaintext,
                paddedPlaintext.Length - expectedZeros,
                pad,
                0,
                expectedZeros
            );
         ...
         */
        private byte[] Rfc3394UnwrapNoIvCheck(
            byte[] input,
            int inOff,
            int inLen,
            byte[] extractedAIV

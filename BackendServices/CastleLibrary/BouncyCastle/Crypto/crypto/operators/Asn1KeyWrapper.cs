...
namespace Org.BouncyCastle.Crypto.Operators
{
    public class Asn1KeyWrapper : IKeyWrapper
    {
        ...
        )
        {
            ...
            if (algorithm.Equals(PkcsObjectIdentifiers.IdRsaesOaep))
            {
                ...
                {
                    AlgorithmIdentifier digAlg = AlgorithmIdentifier.GetInstance(
                        oaepParams.MaskGenAlgorithm.Parameters
                    );

                    provider = new RsaOaepWrapperProvider(
    ...
    internal class KeyWrapperUtil
    {
        ...
        // Provider
        //
        private static readonly Dictionary<string, WrapperProvider> m_providerMap = new Dictionary<
            string,
            WrapperProvider
        ...
        static KeyWrapperUtil()
        {
            ...
            m_providerMap.Add(
                "RSA/NONE/OAEPWITHSHA256ANDMGF1WITHSHA1PADDING",
                new RsaOaepWrapperProvider(
        ...
        public static IKeyUnwrapper UnwrapperForName(string algorithm, ICipherParameters parameters)
        {
            if (!m_providerMap.TryGetValue(algorithm, out var provider))
                throw new ArgumentException(
                    "could not resolve " + algorithm + " to a KeyUnwrapper"
                );
    ...
    internal class RsaOaepWrapper : IKeyWrapper, IKeyUnwrapper
    {
        ...
        )
        {
            ...
            if (
                mgfOid.Equals(NistObjectIdentifiers.IdShake128)
                || mgfOid.Equals(NistObjectIdentifiers.IdShake256)
            )
            {
                this.algId = new AlgorithmIdentifier(
            ...
            this.engine = new OaepEncoding(
                new RsaBlindedEngine(),
                DigestUtilities.GetDigest(digestOid),
                DigestUtilities.GetDigest(mgfOid),
                null
            );
            this.engine.Init(forWrapping, parameters);
        }

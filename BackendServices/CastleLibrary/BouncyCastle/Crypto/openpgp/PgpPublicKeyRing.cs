    ...
    public class PgpPublicKeyRing : PgpKeyRing
    {
                ...
                if (pubKey.IsMasterKey)
                {
                    if (masterFound)
                        throw new ArgumentException(
                            "cannot add a master key to a ring that already has one"
                        );
        ...
        )
        {
            if (!second.GetPublicKey().HasFingerprint(first.GetPublicKey().GetFingerprint()))
                throw new ArgumentException(
                    "Cannot merge certificates with differing primary keys."
                );
            ...
            foreach (var key in first.keys)
            {
                ...
                {
                    merged.Add(
                        PgpPublicKey.Join(key, copy, joinTrustPackets, allowSubkeySigsOnNonSubkey)
                    );

...
namespace Org.BouncyCastle.Bcpg.OpenPgp
{
    ...
    public class PgpSignature
    {
        ...
        public void InitVerify(PgpPublicKey pubKey)
        {
            ...
            if (sig == null)
            {
                this.sig = PgpUtilities.CreateSigner(
                    sigPck.KeyAlgorithm,
                    sigPck.HashAlgorithm,
                    key
                );
            }
        ...
        private void UpdateWithPublicKey(PgpPublicKey key)
        {
            ...
            this.Update((byte)0x99, (byte)(keyBytes.Length >> 8), (byte)(keyBytes.Length));
                ...
                else
                {
                    ...
                    try
                    {
                        signature = new DerSequence(
                            ...
                            new DerInteger(sigValues[1].Value)
                        ).GetEncoded();
                    }
                    catch (IOException e)
                    {
        ...
        public void Encode(Stream outStream, bool forTransfer)
        {
            ...
            if (
                forTransfer
                && (
                    !GetHashedSubPackets().IsExportable() || !GetUnhashedSubPackets().IsExportable()
                )
            )
                return;

...
namespace Org.BouncyCastle.Bcpg.OpenPgp
{
    ...
    public class PgpPublicKey : PgpObject
    {
        ...
        private static readonly PgpKdfParameters DefaultKdfParameters = new PgpKdfParameters(
            ...
            else
            {
                ...
                else if (key is EdDsaPublicBcpgKey eddsaK)
                {
                    ...
                    )
                    {
                        ...
                        EdECObjectIdentifiers.id_Ed448.Equals(curveOid)
        ...
        )
        {
            ...
            else if (pubKey is Ed25519PublicKeyParameters ed25519PubKey)
            {
                ...
                bcpgKey = new EdDsaPublicBcpgKey(
            ...
            get
            {
                ...
                return !(publicPk is PublicSubkeyPacket)
                    && !(
                        ...
                        && publicPk.Algorithm != PublicKeyAlgorithmTag.RsaGeneral
                    );
            }
        }
                ...
                switch (publicPk.Algorithm)
                {
                    ...
                    case PublicKeyAlgorithmTag.Dsa:
                        ...
                        return new DsaPublicKeyParameters(
                            dsaK.Y,
                            new DsaParameters(dsaK.P, dsaK.Q, dsaK.G)
                        );
                    ...
                    case PublicKeyAlgorithmTag.ECDH:
                    {
                        ...
                        {
                            byte[] pEnc = BigIntegers.AsUnsignedByteArray(
                                1 + X25519.PointSize,
                                ecdhK.EncodedPoint
                            );
                            if (pEnc[0] != 0x40)
                    ...
                    case PublicKeyAlgorithmTag.ElGamalGeneral:
                        ...
                        return new ElGamalPublicKeyParameters(
                            elK.Y,
                            new ElGamalParameters(elK.P, elK.G)
                        );
                    default:
        ...
        private ECPublicKeyParameters GetECKey(string algorithm, ECPublicBcpgKey ecK)
        {
            ...
            Span<byte> encoding =
                encodedLength <= 512 ? stackalloc byte[encodedLength] : new byte[encodedLength];
        ...
        public bool IsRevoked()
        {
            ...
            else // Sub-key
            {
                ...
                {
                    if (
                        ((PgpSignature)subSigs[ns++]).SignatureType == PgpSignature.SubkeyRevocation
                    )
                    {
                        revoked = true;
                    }
            ...
            if (key.IsMasterKey)
            {
                if (certification.SignatureType == PgpSignature.SubkeyRevocation)
                    throw new ArgumentException(
                        "signature type incorrect for master key revocation."
                    );
            }
            else
            {
        ...
        )
        {
            ...
            List<PgpSignature> subSigs =
                key.subSigs == null ? null : new List<PgpSignature>(key.subSigs);
            ...
            for (int idIdx = 0; idIdx < copy.ids.Count; idIdx++)
            {
                ...
                if (joinTrustPackets && copyTrust != null)
                {
                    ...
                    if (
                        existingTrust == null
                        || Arrays.AreEqual(
                            copyTrust.GetLevelAndTrustAmount(),
                            existingTrust.GetLevelAndTrustAmount()
                        )
                    )
                    {

...
namespace Org.BouncyCastle.Bcpg.OpenPgp
{
    ...
    public class PgpSecretKey : PgpObject
    {
        ...
        )
        {
            ...
            switch (pubKey.Algorithm)
            {
                ...
                case PublicKeyAlgorithmTag.ECDH:
                {
                    ...
                    else
                    {
                        ...
                        secKey = new ECSecretBcpgKey(
                            new BigInteger(1, xK.DataSpan, bigEndian: false)
                        );
                    }
                    break;
                }
            ...
            try
            {
                ...
                if (encAlgorithm == SymmetricKeyAlgorithmTag.Null)
                {
                    if (isMasterKey)
                    {
                        this.secret = new SecretKeyPacket(
                            pub.publicPk,
                            encAlgorithm,
                            null,
                            null,
        ...
        internal PgpSecretKey(
            ...
            : this(
                keyPair.PrivateKey,
                CertifiedPublicKey(certificationLevel, keyPair, id, hashedPackets, unhashedPackets),
                encAlgorithm,
                rawPassPhrase,
                clearPassPhrase,
                useSha1,
        ...
        private byte[] ExtractKeyData(byte[] rawPassPhrase, bool clearPassPhrase)
        {
            ...
            try
            {
                ...
                if (secret.PublicKeyPacket.Version >= 4)
                {
                    ...
                    if (
                        ...
                        throw new PgpException(
                            "Checksum mismatch in checksum of " + check.Length + " bytes"
                        );
                }
                else // version 2 or 3, RSA only.
                {
                    ...
                    {
                        int encLen =
                            ((((encData[pos] & 0xff) << 8) | (encData[pos + 1] & 0xff)) + 7) / 8;
        ...
        public PgpPrivateKey ExtractPrivateKey(char[] passPhrase) =>
            DoExtractPrivateKey(
                PgpUtilities.EncodePassPhrase(passPhrase, utf8: false),
        ...
        public PgpPrivateKey ExtractPrivateKeyUtf8(char[] passPhrase) =>
            DoExtractPrivateKey(
                PgpUtilities.EncodePassPhrase(passPhrase, utf8: true),
                clearPassPhrase: true
            );
                ...
                switch (pubPk.Algorithm)
                {
                    ...
                    case PublicKeyAlgorithmTag.ECDH:
                    {
                        ...
                        )
                        {
                            ...
                            privateKey = PrivateKeyFactory.CreateKey(
                                new PrivateKeyInfo(
                                    new AlgorithmIdentifier(curveOid),
                                    new DerOctetString(
                                        Arrays.ReverseInPlace(
                                            BigIntegers.AsUnsignedByteArray(ecdhPriv.X)
                                        )
                                    )
                                )
                            );
                    ...
                    case PublicKeyAlgorithmTag.EdDsa_Legacy:
                    {
                        ...
                        if (
                            EdECObjectIdentifiers.id_Ed25519.Equals(curveOid)
        ...
        )
        {
            if (key.IsPrivateKeyEmpty)
                throw new PgpException(
                    "no private key in this SecretKey - public key present only."
                );
        ...
        )
        {
            ...
            KeyParameter encKey = PgpUtilities.DoMakeKeyFromPassPhrase(
                encAlgorithm,
                s2k,
                rawPassPhrase,
                clearPassPhrase
            ...
            {
                int encLen =
                    ((((rawKeyData[pos] & 0xff) << 8) | (rawKeyData[pos + 1] & 0xff)) + 7) / 8;
        ...
        )
        {
            ...
            return DoParseSecretKeyFromSExpr(
                inputStream,
                rawPassPhrase,
                clearPassPhrase: true,
                pubKey
        ...
        private static PgpSecretKey CreateECSecretKey(PgpPublicKey pubKey, byte[] dValue)
        {
            ...
            var secretKeyPacket = new SecretKeyPacket(

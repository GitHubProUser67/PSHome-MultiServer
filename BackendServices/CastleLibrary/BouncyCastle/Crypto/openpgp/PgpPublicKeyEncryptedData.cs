    ...
    public class PgpPublicKeyEncryptedData : PgpEncryptedData
    {
        ...
        public Stream GetDataStream(PgpPrivateKey privKey)
        {
            ...
            try
            {
                ...
                cipher.Init(
                    forEncryption: false,
                    new ParametersWithIV(key, iv: new byte[cipher.GetBlockSize()])
                );
        ...
        private byte[] RecoverSessionData(PgpPrivateKey privKey)
        {
            ...
            if (m_keyData.Algorithm != PublicKeyAlgorithmTag.ECDH)
            {
                ...
                if (
                    m_keyData.Algorithm == PublicKeyAlgorithmTag.RsaEncrypt
            ...
            else
            {
                ...
                ECPublicKeyParameters ephPub = new ECPublicKeyParameters(
                    ecParameters.Curve.DecodePoint(pEnc),
                    ecParameters
                );
            ...
            KeyParameter key = new KeyParameter(
                Rfc6637Utilities.CreateKey(privKey.PublicKeyPacket, secret)
            );

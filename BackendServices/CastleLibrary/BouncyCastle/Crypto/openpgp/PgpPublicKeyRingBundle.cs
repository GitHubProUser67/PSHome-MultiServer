...
namespace Org.BouncyCastle.Bcpg.OpenPgp
{
    ...
    public class PgpPublicKeyRingBundle
    {
            ...
            foreach (var obj in e)
            {
                ...
                if (!(obj is PgpPublicKeyRing pgpPub))
                    throw new PgpException(
                        Platform.GetTypeName(obj) + " found where PgpPublicKeyRing expected"
                    );
        ...
        )
        {
            ...
            if (bundle.m_pubRings.ContainsKey(key))
                throw new ArgumentException(
                    "Bundle already contains a key with a keyId for the passed in ring."
                );
        ...
        )
        {
            ...
            if (!bundle.m_pubRings.ContainsKey(key))
                throw new ArgumentException(
                    "Bundle does not contain a key with a keyId for the passed in ring."
                );

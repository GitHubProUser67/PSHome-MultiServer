...
namespace Org.BouncyCastle.Bcpg.OpenPgp
{
    ...
    public class PgpEncryptedDataGenerator : IStreamGenerator
    {
        ...
        private class PubMethod : EncMethod
        {
            ...
            private byte[] EncryptSessionInfo(byte[] sessionInfo, SecureRandom random)
            {
                    ...
                    switch (pubKey.Algorithm)
                    {
                        ...
                        default:
                            throw new PgpException(
                                "unknown asymmetric algorithm: " + pubKey.Algorithm
                            );
                    }
                ...
                else if (EdECObjectIdentifiers.id_X448.Equals(curveOid))
                {
                    ...
                    Span<byte> ephPubEncoding =
                        stackalloc byte[1 + X448PublicKeyParameters.KeySize];
                ...
                else
                {
                    ...
                    ECDomainParameters ecParams = (
                        (ECPublicKeyParameters)cryptoPublicKey
                    ).Parameters;
            ...
            private byte[] EncryptSessionInfo(
                ...
                byte[] sessionInfo,
                byte[] secret,
                ReadOnlySpan<byte> ephPubEncoding,
                SecureRandom random
            )
            {
                var key = new KeyParameter(
                    Rfc6637Utilities.CreateKey(pubKey.PublicKeyPacket, secret)
                );

                IWrapper w = PgpUtilities.CreateWrapper(ecPubKey.SymmetricKeyAlgorithm);
                w.Init(true, new ParametersWithRandom(key, random));

                byte[] paddedSessionData = PgpPad.PadSessionData(
                    sessionInfo,
            ...
            private byte[][] ProcessSessionInfo(byte[] encryptedSessionInfo)
            {
                ...
                switch (pubKey.Algorithm)
                {
                    ...
                    case PublicKeyAlgorithmTag.ElGamalGeneral:
                        ...
                        data = new byte[2][] { ConvertToEncodedMpi(b1), ConvertToEncodedMpi(b2) };
                        break;
        ...
        internal void ImplAddPbeMethod(
            byte[] rawPassPhrase,
        ...
        )
        {
            ...
            var key = PgpUtilities.DoMakeKeyFromPassPhrase(
                defAlgorithm,
                s2k,
                rawPassPhrase,
                clearPassPhrase
        ...
        private Stream Open(Stream outStr, long length, byte[] buffer)
        {
            ...
            try
            {
                ...
                if (buffer == null)
                {
                    ...
                    if (withIntegrityPacket)
                    {
                        pOut = new BcpgOutputStream(
                            outStr,
                            PacketTag.SymmetricEncryptedIntegrityProtected,
                            length + c.GetBlockSize() + 2 + 1 + 22
                        );
                    ...
                    else
                    {
                        pOut = new BcpgOutputStream(
                            outStr,
                            PacketTag.SymmetricKeyEncrypted,
                            length + c.GetBlockSize() + 2,
                            oldFormat
                        );
                    }
                }

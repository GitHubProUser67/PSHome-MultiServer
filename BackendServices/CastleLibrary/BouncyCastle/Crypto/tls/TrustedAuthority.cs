...
namespace Org.BouncyCastle.Tls
{
    public sealed class TrustedAuthority
    {
        ...
        public TrustedAuthority(short identifierType, object identifier)
        {
            if (!IsCorrectType(identifierType, identifier))
                throw new ArgumentException(
                    "not an instance of the correct type",
                    nameof(identifier)
                );
        ...
        {
            if (
                m_identifierType != expectedIdentifierType
                || !IsCorrectType(expectedIdentifierType, m_identifier)
            )
                throw new InvalidOperationException(
                    "TrustedAuthority is not of type "
        ...
        private static bool IsSha1Hash(object identifier) =>
            identifier is byte[] bytes && bytes.Length == 20;
    }
}

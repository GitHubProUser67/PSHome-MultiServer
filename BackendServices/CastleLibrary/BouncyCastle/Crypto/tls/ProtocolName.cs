...
namespace Org.BouncyCastle.Tls
{
    ...
    public sealed class ProtocolName
    {
        public static ProtocolName AsRawBytes(byte[] bytes) =>
            new ProtocolName(Arrays.Clone(bytes));

        public static ProtocolName AsUtf8Encoding(string name) =>
            new ProtocolName(Strings.ToUtf8ByteArray(name));
        ...
        public static readonly ProtocolName Stun_Nat_Discovery = AsUtf8Encoding(
            "stun.nat-discovery"
        );
        ...
        public static ProtocolName Parse(Stream input) =>
            new ProtocolName(TlsUtilities.ReadOpaque8(input, 1));

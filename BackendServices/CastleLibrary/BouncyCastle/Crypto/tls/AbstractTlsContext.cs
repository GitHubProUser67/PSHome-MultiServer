...
namespace Org.BouncyCastle.Tls
{
    internal abstract class AbstractTlsContext : TlsContext
    {
        ...
        public virtual byte[] ExportChannelBinding(int channelBinding)
        {
            if (!IsConnected)
                throw new InvalidOperationException(
                    "Export of channel bindings unavailable before handshake completion"
                );
            ...
            switch (channelBinding)
            {
                case ChannelBinding.tls_server_end_point:
                {
                    ...
                    return TlsUtilities.IsNullOrEmpty(tlsServerEndPoint)
                        ? null
                        : Arrays.Clone(tlsServerEndPoint);
                }
        ...
        public virtual byte[] ExportEarlyKeyingMaterial(
            string asciiLabel,
            byte[] context,
            int length
        )
        {
            ...
            if (!IsConnected)
                throw new InvalidOperationException(
                    "Export of early key material only available during handshake"
                );

            SecurityParameters sp = SecurityParameters;

            return ExportKeyingMaterial13(
                CheckEarlyExportSecret(sp.EarlyExporterMasterSecret),
                sp.PrfCryptoHashAlgorithm,
                asciiLabel,
                context,
                length
            );
        ...
        public virtual byte[] ExportKeyingMaterial(string asciiLabel, byte[] context, int length)
        {
            if (!IsConnected)
                throw new InvalidOperationException(
                    "Export of key material unavailable before handshake completion"
                );
            ...
            if (!sp.IsExtendedMasterSecret)
            {
                 ...
                 */
                throw new InvalidOperationException(
                    "Export of key material requires extended_master_secret"
                );
            }
            ...
            {
                return ExportKeyingMaterial13(
                    CheckExportSecret(sp.ExporterMasterSecret),
                    sp.PrfCryptoHashAlgorithm,
                    asciiLabel,
                    context,
                    length
            ...
            return TlsUtilities
                .Prf(sp, CheckExportSecret(sp.MasterSecret), asciiLabel, seed, length)
                .Extract();
        }
        ...
        )
        {
            ...
            TlsSecret exporterSecret = TlsUtilities.DeriveSecret(
                SecurityParameters,
                secret,
                asciiLabel,
                emptyTranscriptHash
        ...
        protected virtual TlsSecret CheckEarlyExportSecret(TlsSecret secret)
        {
            if (null == secret)
            {
                ...
                throw new InvalidOperationException(
                    "Export of early key material not available for this handshake"
                );
            }
            return secret;
        }

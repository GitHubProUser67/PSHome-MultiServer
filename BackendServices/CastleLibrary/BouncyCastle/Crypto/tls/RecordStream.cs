...
namespace Org.BouncyCastle.Tls
{
    ...
    internal sealed class RecordStream
    {
        ...
        private readonly SequenceNumber m_readSeqNo = new SequenceNumber(),
            m_writeSeqNo = new SequenceNumber();
        ...
        internal RecordPreview PreviewRecordHeader(byte[] recordHeader)
        {
            ...
            if (ContentType.application_data == recordType && m_handler.IsApplicationDataReady)
            {
                ...
                applicationDataLimit = System.Math.Max(
                    0,
                    System.Math.Min(m_plaintextLimit, plaintextDecodeLimit)
                );
            }
        ...
        internal int PreviewOutputRecordSize(int contentLength)
        {
            ...
            return RecordFormat.FragmentOffset
                + m_writeCipher.GetCiphertextEncodeLimit(contentLength, m_plaintextLimit);
        }
        ...
        internal bool ReadFullRecord(byte[] input, int inputOff, int inputLen)
        {
            ...
            ProtocolVersion recordVersion = TlsUtilities.ReadVersion(
                input,
                inputOff + RecordFormat.VersionOffset
            );
            ...
            TlsDecodeResult decoded = DecodeAndVerify(
                recordType,
                recordVersion,
                input,
                inputOff + RecordFormat.FragmentOffset,
                length
            );
        ...
        internal bool ReadRecord()
        {
            ...
            try
            {
                ...
                decoded = DecodeAndVerify(
                    recordType,
                    recordVersion,
                    m_inputRecord.m_buf,
                    RecordFormat.FragmentOffset,
                    length
                );
            }
            finally
        ...
        )
        {
            ...
            TlsDecodeResult decoded = m_readCipher.DecodeCiphertext(
                seqNo,
                recordType,
                recordVersion,
                ciphertext,
        ...
        internal void WriteRecord(short contentType, ReadOnlySpan<byte> plaintext)
        {
            ...
            TlsEncodeResult encoded = m_writeCipher.EncodePlaintext(
                seqNo,
                contentType,
                recordVersion,
                RecordFormat.FragmentOffset,
            ...
            TlsUtilities.WriteUint8(
                encoded.recordType,
                encoded.buf,
                encoded.off + RecordFormat.TypeOffset
            );
            TlsUtilities.WriteVersion(
                recordVersion,
                encoded.buf,
                encoded.off + RecordFormat.VersionOffset
            );
            TlsUtilities.WriteUint16(
                ciphertextLength,
                encoded.buf,
                encoded.off + RecordFormat.LengthOffset
            );

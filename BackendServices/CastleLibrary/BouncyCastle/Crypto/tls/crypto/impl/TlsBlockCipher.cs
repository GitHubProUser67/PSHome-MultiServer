...
namespace Org.BouncyCastle.Tls.Crypto.Impl
{
    ...
    public class TlsBlockCipher : TlsCipher, TlsCipherExt
    {
        ...
        )
        {
             ...
             */
            this.m_useExtraPadding =
                securityParameters.IsExtendedPadding
            ...
            Span<byte> keyBlock =
                keyBlockSize <= 512 ? stackalloc byte[keyBlockSize] : new byte[keyBlockSize];
            TlsImplUtilities.CalculateKeyBlock(cryptoParams, keyBlock);

            clientMac.SetKey(keyBlock[..clientMac.MacLength]);
            keyBlock = keyBlock[clientMac.MacLength..];
            serverMac.SetKey(keyBlock[..serverMac.MacLength]);
            keyBlock = keyBlock[serverMac.MacLength..];

            clientCipher.SetKey(keyBlock[..cipherKeySize]);
            keyBlock = keyBlock[cipherKeySize..];
            serverCipher.SetKey(keyBlock[..cipherKeySize]);
            keyBlock = keyBlock[cipherKeySize..];
            ...
            if (m_useExplicitIV)
            {
                clientCipher.Init(
                    clientIVLength <= 64
                        ? stackalloc byte[clientIVLength]
                        : new byte[clientIVLength]
                );
                serverCipher.Init(
                    serverIVLength <= 64
                        ? stackalloc byte[serverIVLength]
                        : new byte[serverIVLength]
                );
            }
            else
            {
                clientCipher.Init(keyBlock[..clientIVLength]);
                keyBlock = keyBlock[clientIVLength..];
                serverCipher.Init(keyBlock[..serverIVLength]);
                keyBlock = keyBlock[serverIVLength..];
            }
        ...
        )
        {
            ...
            if (!m_encryptThenMac)
            {
                byte[] mac = m_writeMac.CalculateMac(
                    seqNo,
                    recordType,
                    m_encryptConnectionID,
        ...
        )
        {
            ...
            if (m_encryptThenMac)
            {
                byte[] expectedMac = m_readMac.CalculateMac(
                    seqNo,
                    recordType,
                ...
                bool checkMac = Arrays.FixedTimeEquals(
                    macSize,
                    expectedMac,
                    0,
                    ciphertext,
            ...
            int totalPad = CheckPaddingConstantTime(
                ciphertext,
                offset,
                blocks_length,
                blockSize,
            ...
            if (!m_encryptThenMac)
            {
                ...
                byte[] expectedMac = m_readMac.CalculateMacConstantTime(
                    seqNo,
                    recordType,
                    m_decryptConnectionID,
                    ciphertext,
        ...
        )
        {
            ...
            int totalPadLimit = System.Math.Min(
                m_acceptExtraPadding ? 256 : blockSize,
                len - macSize
            );

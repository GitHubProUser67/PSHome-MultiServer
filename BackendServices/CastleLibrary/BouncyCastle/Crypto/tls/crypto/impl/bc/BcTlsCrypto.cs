...
namespace Org.BouncyCastle.Tls.Crypto.Impl.BC
{
    ...
    public class BcTlsCrypto : AbstractTlsCrypto
    {
        ...
        public override TlsNonceGenerator CreateNonceGenerator(byte[] additionalSeedMaterial)
        {
            ...
            var seed =
                additionalSeedMaterial == null ? Span<byte>.Empty : additionalSeedMaterial.AsSpan();

            return CreateNonceGenerator(seed);
        }

        public override TlsNonceGenerator CreateNonceGenerator(
            ReadOnlySpan<byte> additionalSeedMaterial
        )
        {
            ...
            Span<byte> seed =
                seedLength <= 128 ? stackalloc byte[seedLength] : new byte[seedLength];
        ...
        public override bool HasSignatureAndHashAlgorithm(
        ...
        public override bool HasSignatureScheme(int signatureScheme)
        {
            switch (signatureScheme)
            {
                ...
                default:
                {
                    ...
                    {
                        case CryptoHashAlgorithm.md5:
                            return SignatureAlgorithm.rsa == signature
                                && HasSignatureAlgorithm(signature);
                        default:
                            return HasSignatureAlgorithm(signature);
                    }
                }
            ...
            switch (cryptoHashAlgorithm)
            {
                ...
                default:
                    throw new ArgumentException(
                        "invalid CryptoHashAlgorithm: " + cryptoHashAlgorithm
                    );
            }
        }
        ...
        )
        {
            ...
            BcTlsAeadCipherImpl decrypt = new BcTlsAeadCipherImpl(
        ...
        {
            BcTlsBlockCipherImpl encrypt = new BcTlsBlockCipherImpl(
                CreateCbcBlockCipher(encryptionAlgorithm),
                true
            );
            BcTlsBlockCipherImpl decrypt = new BcTlsBlockCipherImpl(
                CreateCbcBlockCipher(encryptionAlgorithm),
                false
            );
        ...
        protected virtual Tls13NullCipher Create13NullCipher(
            TlsCryptoParameters cryptoParams,
            int macAlgorithm
        ...
        public override TlsSrp6Server CreateSrp6Server(
            TlsSrpConfig srpConfig,
            BigInteger srpVerifier
        )
        {
            BigInteger[] ng = srpConfig.GetExplicitNG();
            Srp6GroupParameters srpGroup = new Srp6GroupParameters(ng[0], ng[1]);
            ...
            srp6Server.Init(
                srpGroup,
                srpVerifier,
                CreateDigest(CryptoHashAlgorithm.sha1),
                SecureRandom
            );
        ...
        public override TlsSecret HkdfInit(int cryptoHashAlgorithm)
        {
            return AdoptLocalSecret(
                new byte[TlsCryptoUtilities.GetHashOutputSize(cryptoHashAlgorithm)]
            );
        }
    }
}

...
namespace Org.BouncyCastle.Tls.Crypto.Impl.BC
{
    ...
    public class BcTlsSecret : AbstractTlsSecret
    {
        ...
        public override TlsSecret HkdfExpand(
        ...
        )
        {
            ...
            if (length > (255 * hashLen))
                throw new ArgumentException(
                    "must be <= 255 * (output size of 'hashAlgorithm')",
                    "length"
                );
            ...
            Span<byte> t = hashLen <= 128 ? stackalloc byte[hashLen] : new byte[hashLen];
            byte counter = 0x00;

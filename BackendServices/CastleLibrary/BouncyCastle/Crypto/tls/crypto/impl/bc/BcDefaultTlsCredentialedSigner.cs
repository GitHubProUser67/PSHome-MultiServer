    ...
    public class BcDefaultTlsCredentialedSigner : DefaultTlsCredentialedSigner
    {
        ...
        )
        {
            ...
            else if (privateKey is MLDsaPrivateKeyParameters mlDsaPrivateKey)
            {
                ...
                {
                    int signatureScheme = SignatureScheme.From(signatureAndHashAlgorithm);
                    TlsSigner signer = BcTlsMLDsaSigner.Create(
                        crypto,
                        mlDsaPrivateKey,
                        signatureScheme
                    );
                ...
                throw new ArgumentException(
                    "ML-DSA private key of wrong type for signature algorithm"
                );
            }
            else if (privateKey is SlhDsaPrivateKeyParameters slhDsaPrivateKey)
            {
                ...
                {
                    int signatureScheme = SignatureScheme.From(signatureAndHashAlgorithm);
                    TlsSigner signer = BcTlsSlhDsaSigner.Create(
                        crypto,
                        slhDsaPrivateKey,
                        signatureScheme
                    );
                ...
                throw new ArgumentException(
                    "SLH-DSA private key of wrong type for signature algorithm"
                );
            }
            else
            {
        ...
        public BcDefaultTlsCredentialedSigner(
            ...
            : base(
                cryptoParams,
                MakeSigner(crypto, privateKey, certificate, signatureAndHashAlgorithm),
                certificate,

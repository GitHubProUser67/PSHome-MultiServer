...
namespace Org.BouncyCastle.Tls.Crypto.Impl
{
    ...
    public class TlsAeadCipher : TlsCipher, TlsCipherExt
    {
        ...
        )
        {
            ...
            if (isServer)
            {
                decryptCipher.SetKey(keyBlock[..keySize]);
                keyBlock = keyBlock[keySize..];
                encryptCipher.SetKey(keyBlock[..keySize]);
                keyBlock = keyBlock[keySize..];

                keyBlock[..m_fixed_iv_length].CopyTo(m_decryptNonce);
                keyBlock = keyBlock[m_fixed_iv_length..];
                keyBlock[..m_fixed_iv_length].CopyTo(m_encryptNonce);
                keyBlock = keyBlock[m_fixed_iv_length..];
            }
            else
            {
        ...
        public virtual int GetPlaintextLimit(int ciphertextLimit) =>
            GetPlaintextEncodeLimit(ciphertextLimit);
        ...
        )
        {
            ...
            if (m_record_iv_length != 0)
            {
                Array.Copy(
                    nonce,
                    nonce.Length - m_record_iv_length,
                    output,
                    outputPos,
                    m_record_iv_length
                );
            ...
            try
            {
                ...
                outputPos += m_encryptCipher.DoFinal(
                    additionalData,
                    output,
                    outputPos,
                    innerPlaintextLength,
        ...
        )
        {
            ...
            try
            {
                outputPos = m_decryptCipher.DoFinal(
                    additionalData,
                    ciphertext,
                    encryptionOffset,
                    encryptionLength,
        ...
        public virtual void RekeyDecoder() =>
            RekeyCipher(
                m_cryptoParams.SecurityParameters,
                m_decryptCipher,
                m_decryptNonce,
                !m_cryptoParams.IsServer
            );
        ...
        private static TlsSecret HkdfExpandLabel(
            TlsSecret secret,
            int cryptoHashAlgorithm,
            string label,
            int length

...
namespace Org.BouncyCastle.Tls
{
    ...
    public abstract class AbstractTlsServer : AbstractTlsPeer, TlsServer
    {
        ...
        protected virtual int GetMaximumDefaultCurveBits() =>
            NamedGroup.GetCurveBits(NamedGroup.secp521r1);

        protected virtual int GetMaximumDefaultFiniteFieldBits() =>
            NamedGroup.GetFiniteFieldBits(NamedGroup.ffdhe8192);
        ...
        protected virtual int GetMaximumNegotiableCurveBits()
        {
            ...
            if (clientSupportedGroups != null)
            {
                ...
                {
                    maxBits = System.Math.Max(
                        maxBits,
                        NamedGroup.GetCurveBits(clientSupportedGroups[i])
                    );
                }
            }
            else
        ...
        protected virtual int GetMaximumNegotiableFiniteFieldBits()
        {
            ...
            if (clientSupportedGroups != null)
            {
                for (int i = 0; i < clientSupportedGroups.Length; ++i)
                {
                    ...
                    maxBits = System.Math.Max(
                        maxBits,
                        NamedGroup.GetFiniteFieldBits(clientSupportedGroups[i])
                    );
                }
            }
            if (!anyPeerFF)
        ...
        public virtual void NotifyFallback(bool isFallback)
        {
            ...
            if (isFallback)
            {
                ...
                if (
                    null != latestServerVersion
                    && latestServerVersion.IsLaterVersionOf(clientVersion)
                )
                {
        ...
        public virtual void ProcessClientExtensions(IDictionary<int, byte[]> clientExtensions)
        {
            ...
            {
                m_clientProtocolNames = TlsExtensionsUtilities.GetAlpnExtensionClient(
                    clientExtensions
                );
                ...
                {
                    m_encryptThenMACOffered = TlsExtensionsUtilities.HasEncryptThenMacExtension(
                        clientExtensions
                    );
                    m_truncatedHMacOffered = TlsExtensionsUtilities.HasTruncatedHmacExtension(
                        clientExtensions
                    );
                    m_statusRequestV2 = TlsExtensionsUtilities.GetStatusRequestV2Extension(
                        clientExtensions
                    );
                    m_trustedCAKeys = TlsExtensionsUtilities.GetTrustedCAKeysExtensionClient(
                        clientExtensions
                    );
                    ...
                    m_clientSentECPointFormats =
                        null
                        != TlsExtensionsUtilities.GetSupportedPointFormatsExtension(
                            clientExtensions
                        );
                }

                m_certificateStatusRequest = TlsExtensionsUtilities.GetStatusRequestExtension(
                    clientExtensions
                );

                m_maxFragmentLengthOffered = TlsExtensionsUtilities.GetMaxFragmentLengthExtension(
                    clientExtensions
                );
                if (
                    m_maxFragmentLengthOffered >= 0
                    && !MaxFragmentLength.IsValid(m_maxFragmentLengthOffered)
                )
        ...
        public virtual int GetSelectedCipherSuite()
        {
            ...
            {
                int commonCipherSuite13 = TlsUtilities.GetCommonCipherSuite13(
                    negotiatedVersion,
            ...
            else
            {
                 ...
                 * cipher suite design.
                 */
                var sigAlgs = TlsUtilities.GetUsableSignatureAlgorithms(
                ...
                int[] cipherSuites = TlsUtilities.GetCommonCipherSuites(
                    m_offeredCipherSuites,
                    GetCipherSuites(),
                    PreferLocalCipherSuites()
                );
        ...
        public virtual IDictionary<int, byte[]> GetServerExtensions()
        {
            ...
            else
            {
                ...
                if (
                    m_clientSentECPointFormats
                    && TlsEccUtilities.IsEccCipherSuite(m_selectedCipherSuite)
                )
                {
                    /*
                     ...
                     */
                    TlsExtensionsUtilities.AddSupportedPointFormatsExtension(
                        m_serverExtensions,
                ...
                if (null != m_statusRequestV2 && AllowMultiCertStatus())
                {
                     ...
                     */
                    TlsExtensionsUtilities.AddEmptyExtensionData(
                        m_serverExtensions,
            ...
            short[] serverCertTypes =
                TlsExtensionsUtilities.GetServerCertificateTypeExtensionClient(m_clientExtensions);
            if (serverCertTypes != null)
            {
                ...
                if (credentials != null)
                {
                    ...
                    TlsExtensionsUtilities.AddServerCertificateTypeExtensionServer(
                        m_serverExtensions,
            ...
            short[] remoteClientCertTypes =
                TlsExtensionsUtilities.GetClientCertificateTypeExtensionClient(m_clientExtensions);
            if (remoteClientCertTypes != null)
            {
                ...
                if (localClientCertTypes != null)
                {
                    ...
                    // outcome 3: we support the extension and have a common type
                    TlsExtensionsUtilities.AddClientCertificateTypeExtensionServer(
                        m_serverExtensions,
                        selectedType
                    );
        ...
        public virtual void GetServerExtensionsForConnection(
        ...
        )
        {
            ...
            {
                TlsExtensionsUtilities.AddAlpnExtensionServer(
                    serverExtensions,
            ...
            if (ProtocolVersion.DTLSv12.Equals(m_context.ServerVersion))
            {
                ...
                if (
                    m_clientExtensions != null
                    && m_clientExtensions.ContainsKey(ExtensionType.connection_id)
                )
                {
                    var serverConnectionID = GetNewConnectionID();
                    if (serverConnectionID != null)
                    {
                        TlsExtensionsUtilities.AddConnectionIDExtension(
                            m_serverExtensions,
                            serverConnectionID
                        );
                    }
        ...
        {
            int minimumFiniteFieldBits = TlsDHUtilities.GetMinimumFiniteFieldBits(

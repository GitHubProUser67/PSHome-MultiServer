...
namespace Org.BouncyCastle.Tls
{
    public class TlsClientProtocol : TlsProtocol
    {
                ...
                case HandshakeType.finished:
                {
                    switch (m_connectionState)
                    {
                        ...
                        case CS_SERVER_CERTIFICATE_VERIFY:
                        {
                            ...
                            byte[] serverFinishedTranscriptHash = TlsUtilities.GetCurrentPrfHash(
                                m_handshakeHash
                            );
                            ...
                            {
                                TlsCredentialedSigner clientCredentials =
                                    TlsUtilities.Establish13ClientCredentials(
                                        m_authentication,
                                ...
                                if (null != clientCredentials)
                                {
                                    DigitallySigned certificateVerify =
                                        TlsUtilities.Generate13CertificateVerify(
                                            m_tlsClientContext,
                                            clientCredentials,
                                            m_handshakeHash
                                        );
                            ...
                            TlsUtilities.Establish13PhaseApplication(
                                m_tlsClientContext,
                ...
                case HandshakeType.certificate:
                {
                    switch (m_connectionState)
                    {
                        ...
                        case CS_SERVER_SUPPLEMENTAL_DATA:
                        {
                             ...
                             */
                            m_authentication = TlsUtilities.ReceiveServerCertificate(
                                m_tlsClientContext,
                                m_tlsClient,
                                buf
                            );
                ...
                case HandshakeType.certificate_status:
                {
                    switch (m_connectionState)
                    {
                        case CS_SERVER_CERTIFICATE:
                        {
                            ...
                            this.m_certificateStatus = CertificateStatus.Parse(
                                m_tlsClientContext,
                                buf
                            );
                ...
                case HandshakeType.server_hello_done:
                {
                    switch (m_connectionState)
                    {
                        ...
                        case CS_SERVER_CERTIFICATE_REQUEST:
                        {
                            ...
                            {
                                clientAuthCredentials = TlsUtilities.EstablishClientCredentials(
                                    m_authentication,
                                    m_certificateRequest
                                );
                                if (clientAuthCredentials != null)
                                {
                                    ...
                                    if (
                                        ...
                                        is TlsCredentialedSigner credentialedSigner
                                    )
                                    {
                                        clientAuthSigner = credentialedSigner;
                                        clientAuthAlgorithm =
                                            TlsUtilities.GetSignatureAndHashAlgorithm(
                                                securityParameters.NegotiatedVersion,
                                                clientAuthSigner
                                            );
                                        clientAuthStreamSigner = clientAuthSigner.GetStreamSigner();

                                        TlsUtilities.Verify12SignatureAlgorithm(
                                            clientAuthAlgorithm,
                                            AlertDescription.internal_error
                                        );

                                        if (
                                            ProtocolVersion.TLSv12.Equals(
                                                securityParameters.NegotiatedVersion
                                            )
                                        )
                                        {
                                            TlsUtilities.VerifySupportedSignatureAlgorithm(
                                            ...
                                            {
                                                TlsUtilities.TrackHashAlgorithmClient(
                                                    m_handshakeHash,
                                                    clientAuthAlgorithm
                                                );
                                            }
                            ...
                            securityParameters.m_sessionHash = TlsUtilities.GetCurrentPrfHash(
                                m_handshakeHash
                            );
                            ...
                            m_recordStream.SetPendingCipher(
                                TlsUtilities.InitCipher(m_tlsClientContext)
                            );

                            if (clientAuthSigner != null)
                            {
                                DigitallySigned certificateVerify =
                                    TlsUtilities.GenerateCertificateVerifyClient(
                                        m_tlsClientContext,
                                        clientAuthSigner,
                                        clientAuthAlgorithm,
                                        clientAuthStreamSigner,
                                        m_handshakeHash
                                    );
                ...
                case HandshakeType.certificate_request:
                {
                    switch (m_connectionState)
                    {
                        ...
                        case CS_SERVER_KEY_EXCHANGE:
                        {
                            ...
                            TlsUtilities.EstablishServerSigAlgs(
                                securityParameters,
                                m_certificateRequest
                            );
                            break;
                ...
                case HandshakeType.new_session_ticket:
                {
                    switch (m_connectionState)
                    {
                        case CS_CLIENT_FINISHED:
                        {
                            ...
                            InvalidateSession();
                            this.m_tlsSession = TlsUtilities.ImportSession(
                                securityParameters.SessionID,
                                null
                            );
        ...
        {
            TlsUtilities.ProcessServerCertificate(
                m_tlsClientContext,
                m_certificateStatus,
                m_keyExchange,
                m_authentication,
        ...
        protected virtual void Process13HelloRetryRequest(ServerHello helloRetryRequest)
        {
            ...
            if (
                !ProtocolVersion.TLSv12.Equals(legacy_version)
                || !Arrays.AreEqual(m_clientHello.SessionID, legacy_session_id_echo)
                || !TlsUtilities.IsValidCipherSuiteSelection(
            ...
            TlsUtilities.CheckExtensionData13(
                extensions,
                ...
                foreach (int extensionType in extensions.Keys)
                {
                    ...
                    if (null == TlsUtilities.GetExtensionData(m_clientExtensions, extensionType))
                    {
                        throw new TlsFatalAlert(
                            ...
                            "Unrequested extension in HelloRetryRequest: "
            ...
            ProtocolVersion server_version =
                TlsExtensionsUtilities.GetSupportedVersionsExtensionServer(extensions);
            if (null == server_version)
            {
                throw new TlsFatalAlert(
                    AlertDescription.missing_extension,
                    "missing extension response: "
                        + ExtensionType.GetText(ExtensionType.supported_versions)
                );
            }

            if (
                !ProtocolVersion.TLSv13.IsEqualOrEarlierVersionOf(server_version)
            ...
            if (null != m_clientBinders)
            {
                if (
                    !Arrays.Contains(
                        m_clientBinders.m_pskKeyExchangeModes,
             ...
             */
            if (
                !TlsUtilities.IsValidKeyShareSelection(
                    server_version,
        ...
        /// <exception cref="IOException"/>
        protected virtual void Process13ServerHello(
        ...
        )
        {
            ...
            if (afterHelloRetryRequest)
            {
                ...
                if (
                    !securityParameters.NegotiatedVersion.Equals(server_version)
            ...
            else
            {
                if (
                    ...
                    || !TlsUtilities.IsValidVersionForCipherSuite(
                        cipherSuite,
                        securityParameters.NegotiatedVersion
                    )
                )
             ...
             */
            securityParameters.m_statusRequestVersion = m_clientExtensions.ContainsKey(
            ...
            {
                int selected_identity = TlsExtensionsUtilities.GetPreSharedKeyServerHello(
                    extensions
                );
                ...
                {
                    if (
                        null == m_clientBinders
                        || selected_identity >= m_clientBinders.m_psks.Length
                    )
                        throw new TlsFatalAlert(AlertDescription.illegal_parameter);
            ...
            {
                KeyShareEntry serverShare = TlsExtensionsUtilities.GetKeyShareServerHello(
                    extensions
                );
                if (null == serverShare)
                {
        ...
        )
        {
            ...
            TlsUtilities.Establish13PhaseHandshake(
                m_tlsClientContext,
        ...
        protected virtual void ProcessServerHello(ServerHello serverHello)
        {
            ...
            ProtocolVersion supported_version =
                TlsExtensionsUtilities.GetSupportedVersionsExtensionServer(serverHelloExtensions);
            ...
            // NOT renegotiating
            {
                ...
                ProtocolVersion legacy_record_version = server_version.IsLaterVersionOf(
...

            {
                ...
                {
                    if (
                        serverHello.CipherSuite != m_sessionParameters.CipherSuite
                        || !securityParameters.NegotiatedVersion.Equals(
                            m_sessionParameters.NegotiatedVersion
                        )
                    )
                    {
                        throw new TlsFatalAlert(
             ...
             */
            {
                ...
                if (
                    !TlsUtilities.IsValidCipherSuiteSelection(offeredCipherSuites, cipherSuite)
                    || !TlsUtilities.IsValidVersionForCipherSuite(
                        cipherSuite,
                        securityParameters.NegotiatedVersion
                    )
            ...
            if (serverHelloExtensions != null)
            {
                foreach (int extType in serverHelloExtensions.Keys)
                {
                    ...
                    if (null == TlsUtilities.GetExtensionData(clientHelloExtensions, extType))
                    {
                        throw new TlsFatalAlert(
                            AlertDescription.unsupported_extension,
                            "Unrequested extension in ServerHello: "
                                + ExtensionType.GetText(extType)
                        );
                    }
            ...
            byte[] renegExtData = TlsUtilities.GetExtensionData(
            ...
            // extended_master_secret
            {
                ...
                {
                    negotiatedEms = TlsExtensionsUtilities.HasExtendedMasterSecretExtension(
                        serverHelloExtensions
                    );

                    if (TlsUtilities.IsExtendedMasterSecretOptional(server_version))
                    {
                        if (!negotiatedEms && m_tlsClient.RequiresExtendedMasterSecret())
                        {
                            throw new TlsFatalAlert(
                    ...
                    else
                    {
                        if (negotiatedEms)
                        {
                            throw new TlsFatalAlert(
                                AlertDescription.illegal_parameter,
                                "Server sent an unexpected extended_master_secret extension negotiating "
                                    + server_version
                            );
                        }
                    }
            ...
            if (
                securityParameters.IsResumedSession
                && securityParameters.IsExtendedMasterSecret
             ...
             */
            securityParameters.m_applicationProtocol =
                TlsExtensionsUtilities.GetAlpnExtensionServer(serverHelloExtensions);
            ...
            {
                {
                     ...
                     * client.
                     */
                    bool serverSentEncryptThenMAC =
                        TlsExtensionsUtilities.HasEncryptThenMacExtension(sessionServerExtensions);
                    if (
                        serverSentEncryptThenMAC
                        && !TlsUtilities.IsBlockCipherSuite(securityParameters.CipherSuite)
                    )
                        throw new TlsFatalAlert(AlertDescription.illegal_parameter);
                ...
                securityParameters.m_maxFragmentLength =
                    TlsUtilities.ProcessMaxFragmentLengthExtension(
                        sessionClientExtensions,
                        sessionServerExtensions,
                        AlertDescription.illegal_parameter
                    );

                securityParameters.m_truncatedHmac =
                    TlsExtensionsUtilities.HasTruncatedHmacExtension(sessionServerExtensions);

                if (!securityParameters.IsResumedSession)
                {
                    // TODO[tls13] See RFC 8446 4.4.2.1
                    if (
                        TlsUtilities.HasExpectedEmptyExtensionData(
                            sessionServerExtensions,
                    ...
                    securityParameters.m_clientCertificateType =
                        TlsUtilities.ProcessClientCertificateTypeExtension(
                            sessionClientExtensions,
                            sessionServerExtensions,
                            AlertDescription.illegal_parameter
                        );
                    securityParameters.m_serverCertificateType =
                        TlsUtilities.ProcessServerCertificateTypeExtension(
                            sessionClientExtensions,
                            sessionServerExtensions,
                            AlertDescription.illegal_parameter
                        );

                    this.m_expectSessionTicket = TlsUtilities.HasExpectedEmptyExtensionData(
                        sessionServerExtensions,
        ...
        protected virtual void Receive13CertificateRequest(MemoryStream buf, bool postHandshakeAuth)
        {
            ...
            CertificateRequest certificateRequest = CertificateRequest.Parse(
                m_tlsClientContext,
                buf
            );
            ...
            TlsUtilities.EstablishServerSigAlgs(
                m_tlsClientContext.SecurityParameters,
                certificateRequest
            );
        }
        ...
        protected virtual void Receive13EncryptedExtensions(MemoryStream buf)
        {
            ...
            this.m_serverExtensions = ReadExtensionsData13(
...

            {
                ...
                foreach (int extType in m_serverExtensions.Keys)
                {
                    if (null == TlsUtilities.GetExtensionData(m_clientExtensions, extType))
                    {
                        throw new TlsFatalAlert(
                            AlertDescription.unsupported_extension,
                            "Unrequested extension in EncryptedExtensions: "
                                + ExtensionType.GetText(extType)
                        );
                    }
                }
            ...
            securityParameters.m_applicationProtocol =
                TlsExtensionsUtilities.GetAlpnExtensionServer(m_serverExtensions);
            securityParameters.m_applicationProtocolSet = true;
            ...
            if (!securityParameters.IsResumedSession)
            {
                ...
                securityParameters.m_clientCertificateType =
                    TlsUtilities.ProcessClientCertificateTypeExtension13(
                        sessionClientExtensions,
                        sessionServerExtensions,
                        AlertDescription.illegal_parameter
                    );
                securityParameters.m_serverCertificateType =
                    TlsUtilities.ProcessServerCertificateTypeExtension13(
                        sessionClientExtensions,
                        sessionServerExtensions,
                        AlertDescription.illegal_parameter
                    );
        ...
        protected virtual void Receive13ServerCertificate(MemoryStream buf)
        {
            ...
            m_authentication = TlsUtilities.Receive13ServerCertificate(
                m_tlsClientContext,
                m_tlsClient,
                buf
            );
        ...
        protected virtual void Receive13ServerCertificateVerify(MemoryStream buf)
        {
            ...
            TlsUtilities.Verify13CertificateVerifyServer(
                m_tlsClientContext,
                m_handshakeHash,
                certificateVerify
            );
        ...
        protected virtual void ReceiveCertificateRequest(MemoryStream buf)
        {
            ...
            m_certificateRequest = TlsUtilities.ValidateCertificateRequest(
                certificateRequest,
                m_keyExchange
            );
        }
        ...
        protected virtual void Send13ClientHelloRetry()
        {
            ...
            if (null != m_clientBinders)
            {
                this.m_clientBinders = TlsUtilities.AddPreSharedKeyToClientHelloRetry(
                    m_tlsClientContext,
                    m_clientBinders,
             ...
             */
            this.m_clientAgreements = TlsUtilities.AddKeyShareToClientHelloRetry(
                m_tlsClientContext,
                clientHelloExtensions,
                m_retryGroup
            );
        ...
        {
            HandshakeMessageOutput message = new HandshakeMessageOutput(
        ...
        protected virtual void SendClientHello()
        {
            ...
            bool offeringTlsV12Minus = ProtocolVersion.TLSv12.IsEqualOrLaterVersionOf(
                earliestVersion
            );
            bool offeringTlsV13Plus = ProtocolVersion.TLSv13.IsEqualOrEarlierVersionOf(
                latestVersion
            );

            {
                ...
                securityParameters.m_clientRandom = CreateRandomBlock(
                    useGmtUnixTime,
                    m_tlsClientContext
                );
            }

            TlsSession sessionToResume = offeringTlsV12Minus
                ? m_tlsClient.GetSessionToResume()
                : null;
            ...
            if (offeringTlsV13Plus)
            {
                ...
                TlsExtensionsUtilities.AddSupportedVersionsExtensionClient(
                    m_clientExtensions,
                    supportedVersions
                );
                 ...
                 */
                if (
                    legacy_session_id.Length < 1
                    && TlsUtilities.ShouldUseCompatibilityMode(m_tlsClient)
                )
                {
            ...
            securityParameters.m_clientServerNames =
                TlsExtensionsUtilities.GetServerNameExtensionClient(m_clientExtensions);
            ...
            securityParameters.m_clientSupportedGroups =
                TlsExtensionsUtilities.GetSupportedGroupsExtension(m_clientExtensions);
            ...
            this.m_clientAgreements = TlsUtilities.AddKeyShareToClientHello(
                m_tlsClientContext,
                m_tlsClient,
                m_clientExtensions
            );
            ...
            if (offeringTlsV12Minus)
            {
                ...
                bool noRenegExt = (
                    null
                    == TlsUtilities.GetExtensionData(
                        m_clientExtensions,
                        ExtensionType.renegotiation_info
                    )
                );
                bool noRenegScsv = !Arrays.Contains(
                    offeredCipherSuites,
                ...
                {
                    offeredCipherSuites = Arrays.Append(
                        offeredCipherSuites,
                        CipherSuite.TLS_EMPTY_RENEGOTIATION_INFO_SCSV
                    );
                }
            ...
            this.m_clientHello = new ClientHello(
                legacy_version,
        ...
        protected virtual void SendClientHelloMessage()
        {
            ...
            if (null != m_clientBinders)
            {
                OfferedPsks.EncodeBinders(
                    message,
                    m_tlsClientContext.Crypto,
                    m_handshakeHash,
                    m_clientBinders
                );
            }

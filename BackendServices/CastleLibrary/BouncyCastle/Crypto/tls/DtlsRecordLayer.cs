...
namespace Org.BouncyCastle.Tls
{
    internal class DtlsRecordLayer : DatagramTransport
    {
        ...
        private volatile ProtocolVersion m_readVersion = null,
            m_writeVersion = null;
        ...
        internal DtlsRecordLayer(TlsContext context, TlsPeer peer, DatagramTransport transport)
        {
            ...
            m_currentEpoch = new DtlsEpoch(
                0,
                TlsNullNullCipher.Instance,
                RecordHeaderLength,
                RecordHeaderLength
            );
            m_pendingEpoch = null;
        ...
        internal virtual void InitPendingEpoch(TlsCipher pendingCipher)
        {
            ...
            int recordHeaderLengthRead =
                RecordHeaderLength + (securityParameters.ConnectionIDPeer?.Length ?? 0);
            int recordHeaderLengthWrite =
                RecordHeaderLength + (securityParameters.ConnectionIDLocal?.Length ?? 0);

            // TODO Check for overflow
            m_pendingEpoch = new DtlsEpoch(
                m_writeEpoch.Epoch + 1,
                pendingCipher,
                recordHeaderLengthRead,
        ...
        {
            int ciphertextLimit =
                m_transport.GetReceiveLimit() - m_readEpoch.RecordHeaderLengthRead;
        ...
        public virtual int Receive(Span<byte> buffer, int waitMillis) =>
            Receive(buffer, waitMillis, null);
        ...
        internal int Receive(Span<byte> buffer, int waitMillis, DtlsRecordCallback recordCallback)
        {
            ...
            {
                if (
                    null != m_retransmitTimeout
                    && m_retransmitTimeout.RemainingMillis(currentTimeMillis) < 1
                )
                {
                    m_retransmit = null;
                ...
                if (Timeout.HasExpired(m_heartbeatTimeout, currentTimeMillis))
                {
                    ...
                    m_heartbeatInFlight = HeartbeatMessage.Create(
                        m_context,
                    ...
                    m_heartbeatResendTimeout = new Timeout(
                        m_heartbeatResendMillis,
                        currentTimeMillis
                    );
                ...
                {
                    m_heartbeatResendMillis = DtlsReliableHandshake.BackOff(
                        m_heartbeatResendMillis
                    );
                    m_heartbeatResendTimeout = new Timeout(
                        m_heartbeatResendMillis,
                ...
                waitMillis = Timeout.ConstrainWaitMillis(
                    waitMillis,
                    m_heartbeatTimeout,
                    currentTimeMillis
                );
        ...
        )
        {
            ...
            ReadOnlySpan<byte> error = stackalloc byte[2]
            {
                (byte)alertLevel,
        ...
        )
        {
            ...
            if (null != m_readVersion && !m_readVersion.Equals(recordVersion))
            {
                ...
                bool isClientHelloFragment =
                    ...
                    && HandshakeType.client_hello
                        == TlsUtilities.ReadUint8(record, recordHeaderLength);
            ...
            try
            {
                decoded = recordEpoch.Cipher.DecodeCiphertext(
                    macSeqNo,
                    recordType,
                    recordVersion,
                    record,
                ...
                when (AlertDescription.bad_record_mac == fatalAlert.AlertDescription)
            {
                /*
            ...
            switch (decoded.contentType)
            {
                case ContentType.alert:
                {
                    if (decoded.len == 2)
                    {
                        ...
                        short alertDescription = TlsUtilities.ReadUint8(
                            decoded.buf,
                            decoded.off + 1
                        );
                ...
                case ContentType.handshake:
                {
                    if (!m_inHandshake)
                    {
                        ...
                        {
                            m_retransmit.ReceivedHandshakeRecord(
                                epoch,
                                decoded.buf,
                                decoded.off,
                                decoded.len
                            ...
                            if (null != heartbeatMessage)
                            {
                                switch (heartbeatMessage.Type)
                                {
                                    case HeartbeatMessageType.heartbeat_request:
                                    {
                                        ...
                                        {
                                            HeartbeatMessage response = HeartbeatMessage.Create(
                                                m_context,
                                    ...
                                    {
                                        if (
                                            null != m_heartbeatInFlight
                                            && Arrays.AreEqual(
                                                heartbeatMessage.Payload,
                                                m_heartbeatInFlight.Payload
                                            )
                                        )
                                        {
                                            ResetHeartbeat();
        ...
        private void SendRecord(short contentType, ReadOnlySpan<byte> buffer)
        {
            ...
            lock (m_writeLock)
            {
                ...
                TlsEncodeResult encoded = m_writeEpoch.Cipher.EncodePlaintext(
                    macSequenceNumber,
                    contentType,
                    recordVersion,
                    recordHeaderLength,
                ...
                TlsUtilities.WriteUint16(
                    ciphertextLength,
                    encoded.buf,
                    encoded.off + (recordHeaderLength - 2)
                );

...
namespace Org.BouncyCastle.Tls
{
    public class TlsServerProtocol : TlsProtocol
    {
        ...
        protected virtual ServerHello Generate13HelloRetryRequest(ClientHello clientHello)
        {
            ...
            TlsExtensionsUtilities.AddSupportedVersionsExtensionServer(
                serverHelloExtensions,
                serverVersion
            );
            if (m_retryGroup >= 0)
            {
                TlsExtensionsUtilities.AddKeyShareHelloRetryRequest(
                    serverHelloExtensions,
                    m_retryGroup
                );
            }
        ...
        )
        {
            ...
            OfferedPsks.SelectedConfig selectedPsk = TlsUtilities.SelectPreSharedKey(
                m_tlsServerContext,
                m_tlsServer,
            ...
            if (afterHelloRetryRequest)
            {
                ...
                clientShare =
                    TlsUtilities.GetRetryKeyShare(clientShares, m_retryGroup)
                    ?? throw new TlsFatalAlert(AlertDescription.illegal_parameter);
            }
            else
            {
                {
                    securityParameters.m_serverRandom = CreateRandomBlock(
                        false,
                        m_tlsServerContext
                    );
                    ...
                    if (
                        !serverVersion.Equals(
                            ProtocolVersion.GetLatestTls(m_tlsServer.GetProtocolVersions())
                        )
                    )
                    {
                        TlsUtilities.WriteDowngradeMarker(
                            serverVersion,
                ...
                securityParameters.m_clientServerNames =
                    TlsExtensionsUtilities.GetServerNameExtensionClient(clientHelloExtensions);
                ...
                int selectedGroup = TlsUtilities.SelectKeyShareGroup(
                    crypto,
                    serverVersion,
            ...
            if (!afterHelloRetryRequest)
            {
                ...
                if (
                    !TlsUtilities.IsNullOrEmpty(serverSupportedGroups)
                ...
                {
                    TlsExtensionsUtilities.AddSupportedGroupsExtension(
            ...
            securityParameters.m_applicationProtocol =
                TlsExtensionsUtilities.GetAlpnExtensionServer(serverEncryptedExtensions);
            ...
            if (serverEncryptedExtensions.Count > 0)
            {
                securityParameters.m_maxFragmentLength =
                    TlsUtilities.ProcessMaxFragmentLengthExtension(
                        securityParameters.IsResumedSession ? null : clientHelloExtensions,
                        serverEncryptedExtensions,
                        AlertDescription.internal_error
                    );
             ...
             */
            securityParameters.m_statusRequestVersion = clientHelloExtensions.ContainsKey(
            ...
            if (null != selectedPsk)
            {
                ...
                TlsExtensionsUtilities.AddPreSharedKeyServerHello(
                    serverHelloExtensions,
                    selectedPsk.m_index
                );
            }
            ...
            TlsSecret sharedSecret;
            {
                ...
                TlsAgreement agreement =
                    TlsUtilities.CreateKeyShare(crypto, negotiatedGroup, isServer: true)
                    ?? throw new TlsFatalAlert(AlertDescription.internal_error);
        ...
        )
        {
            ...
            if (null == m_tlsServerContext.ClientSupportedVersions)
            {
                ...
                m_tlsServerContext.SetClientSupportedVersions(
                    clientVersion.DownTo(ProtocolVersion.SSLv3)
                );
            }
            else
            {
                clientVersion = ProtocolVersion.GetLatestTls(
                    m_tlsServerContext.ClientSupportedVersions
                );
            }
            ...
            if (
                !ProtocolVersion.SERVER_EARLIEST_SUPPORTED_TLS.IsEqualOrEarlierVersionOf(
                    clientVersion
                )
            )
            ...
            m_tlsServer.NotifyFallback(
                Arrays.Contains(m_offeredCipherSuites, CipherSuite.TLS_FALLBACK_SCSV)
            );
            ...
            securityParameters.m_clientSupportedGroups =
                TlsExtensionsUtilities.GetSupportedGroupsExtension(clientHello.Extensions);
            ...
            byte[] clientRenegExtData = TlsUtilities.GetExtensionData(
                m_clientExtensions,
            ...
            // extended_master_secret
            {
                ...
                if (
                    TlsUtilities.IsExtendedMasterSecretOptional(serverVersion)
...

            {
                ...
                if (
                    !TlsUtilities.IsValidCipherSuiteSelection(m_offeredCipherSuites, cipherSuite)
                    || !TlsUtilities.IsValidVersionForCipherSuite(cipherSuite, serverVersion)
                )
                {
                    throw new TlsFatalAlert(AlertDescription.internal_error);
...

            {
                ...
                this.m_serverExtensions = TlsExtensionsUtilities.EnsureExtensionsInitialised(
            ...
            // NOT renegotiating
            {
                ...
                {
                    byte[] serverRenegExtData = TlsUtilities.GetExtensionData(
                        m_serverExtensions,
                        ExtensionType.renegotiation_info
                    );
                    ...
                    if (noRenegExt)
                    {
                         ...
                         */
                        this.m_serverExtensions[ExtensionType.renegotiation_info] =
                            CreateRenegotiationInfo(TlsUtilities.EmptyBytes);
                    }
                }
            }
            ...
            {
                securityParameters.m_encryptThenMac =
                    TlsExtensionsUtilities.HasEncryptThenMacExtension(m_serverExtensions);

                securityParameters.m_maxFragmentLength =
                    TlsUtilities.ProcessMaxFragmentLengthExtension(
                        resumedSession ? null : m_clientExtensions,
                        m_serverExtensions,
                        AlertDescription.internal_error
                    );

                securityParameters.m_truncatedHmac =
                    TlsExtensionsUtilities.HasTruncatedHmacExtension(m_serverExtensions);
                ...
                case HandshakeType.client_hello:
                {
                    switch (m_connectionState)
                    {
                        ...
                        case CS_SERVER_HELLO_RETRY_REQUEST:
                        {
                            ...
                            ServerHello serverHello = Generate13ServerHello(
                                clientHelloRetry,
                                buf,
                                true
                            );
            ...
            switch (type)
            {
                        ...
                        case CS_START:
                        {
                            ...
                            if (securityParameters.IsResumedSession)
                            {
                                ...
                                m_recordStream.SetPendingCipher(
                                    TlsUtilities.InitCipher(m_tlsServerContext)
                                );
                            ...
                            m_keyExchange = TlsUtilities.InitKeyExchangeServer(
                                m_tlsServerContext,
                                m_tlsServer
                            );
                            ...
                            if (
                                !KeyExchangeAlgorithm.IsAnonymous(
                                    securityParameters.KeyExchangeAlgorithm
                                )
                            )
                            {
                                serverCredentials = TlsUtilities.EstablishServerCredentials(
                                    m_tlsServer
                                );
                            }
                            ...
                            if (null != serverCredentials)
                            {
                                ...
                                {
                                    if (
                                        TlsUtilities.IsTlsV12(m_tlsServerContext)
                                        != (
                                            m_certificateRequest.SupportedSignatureAlgorithms
                                            != null
                                        )
                                    )
                                    {
                                    ...
                                    this.m_certificateRequest =
                                        TlsUtilities.ValidateCertificateRequest(
                                    ...
                                    )
                                    {
                                        ...
                                        if (
                                            m_tlsServerContext.Crypto.HasAnyStreamVerifiers(
                                                securityParameters.ServerSigAlgs
                                            )
                                        )
                                        {
                                    ...
                                    {
                                        if (
                                            m_tlsServerContext.Crypto.HasAnyStreamVerifiersLegacy(
                                                m_certificateRequest.CertificateTypes
                                            )
                                        )
                                        {
                                            m_handshakeHash.ForceBuffering();
                ...
                case HandshakeType.supplemental_data:
                {
                    switch (m_connectionState)
                    {
                        ...
                        {
                            m_tlsServer.ProcessClientSupplementalData(
                                ReadSupplementalDataMessage(buf)
                            );
        ...
        protected override void HandleAlertWarningMessage(short alertDescription)
        {
             ...
             */
            if (
                AlertDescription.no_certificate == alertDescription
                && null != m_certificateRequest
        ...
        protected virtual void NotifyClientCertificate(Certificate clientCertificate)
        {
            ...
            TlsUtilities.ProcessClientCertificate(
                m_tlsServerContext,
                clientCertificate,
                m_keyExchange,
                m_tlsServer
        ...
        protected void Receive13ClientCertificateVerify(MemoryStream buf)
        {
            ...
            TlsUtilities.Verify13CertificateVerifyClient(
                m_tlsServerContext,
                m_handshakeHash,
                certificateVerify
            );
        ...
        protected virtual void ReceiveCertificateVerifyMessage(MemoryStream buf)
        {
            ...
            TlsUtilities.VerifyCertificateVerifyClient(
                m_tlsServerContext,
                m_certificateRequest,
                certificateVerify,
                m_handshakeHash
        ...
        protected virtual void ReceiveClientKeyExchangeMessage(MemoryStream buf)
        {
            ...
            m_tlsServerContext.SecurityParameters.m_sessionHash = TlsUtilities.GetCurrentPrfHash(
                m_handshakeHash
            );
        ...
        /// <exception cref="IOException"/>
        protected virtual void Send13EncryptedExtensionsMessage(
        ...
        /// <exception cref="IOException"/>
        protected virtual void Send13ServerHelloCoda(
        ...
        )
        {
            ...
            else
            {
                // CertificateRequest
                {
                    ...
                    {
                        if (
                            !m_certificateRequest.HasCertificateRequestContext(
                ...
                TlsCredentialedSigner serverCredentials =
                    TlsUtilities.Establish13ServerCredentials(m_tlsServer)
                    ?? throw new TlsFatalAlert(AlertDescription.internal_error);

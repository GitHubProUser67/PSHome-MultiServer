...
namespace Org.BouncyCastle.Tls
{
    public class DtlsClientProtocol : DtlsProtocol
    {
        ...
        internal virtual DtlsTransport ClientHandshake(ClientHandshakeState state)
        {
            ...
            DtlsReliableHandshake handshake = new DtlsReliableHandshake(
                clientContext,
                recordLayer,
                client.GetHandshakeTimeoutMillis(),
                TlsUtilities.GetHandshakeResendTimeMillis(client),
                null
            );
            ...
            if (securityParameters.IsResumedSession)
            {
                ...
                securityParameters.m_peerVerifyData = TlsUtilities.CalculateVerifyData(
                    clientContext,
                ...
                securityParameters.m_localVerifyData = TlsUtilities.CalculateVerifyData(
                    clientContext,
                    handshake.HandshakeHash,
                    false
                );
                ...
                if (
                    ...
                    && ProtocolVersion.DTLSv12.IsEqualOrLaterVersionOf(
            ...
            if (state.expectSessionTicket)
            {
                ...
                if (serverMessage.Type == HandshakeType.new_session_ticket)
                {
                    ...
                    InvalidateSession(state);
                    state.tlsSession = TlsUtilities.ImportSession(
        ...
        protected virtual byte[] GenerateClientHello(ClientHandshakeState state)
        {
            ...
            bool offeringDtlsV12Minus = ProtocolVersion.DTLSv12.IsEqualOrLaterVersionOf(
                earliestVersion
            );
            bool offeringDtlsV13Plus = ProtocolVersion.DTLSv13.IsEqualOrEarlierVersionOf(
                latestVersion
            );

            {
                ...
                securityParameters.m_clientRandom = TlsProtocol.CreateRandomBlock(
                    useGmtUnixTime,
                    clientContext
                );
            }
            ...
            )
            {
                ...
                {
                    if (
                        !state.sessionParameters.IsExtendedMasterSecret
            ...
            securityParameters.m_clientServerNames =
                TlsExtensionsUtilities.GetServerNameExtensionClient(state.clientExtensions);
            ...
            securityParameters.m_clientSupportedGroups =
                TlsExtensionsUtilities.GetSupportedGroupsExtension(state.clientExtensions);
            ...
            state.clientAgreements = TlsUtilities.AddKeyShareToClientHello(
                clientContext,
                client,
                state.clientExtensions
            );
            ...
            if (offeringDtlsV12Minus)
            {
                ...
                {
                    state.offeredCipherSuites = Arrays.Append(
                        state.offeredCipherSuites,
                        CipherSuite.TLS_EMPTY_RENEGOTIATION_INFO_SCSV
                    );
                }
            ...
            // Heartbeats
            {
                ...
                if (
                    null != state.heartbeat
                    || HeartbeatMode.peer_allowed_to_send == state.heartbeatPolicy
                )
                {
                    TlsExtensionsUtilities.AddHeartbeatExtension(
                        state.clientExtensions,
            ...
            ClientHello clientHello = new ClientHello(
                ...
                legacy_session_id,
                cookie: TlsUtilities.EmptyBytes,
                state.offeredCipherSuites,
                state.clientExtensions,
                bindersSize
            );
        ...
        protected virtual bool EstablishSession(
            ClientHandshakeState state,
        ...
        )
        {
            ...
            TlsSecret sessionMasterSecret = TlsUtilities.GetSessionMasterSecret(
                crypto,
        ...
        protected virtual void ProcessCertificateRequest(ClientHandshakeState state, byte[] body)
        {
            ...
            state.certificateRequest = TlsUtilities.ValidateCertificateRequest(
                certificateRequest,
                state.keyExchange
            );
        }
        ...
        protected virtual byte[] ProcessHelloVerifyRequest(ClientHandshakeState state, byte[] body)
        {
             ...
             */
            int maxCookieLength = ProtocolVersion.DTLSv12.IsEqualOrEarlierVersionOf(server_version)
                ? 255
                : 32;
        ...
        {
            state.authentication = TlsUtilities.ReceiveServerCertificate(
                state.clientContext,
                state.client,
        ...
        protected virtual void ProcessServerHello(ClientHandshakeState state, byte[] body)
        {
            ...
            {
                if (
                    !ProtocolVersion.DTLSv12.Equals(legacy_version)
...

            {
                ...
                {
                    if (
                        serverHello.CipherSuite != state.sessionParameters.CipherSuite
                        || !securityParameters.NegotiatedVersion.Equals(
                            state.sessionParameters.NegotiatedVersion
                        )
                    )
                    {
             ...
             */
            {
                int cipherSuite = ValidateSelectedCipherSuite(
            ...
            if (ProtocolVersion.DTLSv12.Equals(securityParameters.NegotiatedVersion))
            {
                 ...
                 * negotiated afresh.
                 */
                var serverConnectionID = TlsExtensionsUtilities.GetConnectionIDExtension(
                    serverHelloExtensions
                );
                if (serverConnectionID != null)
                {
                    var clientConnectionID =
                        TlsExtensionsUtilities.GetConnectionIDExtension(state.clientExtensions)
                        ?? throw new TlsFatalAlert(AlertDescription.internal_error);
            ...
            // Heartbeats
            {
                HeartbeatExtension heartbeatExtension =
                    TlsExtensionsUtilities.GetHeartbeatExtension(serverHelloExtensions);
            ...
            if (sessionServerExtensions != null && sessionServerExtensions.Count > 0)
            {
                ...
                if (!securityParameters.IsResumedSession)
                {
                    ...
                    state.expectSessionTicket = TlsUtilities.HasExpectedEmptyExtensionData(
                        sessionServerExtensions,

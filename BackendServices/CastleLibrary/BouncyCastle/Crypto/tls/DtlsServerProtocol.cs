    ...
    public class DtlsServerProtocol : DtlsProtocol
    {
        ...
        )
        {
            ...
            DtlsReliableHandshake handshake = new DtlsReliableHandshake(
                serverContext,
                recordLayer,
                server.GetHandshakeTimeoutMillis(),
                TlsUtilities.GetHandshakeResendTimeMillis(server),
                request
            );
            ...
            if (securityParameters.StatusRequestVersion > 0)
            {
                ...
                {
                    byte[] certificateStatusBody = GenerateCertificateStatus(
                        state,
                        certificateStatus
                    );
            ...
            if (serverCredentials != null)
            {
                ...
                {
                    if (
                        TlsUtilities.IsTlsV12(serverContext)
                    ...
                    if (ProtocolVersion.DTLSv12.Equals(securityParameters.NegotiatedVersion))
                    {
                        ...
                        if (
                            serverContext.Crypto.HasAnyStreamVerifiers(
                                securityParameters.ServerSigAlgs
                            )
                        )
                        {
                    ...
                    {
                        if (
                            serverContext.Crypto.HasAnyStreamVerifiersLegacy(
                                state.certificateRequest.CertificateTypes
                            )
                        )
                        {
                            handshake.HandshakeHash.ForceBuffering();
            ...
            if (null != state.certificateRequest)
            {
                byte[] certificateRequestBody = GenerateCertificateRequest(
                    state,
                    state.certificateRequest
                );
             ...
             */
            {
                ...
                {
                    clientMessage = handshake.ReceiveMessageDelayedDigest(
        ...
        internal virtual byte[] GenerateServerHello(ServerHandshakeState state)
        {
...

            {
                ...
                securityParameters.m_serverRandom = TlsProtocol.CreateRandomBlock(
                    useGmtUnixTime,
                    serverContext
                );
                ...
                if (
                    !serverVersion.Equals(
                        ProtocolVersion.GetLatestDtls(server.GetProtocolVersions())
                    )
                )
                {
            ...
            // extended_master_secret
            {
                ...
                {
                    if (
                        TlsExtensionsUtilities.HasExtendedMasterSecretExtension(
                ...
                if (
                    resumedSession
                    && negotiateEms != state.sessionParameters.IsExtendedMasterSecret
                )
                {
                    resumedSession = false;
                }
...

            {
                ...
                if (
                    !TlsUtilities.IsValidCipherSuiteSelection(
                        state.clientHello.CipherSuites,
                        cipherSuite
                    )
            ...
            // NOT renegotiating
            {
                ...
                if (securityParameters.IsSecureRenegotiation)
                {
                    ...
                    if (noRenegExt)
                    {
                         ...
                         */
                        state.serverExtensions[ExtensionType.renegotiation_info] =
            ...
            securityParameters.m_applicationProtocol =
                TlsExtensionsUtilities.GetAlpnExtensionServer(state.serverExtensions);
            ...
            if (ProtocolVersion.DTLSv12.Equals(securityParameters.NegotiatedVersion))
            {
                ...
                if (serverConnectionID != null)
                {
                    var clientConnectionID =
                        TlsExtensionsUtilities.GetConnectionIDExtension(clientHelloExtensions)
                        ?? throw new TlsFatalAlert(AlertDescription.internal_error);
            ...
            if (state.serverExtensions.Count > 0)
            {
                securityParameters.m_encryptThenMac =
                    TlsExtensionsUtilities.HasEncryptThenMacExtension(state.serverExtensions);

                securityParameters.m_maxFragmentLength =
                    TlsUtilities.ProcessMaxFragmentLengthExtension(
                        resumedSession ? null : clientHelloExtensions,
                        state.serverExtensions,
                        AlertDescription.internal_error
                    );

                securityParameters.m_truncatedHmac =
                    TlsExtensionsUtilities.HasTruncatedHmacExtension(state.serverExtensions);
        ...
        )
        {
            ...
            if (null == serverContext.ClientSupportedVersions)
            {
                ...
                serverContext.SetClientSupportedVersions(
                    client_version.DownTo(ProtocolVersion.DTLSv10)
                );
            }
            else
            {
                client_version = ProtocolVersion.GetLatestDtls(
                    serverContext.ClientSupportedVersions
                );
            }

            if (
                !ProtocolVersion.SERVER_EARLIEST_SUPPORTED_DTLS.IsEqualOrEarlierVersionOf(
                    client_version
                )
            )
            ...
            server.NotifyFallback(
                Arrays.Contains(offeredCipherSuites, CipherSuite.TLS_FALLBACK_SCSV)
            );
            ...
            if (clientHelloExtensions != null)
            {
                ...
                securityParameters.m_clientSupportedGroups =
                    TlsExtensionsUtilities.GetSupportedGroupsExtension(clientHelloExtensions);

                // Heartbeats
                {
                    HeartbeatExtension heartbeatExtension =
                        TlsExtensionsUtilities.GetHeartbeatExtension(clientHelloExtensions);

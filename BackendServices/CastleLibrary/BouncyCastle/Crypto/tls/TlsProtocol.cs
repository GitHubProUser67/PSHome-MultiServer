...
namespace Org.BouncyCastle.Tls
{
    public abstract class TlsProtocol : TlsCloseable
    {
        ...
        public virtual void ResumeHandshake()
        {
            if (!m_blocking)
                throw new InvalidOperationException(
                    "Cannot use ResumeHandshake() in non-blocking mode!"
                );
            if (!IsHandshaking)
        ...
        protected virtual void BeginHandshake()
        {
            ...
            this.m_maxHandshakeMessageSize = System.Math.Max(
                1024,
                peer.GetMaxHandshakeMessageSize()
            );
            ...
            while (queue.Available >= 4)
            {
                ...
                if (length > m_maxHandshakeMessageSize)
                {
                    throw new TlsFatalAlert(
                        ...
                        "Handshake message length exceeds the maximum: "
                            + HandshakeType.GetText(type)
                            + ", "
                            + length
                            + " > "
        ...
        private void ProcessChangeCipherSpec(byte[] buf, int off, int len)
        {
            ...
            for (int i = 0; i < len; ++i)
            {
                ...
                if (
                    this.m_receivedChangeCipherSpec
        ...
        public virtual int ReadApplicationData(Span<byte> buffer)
        {
            if (!m_appDataReady)
                throw new InvalidOperationException(
                    "Cannot read application data until initial handshake completed."
                );
                ...
                if (m_closed)
                {
                    if (m_failed)
                        throw new IOException(
                            "Cannot read application data on failed TLS connection"
                        );
        ...
        public virtual RecordPreview PreviewInputRecord(byte[] recordHeader)
        {
            if (m_blocking)
                throw new InvalidOperationException(
                    "Cannot use PreviewInputRecord() in blocking mode!"
                );
            if (m_inputBuffers.Available != 0)
                throw new InvalidOperationException(
                    "Can only use PreviewInputRecord() for record-aligned input."
                );
            if (m_closed)
        ...
        public virtual int PreviewOutputRecord()
        {
            if (m_blocking)
                throw new InvalidOperationException(
                    "Cannot use PreviewOutputRecord() in blocking mode!"
                );
            ...
            throw new InvalidOperationException(
                "Can only use PreviewOutputRecord() for record-aligned output."
            );
        }
        ...
        public virtual RecordPreview PreviewOutputRecord(int applicationDataSize)
        {
            ...
            if (m_appDataSplitEnabled)
            {
                switch (m_appDataSplitMode)
                {
                    ...
                    default:
                    {
                        ...
                        if (applicationDataSize > 1)
                        {
                            RecordPreview b = m_recordStream.PreviewOutputRecord(
                                applicationDataSize - 1
                            );
            ...
            else
            {
                ...
                if (
                    m_keyUpdateEnabled
                    && (m_keyUpdatePendingSend || m_recordStream.NeedsKeyUpdate())
                )
                {
        ...
        public virtual void OfferInput(byte[] input, int inputOff, int inputLen)
        {
            if (m_blocking)
                throw new InvalidOperationException(
                    "Cannot use OfferInput() in blocking mode! Use Stream instead."
                );
            if (m_closed)
                throw new IOException("Connection is closed, cannot accept any more input");
        ...
        public virtual int GetAvailableInputBytes()
        {
            if (m_blocking)
                throw new InvalidOperationException(
                    "Cannot use GetAvailableInputBytes() in blocking mode!"
                );
        ...
        public virtual int ReadInput(byte[] buf, int off, int len)
        {
            if (m_blocking)
                throw new InvalidOperationException(
                    "Cannot use ReadInput() in blocking mode! Use Stream instead."
                );
        ...
        public virtual int GetAvailableOutputBytes()
        {
            if (m_blocking)
                throw new InvalidOperationException(
                    "Cannot use GetAvailableOutputBytes() in blocking mode! Use Stream instead."
                );
        ...
        public virtual int ReadOutput(byte[] buffer, int offset, int length)
        {
            if (m_blocking)
                throw new InvalidOperationException(
                    "Cannot use ReadOutput() in blocking mode! Use 'Stream() instead."
                );
        ...
        protected virtual void ProcessFinishedMessage(MemoryStream buf)
        {
            ...
            byte[] expected_verify_data = TlsUtilities.CalculateVerifyData(
                context,
                m_handshakeHash,
                !isServerContext
            );
        ...
        /// <exception cref="IOException"/>
        protected virtual void RaiseAlertFatal(
        ...
        )
        {
            ...
            try
            {
                Span<byte> alert = stackalloc byte[]
                {
                    (byte)AlertLevel.fatal,
        ...
        protected virtual void SendCertificateMessage(Certificate certificate, Stream endPointHash)
        {
            ...
            if (
                certificate.IsEmpty
                && !context.IsServer
                && securityParameters.NegotiatedVersion.IsSsl
            )
            {
        ...
        protected virtual void SendFinishedMessage()
        {
            ...
            byte[] verify_data = TlsUtilities.CalculateVerifyData(
                context,
                m_handshakeHash,
                isServerContext
            );
        ...
        protected virtual void Send13KeyUpdate(bool updateRequested)
        {
            ...
            HandshakeMessageOutput.Send(
                ...
                TlsUtilities.EncodeUint8(requestUpdate)
            );
        ...
        internal static void EstablishMasterSecret(TlsContext context, TlsKeyExchange keyExchange)
        {
            TlsSecret preMasterSecret =
                keyExchange.GeneratePreMasterSecret()
                ?? throw new TlsFatalAlert(AlertDescription.internal_error);

            try
            {
                context.SecurityParameters.m_masterSecret = TlsUtilities.CalculateMasterSecret(
                    context,
                    preMasterSecret
                );
            }
        ...
        /// <exception cref="IOException"/>
        internal static IDictionary<int, byte[]> ReadExtensionsData13(
            int handshakeType,
            byte[] extBytes
        )
        {
        ...
        /// <exception cref="IOException"/>
        internal static byte[] WriteExtensionsData(
            IDictionary<int, byte[]> extensions,
            int bindersSize
        )
        {

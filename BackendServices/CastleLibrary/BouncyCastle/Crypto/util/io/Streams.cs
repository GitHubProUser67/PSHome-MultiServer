...
namespace Org.BouncyCastle.Utilities.IO
{
    public static class Streams
    {
        ...
        public static void CopyTo(Stream source, Stream destination, int bufferSize)
        {
            int bytesRead;
            Span<byte> buffer =
                bufferSize <= MaxStackAlloc ? stackalloc byte[bufferSize] : new byte[bufferSize];
        ...
        )
        {
            ...
            while (
                (
                    bytesRead = await ReadAsync(source, new Memory<byte>(buffer), cancellationToken)
        ...
        internal static async ValueTask<int> ReadAsyncCompletion(
            Task<int> readTask,
            byte[] localBuffer,
        ...
        internal static Task WriteAsyncDirect(
            Stream destination,
            byte[] buffer,
            int offset,
            int count,
        ...
        internal static async ValueTask WriteAsyncCompletion(
            ValueTask writeTask,
            byte[] localBuffer
        )
        {
        ...
        internal static ValueTask WriteAsyncDirect(
            Stream destination,
            ReadOnlyMemory<byte> buffer,
            CancellationToken cancellationToken = default
        )

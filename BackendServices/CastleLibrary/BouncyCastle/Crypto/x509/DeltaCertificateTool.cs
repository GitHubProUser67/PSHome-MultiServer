...
namespace Org.BouncyCastle.X509
{
    ...
    {
        public static Asn1.X509.Extension CreateDeltaCertificateExtension(
            bool isCritical,
        ...
        public static Asn1.X509.Extension CreateDeltaCertificateExtension(
            bool isCritical,
            X509Certificate deltaCert
        ) => CreateDeltaCertificateExtension(isCritical, deltaCert.CertificateStructure);

        public static X509CertificateStructure ExtractDeltaCertificate(
            TbsCertificateStructure baseTbsCert
        )
        {
            var baseExtensions = baseTbsCert.Extensions;

            var dcdExtension =
                baseExtensions.GetExtension(X509Extensions.DRAFT_DeltaCertificateDescriptor)
            ...
            var tbsCertificate = new TbsCertificateStructure(
                version,
                serialNumber,
                signature,
                issuer,
        ...
        public static DeltaCertificateDescriptor TrimDeltaCertificateDescriptor(
            DeltaCertificateDescriptor descriptor,
            TbsCertificateStructure tbsCertificate,
            X509Extensions tbsExtensions
        )
        {
            ...
            if (extensions != null)
            {
                ...
                foreach (DerObjectIdentifier oid in tbsExtensions.ExtensionOids)
                {
                    ...
                    if (
                        deltaExtension != null
                        && !deltaExtension.Equals(tbsExtensions.GetExtension(oid))
                    )
                    {
            ...
            return new DeltaCertificateDescriptor(
                serialNumber,
                signature,
                issuer,
                validity,
        ...
        private static X509Extensions ExtractDeltaExtensions(
            X509Extensions descriptorExtensions,

...
namespace Org.BouncyCastle.X509.Store
{
    public class X509CertStoreSelector : ISelector<X509Certificate>
    {
        ...
        public virtual bool Match(X509Certificate c)
        {
            ...
            if (
                m_subjectPublicKeyAlgID != null
                && !m_subjectPublicKeyAlgID.Equals(c.SubjectPublicKeyInfo.Algorithm)
            )
                return false;
            ...
            if (
                !CollectionUtilities.IsNullOrEmpty(m_subjectAlternativeNames)
                && !MatchSubjectAlternativeNames(c)
            )
                return false;
        ...
        protected internal int GetHashCodeOfSubjectKeyIdentifier() =>
            Arrays.GetHashCode(m_subjectKeyIdentifier);

        protected internal bool MatchesIssuer(X509CertStoreSelector other) =>
            IssuersMatch(m_issuer, other.m_issuer);
        ...
        {
            var certificatePolicies = c.GetExtension(
        ...
        {
            var privateKeyUsagePeriod = c.GetExtension(
        ...
        private static bool IssuersMatch(X509Name a, X509Name b) =>
            a == null ? b == null : a.Equivalent(b, true);
        ...
        private static bool PoliciesIntersect(
            ISet<DerObjectIdentifier> policy,
            Asn1Sequence certificatePolicies
        )
        {

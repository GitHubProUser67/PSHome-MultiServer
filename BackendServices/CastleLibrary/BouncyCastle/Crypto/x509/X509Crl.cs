...
namespace Org.BouncyCastle.X509
{
    ...
    // TODO Add interface Crl?
    {
        ...
        {
            return c.Version >= 2 ? c.TbsCertList.Extensions : null;
        }
        ...
        {
            return CheckSignatureValid(
                verifierProvider.CreateVerifierFactory(c.SignatureAlgorithm)
            );
        }
        ...
        public virtual bool IsAlternativeSignatureValid(IVerifierFactoryProvider verifierProvider)
        {
            ...
            return X509Utilities.VerifySignature(
                verifier,
                new DerSequence(v),
                altSigValue.Signature
            );
        }
        ...
        public virtual void VerifyAltSignature(IVerifierFactoryProvider verifierProvider)
        {
            if (!IsAlternativeSignatureValid(verifierProvider))
                throw new InvalidKeyException(
                    "CRL alternative signature does not verify with supplied public key."
                );
        }
        ...
        public virtual string SigAlgName =>
            Objects.EnsureSingletonInitialized(
                ref m_sigAlgName,
                SignatureAlgorithm,
                    ...
                    if (ext.Value != null)
                    {
                        ...
                        try
                        {
                            if (oid.Equals(X509Extensions.CrlNumber))
                            {
                                buf.Append(
                                        new CrlNumber(
                                            DerInteger.GetInstance(asn1Value).PositiveValue
                                        )
                                    )
                                    .AppendLine();
                            }
                            ...
                            {
                                buf.Append(CrlDistPoint.GetInstance((Asn1Sequence)asn1Value))
                                    .AppendLine();
                            }
                            ...
                            else
                            {
                                buf.Append(oid.Id);
                                buf.Append(" value = ")
                                    .Append(Asn1Dump.DumpAsString(asn1Value))
                                    .AppendLine();
                            }
                        }
                        catch (Exception)

    ...
    */
    public class KeyTransRecipientInformation : RecipientInformation
    {
        ...
        private string GetExchangeEncryptionAlgorithmName(AlgorithmIdentifier algID)
        {
            ...
            {
                var rsaesOaepParameters = Asn1.Pkcs.RsaesOaepParameters.GetInstance(
                    algID.Parameters
                );
                var digestName = DigestUtilities.GetAlgorithmName(
                    rsaesOaepParameters.HashAlgorithm.Algorithm
                );
        ...
        internal KeyParameter UnwrapKey(ICipherParameters key)
        {
            ...
            try
            {
                ...
                {
                    IKeyUnwrapper keyWrapper = new Asn1KeyUnwrapper(
                        keyEncAlg.Algorithm,
                    ...
                    return ParameterUtilities.CreateKeyParameter(
                        ...
                        keyWrapper.Unwrap(encryptedKey, 0, encryptedKey.Length).Collect()
                    );
                }
                else
                {
                    ...
                    return ParameterUtilities.CreateKeyParameter(
                        GetContentAlgorithmName(),
                        keyWrapper.Unwrap(encryptedKey, 0, encryptedKey.Length)
                    );
                }
            }

    ...
    */
    public class KeyAgreeRecipientInformation : RecipientInformation
    {
            ...
            try
            {
                foreach (Asn1Encodable element in info.RecipientEncryptedKeys)
                {
                    ...
                    else
                    {
                        ...
                        rid.SubjectKeyIdentifier = karid.RKeyID.SubjectKeyIdentifier.GetEncoded(
                            Asn1Encodable.Der
                        );
                    }

                    infos.Add(
                        new KeyAgreeRecipientInformation(info, rid, id.EncryptedKey, secureReadable)
                    );
                }
            }
            catch (IOException e)
        ...
        )
        {
            ...
            {
                origID.SubjectKeyIdentifier = originator.SubjectKeyIdentifier.GetEncoded(
                    Asn1Encodable.Der
                );
            }
        ...
        private AsymmetricKeyParameter GetPublicKeyFromOriginatorID(OriginatorID origID)
        {
            ...
            throw new CmsException(
                "No support for 'originator' as IssuerAndSerialNumber or SubjectKeyIdentifier"
            );
        }
        ...
        internal KeyParameter GetSessionKey(AsymmetricKeyParameter receiverPrivateKey)
        {
            try
            {
                var wrapAlgOid = DerObjectIdentifier.GetInstance(
                    Asn1Sequence.GetInstance(keyEncAlg.Parameters)[0]
                );
                ...
                KeyParameter agreedWrapKey = CalculateAgreedWrapKey(
                    wrapAlgOid,
                    senderPublicKey,
                    receiverPrivateKey
                );
                ...
                if (
                    CryptoProObjectIdentifiers.id_Gost28147_89_None_KeyWrap.Equals(wrapAlgOid)
                    || CryptoProObjectIdentifiers.id_Gost28147_89_CryptoPro_KeyWrap.Equals(
                        wrapAlgOid
                    )
                )
                {

...
namespace Org.BouncyCastle.Cms
{
    ...
    public class SignerInformation
    {
        ...
        public SignerInformationStore GetCounterSignatures()
        {
             ...
             */
            Asn1EncodableVector allCSAttrs = unsignedAttributeTable.GetAll(
            ...
            foreach (Asn1.Cms.Attribute counterSignatureAttribute in allCSAttrs)
            {
                ...
                foreach (
                    var signerInfo in CollectionUtilities.Select(values, SignerInfo.GetInstance)
                )
                {
                    /*
        ...
        private bool DoVerify(AsymmetricKeyParameter publicKey)
        {
                ...
                else
                {
                    if (!NistObjectIdentifiers.IdSha512.Equals(digAlgOid) || digAlgParams != null)
                        throw new CmsException(
                            "Ed25519 signature used with unsupported digest algorithm"
                        );
            ...
            //}
                ...
                MLDsaParameters.ByOid.TryGetValue(sigAlgOid, out MLDsaParameters mlDsaParameters)
            )
            {
                ...
                if (sigAlgParams != null)
                    throw new CmsException(
                        $"{mlDsaParameters} signature cannot specify algorithm parameters"
                    );
                ...
                else
                {
                    ...
                    if (
                        !NistObjectIdentifiers.IdSha512.Equals(digAlgOid)
                        && !NistObjectIdentifiers.IdShake256.Equals(digAlgOid)
                    )
                    {
                        throw new CmsException(
                            $"{mlDsaParameters} signature used with unsupported digest algorithm"
                        );
                    }
                ...
                SlhDsaParameters.ByOid.TryGetValue(sigAlgOid, out SlhDsaParameters slhDsaParameters)
            )
            {
                if (slhDsaParameters.IsPreHash)
                    throw new CmsException(
                        $"{slhDsaParameters} prehash signature is not supported"
                    );

                if (sigAlgParams != null)
                    throw new CmsException(
                        $"{slhDsaParameters} signature cannot specify algorithm parameters"
                    );
                ...
                else
                {
                    ...
                    if (!defaultDigAlgOid.Equals(digAlgOid) || digAlgParams != null)
                        throw new CmsException(
                            $"{slhDsaParameters} signature used with unsupported digest algorithm"
                        );
            ...
            else if (Asn1.Pkcs.PkcsObjectIdentifiers.IdRsassaPss.Equals(sigAlgOid))
            {
                ...
                try
                {
                    ...
                    if (!pss.HashAlgorithm.Algorithm.Equals(digAlgOid))
                        throw new CmsException(
                            "RSASSA-PSS signature parameters specified incorrect hash algorithm"
                        );
                    if (
                        !pss.MaskGenAlgorithm.Algorithm.Equals(
                            Asn1.Pkcs.PkcsObjectIdentifiers.IdMgf1
                        )
                    )
                        throw new CmsException(
                            "RSASSA-PSS signature parameters specified unknown MGF"
                        );
                    ...
                    if (!Asn1.Pkcs.RsassaPssParameters.DefaultTrailerField.Equals(pss.TrailerField))
                        throw new CmsException(
                            "RSASSA-PSS signature parameters must have trailerField of 1"
                        );
            ...
            else
            {
                ...
                string signatureName =
                    digestName + "with" + CmsSignedHelper.GetEncryptionAlgName(sigAlgOid);
            ...
            // RFC 3852 11.4 Validate countersignature attribute(s)
            {
                ...
                {
                    throw new CmsException(
                        "A countersignature attribute MUST NOT be a signed attribute"
                    );
                }
                ...
                {
                    foreach (
                        Asn1.Cms.Attribute csAttr in unsignedAttrTable.GetAll(
                            CmsAttributes.CounterSignature
                        )
                    )
                    {
        ...
        )
        {
            ...
            var newInfo = new SignerInfo(
                oldInfo.SignerID,
                oldInfo.DigestAlgorithm,
                oldInfo.SignedAttrs,
                oldInfo.SignatureAlgorithm,
                oldInfo.Signature,
                newUnsignedAttrs
            );
        ...
        )
        {
            ...
            var attrValues = DerSet.Map(
                counterSigners.SignersInternal,
                sigInf => sigInf.SignerInfo
            );

            var attributeTable = (
                signerInformation.UnsignedAttributes ?? DerSet.Empty.ToAttributeTable()

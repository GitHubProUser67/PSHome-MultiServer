...
namespace Org.BouncyCastle.Cms
{
    ...
    {
        private readonly List<SignerInfoGeneratorImpl> m_signerInfoGens =
            new List<SignerInfoGeneratorImpl>();
        private readonly HashSet<DerObjectIdentifier> m_messageDigestOids =
        ...
        private class SignerInfoGeneratorImpl
        {
            ...
            )
            {
                ...
                else
                {
                    ...
                    {
                        signer = SignerUtilities.InitSigner(
                            "NONEwithDSA",
                            true,
                            key,
                            outer.m_random
                    ...
                    else
                    {
                        throw new SignatureException(
                            "algorithm: " + m_encName + " not supported in base signatures."
                        );
                    }
                }
            ...
            internal SignerInfo Generate(DerObjectIdentifier contentType, byte[] calculatedDigest)
            {
                ...
                try
                {
                    ...
                    if (m_sAttrGen != null)
                    {
                        var parameters = m_outer.GetBaseParameters(
                            contentType,
                            digAlgID,
                            calculatedDigest
                        );
                    ...
                    AlgorithmIdentifier sigAlgID = CmsSignedHelper.GetSigAlgID(
                        m_sigAlgOid,
                        sigAlgParams
                    );
        ...
        */
        ...
        )
        {
            ...
            SignerInfoGeneratorImpl signerInfoGen = new SignerInfoGeneratorImpl(
                this,
                privateKey,
                signerIdentifier,
                digestOid,
        ...
        */
        public Stream Open(Stream outStream, bool encapsulate) =>
            Open(outStream, signedContentType: Data, encapsulate);
        ...
        */
        ...
        )
        {
            ...
            return new CmsSignedDataOutputStream(
                this,
                digStream,
                contentType,
                sGen,
        ...
        private void RegisterDigestOid(DerObjectIdentifier digestOid)
        {
            ...
            else if (!m_messageDigestOids.Contains(digestOid))
            {
                throw new InvalidOperationException(
                    "Cannot register new digest OIDs after the data stream is opened"
                );
            }
        }
        ...
        private void ConfigureDigest(DerObjectIdentifier digestOid)
        {
            ...
            if (!m_messageDigests.ContainsKey(digestOid))
            {
                if (_messageDigestsLocked)
                    throw new InvalidOperationException(
                        "Cannot configure new digests after the data stream is opened"
                    );
        ...
        private DerInteger CalculateVersion(DerObjectIdentifier contentOid)
        {
            ...
            if (
                attrCertV1Found
                || !CmsObjectIdentifiers.Data.Equals(contentOid)
        ...
        private static bool CheckForVersion3(
        ...
        private class CmsSignedDataOutputStream : BaseOutputStream
        {
            ...
            internal CmsSignedDataOutputStream(
                CmsSignedDataStreamGenerator outer,
                Stream outStream,
                DerObjectIdentifier contentType,
                BerSequenceGenerator sGen,
                BerSequenceGenerator sigGen,
                BerSequenceGenerator eciGen,
                BerOctetStringGenerator ecGen
            )
            {
                m_outer = outer;

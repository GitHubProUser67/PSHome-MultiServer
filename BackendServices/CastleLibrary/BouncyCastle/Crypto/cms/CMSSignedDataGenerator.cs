    ...
    public class CmsSignedDataGenerator : CmsSignedGenerator
    {
        ...
        private class SignerInf
        {
            ...
            )
            {
                ...
                //}
                    ...
                    MLDsaParameters.ByOid.TryGetValue(
                        sigAlgOid,
                        out MLDsaParameters mlDsaParameters
                    )
                )
                {
                    if (mlDsaParameters.IsPreHash)
                        throw new CmsException(
                            $"{mlDsaParameters} prehash signature is not supported"
                        );
                    ...
                    SlhDsaParameters.ByOid.TryGetValue(
                        sigAlgOid,
                        out SlhDsaParameters slhDsaParameters
                    )
                )
                {
            ...
            )
            {
                ...
                if (EdECObjectIdentifiers.id_Ed25519.Equals(sigAlgOid))
                {
                    if (sigAlgParams != null)
                        throw new CmsException(
                            "Ed25519 signature cannot specify algorithm parameters"
                        );
            ...
            )
            {
                ...
                else
                {
                    string digestName = CmsSignedHelper.GetDigestAlgName(digAlgOid);
                    string signatureName =
                        digestName + "with" + CmsSignedHelper.GetEncryptionAlgName(m_sigAlgOid);

                    // TODO[RSAPSS] Need the ability to specify non-default parameters
                    Asn1Encodable sigAlgParams = SignerUtilities.GetDefaultX509Parameters(
                        signatureName
                    );
        ...
        public void AddSigner(
            ...
            AddSigner(
                privateKey,
                cert,
                CmsSignedHelper.GetEncOid(privateKey, digestOID)?.GetID(),
                digestOID
            );
        ...
        )
        {
            DoAddSigner(
                privateKey,
                GetSignerIdentifier(subjectKeyID),
                new DerObjectIdentifier(encryptionOID),
                new DerObjectIdentifier(digestOID),
                new DefaultSignedAttributeTableGenerator(),
                null,
        ...
        public void AddSignerInfoGenerator(SignerInfoGenerator signerInfoGenerator)
        {
            signerInfs.Add(
                new SignerInf(
                    this,
                    signerInfoGenerator.SignatureFactory,
                    signerInfoGenerator.SignerID,
                    signerInfoGenerator.SignedAttributeTableGenerator,
                    signerInfoGenerator.UnsignedAttributeTableGenerator,
                    baseSignedTable: null
                )
            );
        }
        ...
        public CmsSignedData Generate(CmsProcessable content) =>
            Generate(content, encapsulate: false);
        ...
        public SignerInformationStore GenerateCounterSigners(SignerInformation signer)
        {
            ...
            {
                signerInformations.Add(
                    new SignerInformation(_signer.SignerInfo, null, content, null)
                );
            }

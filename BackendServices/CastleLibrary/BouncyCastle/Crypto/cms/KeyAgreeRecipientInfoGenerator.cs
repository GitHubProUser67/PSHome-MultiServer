...
namespace Org.BouncyCastle.Cms
{
    ...
    {
        private readonly List<KeyAgreeRecipientIdentifier> m_recipientIDs =
            new List<KeyAgreeRecipientIdentifier>();
        private readonly List<AsymmetricKeyParameter> m_recipientKeys =
            new List<AsymmetricKeyParameter>();
        ...
        {
            m_recipientIDs.Add(
                new KeyAgreeRecipientIdentifier(new RecipientKeyIdentifier(subjectKeyID))
            );
        ...
        public RecipientInfo Generate(KeyParameter contentEncryptionKey, SecureRandom random)
        {
            ...
            if (CmsUtilities.IsMqv(m_keyAgreementOid))
            {
                try
                {
                    ...
                    ephemKPG.Init(
                        ((ECPublicKeyParameters)senderPublicKey).CreateKeyGenerationParameters(
                            random
                        )
                    );
                    ...
                    ukm = new DerOctetString(
                        new MQVuserKeyingMaterial(CreateOriginatorPublicKey(ephemKP.Public), null)
                    );
            ...
            for (int i = 0; i < m_recipientIDs.Count; ++i)
            {
                ...
                int keyEncryptionKeySize =
                    GeneratorUtilities.GetDefaultKeySize(m_keyEncryptionOid) / 8;
                byte[] keyEncryptionKeyBytes = X9IntegerConverter.IntegerToBytes(
                    agreedValue,

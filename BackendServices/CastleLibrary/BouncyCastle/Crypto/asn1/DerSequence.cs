    ...
    public class DerSequence : Asn1Sequence
    {
        ...
        )
        {
            return sequence.Count < 1
                ? Empty
                : new DerSequence(sequence.MapElements(func), clone: false);
        }
        ...
        public static DerSequence Map<T>(T[] ts, Func<T, Asn1Encodable> func)
        {
            return ts.Length < 1
                ? Empty
                : new DerSequence(CollectionUtilities.Map(ts, func), clone: false);
        }
        ...
        public static DerSequence Map<T>(IReadOnlyCollection<T> c, Func<T, Asn1Encodable> func)
        {
            return c.Count < 1
                ? Empty
                : new DerSequence(CollectionUtilities.Map(c, func), clone: false);
        }
        ...
        {
            return new ConstructedDLEncoding(
                Asn1Tags.Universal,
                Asn1Tags.Sequence,
        ...
        internal override Asn1OctetString ToAsn1OctetString() =>
            DerOctetString.WithContents(
                BerOctetString.FlattenOctetStrings(GetConstructedOctetStrings())
            );

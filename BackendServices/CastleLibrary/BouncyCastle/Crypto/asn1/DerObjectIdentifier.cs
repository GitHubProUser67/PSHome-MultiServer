...
namespace Org.BouncyCastle.Asn1
{
    public class DerObjectIdentifier : Asn1Object
    {
        internal class Meta : Asn1UniversalType
        {
            ...
            private Meta()
                : base(typeof(DerObjectIdentifier), Asn1Tags.ObjectIdentifier) { }
            ...
            else if (obj is byte[] bytes)
            {
                ...
                catch (IOException e)
                {
                    throw new ArgumentException(
                        "failed to construct object identifier from byte[]: " + e.Message
                    );
                }
            }
        ...
        public static DerObjectIdentifier GetTagged(
            Asn1TaggedObject taggedObject,
            bool declaredExplicit
        )
        {
        ...
        internal static DerObjectIdentifier CreatePrimitive(byte[] contents, bool clone)
        {
            ...
            var newEntry = new DerObjectIdentifier(
                clone ? Arrays.Clone(contents) : contents,
                identifier: null
            );

            var exchangedEntry = Interlocked.CompareExchange(
                ref Cache[index],
                newEntry,
                originalEntry
            );
        ...
        private static byte[] ParseIdentifier(string identifier)
        {
            ...
            {
                Asn1RelativeOid.WriteField(
                    bOut,
                    new BigInteger(token).Add(BigInteger.ValueOf(first))
                );
            }

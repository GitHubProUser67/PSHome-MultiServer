...
namespace Org.BouncyCastle.Asn1.Mozilla
{
    ...
    public class SignedPublicKeyAndChallenge : Asn1Encodable
    {
        ...
        public static SignedPublicKeyAndChallenge GetInstance(
            Asn1TaggedObject taggedObject,
            bool declaredExplicit
        ) =>
            new SignedPublicKeyAndChallenge(
                Asn1Sequence.GetInstance(taggedObject, declaredExplicit)
            );

        public static SignedPublicKeyAndChallenge GetTagged(
            Asn1TaggedObject taggedObject,
            bool declaredExplicit
        ) =>
        ...
        {
            m_publicKeyAndChallenge =
                publicKeyAndChallenge
                ?? throw new ArgumentNullException(nameof(publicKeyAndChallenge));
            m_signatureAlgorithm =
                signatureAlgorithm ?? throw new ArgumentNullException(nameof(signatureAlgorithm));
        ...
        private SignedPublicKeyAndChallenge(Asn1Sequence seq)
        {
            ...
            if (seq.Count != 3)
                throw new ArgumentException(
                    $"Expected 3 elements, but found {seq.Count}",
                    nameof(seq)
                );

...
namespace Org.BouncyCastle.Asn1.Cmp
{
    public class PollReqContent : Asn1Encodable
    {
        ...
        public static PollReqContent GetTagged(
            Asn1TaggedObject taggedObject,
            bool declaredExplicit
        ...
        public PollReqContent(DerInteger[] certReqIds)
            : this(
                DerSequence.WithElements(
                    CollectionUtilities.Map(certReqIds, id => new DerSequence(id))
                )
            ) { }
        ...
        public virtual DerInteger[][] GetCertReqIDs() =>
            m_content.MapElements(element =>
                Asn1Sequence.GetInstance(element).MapElements(DerInteger.GetInstance)
            );

        public virtual BigInteger[] GetCertReqIDValues() =>
            m_content.MapElements(element =>
                DerInteger.GetInstance(Asn1Sequence.GetInstance(element)[0]).Value
            );

    ...
    public class GeneralName : Asn1Encodable, IAsn1Choice
    {
            ...
            switch (tag)
            {
                ...
                default:
                {
                    string message = string.Format(
                        "can't process string for tag: {0}",
        ...
        private byte[] ToGeneralNameEncoding(string ip)
        {
            if (NetUtils.IPAddress.IsValidIPv6WithNetmask(ip) || NetUtils.IPAddress.IsValidIPv6(ip))
            {
                ...
                NetUtils.IPAddress.IsValidIPv4WithNetmask(ip) || NetUtils.IPAddress.IsValidIPv4(ip)
            )
            {
                int slashIndex = Platform.IndexOf(ip, '/');

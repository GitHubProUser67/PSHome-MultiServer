...
namespace Org.BouncyCastle.Asn1.X509
{
    ...
    public class X509ExtensionsGenerator
    {
        ...
        private static readonly HashSet<DerObjectIdentifier> m_dupsAllowed =
        ...
        public void AddExtension(DerObjectIdentifier oid, bool critical, Asn1Encodable extValue)
        {
            if (m_extensions.TryGetValue(oid, out X509Extension existingExtension))
            {
                ImplAddExtensionDup(
                    ...
                    oid,
                    critical,
                    extValue.GetEncoded(Asn1Encodable.Der)
                );
            }
            else
            {
        ...
        public void AddExtension(DerObjectIdentifier oid, bool critical, byte[] extValue)
        {
            ...
            else
            {
                ImplAddExtension(
                    oid,
                    new X509Extension(critical, DerOctetString.FromContents(extValue))
                );
            }
        }
        ...
        private void ImplAddExtensionDup(
            X509Extension existingExtension,
            DerObjectIdentifier oid,
            bool critical,
            byte[] extValue
        )

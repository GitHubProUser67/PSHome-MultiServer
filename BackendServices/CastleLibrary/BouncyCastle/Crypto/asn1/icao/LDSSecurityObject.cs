...
namespace Org.BouncyCastle.Asn1.Icao
{
    ...
    public class LdsSecurityObject : Asn1Encodable
    {
        ...
        public static LdsSecurityObject GetTagged(
            Asn1TaggedObject taggedObject,
            bool declaredExplicit
        ...
        private LdsSecurityObject(Asn1Sequence seq)
        {
            ...
            if (m_ldsVersionInfo != null && !m_version.HasValue(1))
                throw new ArgumentException(
                    "'ldsVersionInfo' is present, but 'version' is NOT 'v1'"
                );
        }
        ...
        )
        {
            ...
            m_hashAlgorithm =
                ...
                ?? throw new ArgumentNullException(nameof(digestAlgorithmIdentifier));
            ;
            m_datagroupHashValues =
                datagroupHash ?? throw new ArgumentNullException(nameof(datagroupHash));
        ...
        private static void CheckDatagroupHashCount(int count)
        {
            if (count < 2 || count > UBDataGroups)
                throw new ArgumentException(
                    "wrong size in DataGroupHashValues : not in (2.." + UBDataGroups + ")"
                );
        }

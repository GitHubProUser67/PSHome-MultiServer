...
namespace Org.BouncyCastle.Asn1.Cms
{
    public class OriginatorIdentifierOrKey : Asn1Encodable, IAsn1Choice
    {
        ...
        public static OriginatorIdentifierOrKey GetOptional(Asn1Encodable element)
        {
            ...
            IssuerAndSerialNumber issuerAndSerialNumber = IssuerAndSerialNumber.GetOptional(
                element
            );
            ...
            if (taggedObject != null)
            {
                if (taggedObject.HasContextTag(0))
                    return new OriginatorIdentifierOrKey(
                        SubjectKeyIdentifier.GetTagged(taggedObject, false)
                    );

                if (taggedObject.HasContextTag(1))
                    return new OriginatorIdentifierOrKey(
                        OriginatorPublicKey.GetTagged(taggedObject, false)
                    );
            }
        ...
        public static OriginatorIdentifierOrKey GetTagged(
        ...
        public IssuerAndSerialNumber IssuerAndSerialNumber =>
            IssuerAndSerialNumber.GetInstance(m_id);

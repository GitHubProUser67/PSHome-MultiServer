...
namespace Org.BouncyCastle.Asn1.Tsp
{
    ...
    public class PartialHashtree : Asn1Encodable
    {
        ...
        public static PartialHashtree GetInstance(
        ...
        public static PartialHashtree GetTagged(
            Asn1TaggedObject taggedObject,
            bool declaredExplicit
            ...
            for (int i = 0; i != values.Count; i++)
            {
                if (!(values[i] is Asn1OctetString))
                    throw new ArgumentException(
                        "unknown object in constructor: " + Platform.GetTypeName(values[i])
                    );
            }
            m_values = values;
        }
        ...
        public virtual byte[][] GetValues() =>
            m_values.MapElements(element =>
                Arrays.Clone(Asn1OctetString.GetInstance(element).GetOctets())
            );

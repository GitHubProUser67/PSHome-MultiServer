...
namespace Org.BouncyCastle.Asn1
{
    public class DLSequence : DerSequence
    {
        ...
        public static new DLSequence Map(
        ...
        )
        {
            return sequence.Count < 1
                ? Empty
                : new DLSequence(sequence.MapElements(func), clone: false);
        }
        ...
        public static new DLSequence Map<T>(T[] ts, Func<T, Asn1Encodable> func)
        {
            return ts.Length < 1
                ? Empty
                : new DLSequence(CollectionUtilities.Map(ts, func), clone: false);
        }
        ...
        public static new DLSequence Map<T>(IReadOnlyCollection<T> c, Func<T, Asn1Encodable> func)
        {
            return c.Count < 1
                ? Empty
                : new DLSequence(CollectionUtilities.Map(c, func), clone: false);
        }

...
namespace Org.BouncyCastle.Asn1
{
    public class DerInteger : Asn1Object
    {
        internal class Meta : Asn1UniversalType
        {
            ...
            private Meta()
                : base(typeof(DerInteger), Asn1Tags.Integer) { }
            ...
            else if (obj is byte[] bytes)
            {
                ...
                catch (IOException e)
                {
                    throw new ArgumentException(
                        "failed to construct integer from byte[]: " + e.Message
                    );
                }
            }
        ...
        {
            return (bytes.Length - start) <= 4 && IntValue(bytes, start, SignExtSigned) == x;
        }

        public bool HasValue(long x)
        {
            return (bytes.Length - start) <= 8 && LongValue(bytes, start, SignExtSigned) == x;
        }
        ...
        {
            return Asn1OutputStream.GetLengthOfEncodingDL(
                Asn1Tags.Integer,
                BigIntegers.GetByteLength(x)
            );
        }
        ...
        internal static int SignBytesToSkip(byte[] bytes)
        {
            int pos = 0,
                last = bytes.Length - 1;
            while (pos < last && (sbyte)bytes[pos] == ((sbyte)bytes[pos + 1] >> 7))
            {
                ++pos;
            }
            return pos;

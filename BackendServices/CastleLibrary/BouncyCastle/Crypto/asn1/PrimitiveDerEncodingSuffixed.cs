...
namespace Org.BouncyCastle.Asn1
{
    internal class PrimitiveDerEncodingSuffixed : DerEncoding
    {
        ...
        protected internal override int CompareLengthAndContents(DerEncoding other)
        {
            ...
            {
                return CompareSuffixed(
                    this.m_contentsOctets,
                    this.m_contentsSuffix,
                    suff.m_contentsOctets,
                    suff.m_contentsSuffix
                );
            }
            else if (other is PrimitiveDerEncoding that)
            {
                ...
                return CompareSuffixed(
                    this.m_contentsOctets,
                    this.m_contentsSuffix,
                    that.m_contentsOctets,
                    that.m_contentsOctets[length - 1]
                );
            }
            else
            {
        ...
        private static int CompareSuffixed(
            byte[] octetsA,
            byte suffixA,
            byte[] octetsB,
            byte suffixB
        )
        {
            ...
            int last = length - 1;
            int c = octetsA.AsSpan(0, last).SequenceCompareTo(octetsB.AsSpan(0, last));
            if (c != 0)
                return c;

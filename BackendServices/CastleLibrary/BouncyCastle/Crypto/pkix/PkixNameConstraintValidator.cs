...
namespace Org.BouncyCastle.Pkix
{
    public class PkixNameConstraintValidator
    {
        ...
        private static bool WithinDNSubtree(Asn1Sequence dns, Asn1Sequence subtree)
        {
            ...
            for (int j = 0; j < subtree.Count; ++j)
            {
                ...
                )
                {
                    if (
                        !Platform.StartsWith(
                            dnsRdn.GetFirst().Value.ToString(),
                            subtreeRdn.GetFirst().Value.ToString()
                        )
                    )
                        return false;
                }
        ...
        {
            var union =
                permitted != null ? new HashSet<OtherName>(permitted) : new HashSet<OtherName>();
            union.Add(otherName);
            return union;
        }
        ...
        private HashSet<string> IntersectEmail(
            HashSet<string> permitted,
            HashSet<GeneralSubtree> emails
        )
        {
        ...
        private HashSet<string> IntersectDns(
            HashSet<string> permitted,
            HashSet<GeneralSubtree> dnss
        )
        {
            var intersect = new HashSet<string>();
        ...
        private HashSet<string> IntersectUri(
            HashSet<string> permitted,
            HashSet<GeneralSubtree> uris
        )
        {
            var intersect = new HashSet<string>();
            ...
            switch (name.TagNo)
            {
                case GeneralName.OtherName:
                    CheckPermittedOtherName(
                        ...
                        OtherName.GetInstance(name.Name)
                    );
                    break;
                ...
                case GeneralName.IPAddress:
                    CheckPermittedIP(
                        permittedSubtreesIP,
                        Asn1OctetString.GetInstance(name.Name).GetOctets()
                    );
                    break;
            }
        }
        ...
        public void IntersectPermittedSubtree(Asn1Sequence permitted)
        {
            ...
            foreach (var entry in subtreesMap)
            {
                ...
                {
                    case GeneralName.OtherName:
                        permittedSubtreesOtherName = IntersectOtherName(
                            permittedSubtreesOtherName,
                            entry.Value
                        );
                        break;
        ...
        public void AddExcludedSubtree(GeneralSubtree subtree)
        {
            ...
            {
                case GeneralName.OtherName:
                    excludedSubtreesOtherName = UnionOtherName(
                        excludedSubtreesOtherName,
                        OtherName.GetInstance(subTreeBase.Name)
                    );
                    break;
                case GeneralName.Rfc822Name:
                    excludedSubtreesEmail = UnionEmail(
                        excludedSubtreesEmail,
                        ExtractNameAsString(subTreeBase)
                    );
                    break;
                case GeneralName.DnsName:
                    excludedSubtreesDns = UnionDns(
                        excludedSubtreesDns,
                        ExtractNameAsString(subTreeBase)
                    );
                    break;
                case GeneralName.DirectoryName:
                    excludedSubtreesDN = UnionDN(
                        excludedSubtreesDN,
                        (Asn1Sequence)subTreeBase.Name.ToAsn1Object()
                    );
                    break;
                case GeneralName.UniformResourceIdentifier:
                    excludedSubtreesUri = UnionUri(
                        excludedSubtreesUri,
                        ExtractNameAsString(subTreeBase)
                    );
                    break;
                case GeneralName.IPAddress:
                    excludedSubtreesIP = UnionIP(
                        excludedSubtreesIP,

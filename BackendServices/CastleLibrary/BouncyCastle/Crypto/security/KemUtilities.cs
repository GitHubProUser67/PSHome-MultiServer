...
namespace Org.BouncyCastle.Security
{
    public static class KemUtilities
    {
        ...
        static KemUtilities()
        {
            ...
            foreach (var mechanism in mechanisms)
            {
                ...
                if (!ByName.TryGetValue(mechanism, out var check) || check != mechanism)
                    throw new Exception(
                        $"Mechanism must have {nameof(ByName)} mapping to self: {mechanism}"
                    );
            }
#endif
        }
        ...
        internal static byte[] Decapsulate(
            IKemDecapsulator kemDecapsulator,
            byte[] encBuf,
            int encOff,
            int encLen
        )
        ...
        public static bool TryGetDecapsulator(
            DerObjectIdentifier oid,
            out IKemDecapsulator decapsulator
        )
        {
            if (oid == null)
        ...
        public static bool TryGetEncapsulator(
            DerObjectIdentifier oid,
            out IKemEncapsulator encapsulator
        )
        {
            if (oid == null)
                throw new ArgumentNullException(nameof(oid));

...
namespace Org.BouncyCastle.Security
{
    ...
    {
        private static readonly IDictionary<string, string> KgAlgorithms = new Dictionary<
            string,
            string
        >(StringComparer.OrdinalIgnoreCase);
        private static readonly IDictionary<string, string> KpgAlgorithms = new Dictionary<
            string,
            string
        >(StringComparer.OrdinalIgnoreCase);
        private static readonly IDictionary<string, int> DefaultKeySizes = new Dictionary<
            string,
            int
        >(StringComparer.OrdinalIgnoreCase);
        ...
        static GeneratorUtilities()
        {
            ...
            AddHMacKeyGenerator("SHA512/224", PkcsObjectIdentifiers.IdHmacWithSha512_224);
            AddHMacKeyGenerator("SHA512-224");
            AddHMacKeyGenerator("SHA512/256", PkcsObjectIdentifiers.IdHmacWithSha512_256);
        ...
        public static CipherKeyGenerator GetKeyGenerator(string algorithm)
        {
            ...
            if (canonicalName == null)
                throw new SecurityUtilityException(
                    "KeyGenerator " + algorithm + " not recognised."
                );

            int defaultKeySize = FindDefaultKeySize(canonicalName);
            if (defaultKeySize == -1)
                throw new SecurityUtilityException(
                    "KeyGenerator " + algorithm + " (" + canonicalName + ") not supported."
                );
        ...
        public static IAsymmetricCipherKeyPairGenerator GetKeyPairGenerator(string algorithm)
        {
            ...
            if (canonicalName == null)
                throw new SecurityUtilityException(
                    "KeyPairGenerator " + algorithm + " not recognised."
                );
            ...
            throw new SecurityUtilityException(
                "KeyPairGenerator " + algorithm + " (" + canonicalName + ") not supported."
            );
        }

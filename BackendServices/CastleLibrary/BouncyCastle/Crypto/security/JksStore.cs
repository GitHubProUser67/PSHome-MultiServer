...
namespace Org.BouncyCastle.Security
{
    public class JksStore
    {
        ...
        private readonly Dictionary<string, JksKeyEntry> m_keyEntries = new Dictionary<
            string,
            JksKeyEntry
        ...
        private byte[] GetKeyChecksum(
            IDigest digest,
            ReadOnlySpan<char> password,
            ReadOnlySpan<byte> pkcs8Key
        )
        {
        ...
        private byte[] CalculateKeyStream(
            IDigest digest,
            ReadOnlySpan<char> password,
            ...
            while (i < mUtfBytes.Length)
            {
                ...
                if (mUtfByte == 0)
                    throw new NotSupportedException(
                        "Unexpected NULL byte in modified UTF-8 encoding in JKS"
                    );
                ...
                else if ((mUtfByte & 0xE0) == 0xC0)
                {
                    ...
                    if ((i + 1) >= mUtfBytes.Length)
                        throw new NotSupportedException(
                            "Two-byte sentinel found at end of input stream"
                        );

                    byte mUtfByteSecond = mUtfBytes[i + 1];
                    if ((mUtfByteSecond & 0xC0) != 0x80)
                        throw new NotSupportedException(
                            "Second byte in two-byte modified UTF-8 encoding malformed"
                        );
                ...
                else if ((mUtfByte & 0xF0) == 0xE0)
                {
                    ...
                    if ((i + 2) >= mUtfBytes.Length)
                        throw new NotSupportedException(
                            "Three-byte sentinel found at end of input stream"
                        );
                    ...
                    if ((mUtfByteThird & 0xC0) != 0x80)
                        throw new NotSupportedException(
                            "Third byte in two-byte modified UTF-8 encoding malformed"
                        );
                ...
                else
                {
                    ...
                    throw new NotSupportedException(
                        "Malformed modified UTF-8 encoding at index " + i
                    );
                }
            }
            ...
            while (i < utfBytes.Length)
            {
                ...
                else if ((utfByte & 0xE0) == 0xC0)
                {
                    ...
                    if ((i + 1) >= utfBytes.Length)
                        throw new NotSupportedException(
                            "Malformed UTF-8: trailing two-byte character at end of string"
                        );

                    if ((utfBytes[i + 1] & 0xC0) != 0x80)
                        throw new NotSupportedException(
                            "Malformed UTF-8: second byte has invalid prefix"
                        );
                ...
                else if ((utfByte & 0xF0) == 0xE0)
                {
                    ...
                    if ((i + 2) >= utfBytes.Length)
                        throw new NotSupportedException(
                            "Malformed UTF-8: trailing three-byte character at end of string"
                        );
                    ...
                    if ((utfBytes[i + 2] & 0xC0) != 0x80)
                        throw new NotSupportedException(
                            "Malformed UTF-8: third byte has invalid prefix"
                        );

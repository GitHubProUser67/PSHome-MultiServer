...
namespace Org.BouncyCastle.Security
{
    ...
    public static class CipherUtilities
    {
        ...
        static CipherUtilities()
        {
            ...
            AlgorithmOidMap[PkcsObjectIdentifiers.PbeWithShaAnd128BitRC4] =
                "PBEWITHSHAAND128BITRC4";
            ...
            AlgorithmOidMap[PkcsObjectIdentifiers.PbeWithShaAnd3KeyTripleDesCbc] =
            ...
            AlgorithmOidMap[PkcsObjectIdentifiers.PbeWithShaAnd2KeyTripleDesCbc] =
                "PBEWITHSHAAND2-KEYTRIPLEDES-CBC";

            AlgorithmMap["PBEWITHSHA1AND128BITRC2-CBC"] = "PBEWITHSHAAND128BITRC2-CBC";
            AlgorithmOidMap[PkcsObjectIdentifiers.PbeWithShaAnd128BitRC2Cbc] =
                "PBEWITHSHAAND128BITRC2-CBC";

            AlgorithmMap["PBEWITHSHA1AND40BITRC2-CBC"] = "PBEWITHSHAAND40BITRC2-CBC";
            AlgorithmOidMap[PkcsObjectIdentifiers.PbewithShaAnd40BitRC2Cbc] =
            ...
            AlgorithmOidMap[CryptoProObjectIdentifiers.GostR28147Gcfb] =
        ...
        private static IBufferedCipher GetCipherForMechanism(string mechanism)
        {
            ...
            if (Platform.StartsWith(mechanism, "PBE"))
            {
                if (Platform.EndsWith(mechanism, "-CBC"))
                {
                    ...
                    Platform.EndsWith(mechanism, "-BC") || Platform.EndsWith(mechanism, "-OPENSSL")
                )
                {
                    if (
                        Strings.IsOneOf(
            ...
            string algorithmName = CollectionUtilities
                .GetValueOrKey(AlgorithmMap, parts[0])
                .ToUpperInvariant();
            ...
            if (parts.Length > 2)
            {
                ...
                switch (cipherPadding)
                {
                    ...
                    case CipherPadding.OAEPWITHSHA_256ANDMGF1WITHSHA_1PADDING:
                        asymBlockCipher = new OaepEncoding(
                            asymBlockCipher,
                            new Sha256Digest(),
                            new Sha1Digest(),
                            null
                ...
                switch (cipherMode)
                {
                    ...
                    case CipherMode.OCB:
                        aeadBlockCipher = new OcbBlockCipher(
                            blockCipher,
                            CreateBlockCipher(cipherAlgorithm)
                        );
                        break;
                    case CipherMode.OFB:
                    {
        ...
        private static IBlockCipher CreateBlockCipher(CipherAlgorithm cipherAlgorithm)
        {
            switch (cipherAlgorithm)
            {
                ...
                case CipherAlgorithm.THREEFISH_256:
                    return new ThreefishEngine(ThreefishEngine.BLOCKSIZE_256);
                case CipherAlgorithm.THREEFISH_512:
                    return new ThreefishEngine(ThreefishEngine.BLOCKSIZE_512);
                case CipherAlgorithm.THREEFISH_1024:
                    return new ThreefishEngine(ThreefishEngine.BLOCKSIZE_1024);
                case CipherAlgorithm.TNEPRES:
                ...
                case CipherAlgorithm.LIBSECUREXTEA:
                    return new LibSecureXteaEngine();
                default:
                    throw new SecurityUtilityException(
                        "Cipher " + cipherAlgorithm + " not recognised or not a block cipher"
                    );
            }
        }
    }

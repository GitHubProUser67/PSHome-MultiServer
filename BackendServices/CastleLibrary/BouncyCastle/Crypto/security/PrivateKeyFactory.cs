...
namespace Org.BouncyCastle.Security
{
    public static class PrivateKeyFactory
    {
        ...
        public static AsymmetricKeyParameter CreateKey(PrivateKeyInfo keyInfo)
        {
            ...
            if (
                algOid.Equals(PkcsObjectIdentifiers.RsaEncryption)
            ...
            {
                ECPrivateKeyStructure ecPrivateKey = ECPrivateKeyStructure.GetInstance(
                    keyInfo.ParsePrivateKey()
                );
                X962Parameters parameters = X962Parameters.GetInstance(algID.Parameters);
                ECDomainParameters domainParameters = ECDomainParameters.FromX962Parameters(
                    parameters
                );
            ...
            )
            {
                ...
                Gost3410PublicKeyAlgParameters gostParams =
                    Gost3410PublicKeyAlgParameters.GetInstance(p);
                ...
                {
                    X9ECParameters ecP = ECGost3410NamedCurves.GetByOid(
                        gostParams.PublicKeyParamSet
                    );
                    if (ecP == null)
                        throw new ArgumentException(
                            "Unrecognized curve OID for GostR3410x2001 private key"
                        );
            ...
            {
                Gost3410PublicKeyAlgParameters gostParams =
                    Gost3410PublicKeyAlgParameters.GetInstance(algID.Parameters);
                ...
                else
                {
                    x = new BigInteger(
                        1,
                        Asn1OctetString.GetInstance(privKey).GetOctets(),
                        bigEndian: false
                    );
                }
                ...
                algOid.Equals(EdECObjectIdentifiers.id_X25519)
            ...
            else if (algOid.Equals(EdECObjectIdentifiers.id_X448))
            {
                ...
                algOid.Equals(EdECObjectIdentifiers.id_Ed25519)
            ...
            {
                Gost3410PublicKeyAlgParameters gostParams =
                    Gost3410PublicKeyAlgParameters.GetInstance(
                ...
                if (
                    p is Asn1Sequence
                    && (
                        Asn1Sequence.GetInstance(p).Count == 2
                        || Asn1Sequence.GetInstance(p).Count == 3
                    )
                )
                {
                    X9ECParameters ecP = ECGost3410NamedCurves.GetByOid(
            ...
            else if (MLDsaParameters.ByOid.TryGetValue(algOid, out MLDsaParameters mlDsaParameters))
            {
                ...
                // TODO[api] Eventually remove legacy support for raw octets
                {
                    ...
                    if (length == parameterSet.SeedLength)
                        return MLDsaPrivateKeyParameters.FromSeed(
                            mlDsaParameters,
                            seed: privateKey.GetOctets()
                        );

                    if (length == parameterSet.PrivateKeyLength)
                        return MLDsaPrivateKeyParameters.FromEncoding(
                            mlDsaParameters,
                            encoding: privateKey.GetOctets()
                        );
                }
                ...
                try
                {
                    ...
                    if (asn1Object is Asn1TaggedObject taggedSeedOnly)
                    {
                        ...
                        if (taggedSeedOnly.HasContextTag(0))
                        {
                            var seed = Asn1OctetString
                                .GetTagged(taggedSeedOnly, declaredExplicit: false)
                                .GetOctets();
                    ...
                    else if (asn1Object is Asn1Sequence sequence)
                    {
                        ...
                        if (sequence.Count == 2)
                        {
                            ...
                            var fromSeed = MLDsaPrivateKeyParameters.FromSeed(
                                mlDsaParameters,
                                seed,
                            ...
                            if (!Arrays.FixedTimeEquals(fromSeed.GetEncoded(), encoding))
                                throw new ArgumentException(
                                    "inconsistent " + mlDsaParameters.Name + " private key"
                                );
            ...
            else if (MLKemParameters.ByOid.TryGetValue(algOid, out MLKemParameters mlKemParameters))
            {
                ...
                // TODO[api] Eventually remove legacy support for raw octets
                {
                    ...
                    if (length == parameterSet.SeedLength)
                        return MLKemPrivateKeyParameters.FromSeed(
                            mlKemParameters,
                            seed: privateKey.GetOctets()
                        );

                    if (length == parameterSet.PrivateKeyLength)
                        return MLKemPrivateKeyParameters.FromEncoding(
                            mlKemParameters,
                            encoding: privateKey.GetOctets()
                        );
                }
                ...
                try
                {
                    ...
                    else if (asn1Object is Asn1Sequence sequence)
                    {
                        ...
                        if (sequence.Count == 2)
                        {
                            ...
                            var fromSeed = MLKemPrivateKeyParameters.FromSeed(
                                mlKemParameters,
                                seed,
                            ...
                            if (!Arrays.FixedTimeEquals(fromSeed.GetEncoded(), encoding))
                                throw new ArgumentException(
                                    "inconsistent " + mlKemParameters.Name + " private key"
                                );
                ...
                SlhDsaParameters.ByOid.TryGetValue(algOid, out SlhDsaParameters slhDsaParameters)
            )
            {
                // NOTE: We ignore the publicKey field since the private key already includes the public key
                // TODO[pqc] Validate the public key if it is included?
                ...
                if (octetsLength == privateKeyLength)
                    return SlhDsaPrivateKeyParameters.FromEncoding(
                        slhDsaParameters,
                        encoding: privateKey.GetOctets()
                    );
            ...
            else
            {
                throw new SecurityUtilityException(
                    "algorithm identifier in private key not recognised"
                );
            }
        }
        ...
        public static AsymmetricKeyParameter DecryptKey(
            ...
            DecryptKey(
                passPhrase,
                EncryptedPrivateKeyInfo.GetInstance(encryptedPrivateKeyInfoData)
            );
        ...
        )
        {
            return DecryptKey(
                ...
                EncryptedPrivateKeyInfo.GetInstance(
                    Asn1Object.FromStream(encryptedPrivateKeyInfoStream)
                )
            );
        }
        ...
        )
        {
            return EncryptedPrivateKeyInfoFactory
                .CreateEncryptedPrivateKeyInfo(algorithm, passPhrase, salt, iterationCount, key)
                .GetEncoded();
        }

...
namespace Org.BouncyCastle.Pkcs
{
    ...
    public class Pkcs10CertificationRequest : CertificationRequest
    {
        ...
        internal static readonly Dictionary<string, Asn1Encodable> m_exParams = new Dictionary<
            string,
            Asn1Encodable
        ...
        static Pkcs10CertificationRequest()
        {
            ...
            m_algorithms.Add("SHA3-224WITHDSA", NistObjectIdentifiers.IdDsaWithSha3_224);
            m_algorithms.Add("SHA3-256WITHDSA", NistObjectIdentifiers.IdDsaWithSha3_256);
            m_algorithms.Add("SHA3-384WITHDSA", NistObjectIdentifiers.IdDsaWithSha3_384);
            m_algorithms.Add("SHA3-512WITHDSA", NistObjectIdentifiers.IdDsaWithSha3_512);
            ...
            m_algorithms.Add("SHA3-224WITHECDSA", NistObjectIdentifiers.IdEcdsaWithSha3_224);
            m_algorithms.Add("SHA3-256WITHECDSA", NistObjectIdentifiers.IdEcdsaWithSha3_256);
        ...
        public Pkcs10CertificationRequest(
            ...
            : this(
                new Asn1SignatureFactory(signatureAlgorithm, signingKey),
                subject,
                publicKey,
                attributes
            ) { }
        ...
        {
            SubjectPublicKeyInfo pubInfo = SubjectPublicKeyInfoFactory.CreateSubjectPublicKeyInfo(
                publicKey
            );
                ...
                foreach (Asn1Encodable item in reqInfo.Attributes)
                {
                    ...
                    catch (ArgumentException ex)
                    {
                        throw new ArgumentException(
                            "encountered non PKCS attribute in extensions block",
                            ex
                        );
                    }

                    if (
                        PkcsObjectIdentifiers.Pkcs9AtExtensionRequest.Equals(attributePkcs.AttrType)
                    )
                    {
                        ...
                        if (attrValues == null || attrValues.Count == 0)
                            throw new InvalidOperationException(
                                "pkcs_9_at_extensionRequest present but has no value"
                            );
                            ...
                            foreach (Asn1Encodable seqItem in extensionSequence)
                            {
                                ...
                                if (itemSeq.Count == 2)
                                {
                                    generator.AddExtension(
                                        DerObjectIdentifier.GetInstance(itemSeq[0]),
                                        false,
                                        Asn1OctetString.GetInstance(itemSeq[1]).GetOctets()
                                    );
                                }
                                ...
                                {
                                    throw new InvalidOperationException(
                                        "incorrect sequence size of X509Extension got "
                                            + itemSeq.Count
                                            + " expected 2 or 3"
                                    );
                                }
                        ...
                        {
                            throw new InvalidOperationException(
                                "asn1 processing issue: " + e.Message,
                                e
                            );
                        }

...
namespace Org.BouncyCastle.Pkcs
{
    public class Pkcs12Store
    {
        ...
        private readonly Dictionary<string, AsymmetricKeyEntry> m_keys = new Dictionary<
            string,
            AsymmetricKeyEntry
        ...
        private readonly Dictionary<string, X509CertificateEntry> m_certs = new Dictionary<
            string,
        ...
        private readonly Dictionary<string, X509CertificateEntry> m_keyCerts = new Dictionary<
            string,
            X509CertificateEntry
        >(StringComparer.OrdinalIgnoreCase);
        ...
        internal Pkcs12Store(
            ...
            bool useDerEncoding,
            bool reverseCertificates,
            bool overwriteFriendlyName,
            bool enableOracleTrustedKeyUsage
        )
        {
        ...
        public void Load(Stream input, char[] password)
        {
            ...
            if (macData != null) // check the mac code
            {
                ...
                if (password == null)
                    throw new ArgumentNullException(
                        nameof(password),
                        "no password supplied when one expected"
                    );
                ...
                if (!VerifyPbeMac(macData, password, wrongPkcs12Zero: false, data))
                {
                    ...
                    if (
                        password.Length == 0
                        && VerifyPbeMac(macData, password, wrongPkcs12Zero: true, data)
                    )
                    {
            ...
            if (PkcsObjectIdentifiers.Data.Equals(info.ContentType))
            {
                ...
                foreach (ContentInfo ci in cis)
                {
                    ...
                    foreach (var element in seq)
                    {
                        ...
                        {
                            LoadKeyBag(
                                PrivateKeyInfo.GetInstance(safeBag.BagValueEncodable),
                        ...
                        else if (PkcsObjectIdentifiers.Pkcs8ShroudedKeyBag.Equals(safeBagID))
                        {
                            ...
                            LoadPkcs8ShroudedKeyBag(
                                EncryptedPrivateKeyInfo.GetInstance(safeBag.BagValueEncodable),
                                safeBag.BagAttributes,
                                password,
                                wrongPkcs12Zero
                            );
                ...
                if (b.BagAttributes != null)
                {
                    foreach (var bagAttribute in b.BagAttributes)
                    {
                        ...
                        if (attributes.TryGetValue(aOid, out var attributeValue))
                        {
                            ...
                            {
                                string id = Hex.ToHexString(
                                    Asn1OctetString.GetInstance(attr).GetOctets()
                                );
            ...
            {
                string ignoreProperty = Platform.GetEnvironmentVariable(
                    IgnoreUselessPasswordProperty
                );
                bool ignore =
                    ignoreProperty != null && Platform.EqualsIgnoreCase("true", ignoreProperty);

                if (!ignore)
                    throw new IOException(
                        "password supplied for keystore that does not require one"
                    );
            }
        }
        ...
        public X509CertificateEntry[] GetCertificateChain(string alias)
        {
            ...
            while (c != null)
            {
                ...
                AuthorityKeyIdentifier aki = X509ExtensionUtilities.GetAuthorityKeyIdentifier(
                    x509c
                );
                if (aki != null)
                {
        ...
        public void Save(Stream stream, char[] password, SecureRandom random)
        {
            ...
            for (
                uint i = reverseCertificates ? (uint)m_keysOrder.Count - 1 : 0;
                i < m_keysOrder.Count;
            ...
            ContentInfo keysInfo = new ContentInfo(
                PkcsObjectIdentifiers.Data,
                new BerOctetString(keyBagsEncoding)
            );
            ...
            )
            {
                ...
                certBags.Add(
                    new SafeBag(
                        ...
                        certBag,
                        DerSet.FromVector(bagAttributes)
                    )
                );
            ...
            for (
                uint j = reverseCertificates ? (uint)m_certsOrder.Count - 1 : 0;
            ...
            )
            {
                ...
                {
                    Asn1Object eku = certEntry.Certificate.GetExtensionParsedValue(
                    ...
                    if (eku != null)
                    {
                        attrValue = new DerSet(
                            ExtendedKeyUsage.GetInstance(eku).GetAllUsagesArray()
                        );
                    }
                    else
                    {
            ...
            for (
                uint i = reverseCertificates ? (uint)m_chainCertsOrder.Count - 1 : 0;
            ...
            )
            {
                ...
                certBags.Add(
                    new SafeBag(PkcsObjectIdentifiers.CertBag, certBag, DerSet.FromVector(fName))
                );
            }
            ...
            {
                certsInfo = new ContentInfo(
                    PkcsObjectIdentifiers.Data,
                    new BerOctetString(certBagsEncoding)
                );
            }
            else
            {
                ...
                {
                    var encParams = PbeUtilities.GenerateAlgorithmParameters(
                        certAlgorithm,
                        certPrfAlgorithm,
                        cSalt,
                        cIterations,
            ...
            ContentInfo mainInfo = new ContentInfo(
                PkcsObjectIdentifiers.Data,
            ...
            if (password != null)
            {
                ...
                var macDigestAlgorithm = DefaultDigestAlgorithmFinder.Instance.Find(
                ...
                byte[] macResult = CalculatePbeMac(
                    macDigestAlgorithm,
                    salt,
                    itCount,
                    password,
        ...
        private DerSequence CreateEntryFriendlyName(string alias, Pkcs12Entry entry)
        {
            ...
            if (overwriteFriendlyName)
            {
                ...
                entry.TryGetAttribute(PkcsObjectIdentifiers.Pkcs9AtFriendlyName, out var attribute)
            )
            {
        ...
        internal static byte[] CalculatePbeMac(
            AlgorithmIdentifier macDigestAlgorithm,
            byte[] salt,
            int iterations,
            char[] password,
            bool wrongPkcs12Zero,
        ...
        )
        {
            ...
            var pbeParameters = PbeUtilities.GenerateAlgorithmParameters(
                hmacDigestOid,
                salt,
                iterations
            );
            var cipherParameters = PbeUtilities.GenerateCipherParameters(
                hmacDigestOid,
                password,
                wrongPkcs12Zero,
                pbeParameters
        ...
        internal static bool VerifyPbeMac(
            MacData macData,
            char[] password,
            bool wrongPkcs12Zero,
            byte[] data
        ...
        private static void ReplaceOrdering<K>(
            IEqualityComparer<K> c,
            List<K> o,
            K oldKey,
            K newKey
        )

...
namespace Org.BouncyCastle.Pkcs
{
    public abstract class Pkcs12Entry
    {
        ...
        public Asn1Encodable this[DerObjectIdentifier oid] =>
            CollectionUtilities.GetValueOrNull(m_attributes, oid);

        public IEnumerable<DerObjectIdentifier> BagAttributeKeys =>
            CollectionUtilities.Proxy(m_attributes.Keys);

        public bool HasFriendlyName =>
            m_attributes.ContainsKey(PkcsObjectIdentifiers.Pkcs9AtFriendlyName);

        public void SetFriendlyName(string friendlyName)
        {
            m_attributes[PkcsObjectIdentifiers.Pkcs9AtFriendlyName] = new DerBmpString(
                friendlyName
            );
        }

...
namespace Org.BouncyCastle.Tsp
{
    public class TspUtil
    {
        ...
        public static IList<TimeStampToken> GetSignatureTimestamps(SignerInformation signerInfo)
        {
            ...
            {
                foreach (
                    Asn1.Cms.Attribute tsAttr in unsignedAttrs.GetAll(
                ...
                )
                {
                    foreach (Asn1Encodable asn1 in tsAttr.AttrValues)
                    {
                        try
                        {
                            ...
                            if (
                                ...
                                throw new TspValidationException(
                                    "Incorrect digest in message imprint"
                                );
                        ...
                        {
                            throw new TspValidationException(
                                "Unknown hash algorithm specified in timestamp"
                            );
                        }
                        catch (Exception)
                        {

    ...
    public class TimeStampResponse
    {
        ...
        public void Validate(TimeStampRequest request)
        {
            ...
            if (tok != null)
            {
                ...
                if (
                    this.Status != (int)PkiStatus.Granted
                    && this.Status != (int)PkiStatus.GrantedWithMods
                )
                ...
                if (
                    !Arrays.FixedTimeEquals(
                        request.MessageImprintDigest.GetOctets(),
                        tstInfo.MessageImprintDigest.GetOctets()
                    )
                )
                    throw new TspValidationException(
                ...
                Asn1.Cms.Attribute scV1 = tok.SignedAttributes[
                    PkcsObjectIdentifiers.IdAASigningCertificate
                ];
                Asn1.Cms.Attribute scV2 = tok.SignedAttributes[
        ...
        public byte[] GetEncoded(string encoding)
        {
            ...
            {
                asn1Encodable =
                    m_timeStampToken == null
                        ? new DLSequence(m_resp.Status)
                        : new DLSequence(
                            m_resp.Status,
                            m_timeStampToken.ToCmsSignedData().ContentInfo
                        );
            }

...
namespace Org.BouncyCastle.Tsp
{
    public class TimeStampToken
    {
        ...
        public TimeStampToken(CmsSignedData signedData)
        {
            ...
            try
            {
                m_tstInfo = new TimeStampTokenInfo(
                    TstInfo.GetInstance(CmsUtilities.GetByteArray(m_tsToken.SignedContent))
                );

                Asn1.Cms.Attribute attr = m_tsaSignerInfo.SignedAttributes[
                ...
                {
                    attr =
                        m_tsaSignerInfo.SignedAttributes[
                    ...
                    SigningCertificateV2 signCertV2 = SigningCertificateV2.GetInstance(
                        attr.AttrValues[0]
                    );
        ...
        public IStore<X509V2AttributeCertificate> GetAttributeCertificates() =>
            m_tsToken.GetAttributeCertificates();
        ...
        public void Validate(X509Certificate cert)
        {
            try
            {
                ...
                byte[] hash = DigestUtilities.CalculateDigest(
                    m_certID.HashAlgorithm.Algorithm,
                    cert.GetEncoded()
                );
                ...
                if (!cert.IsValid(m_tstInfo.GenTime))
                    throw new TspValidationException(
                        "certificate not valid when time stamp created."
                    );
        ...
        private static bool ValidateIssuer(GeneralNames issuerNames, X509Name issuer)
        {
            ...
            {
                if (
                    GeneralName.DirectoryName == issuerName.TagNo

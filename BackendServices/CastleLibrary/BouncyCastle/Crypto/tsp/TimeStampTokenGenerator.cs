...
namespace Org.BouncyCastle.Tsp
{
    ...
    public class TimeStampTokenGenerator
    {
        ...
        )
        {
            ...
            X509Certificate assocCert =
                signerInfoGen.Certificate
                ?? throw new ArgumentException(
                    "SignerInfoGenerator must have an associated certificate"
                );
            ...
            try
            {
                ...
                var issuerSerial = isIssuerSerialIncluded
                    ? X509.X509Utilities.CreateIssuerSerial(assocCert)
                    : null;
                ...
                else
                {
                    ...
                    signedGen = new TableGen2(
                        signerInfoGen,
                        new EssCertIDv2(digestAlgID, certHash, issuerSerial)
                    );
                }
        ...
        )
        {
            ...
            string signatureName =
                digestName + "with" + CmsSignedHelper.GetEncryptionAlgName(encOid);
            var contentSigner = new Asn1SignatureFactory(signatureName, key);

            var signedGen = new DefaultSignedAttributeTableGenerator(
        ...
        )
        {
            ...
            try
            {
                ...
                foreach (
                    KeyValuePair<DerObjectIdentifier, IStore<Asn1Encodable>> entry in otherRevoc
                )
                {
        ...
        private class TableGen : CmsAttributeTableGenerator
        {
            ...
            public Asn1.Cms.AttributeTable GetAttributes(
                IDictionary<CmsAttributeTableParameter, object> parameters
            )
            {
                Asn1.Cms.AttributeTable tab = infoGen.SignedAttributeTableGenerator.GetAttributes(
                    parameters
                );

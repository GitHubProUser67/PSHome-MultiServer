...
namespace Org.BouncyCastle.Pqc.Crypto.Picnic
{
    internal sealed class Tree
    {
        ...
        internal Tree(PicnicEngine engine, uint numLeaves, int dataSize)
        {
            this.engine = engine;
            MAX_AUX_BYTES = (
                (PicnicEngine.LOWMC_MAX_AND_GATES + PicnicEngine.LOWMC_MAX_KEY_BITS) / 8 + 1
            );

            this.depth = PicnicUtilities.ceil_log2(numLeaves) + 1;
            this.numNodes = (uint)(
                ((1 << ((int)this.depth)) - 1) - ((1 << ((int)this.depth - 1)) - numLeaves)
        ...
        )
        {
            ...
            for (int i = 0; i < revealedSize[0]; i++)
            {
                ...
                System.Array.Copy(
                    input,
                    i * engine.seedSizeBytes,
                    this.nodes[revealed[i]],
                    0,
                    engine.seedSizeBytes
                );
        ...
        {
            uint[] revealedSize = new uint[1];
            uint[] revealed = this.GetRevealedMerkleNodes(
                missingLeaves,
                missingLeavesSize,
                revealedSize
            );
        ...
        )
        {
            ...
            uint[] revealedSize = new uint[1];
            revealedSize[0] = 0;
            uint[] revealed = GetRevealedMerkleNodes(
                missingLeaves,
                missingLeavesSize,
                revealedSize
            );

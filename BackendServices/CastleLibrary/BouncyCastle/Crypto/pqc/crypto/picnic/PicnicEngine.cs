...
namespace Org.BouncyCastle.Pqc.Crypto.Picnic
{
    internal sealed class PicnicEngine
    {
        ...
        private static readonly uint MAX_AUX_BYTES = (
            (LOWMC_MAX_AND_GATES + LOWMC_MAX_KEY_BITS) / 8 + 1
        );

        /* Maximum lengths in bytes */
        private static readonly uint PICNIC_MAX_LOWMC_BLOCK_SIZE = 32;
        private static readonly uint PICNIC_MAX_PUBLICKEY_SIZE = (
            2 * PICNIC_MAX_LOWMC_BLOCK_SIZE + 1
        );

        /** Largest serialized public key size, in bytes */
        private static readonly uint PICNIC_MAX_PRIVATEKEY_SIZE = (
            3 * PICNIC_MAX_LOWMC_BLOCK_SIZE + 2
        );
        ...
        private int picnic_verify(byte[] pk, byte[] message, byte[] signature, uint sigLen)
        {
            ...
            if (is_picnic3(parameters))
            {
                ...
                if (ret != 0)
                {
                    LoggerAccessor.LogError("[BC] - Error couldn't deserialize signature (2)!");
                    return -1;
                }
            ...
            else
            {
                ...
                if (ret != 0)
                {
                    LoggerAccessor.LogError("[BC] - Error couldn't deserialize signature!");
                    return -1;
                }
        ...
        private int Verify(Signature sig, uint[] pubKey, uint[] plaintext, byte[] message)
        {
            ...
            byte[] tmp = new byte[
                System.Math.Max(6 * stateSizeBytes, stateSizeBytes + andSizeBytes)
            ];
            ...
            for (int i = 0; i < numMPCRounds; i++)
            {
                ...
                {
                    LoggerAccessor.LogError("[BC] - Invalid signature. Did not verify");
                    return -1;
                }
                ...
                if (transform == TRANSFORM_UR)
                {
                    ...
                    int view3UnruhLength =
                        (challenge == 0) ? UnruhGWithInputBytes : UnruhGWithoutInputBytes;
                    Array.Copy(
                        proofs[i].view3UnruhG,
                        0,
                        gs[i][(challenge + 2) % 3],
                        0,
                        view3UnruhLength
            ...
            if (
                !SubarrayEquals(
                    received_challengebits,
                    computed_challengebits,
                    PicnicUtilities.NumBytes(2 * numMPCRounds)
                )
            )
            {
                LoggerAccessor.LogError("[BC] - Invalid signature. Did not verify");
        ...
        )
        {
            ...
            //        LoggerAccessor.LogError("[BC] - tmp: " + Hex.toHexString(tmp));
            ...
            switch (challenge)
            {
                ...
                default:
                {
                    LoggerAccessor.LogError("[BC] - Invalid Challenge!");
                    break;
                }
            }
            ...
            if (!status)
            {
                LoggerAccessor.LogError(
                    "[BC] - Failed to generate random tapes, signature verification will fail (but signature may actually be valid)"
                );
                return false;
            }
        ...
        private void mpc_substitution_verify(uint[] state, Tape rand, View view1, View view2)
        {
            ...
            for (int i = 0; i < numSboxes * 3; i += 3)
            {
                ...
                for (int j = 0; j < 2; j++)
                {
                    ...
                    PicnicUtilities.SetBitInWordArray(
                        state,
                        stateOffset + i + 1,
                        a[j] ^ b[j] ^ (ca[j])
                    );
        ...
        )
        {
            ...
            {
                LoggerAccessor.LogError(
                    "[BC] - sigBytesLen = %d, expected bytesRequired = %d",
                    sigBytesLen,
                    bytesRequired
                );
                return -1;
            ...
            for (int i = 0; i < numMPCRounds; i++)
            {
                ...
                if (challenge == 1 || challenge == 2)
                {
                    ...
                    {
                        proofs[i].inputShare[stateSizeWords - 1] = (uint)
                            sigBytes[sigBytesOffset + stateSizeBytes - 1];
                    }
        ...
        private int CountNonZeroChallenges(byte[] challengeBits, int challengeBitsOffset)
        {
            ...
            if (remainingBits > 0)
            {
                ...
                uint challenges = Pack.LE_To_UInt32_Low(
                    challengeBits,
                    challengeBitsOffset + (i >> 2),
                    remainingBytes
                );
        ...
        private void picnic_read_public_key(uint[] ciphertext, uint[] plaintext, byte[] pk)
        {
            ...
            {
                int fullWordBytes = fullWords * 4,
                    partialWordBytes = stateSizeBytes - fullWordBytes;
                ciphertext[fullWords] = Pack.LE_To_UInt32_Low(
                    pk,
                    ciphertextPos + fullWordBytes,
                    partialWordBytes
                );
                plaintext[fullWords] = Pack.LE_To_UInt32_Low(
                    pk,
                    plaintextPos + fullWordBytes,
                    partialWordBytes
                );
            }
        }
        ...
        private int verify_picnic3(Signature2 sig, uint[] pubKey, uint[] plaintext, byte[] message)
        {
            ...
            int ret = iSeedsTree.ReconstructSeeds(
                sig.challengeC,
                (uint)numOpenedRounds,
                sig.iSeedInfo,
                (uint)sig.iSeedInfoLen,
                sig.salt,
                0
            );
                ...
                else
                {
                    ...
                    ret = seeds[t]
                        .ReconstructSeeds(
                            hideList,
                            1,
                            sig.proofs[t].seedInfo,
                            (uint)sig.proofs[t].seedInfoLen,
                            sig.salt,
                            t
                        );
                    if (ret != 0)
                    {
                        LoggerAccessor.LogError(
                            "[BC] - Failed to reconstruct seeds for round %d",
                            t
                        );
                        return -1;
                    }
            ...
            for (uint t = 0; t < numMPCRounds; t++)
            {
                ...
                if (Contains(sig.challengeC, numOpenedRounds, (uint)t))
                {
                    ...
                    {
                        LoggerAccessor.LogError(
                            "[BC] - MPC simulation failed for round %d, signature invalid",
                            t
                        );
                        return -1;
                    }
            ...
            {
                LoggerAccessor.LogError("[BC] - Challenge does not match, signature invalid");
                return -1;
            }
        ...
        )
        {
            ...
            //        LoggerAccessor.LogDebug("[BC] - iSeedInfoLen: %04x", sig.iSeedInfoLen);
            ...
            {
                LoggerAccessor.LogError(
                    "[BC] - sigLen = %d, expected bytesRequired = %d",
                    sigLen,
                    bytesRequired
                );
                return -1;
            ...
            //        LoggerAccessor.LogError("[BC] - iSeedInfo: " + Hex.toHexString(sig.iSeedInfo));
            ...
            for (uint t = 0; t < numMPCRounds; t++)
            {
                if (Contains(sig.challengeC, numOpenedRounds, t))
                {
                    ...
                    if (P_t != (numMPCParties - 1))
                    {
                        ...
                        {
                            LoggerAccessor.LogError("[BC] - failed while deserializing aux bits");
                            return -1;
                        }
                    }
                    ...
                    {
                        LoggerAccessor.LogError("[BC] - failed while deserializing msgs bits");
                        return -1;
                    }
        ...
        private bool picnic_sign(byte[] sk, byte[] message, byte[] signature)
        {
            ...
            int dataPos = 1,
                ciphertextPos = 1 + stateSizeBytes,
                plaintextPos = 1 + 2 * stateSizeBytes;
            int fullWords = stateSizeBytes / 4;
            ...
            if (fullWords < stateSizeWords)
            {
                int fullWordBytes = fullWords * 4,
                    partialWordBytes = stateSizeBytes - fullWordBytes;
                data[fullWords] = Pack.LE_To_UInt32_Low(
                    sk,
                    dataPos + fullWordBytes,
                    partialWordBytes
                );
                ciphertext[fullWords] = Pack.LE_To_UInt32_Low(
                    sk,
            ...
            if (!is_picnic3(parameters))
            {
                ...
                if (ret != 0)
                {
                    LoggerAccessor.LogError("[BC] - Failed to create signature");
                    return false;
                }
                ...
                if (len < 0)
                {
                    LoggerAccessor.LogError("[BC] - Failed to serialize signature");
                    return false;
                }
        ...
        private int SerializeSignature(Signature sig, byte[] sigBytes, int sigOffset)
        {
            ...
            int bytesRequired =
                ...
                + saltSizeBytes
                + numMPCRounds
                    * (2 * seedSizeBytes + stateSizeBytes + andSizeBytes + digestSizeBytes);
        ...
        )
        {
            ...
            byte[] tmp = new byte[
                System.Math.Max(9 * stateSizeBytes, stateSizeBytes + andSizeBytes)
            ];
            ...
            for (int k = 0; k < numMPCRounds; k++)
            {
                ...
                for (int j = 0; j < 2; j++)
                {
                    ...
                    if (!status)
                    {
                        LoggerAccessor.LogError("[BC] - createRandomTape failed");
                        return -1;
                    }
                ...
                {
                    LoggerAccessor.LogError(
                        "[BC] - Simulation failed; output does not match public key (round = "
                            + k
                            + ")"
                    );
                    return -1;
            ...
            for (int i = 0; i < numMPCRounds; i++)
            {
                ...
                Prove(
                    ...
                    views[i],
                    AS[i],
                    (transform != TRANSFORM_UR) ? null : gs[i]
                ); //todo check if
            }
        ...
        )
        {
            ...
            Array.Copy(
                views[(challenge + 1) % 3].communicatedBits,
                0,
                proof.communicatedBits,
                0,
                andSizeBytes
            );

            Array.Copy(
                commitments[(challenge + 2) % 3],
                0,
                proof.view3Commitment,
                0,
                digestSizeBytes
            );
            if (transform == TRANSFORM_UR)
        ...
        )
        {
            ...
            if (transform == TRANSFORM_UR)
            {
                for (int i = 0; i < numMPCRounds; i++)
                {
                    ...
                    {
                        int view3UnruhLength =
                            (j == 2) ? UnruhGWithInputBytes : UnruhGWithoutInputBytes;
        ...
        private void mpc_LowMC(Tape tapes, View[] views, uint[] plaintext, uint[] slab)
        {
            ...
            for (int player = 0; player < 3; player++)
            {
                matrix_mul_offset(
                    slab,
                    player * stateSizeWords,
                    views[player].inputShare,
                    0,
                    current.GetData(),
                    current.GetMatrixPointer()
                );
        ...
        )
        {
            for (int player = 0; player < players; player++)
            {
                matrix_mul_offset(
                    output,
                    outputOffset + player * stateSizeWords,
                    state,
                    stateOffset + player * stateSizeWords,
                    matrix,
                    matrixOffset
                );
            }
        ...
        {
            byte[] allSeeds = new byte[
                seedSizeBytes * (numMPCParties * numMPCRounds) + saltSizeBytes
            ];
        ...
        )
        {
            ...
            for (int t = 0; t < numMPCRounds; t++)
            {
                ...
                if (rv != 0)
                {
                    LoggerAccessor.LogError("[BC] - MPC simulation failed, aborting signature");
                    return false;
                }
            ...
            sig.iSeedInfoLen = iSeedsTree.RevealSeeds(
                sig.challengeC,
                (uint)numOpenedRounds,
                sig.iSeedInfo,
                numMPCRounds * seedSizeBytes
            );
            ...
            for (uint t = 0; t < numMPCRounds; t++)
            {
                if (Contains(sig.challengeC, numOpenedRounds, t))
                {
                    ...
                    sig.proofs[t].seedInfoLen = seeds[t]
                        .RevealSeeds(
                            hideList,
                            1,
                            sig.proofs[t].seedInfo,
                    ...
                    Array.Copy(
                        msgs[t].msgs[sig.challengeP[P_index]],
                        0,
                        sig.proofs[t].msgs,
                        0,
                        andSizeBytes
        ...
        private void ExpandChallengeHash(byte[] challengeHash, uint[] challengeC, uint[] challengeP)
        {
            ...
            uint[] chunks = new uint[
                digestSizeBytes * 8 / System.Math.Min(bitsPerChunkC, bitsPerChunkP)
            ];
        ...
        private void aux_mpc_AND(uint mask_a, uint mask_b, uint fresh_output_mask, Tape tape)
        {
            ...
            and_helper =
                PicnicUtilities.Parity16(and_helper)
                ^ PicnicUtilities.GetBit(tape.tapes[lastParty], tape.pos - 1);
        ...
        private void GetAuxBits(byte[] output, Tape tape)
        {
            ...
            for (int j = 0; j < numRounds; j++)
            {
                ...
                for (int i = 0; i < n; i++)
                {
                    PicnicUtilities.SetBit(
                        output,
                        pos++,
                        PicnicUtilities.GetBit(lastTape, tapePos++)
                    );
                }
            }
        }
        ...
        )
        {
            ...
            {
                LoggerAccessor.LogError("[BC] - Failed writing private key!");
                return -1;
            }
        ...
        private int picnic_write_public_key(byte[] ciphertext, byte[] plaintext, byte[] buf)
        {
            ...
            if (buf.Length < bytesRequired)
            {
                LoggerAccessor.LogError("[BC] - Failed writing public key!");
                return -1;
            }

...
namespace Org.BouncyCastle.Pqc.Crypto.Hqc
{
    ...
    {
        static int[,] alpha128 = new int[30, 45]
        {
            {
                2,
                4,
        ...
        static int[,] alpha192 = new int[32, 55]
        {
            {
                2,
                4,
        ...
        static int[,] alpha256 = new int[58, 89]
        {
            {
                2,
                4,
        ...
        )
        {
            ...
            {
                int gateValue = Utils.ToUnsigned8bits(
                    message[paramK - 1 - i] ^ encodedBytes[n1 - paramK - 1]
                );
        ...
        private static void ComputeSyndromes(int[] syndromes, byte[] codeWord, int delta, int n1)
        {
            if (n1 == 46)
            {
                for (int i = 0; i < 2 * delta; i++)
                {
                    for (int j = 1; j < n1; j++)
                    {
                        syndromes[i] ^= GFCalculator.Mul(
                            Utils.ToUnsigned8bits(codeWord[j]),
                            alpha128[i, j - 1]
                        );
                    }
        ...
        )
        {
            ...
            for (int i = 0; i < delta; i++)
            {
                ...
                for (int j = 1; j < delta; j++)
                {
                    temp2 = GFCalculator.Mul(
                        temp2,
                        (1 ^ GFCalculator.Mul(inv, betaSet[(i + j) % delta]))
                    );
                }

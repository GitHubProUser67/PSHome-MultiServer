...
namespace Org.BouncyCastle.Pqc.Crypto.Frodo
{
    public class FrodoEngine
    {
        ...
        public void kem_keypair(byte[] pk, byte[] sk, SecureRandom random)
        {
            ...
            byte[] seedSE = Arrays.CopyOfRange(
                s_seedSE_z,
                len_s_bytes,
                len_s_bytes + len_seedSE_bytes
            );
            byte[] z = Arrays.CopyOfRange(
                s_seedSE_z,
                len_s_bytes + len_seedSE_bytes,
            ...
            Array.Copy(Arrays.Concatenate(s, pk), 0, sk, 0, len_s_bytes + len_pk_bytes);
            ...
            for (int i = 0; i < nbar; i++)
            {
                for (int j = 0; j < n; j++)
                {
                    ...
                    Array.Copy(temp, 0, sk, len_s_bytes + len_pk_bytes + i * n * 2 + j * 2, 2);
                }
            }
        ...
        private short[] FrodoUnpack(byte[] input, int n1, int n2)
        {
            ...
            while (i < output.Length && (j < input.Length || ((j == input.Length) && (bits > 0))))
            {
                ...
                while (b < D)
                {
                    ...
                    byte t = (byte)(
                        (((w & 0xff) >> ((bits & 0xff) - nbits)) & (mask & 0xffff)) & 0xff
                    ); // the bits to copy from w to out
                    output[i] = (short)(
                        (output[i] & 0xffff) + (((t & 0xff) << (D - (b & 0xff) - nbits))) & 0xffff
                    );
                    b += (byte)nbits;
                    bits -= (byte)nbits;
                    w &= (byte)~(mask << bits);
        ...
        public void kem_dec(byte[] ss, byte[] ct, byte[] sk)
        {
            ...
            byte[] kprime = Arrays.CopyOfRange(
                seedSEprime_kprime,
                len_seedSE_bytes,
                len_seedSE_bytes + len_k_bytes
            );

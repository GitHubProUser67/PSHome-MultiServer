...
namespace Org.BouncyCastle.Pqc.Crypto.Utilities
{
    ...
    {
        private delegate AsymmetricKeyParameter Converter(
...
#pragma warning disable CS0618 // Type or member is obsolete
            ...
            Converters[BCObjectIdentifiers.sphincsPlus_haraka_128f_r3_simple] =
                SphincsPlusConverter;
            Converters[BCObjectIdentifiers.sphincsPlus_haraka_128s_r3_simple] =
                SphincsPlusConverter;
            Converters[BCObjectIdentifiers.sphincsPlus_haraka_192f_r3_simple] =
                SphincsPlusConverter;
            Converters[BCObjectIdentifiers.sphincsPlus_haraka_192s_r3_simple] =
                SphincsPlusConverter;
            Converters[BCObjectIdentifiers.sphincsPlus_haraka_256f_r3_simple] =
                SphincsPlusConverter;
            Converters[BCObjectIdentifiers.sphincsPlus_haraka_256s_r3_simple] =
                SphincsPlusConverter;
        ...
        )
        {
            ...
            if (!Converters.TryGetValue(algOid, out var converter))
                throw new IOException(
                    "algorithm identifier in public key not recognised: " + algOid
                );
...
#pragma warning disable CS0618 // Type or member is obsolete
        internal static DilithiumPublicKeyParameters GetDilithiumPublicKey(
...
#pragma warning restore CS0618 // Type or member is obsolete
        ...
        private static AsymmetricKeyParameter LmsConverter(
            SubjectPublicKeyInfo keyInfo,
            object defaultParams
        )
        {
            ...
            if (publicKey.IsOctetAligned())
            {
                ...
                //if (bytesLength > expectedLength)
                {
                    try
                    {
                        Asn1Object obj = Asn1Object.FromMemoryStream(
                            publicKey.GetOctetMemoryStream()
                        );
...
#pragma warning disable CS0618 // Type or member is obsolete
        private static AsymmetricKeyParameter SphincsPlusConverter(
            SubjectPublicKeyInfo keyInfo,
            object defaultParams
        )
        {
            try
            {
                ...
                SphincsPlusParameters spParams = PqcUtilities.SphincsPlusParamsLookup(
                    keyInfo.Algorithm.Algorithm
                );

                return new SphincsPlusPublicKeyParameters(
                    spParams,
                    Arrays.CopyOfRange(keyEnc, 4, keyEnc.Length)
                );
            }
            catch (Exception)
            {
...
#pragma warning restore CS0618 // Type or member is obsolete
        ...
        private static AsymmetricKeyParameter CmceConverter(
            SubjectPublicKeyInfo keyInfo,
            object defaultParams
        )
        {
            ...
            CmceParameters spParams = PqcUtilities.McElieceParamsLookup(
                keyInfo.Algorithm.Algorithm
            );
        ...
        private static AsymmetricKeyParameter FrodoConverter(
            SubjectPublicKeyInfo keyInfo,
            object defaultParams
        )
        {
        ...
        private static AsymmetricKeyParameter SaberConverter(
            SubjectPublicKeyInfo keyInfo,
            object defaultParams
        )
        {
            byte[] keyEnc = Asn1OctetString
                .GetInstance(Asn1Sequence.GetInstance(keyInfo.ParsePublicKey())[0])
                .GetOctets();

            SaberParameters saberParams = PqcUtilities.SaberParamsLookup(
                keyInfo.Algorithm.Algorithm
            );
        ...
        private static AsymmetricKeyParameter PicnicConverter(
            SubjectPublicKeyInfo keyInfo,
            object defaultParams
        )
        {
            ...
            PicnicParameters picnicParams = PqcUtilities.PicnicParamsLookup(
                keyInfo.Algorithm.Algorithm
            );
        ...
        private static AsymmetricKeyParameter DilithiumConverter(
            SubjectPublicKeyInfo keyInfo,
            object defaultParams
        )
        {
            var dilithiumParameters = PqcUtilities.DilithiumParamsLookup(
                keyInfo.Algorithm.Algorithm
            );
        ...
        private static AsymmetricKeyParameter NtruConverter(
            SubjectPublicKeyInfo keyInfo,
            object defaultParams
        )
        {
        ...
        private static NtruPublicKeyParameters GetNtruPublicKey(
        ...
        )
        {
            if (publicKey.IsOctetAligned())
            {
                ...
                if (bytesLength == expectedLength)
                    ...
                    return NtruPublicKeyParameters.FromEncoding(
                        ntruParameters,
                        encoding: publicKey.GetOctets()
                    );
        ...
        private static AsymmetricKeyParameter FalconConverter(
            SubjectPublicKeyInfo keyInfo,
            object defaultParams
        )
        {
            FalconParameters falconParams = PqcUtilities.FalconParamsLookup(
                keyInfo.Algorithm.Algorithm
            );
            ...
            try
            {
                ...
                {
                    byte[] keyEnc = Asn1OctetString
                        .GetInstance(Asn1Sequence.GetInstance(obj)[0])
                        .GetOctets();
                ...
                else
                {
                    ...
                    if (keyEnc[0] != (byte)(0x00 + falconParams.LogN))
                    {
                        throw new ArgumentException(
                            "byte[] enc of Falcon h value not tagged correctly"
                        );
                    }
                    return new FalconPublicKeyParameters(
                        falconParams,
                        Arrays.CopyOfRange(keyEnc, 1, keyEnc.Length)
                    );
                }
            }
            catch (Exception)
        ...
        private static AsymmetricKeyParameter BikeConverter(
            SubjectPublicKeyInfo keyInfo,
            object defaultParams
        )
        {
            try
            {
                ...
                BikeParameters bikeParams = PqcUtilities.BikeParamsLookup(
                    keyInfo.Algorithm.Algorithm
                );
        ...
        private static AsymmetricKeyParameter HqcConverter(
            SubjectPublicKeyInfo keyInfo,
            object defaultParams
        )
        {

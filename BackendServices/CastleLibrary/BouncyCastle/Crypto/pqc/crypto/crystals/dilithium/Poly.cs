...
namespace Org.BouncyCastle.Pqc.Crypto.Crystals.Dilithium
{
    internal class Poly
    {
        ...
        public Poly(DilithiumEngine engine)
        {
            ...
            PolyUniformNBlocks =
                (768 + Symmetric.Stream128BlockBytes - 1) / Symmetric.Stream128BlockBytes;
        ...
        public void UniformEta(byte[] seed, ushort nonce)
        {
            ...
            if (eta == 2)
            {
                PolyUniformEtaNBlocks = (
                    (136 + Symmetric.Stream256BlockBytes - 1) / Symmetric.Stream256BlockBytes
                );
            }
            else if (eta == 4)
            {
                PolyUniformEtaNBlocks = (
                    (227 + Symmetric.Stream256BlockBytes - 1) / Symmetric.Stream256BlockBytes
                );
            }
            else
            {
        ...
        public void PolyT0Unpack(byte[] a, int off)
        {
            ...
            {
                Coeffs[8 * i + 0] =
                    ((a[off + 13 * i + 0] & 0xFF) | ((a[off + 13 * i + 1] & 0xFF) << 8)) & 0x1FFF;
                Coeffs[8 * i + 1] =
                    (
                        (((a[off + 13 * i + 1] & 0xFF) >> 5) | ((a[off + 13 * i + 2] & 0xFF) << 3))
                        | ((a[off + 13 * i + 3] & 0xFF) << 11)
                    ) & 0x1FFF;

                Coeffs[8 * i + 2] =
                    ((((a[off + 13 * i + 3] & 0xFF) >> 2) | ((a[off + 13 * i + 4] & 0xFF) << 6)))
                    & 0x1FFF;

                Coeffs[8 * i + 3] =
                    (
                        (((a[off + 13 * i + 4] & 0xFF) >> 7) | ((a[off + 13 * i + 5] & 0xFF) << 1))
                        | ((a[off + 13 * i + 6] & 0xFF) << 9)
                    ) & 0x1FFF;

                Coeffs[8 * i + 4] =
                    (
                        (((a[off + 13 * i + 6] & 0xFF) >> 4) | ((a[off + 13 * i + 7] & 0xFF) << 4))
                        | ((a[off + 13 * i + 8] & 0xFF) << 12)
                    ) & 0x1FFF;

                Coeffs[8 * i + 5] =
                    ((((a[off + 13 * i + 8] & 0xFF) >> 1) | ((a[off + 13 * i + 9] & 0xFF) << 7)))
                    & 0x1FFF;

                Coeffs[8 * i + 6] =
                    (
                        (((a[off + 13 * i + 9] & 0xFF) >> 6) | ((a[off + 13 * i + 10] & 0xFF) << 2))
                        | ((a[off + 13 * i + 11] & 0xFF) << 10)
                    ) & 0x1FFF;

                Coeffs[8 * i + 7] =
                    (((a[off + 13 * i + 11] & 0xFF) >> 3 | ((a[off + 13 * i + 12] & 0xFF) << 5)))
                    & 0x1FFF;
        ...
        public void PolyEtaUnpack(byte[] a, int off)
        {
            ...
            if (eta == 2)
            {
                for (int i = 0; i < N / 8; ++i)
                {
                    ...
                    Coeffs[8 * i + 2] = (
                        ((a[off + 3 * i + 0] & 0xFF) >> 6) | ((a[off + 3 * i + 1] & 0xFF) << 2) & 7
                    );
                    Coeffs[8 * i + 3] = ((((a[off + 3 * i + 1] & 0xFF) >> 1)) & 7);
                    Coeffs[8 * i + 4] = ((((a[off + 3 * i + 1] & 0xFF) >> 4)) & 7);
                    Coeffs[8 * i + 5] = (
                        ((a[off + 3 * i + 1] & 0xFF) >> 7) | ((a[off + 3 * i + 2] & 0xFF) << 1) & 7
                    );
        ...
        public void UnpackZ(byte[] a)
        {
            ...
            if (gamma1 == (1 << 17))
            {
                for (int i = 0; i < N / 4; ++i)
                {
                    Coeffs[4 * i + 0] =
                        (
                            (((a[9 * i + 0] & 0xFF)) | ((a[9 * i + 1] & 0xFF) << 8))
                            | ((a[9 * i + 2] & 0xFF) << 16)
                        ) & 0x3FFFF;
                    Coeffs[4 * i + 1] =
                        (
                            (((a[9 * i + 2] & 0xFF) >> 2) | ((a[9 * i + 3] & 0xFF) << 6))
                            | ((a[9 * i + 4] & 0xFF) << 14)
                        ) & 0x3FFFF;
                    Coeffs[4 * i + 2] =
                        (
                            (((a[9 * i + 4] & 0xFF) >> 4) | ((a[9 * i + 5] & 0xFF) << 4))
                            | ((a[9 * i + 6] & 0xFF) << 12)
                        ) & 0x3FFFF;
                    Coeffs[4 * i + 3] =
                        (
                            (((a[9 * i + 6] & 0xFF) >> 6) | ((a[9 * i + 7] & 0xFF) << 2))
                            | ((a[9 * i + 8] & 0xFF) << 10)
                        ) & 0x3FFFF;
            ...
            else if (gamma1 == (1 << 19))
            {
                for (int i = 0; i < N / 2; ++i)
                {
                    Coeffs[2 * i + 0] =
                        (
                            (((a[5 * i + 0] & 0xFF)) | ((a[5 * i + 1] & 0xFF) << 8))
                            | ((a[5 * i + 2] & 0xFF) << 16)
                        ) & 0xFFFFF;
                    Coeffs[2 * i + 1] =
                        (
                            (((a[5 * i + 2] & 0xFF) >> 4) | ((a[5 * i + 3] & 0xFF) << 4))
                            | ((a[5 * i + 4] & 0xFF) << 12)
                        ) & 0xFFFFF;
        ...
        internal void PackW1(byte[] r, int off)
        {
            ...
            if (gamma2 == (DilithiumEngine.Q - 1) / 88)
            {
                ...
                {
                    r[off + 3 * i + 0] = (byte)(
                        ((byte)Coeffs[4 * i + 0]) | (Coeffs[4 * i + 1] << 6)
                    );
                    r[off + 3 * i + 1] = (byte)(
                        (byte)(Coeffs[4 * i + 1] >> 2) | (Coeffs[4 * i + 2] << 4)
                    );
                    r[off + 3 * i + 2] = (byte)(
                        (byte)(Coeffs[4 * i + 2] >> 4) | (Coeffs[4 * i + 3] << 2)
                    );
                }
            }

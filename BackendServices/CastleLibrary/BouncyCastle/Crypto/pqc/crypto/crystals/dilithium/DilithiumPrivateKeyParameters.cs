...
namespace Org.BouncyCastle.Pqc.Crypto.Crystals.Dilithium
{
    ...
    public sealed class DilithiumPrivateKeyParameters : DilithiumKeyParameters
    {
            ...
            : base(true, parameters)
        {
            ...
            m_k = Arrays.CopyOfRange(encoding, index, index + DilithiumEngine.SeedBytes);
            index += DilithiumEngine.SeedBytes;
            m_tr = Arrays.CopyOfRange(encoding, index, index + DilithiumEngine.TrBytes);
            index += DilithiumEngine.TrBytes;
            int delta = eng.L * eng.PolyEtaPackedBytes;
            m_s1 = Arrays.CopyOfRange(encoding, index, index + delta);
            index += delta;
            delta = eng.K * eng.PolyEtaPackedBytes;
            m_s2 = Arrays.CopyOfRange(encoding, index, index + delta);
            index += delta;

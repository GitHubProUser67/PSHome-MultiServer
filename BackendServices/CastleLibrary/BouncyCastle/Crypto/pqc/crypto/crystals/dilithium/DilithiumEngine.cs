...
namespace Org.BouncyCastle.Pqc.Crypto.Crystals.Dilithium
{
    internal class DilithiumEngine
    {
        ...
        internal DilithiumEngine(int mode, SecureRandom random, bool usingAes)
        {
            ...
            switch (Mode)
            {
                ...
                default:
                    throw new ArgumentException(
                        "The mode " + mode + "is not supported by Crystals Dilithium!"
                    );
            }
            ...
            if (Gamma1 == (1 << 17))
            {
                PolyUniformGamma1NBytes = (
                    (576 + Symmetric.Stream256BlockBytes - 1) / Symmetric.Stream256BlockBytes
                );
            }
            else if (Gamma1 == (1 << 19))
            {
                PolyUniformGamma1NBytes = (
                    (640 + Symmetric.Stream256BlockBytes - 1) / Symmetric.Stream256BlockBytes
                );
            }
            else
            {
        ...
        internal void GenerateKeyPairInternal(
            byte[] seed,
            bool legacy,
            out byte[] rho,
            out byte[] k,
        ...
        )
        {
            ...
            PolyVec s2 = new PolyVec(this, K),
                t1 = new PolyVec(this, K),
                t0 = new PolyVec(this, K);

            ShakeDigest shake256Digest = new ShakeDigest(256);
            shake256Digest.BlockUpdate(seed, 0, SeedBytes);
            if (!legacy)
            {
                shake256Digest.Update((byte)K);
                shake256Digest.Update((byte)L);
            }
        ...
        internal void MsgRepEndSignInternal(
            ShakeDigest d,
            byte[] sig,
            int siglen,
            byte[] rho,
        ...
        )
        {
            ...
            PolyVecMatrix matrix = new PolyVecMatrix(this);
            PolyVec s1 = new PolyVec(this, L),
                y = new PolyVec(this, L),
                z = new PolyVec(this, L);
            PolyVec t0 = new PolyVec(this, K),
                s2 = new PolyVec(this, K),
                w1 = new PolyVec(this, K),
                w0 = new PolyVec(this, K),
                h = new PolyVec(this, K);
        ...
        internal bool MsgRepEndVerifyInternal(
            ShakeDigest d,
            byte[] sig,
            int siglen,
            byte[] rho,

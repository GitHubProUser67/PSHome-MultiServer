    ...
    internal class Poly
    {
        ...
        public void GenSecret(short[][] s, byte[] seed)
        {
            ...
            for (int i = 0; i < SABER_L; i++)
            {
                ...
                else
                {
                    ...
                    {
                        s[i][4 * j] = (short)(
                            (((buf[j + i * engine.PolyCoinBytes]) & 0x03) ^ 2) - 2
                        );
                        s[i][4 * j + 1] = (short)(
                            (((buf[j + i * engine.PolyCoinBytes] >> 2) & 0x03) ^ 2) - 2
                        );
                        s[i][4 * j + 2] = (short)(
                            (((buf[j + i * engine.PolyCoinBytes] >> 4) & 0x03) ^ 2) - 2
                        );
                        s[i][4 * j + 3] = (short)(
                            (((buf[j + i * engine.PolyCoinBytes] >> 6) & 0x03) ^ 2) - 2
                        );
                    }
                }
            }
        ...
        private void karatsuba_simple(int[] a_1, int[] b_1, int[] result_final)
        {
            ...
            for (i = 0; i < KARATSUBA_N / 2 - 1; i++)
            {
                d0123[i] = (
                    d0123[i]
                    - result_d01[i + 0 * KARATSUBA_N / 4]
                    - result_d01[i + 2 * KARATSUBA_N / 4]
                );
                d01[i] = (
                    d01[i]
                    - result_final[i + 0 * KARATSUBA_N / 4]
                    - result_final[i + 2 * KARATSUBA_N / 4]
                );
                d23[i] = (
                    d23[i]
                    - result_final[i + 4 * KARATSUBA_N / 4]

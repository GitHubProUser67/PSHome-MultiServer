...
{
    internal class Hrss1373Polynomial : HrssPolynomial
    {
        ...
        public override byte[] SqToBytes(int len)
        {
            ...
            {
                t[0] = (short)ModQ(
                    (uint)this.coeffs[ParameterSet.PackDegree() - 2] & 0xffff,
                    (uint)ParameterSet.Q()
                );
                t[1] = (short)ModQ(
                    (uint)this.coeffs[ParameterSet.PackDegree() - 1] & 0xffff,
                    (uint)ParameterSet.Q()
                );
                r[7 * i + 0] = (byte)(t[0] & 0xff);
                r[7 * i + 1] = (byte)((t[0] >> 8) | ((t[1] & 0x03) << 6));
        ...
        public override void SqFromBytes(byte[] a)
        {
            ...
            for (i = 0; i < ParameterSet.PackDegree() / 4; i++)
            {
                this.coeffs[4 * i + 0] = (ushort)(
                    (a[7 * i + 0] & 0xff) | (((ushort)(a[7 * i + 1] & 0xff) & 0x3f) << 8)
                );
                this.coeffs[4 * i + 1] = (ushort)(
                    ((a[7 * i + 1] & 0xff) >> 6)
                    | (((ushort)(a[7 * i + 2] & 0xff)) << 2)
                    | ((short)(a[7 * i + 3] & 0x0f) << 10)
                );
                this.coeffs[4 * i + 2] = (ushort)(
                    ((a[7 * i + 3] & 0xff) >> 4)
                    | (((ushort)(a[7 * i + 4] & 0xff) & 0xff) << 4)
                    | ((short)(a[7 * i + 5] & 0x03) << 12)
                );
                this.coeffs[4 * i + 3] = (ushort)(
                    ((a[7 * i + 5] & 0xff) >> 2) | (((ushort)(a[7 * i + 6] & 0xff)) << 6)
                );
            }
            // i=NtruPackDeg/4;
            if (ParameterSet.PackDegree() % 4 == 2)

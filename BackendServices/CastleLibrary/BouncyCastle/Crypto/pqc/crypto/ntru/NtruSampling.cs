    ...
    internal class NtruSampling
    {
        ...
        internal PolynomialPair SampleRm(byte[] uniformBytes)
        {
            switch (_parameterSet)
            {
                case NtruHrssParameterSet _:
                {
                    var r = (HrssPolynomial)SampleIid(
                        Arrays.CopyOfRange(uniformBytes, 0, _parameterSet.SampleIidBytes())
                    );
                    var m = (HrssPolynomial)SampleIid(
                        Arrays.CopyOfRange(
                            uniformBytes,
                            _parameterSet.SampleIidBytes(),
                            uniformBytes.Length
        ...
        internal HpsPolynomial SampleFixedType(byte[] uniformBytes)
        {
            ...
            {
                s[4 * i + 0] =
                    (uniformBytes[15 * i + 0] << 2)
                    + (uniformBytes[15 * i + 1] << 10)
                    + (uniformBytes[15 * i + 2] << 18)
                    + (uniformBytes[15 * i + 3] << 26);
                s[4 * i + 1] =
                    ((uniformBytes[15 * i + 3] & 0xc0) >> 4)
                    + (uniformBytes[15 * i + 4] << 4)
                    + (uniformBytes[15 * i + 5] << 12)
                    + (uniformBytes[15 * i + 6] << 20)
                    + (uniformBytes[15 * i + 7] << 28);
                s[4 * i + 2] =
                    ((uniformBytes[15 * i + 7] & 0xf0) >> 2)
                    + (uniformBytes[15 * i + 8] << 6)
                    + (uniformBytes[15 * i + 9] << 14)
                    + (uniformBytes[15 * i + 10] << 22)
                    + (uniformBytes[15 * i + 11] << 30);
                s[4 * i + 3] =
                    (uniformBytes[15 * i + 11] & 0xfc)
                    + (uniformBytes[15 * i + 12] << 8)
                    + (uniformBytes[15 * i + 13] << 16)
                    + (uniformBytes[15 * i + 14] << 24);
            }

...
namespace Org.BouncyCastle.Pqc.Crypto.Cmce
{
    ...
    internal class CmceEngine<GFImpl> : ICmceEngine
        where GFImpl : struct, GF
    {
        ...
        public int PublicKeySize =>
            usePadding ? PK_NROWS * ((SYS_N / 8 - ((PK_NROWS - 1) / 8))) : PK_NROWS * PK_NCOLS / 8;
        ...
        private void Syndrome(byte[] cipher_text, byte[] pk, byte[] error_vector)
        {
            ...
            for (i = 0; i < PK_NROWS; i++)
            {
                ...
                if (usePadding)
                {
                    ...
                    {
                        row[j] = (short)(
                            (((row[j] & 0xff) << tail) | ((row[j - 1] & 0xff) >> (8 - tail))) & 0xff
                        );
                    }
                }
        ...
        public int KemDec(byte[] key, byte[] cipher_text, byte[] sk)
        {
            ...
            {
                preimage[1 + i] = (byte)(
                    (~m & sk[i + 40 + IRR_BYTES + COND_BYTES]) | (m & error_vector[i])
                );
            }
            for (i = 0; i < SYND_BYTES; i++)
            {
        ...
        private int MovColumns(byte[][] mat, ushort[] pi, ulong[] pivots)
        {
            ...
            for (i = 0; i < PK_NROWS; i++)
            {
                ...
                if (usePadding)
                {
                    ...
                    mat[i][block_idx + 8] = (byte)(
                        ((mat[i][block_idx + 8] & 0xff) >> tail << tail)
                        | ((tmp[7] & 0xff) >> (8 - tail))
                    );
                    mat[i][block_idx + 0] = (byte)(
                        ((tmp[0] & 0xff) << tail)
                        | ((mat[i][block_idx] & 0xff) << (8 - tail) >> (8 - tail))
                    );

                    for (k = 7; k >= 1; k--)
                    {
                        mat[i][block_idx + k] = (byte)(
                            ((tmp[k] & 0xff) << tail) | ((tmp[k - 1] & 0xff) >> (8 - tail))
                        );
                    }
                }
                else
        ...
        )
        {
            ...
            if (w <= 10)
            {
                ...
                {
                    temp[(int)(n + x)] =
                        ((temp[(int)x] & 0xffff) << 10) | (temp[(int)(n + x)] & 0x3ff);
                }
            ...
            else
            {
                ...
                for (i = 1; i < w - 1; ++i)
                {
                    ...
                    if (i < w - 2)
                    {
                        ...
                        {
                            temp[(int)(n + x)] =
                                (temp[(int)x] & ~0xffff) | (temp[(int)(n + x)] >> 16);
                        }
                        /* B = (p^(-1)<<16)+p */
                        ...
                        {
                            temp[(int)(n + x)] =
                                (temp[(int)(n + x)] << 16) | (temp[(int)x] & 0xffff);
                        }
                        /* B = (p^(-2)<<16)+c */
                    }
            ...
            {
                temp[(int)(n + n / 4 + i)] =
                    (((int)q[(int)(i * 2 + 1)]) << 16) | ((int)q[(int)(i * 2)]);
            }

...
namespace Org.BouncyCastle.Pqc.Crypto.SphincsPlus
{
    internal abstract class SphincsPlusEngine
    {
            ...
            if (w == 16)
            {
                ...
                else
                {
                    throw new ArgumentException(
                        "cannot precompute SPX_WOTS_LEN2 for n outside {2, .., 256}"
                    );
                }
            }
            else if (w == 256)
        ...
        internal class Sha2Engine : SphincsPlusEngine
        {
            ...
            public override IndexedDigest H_msg(
                byte[] prf,
                int prfOff,
                byte[] pkSeed,
                byte[] pkRoot,
            ...
            )
            {
                ...
                ulong treeIndex =
                    Pack.BE_To_UInt64_Low(output, forsMsgBytes, (int)treeBytes)
                    & ulong.MaxValue >> (64 - (int)treeBits);

                uint leafIndex =
                    Pack.BE_To_UInt32_Low(output, forsMsgBytes + (int)treeBytes, (int)leafBytes)
                    & uint.MaxValue >> (32 - (int)leafBits);

                return new IndexedDigest(
                    treeIndex,
                    leafIndex,
                    Arrays.CopyOfRange(output, 0, forsMsgBytes)
                );
            }
        ...
        internal class HarakaSEngine_X86 : SphincsPlusEngine
        {
            public static bool IsSupported =>
                Haraka256_X86.IsSupported && Haraka512_X86.IsSupported && HarakaS_X86.IsSupported;
            ...
            )
            {
                ...
                // currently, only indexes up to 64 bits are supported
                ulong treeIndex =
                    Pack.BE_To_UInt64_Low(indices[..treeBytes]) & ulong.MaxValue >> (64 - treeBits);

                uint leafIndex =
                    Pack.BE_To_UInt32_Low(indices[treeBytes..]) & uint.MaxValue >> (32 - leafBits);

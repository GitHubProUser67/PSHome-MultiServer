...
namespace Org.BouncyCastle.Pqc.Crypto.Lms
{
    public class HssPrivateKeyParameters : LmsKeyParameters, ILmsContextBasedSigner
    {
        ...
        internal static HssPrivateKeyParameters Parse(
            byte[] buf,
            int off,
            int len,
        ...
        public LmsParameters[] GetLmsParameters()
        {
            lock (this)
            {
                ...
                for (int i = 0; i < len; i++)
                {
                    ...
                    parameters[i] = new LmsParameters(
                        lmsPrivateKey.SigParameters,
                        lmsPrivateKey.OtsParameters
                    );
                }
        ...
        protected void UpdateHierarchy(
            IList<LmsPrivateKeyParameters> newKeys,
            IList<LmsSignature> newSig
        )
        {
            lock (this)
            {
            ...
            lock (this)
            {
                ...
                if (usageCount > m_indexLimit - m_index)
                    throw new ArgumentException(
                        "exceeds usages remaining in current leaf",
                        nameof(usageCount)
                    );
        ...
        void ResetKeyToIndex()
        {
            ...
            for (int i = 1; i < qTreePath.Length; i++)
            {
                ...
                //
                bool seedEquals =
                    Arrays.AreEqual(childI, m_keys[i].GetI())
                ...
                if (!seedEquals)
                {
                    ...
                    //
                    m_sig[i - 1] = Lms.GenerateSign(
                        (LmsPrivateKeyParameters)m_keys[i - 1],
                        ((LmsPrivateKeyParameters)m_keys[i]).GetPublicKey().ToByteArray()
                    );
                    changed = true;
                }
                else if (!lmsQMatch)
        ...
        internal void ReplaceConsumedKey(int d)
        {
            ...
            newSig[d - 1] = Lms.GenerateSign(
                newKeys[d - 1],
                newKeys[d].GetPublicKey().ToByteArray()
            );

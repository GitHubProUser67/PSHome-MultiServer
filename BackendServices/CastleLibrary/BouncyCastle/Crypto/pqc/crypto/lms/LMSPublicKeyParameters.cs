...
namespace Org.BouncyCastle.Pqc.Crypto.Lms
{
    public sealed class LmsPublicKeyParameters : LmsKeyParameters, ILmsContextBasedVerifier
    {
        ...
        public LmsParameters GetLmsParameters() =>
            new LmsParameters(GetSigParameters(), GetOtsParameters());
        ...
        internal LmsContext GenerateOtsContext(LmsSignature S)
        {
            ...
            if (S.OtsSignature.ParamType.ID != ots_typecode)
            {
                throw new ArgumentException(
                    "ots type from lsm signature does not match ots"
                        + " signature type from embedded ots signature"
                );
            }

            return new LMOtsPublicKey(
                LMOtsParameters.GetParametersByID(ots_typecode),
                I,
                S.Q,
                null
            ).CreateOtsContext(S);

...
using CastleLibrary.BouncyCastle.Lextm;
using CustomLogger;
using Lextm.SharpSnmpLib;
using Lextm.SharpSnmpLib.Messaging;
using Lextm.SharpSnmpLib.Security;

namespace MultiServerLibrary.SNMP
{
    ...
    public class SnmpTrapSender : IDisposable
    {
        ...
        private readonly BlockingCollection<(string Severity, string Message)> trapQueue = new();
...
#pragma warning disable
            if (AESPrivacyProvider.IsSupported)
                snmpAuthProvider = new AESPrivacyProvider(
                    new OctetString(privPwd),
                    GetAuthenticationProvider(hashAlgorithm, new OctetString(authPwd))
                );
            else
                snmpAuthProvider = new BouncyCastleAESPrivacyProvider(
                    new OctetString(privPwd),
                    GetAuthenticationProvider(hashAlgorithm, new OctetString(authPwd))
                );
#pragma warning restore
            ...
            {
                report = Messenger
                    .GetNextDiscovery(SnmpType.InformRequestPdu)
                    .GetResponse(timeoutValue, snmpEndPoint);

                var provider = snmpAuthProvider as AESPrivacyProvider;
                if (provider != null)
                {
            ...
            catch
            {
                LoggerAccessor.LogError(
                    $"[SnmpTrapSender] - SNMPv3 failed to Inform the supervisor, SNMP reporting will be disabled!"
                );
            }
        ...
        private void SendTrap(string severity, string message)
        {
            if (
                report == null
                || report.Parameters.IsInvalid
                || report.Parameters.EngineBoots == null
                || report.Parameters.EngineTime == null
            )
                return;

            var now = DateTime.Now;
            var offset = TimeZoneInfo.Local.GetUtcOffset(now);
            var sysUpTime = (uint)Environment.TickCount / 10;
            ...
            if (provider != null)
            {
                new TrapV2Message(
                    ...
                    {
                        new(
                            new ObjectIdentifier($"{snmpEnterpriseOid}.1"),
                            new OctetString(severity)
                        ),
                        new(
                            new ObjectIdentifier($"{snmpEnterpriseOid}.2"),
                            new OctetString(message)
                        ),
                        new(
                            new ObjectIdentifier($"{snmpEnterpriseOid}.3"),
                            new TimeTicks(sysUpTime)
                        ),
                        new(
                            new ObjectIdentifier($"{snmpEnterpriseOid}.4"),
                            new OctetString(DateTime.UtcNow.ToString("o"))
                        ),
                        new(
                            new ObjectIdentifier($"{snmpEnterpriseOid}.5"),
                            new OctetString(
                                new byte[]
                                {
                                    (byte)(now.Year >> 8),
                    ...
                    report.Parameters.EngineTime.ToInt32()
                ).Send(snmpEndPoint);
            }
            else
            {
        ...
        private static IAuthenticationProvider GetAuthenticationProvider(
            string providerName,
            OctetString phrase
        )
        {
            ...
            var providerNameUpper = providerName.ToUpper();
...
#pragma warning restore
            ...
            var providerType = assembly
                .GetTypes()
                .FirstOrDefault(t =>
                    t.Name.Equals("BouncyCastle" + providerNameUpper + "AuthenticationProvider")
                    && typeof(IAuthenticationProvider).IsAssignableFrom(t)
                );

            return providerType == null
                ? throw new InvalidOperationException(
                    $"[SnmpTrapSender] - Bouncy Castle Authentication provider '{providerName}' not found in {cryptoLibrary}."
                )
                : (IAuthenticationProvider)Activator.CreateInstance(providerType, phrase);
        }
    }
}

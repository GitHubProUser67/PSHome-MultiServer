...
using System.Timers;
using MultiServerLibrary.ManagedMail.Auth;
using MultiServerLibrary.ManagedMail.Auth.Sasl;
...
namespace MultiServerLibrary.ManagedMail
{
    ...
    /// Protocol (IMAP).
    /// </summary>
    public partial class ImapClient : IImapClient
    {
        Stream stream;
        TcpClient client;
        bool disposed;
        ...
        readonly object sequenceLock = new();
        string[] capabilities;
        int tag = 0;
        ...
        bool hasEvents
        {
            get { return newMessageEvent != null || messageDeleteEvent != null; }
        }
        bool idling;
        Thread idleThread,
            idleDispatch;
        int pauseRefCount = 0;
        readonly SafeQueue<string> idleEvents = new();
        readonly System.Timers.Timer noopTimer = new();
        static readonly TraceSource ts = new(" MultiServerLibrary.ManagedMail");
        ...
        static bool IsResponseOK(string response, string tag = null)
        {
            if (tag != null)
                return response.StartsWith(tag + "OK");
            var v = response.Substring(response.IndexOf(' ')).Trim();
        ...
        {
            var methods = new string[] { "Srp", "Ntlm", "ScramSha1", "DigestMd5", "CramMd5" };
            foreach (var m in methods)
            {
                try
                {
                    var response = Authenticate(tag, username, password, m);
        ...
        {
            return SendCommandGetResponse(
                tag + "LOGIN " + username.QuoteString() + " " + password.QuoteString()
            );
        }
        ...
        /// <exception cref="NotSupportedException">The specified authentication method is not
        /// supported by the server.</exception>
        string SspiAuthenticate(string tag, string username, string password, bool useNtlm)
        {
            var response = SendCommandGetResponse(
            ...
            using (var fs = new FilterStream(stream, true))
            {
                using (var ns = new NegotiateStream(fs, true))
                {
                    try
                    {
        ...
        string Authenticate(string tag, string username, string password, string mechanism)
        {
            var m = SaslFactory.Create(mechanism);
            ...
            var response = SendCommandGetResponse(tag + "AUTHENTICATE " + m.Name);
            // If we get a BAD or NO response, the mechanism is not supported.
            if (response.StartsWith(tag + "BAD") || response.StartsWith(tag + "NO"))
            {
                throw new NotSupportedException(
            ...
            while (!m.IsCompleted)
            {
                ...
                var challenge = MyRegex().Replace(response, String.Empty);
        ...
        public void Logout()
        {
            ...
            lock (sequenceLock)
            {
                StopIdling();
                var tag = GetTag();
                var bye = SendCommandGetResponse(tag + "LOGOUT");
                        ...
                        if (b == Newline)
                        {
                            ...
                            if (resolveLiterals)
                            {
                                s = Regex.Replace(
                                    ...
                                    {
                                        return "\""
                                            + GetData(Convert.ToInt32(m.Groups[1].Value))
                                            + "\""
                                            + GetResponse(false);
                                    }
                                );
        ...
        string GetData(int byteCount)
        {
            ...
            using (var mem = new MemoryStream())
            {
                lock (readLock)
                {
                    while (byteCount > 0)
                    {
                        var request = byteCount > buffer.Length ? buffer.Length : byteCount;
        ...
        public IEnumerable<string> Capabilities()
        {
            ...
            lock (sequenceLock)
            {
                ...
                if (Authed)
                    ResumeIdling();
                return !IsResponseOK(response, tag)
                    ? throw new BadServerResponseException(response)
        ...
        public bool Supports(string capability)
        {
            ...
            return (capabilities ?? Capabilities()).Contains(
                capability,
        ...
        public void DeleteMailbox(string mailbox)
        {
            ...
            lock (sequenceLock)
            {
                ...
                var tag = GetTag();
                var response = SendCommandGetResponse(
                    tag + "DELETE " + Util.UTF7Encode(mailbox).QuoteString()
                );
                ResumeIdling();
                if (!IsResponseOK(response, tag))
                    throw new BadServerResponseException(response);
        ...
        public void CreateMailbox(string mailbox)
        {
            ...
            lock (sequenceLock)
            {
                ...
                var tag = GetTag();
                var response = SendCommandGetResponse(
                    tag + "CREATE " + Util.UTF7Encode(mailbox).QuoteString()
                );
                ResumeIdling();
                if (!IsResponseOK(response, tag))
                    throw new BadServerResponseException(response);
        ...
        public IEnumerable<string> ListMailboxes()
        {
            ...
            lock (sequenceLock)
            {
                ...
                var tag = GetTag();
                var response = SendCommandGetResponse(tag + "LIST \"\" \"*\"");
                while (response.StartsWith("*"))
                {
                    var m = Regex.Match(response, "\\* LIST \\((.*)\\)\\s+\"([^\"]+)\"\\s+(.+)");
                    if (m.Success)
                    {
                        var attr = m.Groups[1].Value.Split(' ');
                        var add = true;
                        foreach (var a in attr)
                        {
                            // Only list mailboxes that can actually be selected.
                            if (a.Equals(@"\noselect", StringComparison.CurrentCultureIgnoreCase))
                                add = false;
                        }
                        ...
                        var name = Regex.Replace(m.Groups[3].Value, "^\"(.+)\"$", "$1");
                        try
                        {
        ...
        public void Expunge(string mailbox = null)
        {
            ...
            lock (sequenceLock)
            {
                ...
                var tag = GetTag();
                var response = SendCommandGetResponse(tag + "EXPUNGE");
        ...
        public MailboxInfo GetMailboxInfo(string mailbox = null)
        {
            ...
            lock (sequenceLock)
            {
                PauseIdling();
                mailbox ??= defaultMailbox;
                var status = GetMailboxStatus(mailbox);
                // Collect quota information if the server supports it.
                UInt64 used = 0,
                    free = 0;
                if (Supports("QUOTA"))
                {
                    var quotas = GetQuota(mailbox);
                ...
                var flags = GetMailboxFlags(mailbox);
                ResumeIdling();
        ...
        {
            var dictFlags = new Dictionary<string, MailboxFlag>(StringComparer.OrdinalIgnoreCase)
            {
            ...
            lock (sequenceLock)
            {
                ...
                var command = Supports("XLIST") ? "XLIST" : "LIST";
                var response = SendCommandGetResponse(
                    tag + command + " \"\" " + Util.UTF7Encode(mailbox).QuoteString()
                );
                while (response.StartsWith("*"))
                {
                    var m = Regex.Match(response, "\\* X?LIST \\((.*)\\)\\s+\"([^\"]+)\"\\s+(.+)");
                    if (m.Success)
                    {
                        var flags = m.Groups[1].Value.Split(' ');
                        foreach (var f in flags)
                        {
                            if (dictFlags.TryGetValue(f, out var value))
                                list.Add(value);
                        }
                    }
                    response = GetResponse();
        ...
        MailboxStatus GetMailboxStatus(string mailbox = null)
        {
            ...
            lock (sequenceLock)
            {
                ...
                while (response.StartsWith("*"))
                {
                    var m = Regex.Match(response, @"\* STATUS.*MESSAGES (\d+)");
                    if (m.Success)
        ...
        public IEnumerable<uint> Search(SearchCondition criteria, string mailbox = null)
        {
            ...
            lock (sequenceLock)
            {
                ...
                var useUTF8 = str.Contains("\r\n");
                var line = reader.ReadLine();
                var response = SendCommandGetResponse(
                    tag + "UID SEARCH " + (useUTF8 ? "CHARSET UTF-8 " : "") + line
                );
                ...
                while ((line = reader.ReadLine()) != null)
                {
                    if (!response.StartsWith("+"))
                    {
                        ResumeIdling();
                        throw new NotSupportedException(
                            "Please restrict your search " + "to ASCII-only characters.",
                            new BadServerResponseException(response)
                        );
                    }
                ...
                while (response.StartsWith("*"))
                {
                    ...
                    if (m.Success)
                    {
                        var v = m.Groups[1].Value.Trim().Split(' ');
                        foreach (var s in v)
                        {
                            try
                            {
                ...
                return !IsResponseOK(response, tag)
                    ? throw new BadServerResponseException(response)
                    : (IEnumerable<uint>)result.ToArray();
            }
        }
            ...
            switch (options)
            {
                ...
                case FetchOptions.NoAttachments:
                    return GetMessage(
                        ...
                        p =>
                        {
                            return p.Disposition.Type != ContentDispositionType.Attachment;
                        },
                        seen,
                        mailbox
                    );
                case FetchOptions.TextOnly:
                    return GetMessage(
                        ...
                        p =>
                        {
                            return p.Type == ContentType.Text;
                        },
                        seen,
                        mailbox
                    );
        ...
        )
        {
            ...
            lock (sequenceLock)
            {
                PauseIdling();
                SelectMailbox(mailbox);
                var header = GetMailHeader(uid, seen, mailbox);
                var message = MessageBuilder.FromHeader(header);
                var structure = GetBodystructure(uid, mailbox);
                try
                {
                    var parts = Bodystructure.Parse(structure);
                    foreach (var part in parts)
                    {
                        // Let the delegate decide whether the part should be fetched or not.
                        if (callback(part) == true)
                        {
                            var content = GetBodypart(uid, part.PartNumber, seen, mailbox);
                ...
                catch (FormatException)
                {
                    throw new BadServerResponseException(
                        "The server returned an erroneous " + "body structure:" + structure
                    );
                }
                ResumeIdling();
                return message;
        ...
        string GetMailHeader(uint uid, bool seen = true, string mailbox = null)
        {
            ...
            lock (sequenceLock)
            {
                ...
                var tag = GetTag();
                var response = SendCommandGetResponse(
                    tag + "UID FETCH " + uid + " (BODY" + (seen ? null : ".PEEK") + "[HEADER])",
                    false
                );
                while (response.StartsWith("*"))
                {
                    var m = Regex.Match(response, @"\* \d+ FETCH .* {(\d+)}");
                    if (m.Success)
                    {
                        var size = Convert.ToInt32(m.Groups[1].Value);
        ...
        string GetBodystructure(uint uid, string mailbox = null)
        {
            ...
            lock (sequenceLock)
            {
                ...
                var tag = GetTag();
                var response = SendCommandGetResponse(
                    tag + "UID FETCH " + uid + " (BODYSTRUCTURE)"
                );
        ...
        /// <exception cref="NotAuthenticatedException">The method was called in non-authenticated
        /// state, i.e. before logging in.</exception>
        string GetBodypart(uint uid, string partNumber, bool seen = true, string mailbox = null)
        {
            AssertValid();
            lock (sequenceLock)
            {
        ...
        string GetMessageData(uint uid, bool seen = true, string mailbox = null)
        {
            ...
            lock (sequenceLock)
            {
                ...
                var tag = GetTag();
                var response = SendCommandGetResponse(
                    tag + "UID FETCH " + uid + " (BODY" + (seen ? null : ".PEEK") + "[])",
                    false
                );
                while (response.StartsWith("*"))
                {
        ...
        uint GetHighestUID(string mailbox = null)
        {
            ...
            lock (sequenceLock)
            {
                ...
                var tag = GetTag();
                var response = SendCommandGetResponse(
                    tag + "STATUS " + Util.UTF7Encode(selectedMailbox).QuoteString() + " (UIDNEXT)"
                );
                uint nextUID = 0;
                while (response.StartsWith("*"))
                {
                    var m = Regex.Match(response, @"\* STATUS.*UIDNEXT (\d+)");
                    if (m.Success)
        ...
        public void CopyMessages(IEnumerable<uint> uids, string destination, string mailbox = null)
        {
            ...
            var set = Util.BuildSequenceSet(uids);
            lock (sequenceLock)
            {
                PauseIdling();
                SelectMailbox(mailbox);
                var tag = GetTag();
                var response = SendCommandGetResponse(
                    tag + "UID COPY " + set + " " + destination.QuoteString()
                );
                while (response.StartsWith("*"))
                    response = GetResponse();
                ResumeIdling();
        ...
        public void DeleteMessages(IEnumerable<uint> uids, string mailbox = null)
        {
            ...
            lock (sequenceLock)
            {
                ...
                var tag = GetTag();
                var response = SendCommandGetResponse(
                    tag + "UID STORE " + set + @" +FLAGS.SILENT (\Deleted \Seen)"
                );
                while (response.StartsWith("*"))
                    response = GetResponse();
                ResumeIdling();
        ...
        {
            var messageFlagsMapping = new Dictionary<string, MessageFlag>(
            ...
            lock (sequenceLock)
            {
                ...
                SelectMailbox(mailbox);
                var tag = GetTag();
                var response = SendCommandGetResponse(tag + "UID FETCH " + uid + " (FLAGS)");
                var flags = new List<MessageFlag>();
                while (response.StartsWith("*"))
                {
                    var m = Regex.Match(response, @"FLAGS \(([\w\s\\$-]*)\)");
                    if (m.Success)
                    {
                        var setFlags = m.Groups[1].Value.Split(' ');
                        foreach (var flag in setFlags)
                        {
                            if (messageFlagsMapping.TryGetValue(flag, out var value))
                                flags.Add(value);
                        }
                    }
                    response = GetResponse();
        ...
        public void SetMessageFlags(uint uid, string mailbox, params MessageFlag[] flags)
        {
            ...
            lock (sequenceLock)
            {
                ...
                var tag = GetTag();
                var response = SendCommandGetResponse(
                    tag + "UID STORE " + uid + @" FLAGS.SILENT (" + flagsString.Trim() + ")"
                );
                while (response.StartsWith("*"))
                    response = GetResponse();
                ResumeIdling();
        ...
        void StartIdling()
        {
            ...
            if (!Supports("IDLE"))
                throw new InvalidOperationException(
                    "The server does not support the IMAP4 IDLE command."
                );
            lock (sequenceLock)
            {
                ...
                SelectMailbox(null);
                var tag = GetTag();
                var response = SendCommandGetResponse(tag + "IDLE");
        ...
        void EventDispatcher()
        {
            ...
            while (true)
            {
                var response = idleEvents.Dequeue();
                var m = Regex.Match(response, @"\*\s+(\d+)\s+(\w+)");
                if (!m.Success)
                    continue;
                try
                {
        ...
        IEnumerable<MailboxQuota> GetQuota(string mailbox = null)
        {
            ...
            lock (sequenceLock)
            {
                PauseIdling();
                mailbox ??= DefaultMailbox;
                var quotas = new List<MailboxQuota>();
                var tag = GetTag();
                var response = SendCommandGetResponse(
                    tag + "GETQUOTAROOT " + Util.UTF7Encode(mailbox).QuoteString()
                );
                while (response.StartsWith("*"))
                {
                    var m = Regex.Match(
                ...
                return !IsResponseOK(response, tag)
                    ? throw new BadServerResponseException(response)
                    : (IEnumerable<MailboxQuota>)quotas;
            }
        }

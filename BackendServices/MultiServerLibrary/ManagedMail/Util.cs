...
namespace MultiServerLibrary.ManagedMail
{
    ...
    internal static partial class Util
    {
        ...
        {
            return str.Contains(value, comparer);
        }
        ...
        public static string DecodeWords(string words)
        {
            if (String.IsNullOrEmpty(words))
                return String.Empty;
            var matches = rxDecodeWord.Matches(words);
            if (matches.Count == 0)
                return words;
            ...
            foreach (Match m in matches)
            {
                if (m.Index > LastKnownMatchPos)
                    HandleFillData(
                        decoded,
                        words.Substring(LastKnownMatchPos, m.Index - LastKnownMatchPos)
                    );
        ...
        /// </summary>
        static readonly Regex rxDecodeWord = MyRegex();
        ...
        internal static string DecodeWord(string word)
        {
            ...
            switch (type)
            {
                ...
                case "B":
                    return encoding.GetString(text.IsBase64().DecodedBytes);
                default:
        ...
        internal static string QPDecode(string value, Encoding encoding)
        {
            ...
            catch
            {
                throw new FormatException(
                    "The value is not a valid quoted-printable encoded string."
                );
            }
        }
        ...
        internal static string UTF7Encode(string s)
        {
            ...
            while (reader.Peek() != -1)
            {
                ...
                else
                {
                    ...
                    var sequence = new StringBuilder(c.ToString());
                    while (reader.Peek() != -1)
                    {
                    ...
                    var buffer = Encoding.BigEndianUnicode.GetBytes(sequence.ToString());
                    var encoded = Base64.ToBase64String(buffer).Replace('/', ',').TrimEnd('=');
        ...
        internal static string UTF7Decode(string s)
        {
            ...
            while (reader.Peek() != -1)
            {
                ...
                if (c == '&' && reader.Peek() != '-')
                {
                    ...
                    var encoded = sequence.ToString().Replace(',', '/');
        ...
        internal static string BuildSequenceSet(IEnumerable<uint> uids)
        {
            ...
            var list = uids.ToList<uint>();
            if (list.Count == 1)
                return list[0].ToString();
            list.Sort();
            var count = uids.Count();
            IList<uint> fromNums = new List<uint>(),
                toNums = new List<uint>();
            ...
            var ranges = Enumerable
                .Range(0, toNums.Count)
                .Select(i => fromNums[i] + (toNums[i] == fromNums[i] ? "" : ":" + toNums[i]))
                .ToArray();

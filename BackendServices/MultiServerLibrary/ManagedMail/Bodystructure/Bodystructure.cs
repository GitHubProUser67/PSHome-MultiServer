namespace MultiServerLibrary.ManagedMail
{
    ...
    internal class Bodystructure
    {
        ...
        {
            var s = new Bodystructure(text);
            var list = new List<Bodypart>();
            try
            {
                var c = (char)s.reader.Peek();
                if (c == '(')
        ...
        {
            var part = new Bodypart(partNumber);
            // Mandatory fields:
            ...
            if (
                part.Type == ContentType.Message
                && part.Subtype.Equals("RFC822", StringComparison.CurrentCultureIgnoreCase)
            )
        ...
        private Bodypart[] ParseList(string nestPrefix = "")
        {
            ...
            {
                reader.Read();
                var c = (char)reader.Peek(true);
                if (c == '(')
                    list.AddRange(ParseList(nestPrefix + count + "."));
                else
                {
                    var partNumber = nestPrefix + count;
                    var part = ParseBodypart(partNumber);
                    list.Add(part);
                }
                count++;
            }

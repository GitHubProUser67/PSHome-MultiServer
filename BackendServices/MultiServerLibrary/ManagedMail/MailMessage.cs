...
namespace MultiServerLibrary.ManagedMail
{
    ...
    ///  MultiServerLibrary.ManagedMail assembly.
    /// </remarks>
    public static partial class MailMessageExtension
    {
        ...
        public static DateTime? Date(this MailMessage message)
        {
            ...
            date = MyRegex().Replace(date, String.Empty);
            try
            {
        ...
        static NameValueCollection BuildHeader(MailMessage m)
        {
            ...
            var date = String.IsNullOrEmpty(m.Headers["Date"])
            ...
            var multipart = m.AlternateViews.Count > 0 || m.Attachments.Count > 0;
            if (!multipart)
            {
                var contentType = m.IsBodyHtml ? "text/html" : "text/plain";
            ...
            {
                var contentType =
                    (m.Attachments.Count == 0 ? "multipart/alternative" : "multipart/mixed")
                    + "; boundary="
        ...
        /// </summary>
        static readonly Dictionary<MailPriority, string> PriorityMap = new()
        {
        ...
        /// </summary>
        static readonly Dictionary<MailPriority, string> ImportanceMap = new()
        {
        ...
        {
            var builder = new StringBuilder("=?UTF-8?Q?");
            var chars = Encoding.Unicode.GetChars(Encoding.Unicode.GetBytes(s));
        ...
        {
            return new StringBuilder("=?UTF-8?B?")
                    .Append(Base64.ToBase64String(Encoding.UTF8.GetBytes(s)))
                    .ToString() + "?=";
        }
        ...
        internal static string To822Address(this MailAddress address)
        {
            ...
            {
                var name = address.DisplayName.IsASCII()
        ...
        {
            var builder = new StringBuilder();
            var multipart = header["Content-Type"].Contains("boundary");
            ...
            var match = Regex.Match(header["Content-Type"], @"boundary=(\w+)");
            var boundary = match.Groups[1].Value;
            // Add the start boundary.
            builder.AppendLine("--" + boundary);
            var nestParts = m.AlternateViews.Count > 0 && m.Attachments.Count > 0;
            if (nestParts)
            {
            ...
            else
            {
                ...
                foreach (var v in m.AlternateViews)
                {
                    builder.AppendLine("--" + boundary);
        ...
        {
            var base64 = header["Content-Transfer-Encoding"] == "base64";
            if (addHeaders)
            {
                var contentType = m.IsBodyHtml ? "text/html" : "text/plain";
                if (m.BodyEncoding != null)
            ...
            if (base64)
            {
                var bytes = m.BodyEncoding.GetBytes(m.Body);
                body = Base64.ToBase64String(bytes);
            }
        ...
        static void AddAttachment(StringBuilder builder, AttachmentBase view)
        {
            ...
            {
                contentType = contentType + "; " + key + "=" + view.ContentType.Parameters[key];
            }
            ...
            using (var memstream = new MemoryStream())
            {
                ...
                var str = Base64.ToBase64String(memstream.ToArray());
        ...
        /// parts.</remarks>
        static void AddNestedAlternative(
            StringBuilder builder,
            MailMessage m,
            NameValueCollection header
        )
        {
            var boundary = GenerateContentBoundary();

...
namespace MultiServerLibrary.ManagedMail.Auth.Sasl.Mechanisms.Ntlm
{
    ...
    internal class Type3Message
    {
        ...
        int NtlmOffset
        {
            get { return LMOffset + LMResponse.Length; }
        }
        ...
        int targetOffset
        {
            get { return NtlmOffset + NtlmResponse.Length; }
        }
        ...
        int usernameOffset
        {
            get { return targetOffset + targetName.Length; }
        }

        /// <summary>
        /// The client workstation's name.
        /// </summary>
        byte[] workstation { get; set; }
        ...
        {
            get { return usernameOffset + username.Length; }
        }
        ...
        {
            get { return workstationOffset + workstation.Length; }
        }

        /// <summary>
        /// Contains the data present in the OS version structure.
        /// </summary>
        OSVersion OSVersion { get; set; }
        ...
        )
        {
            ...
            this.targetName = String.IsNullOrEmpty(targetName)
                ? Array.Empty<byte>()
                : encoding.GetBytes(targetName);
            // The session key is not relevant to authentication.
            this.sessionKey = Array.Empty<byte>();
            ...
            else
            {
                ...
                LMResponse = Responses.ComputeLMv2Response(
                    targetName,
                    username,
                    password,
                    challenge,
                ...
                NtlmResponse = Responses.ComputeNtlmv2Response(
                    targetName,
                    username,
                    password,
                    targetInformation,

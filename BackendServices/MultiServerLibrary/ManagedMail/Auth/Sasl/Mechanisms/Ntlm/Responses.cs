...
namespace MultiServerLibrary.ManagedMail.Auth.Sasl.Mechanisms.Ntlm
{
    /// <summary>
    ...
    internal static class Responses
    {
        ...
        internal static byte[] ComputeLMResponse(byte[] challenge, string password)
        {
        ...
        /// <returns>An array of bytes representing the response to the
        /// specified challenge.</returns>
        internal static byte[] ComputeNtlmResponse(byte[] challenge, string password)
        {
        ...
        /// <returns>An array of bytes representing the response to the
        /// specified challenge.</returns>
        internal static byte[] ComputeNtlmv2Response(
            string target,
            string username,
            string password,
        ...
        /// <returns>An array of bytes representing the response to the
        /// specified challenge.</returns>
        internal static byte[] ComputeLMv2Response(
            string target,
            string username,
            string password,
            byte[] challenge,
        ...
        {
            var ntlmv2Hash = Ntlmv2Hash(target, username, password);
        ...
        private static byte[] LMResponse(byte[] hash, byte[] challenge)
        {
            ...
            byte[] lowKey = CreateDESKey(keyBytes, 0),
                middleKey = CreateDESKey(keyBytes, 7),
                highKey = CreateDESKey(keyBytes, 14);
            using (var des = DES.Create("DES"))
            {
                des.Mode = CipherMode.ECB;
                des.Key = lowKey;
                using (var encryptor = des.CreateEncryptor())
                {
                    encryptor.TransformBlock(challenge, 0, challenge.Length, lmResponse, 0);
                }
                des.Key = middleKey;
                using (var encryptor = des.CreateEncryptor())
                {
                    encryptor.TransformBlock(challenge, 0, challenge.Length, lmResponse, 8);
                }
                des.Key = highKey;
                using (var encryptor = des.CreateEncryptor())
                {
                    encryptor.TransformBlock(challenge, 0, challenge.Length, lmResponse, 16);
                }
                return lmResponse;
            }
        }
        ...
        private static byte[] Ntlmv2Hash(string target, string username, string password)
        {
            username.ThrowIfNull("username");
            password.ThrowIfNull("password");
            var ntlmHash = NtlmHash(password);
            var identity = username.ToUpperInvariant() + target;
        ...
        {
            var data = new ByteBuilder().Append(challenge).Append(clientData).ToArray();

...
namespace  MultiServerLibrary.ManagedMail.Auth.Sasl.Mechanisms.Ntlm
{
    ...
    internal class Type2Message
    {
        ...
        static Type2Version GetType2Version(int targetOffset)
        {
            ...
            return dict.TryGetValue(targetOffset, out var value) ? value :
        ...
        {
            var info = new Type2TargetInformation();
            while (true)
            {
                ...
                var content = Encoding.Unicode.GetString(r.ReadBytes(length));
                switch (_type)
                {

...
namespace MultiServerLibrary.ManagedMail.Auth.Sasl.Mechanisms
{
    ...
    internal class SaslDigestMd5 : SaslMechanism
    {
        ...
        protected override byte[] ComputeResponse(byte[] challenge)
        {
            ...
            // with a CRLF.
            var ret = Step == 0 ? ComputeDigestResponse(challenge) : Array.Empty<byte>();
            Step++;
            return ret;
        }
        ...
        private byte[] ComputeDigestResponse(byte[] challenge)
        {
            ...
            var decoded = Encoding.ASCII.GetString(challenge);
            var fields = ParseDigestChallenge(decoded);
            var digestUri = "imap/" + fields["realm"];
            var responseValue = ComputeDigestResponseValue(
                fields,
                Cnonce,
                digestUri,
                Username,
            ...
            var challengeResponse = String.Join(",", directives);
        ...
        private static string ComputeDigestResponseValue(
            NameValueCollection challenge,
            string cnonce,
            string digestUri,
            string username,
            string password
        )
        {
            ...
            // Construct A1.
            var data = enc.GetBytes(username + ":" + realm + ":" + password);
            data = CastleLibrary.NetHasher.DotNetHasher.ComputeMD5(data);
            var A1 = enc.GetString(data) + ":" + challenge["nonce"] + ":" + cnonce;
            // Construct A2.
        ...
        private static string MD5(string s, Encoding encoding = null)
        {
            ...
            return CastleLibrary
                .NetHasher.DotNetHasher.ComputeMD5String(encoding.GetBytes(s))
                .ToLower();
        }

using MultiServerLibrary.ManagedMail.Auth.Sasl.Mechanisms.Ntlm;
    ...
    internal class SaslNtlm : SaslMechanism
    {
        ...
        protected override byte[] ComputeResponse(byte[] challenge)
        {
            if (step == 1)
                completed = true;
            var ret =
                step == 0 ? ComputeInitialResponse(challenge) : ComputeChallengeResponse(challenge);
            step++;
            return ret;
        }
        ...
        protected byte[] ComputeInitialResponse(byte[] challenge)
        {
            try
            {
                var domain = Properties.TryGetValue("Domain", out var value)
                    ? value as string
                    : "domain";
                var workstation = Properties.TryGetValue("Workstation", out value)
                    ? value as string
                    : "workstation";
                var msg = new Type1Message(domain, workstation);
            ...
            catch (Exception e)
            {
                throw new SaslException(
                    "The initial client response could not " + "be computed.",
                    e
                );
            }
        }
        ...
        protected byte[] ComputeChallengeResponse(byte[] challenge)
        {
            try
            {
                var msg = Type2Message.Deserialize(challenge);
                var data = new Type3Message(
                    Username,
                    Password,
                    msg.Challenge,
                    "Workstation"

...
namespace MultiServerLibrary.ManagedMail.Auth.Sasl.Mechanisms.Srp
{
    ...
    internal static class Helper
    {
        ...
        /// </summary>
        static readonly TraceSource ts = new(" MultiServerLibrary.ManagedMail.Sasl.Srp");
        ...
        {
            foreach (var s in moduli)
            {
                var a = BigInteger.Parse(s, NumberStyles.HexNumber);
        ...
        /// </remarks>
        public static Mpi ComputeClientPublicKey(
            Mpi generator,
            Mpi safePrimeModulus,
            Mpi privateKey
        )
        {
            // A = g ^ a % N
        ...
        /// K = Shared Public Key
        /// </remarks>
        public static Mpi ComputeSharedKey(
            byte[] salt,
            string username,
            string password,
            Mpi clientPublicKey,
            Mpi serverPublicKey,
            Mpi clientPrivateKey,
            Mpi generator,
            Mpi safePrimeModulus,
        ...
        {
            // u = H(A | B)
            var u = hashAlgorithm.ComputeHash(
                new ByteBuilder()
            ...
            var @base = BigInteger.Subtract(serverPublicKey.Value, gx3) % safePrimeModulus.Value;
            if (@base.Sign < 0)
                @base = BigInteger.Add(@base, safePrimeModulus.Value);
            ts.TraceInformation("ComputeSharedKey: @base = " + @base.ToString("X"));

            // exp = a + u * x
            BigInteger exp = BigInteger.Add(
        ...
        )
        {
            byte[] N = safePrimeModulus.ToBytes(),
                g = generator.ToBytes(),
                U = Encoding.UTF8.GetBytes(username),
                s = salt,
        ...
        {
            byte[] A = clientPublicKey.ToBytes(),
                M1 = clientProof,
                K = sharedKey.ToBytes(),
                I = Encoding.UTF8.GetBytes(authId),
                o = Encoding.UTF8.GetBytes(options),
                _sid = Encoding.UTF8.GetBytes(sid);
            var H = hashAlgorithm;
            // The proof is calculated as follows:
            // H( bytes(A)
            //  | bytes(M1)
        ...
        /// </summary>
        static readonly string[] moduli = new string[]
        {

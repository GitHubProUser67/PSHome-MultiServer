...
namespace MultiServerLibrary.ManagedMail.Auth.Sasl.Mechanisms
{
    /// <summary>
    ...
    internal class SaslScramSha1 : SaslMechanism
    {
        ...
        /// </summary>
        readonly string Cnonce = GenerateCnonce();
        ...
        string Username
        {
            get
            {
                return Properties.TryGetValue("Username", out var value) ? value as string : null;
            }
        ...
        string Password
        {
            get
            {
                return Properties.TryGetValue("Password", out var value) ? value as string : null;
            }
        ...
        protected override byte[] ComputeResponse(byte[] challenge)
        {
            ...
            if (String.IsNullOrEmpty(Username) || Password == null)
            {
                throw new SaslException(
                    "The username must not be null or empty and " + "the password must not be null."
                );
            }
            if (Step == 2)
                Completed = true;
            var ret =
                Step == 0
                    ? ComputeInitialResponse()
                    : (
                        Step == 1
                            ? ComputeFinalResponse(challenge)
        ...
        private byte[] ComputeFinalResponse(byte[] challenge)
        {
            ...
            string salt = nv["s"],
                nonce = nv["r"];
            var iterationCount = Int32.Parse(nv["i"]);
            ...
            string clientFirstBare = "n=" + SaslPrep(Username) + ",r=" + Cnonce,
                ...
                withoutProof =
                    "c=" + Base64.ToBase64String(Encoding.UTF8.GetBytes("n,,")) + ",r=" + nonce;
            AuthMessage = clientFirstBare + "," + serverFirstMessage + "," + withoutProof;
            ...
            return Encoding.UTF8.GetBytes(
                withoutProof + ",p=" + Base64.ToBase64String(clientProof)
            );
        }
        ...
        {
            var s = Encoding.UTF8.GetString(challenge);
            ...
            var serverSignature = s.Substring(2).IsBase64().DecodedBytes;
        ...
        /// is null.</exception>
        private static NameValueCollection ParseServerFirstMessage(byte[] challenge)
        {
            challenge.ThrowIfNull("challenge");
            var message = Encoding.UTF8.GetString(challenge);
        ...
        /// </remarks>
        private static byte[] Hi(string password, string salt, int count)
        {
            // The salt is sent by the server as a base64-encoded string.
            var saltBytes = salt.IsBase64().DecodedBytes;
        ...
        {
            return CastleLibrary.NetHasher.DotNetHasher.ComputeSHA1(data, key);
        }
        ...
        {
            return CastleLibrary.NetHasher.DotNetHasher.ComputeSHA1(data);
        }

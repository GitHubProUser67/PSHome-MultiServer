...
namespace MultiServerLibrary.ManagedMail.Auth.Sasl.Mechanisms
{
    ...
    internal class SaslOAuth2 : SaslMechanism
    {
        ...
        string AccessToken
        {
            get
            {
                return Properties.TryGetValue("AccessToken", out var value)
                    ? value as string
                    : null;
            }
        ...
        protected override byte[] ComputeResponse(byte[] challenge)
        {
            ...
            // with a CRLF.
            var ret = Step == 0 ? ComputeInitialResponse(challenge) : Array.Empty<byte>();
            Step++;
            return ret;
        }
        ...
        private byte[] ComputeInitialResponse(byte[] challenge)
        {
            ...
            if (String.IsNullOrEmpty(Username) || String.IsNullOrEmpty(AccessToken))
            {
                throw new SaslException(
                    "The username and access token must not be" + " null or empty."
                );
            }
            // ^A = Control A = (U+0001)
            var A = '\u0001';
            var s = "user=" + Username + A + "auth=Bearer " + AccessToken + A + A;

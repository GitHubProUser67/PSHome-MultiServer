...
using MultiServerLibrary.ManagedMail.Auth.Sasl.Mechanisms.Srp;
...
namespace MultiServerLibrary.ManagedMail.Auth.Sasl.Mechanisms
{
    ...
    internal class SaslSrp : SaslMechanism
    {
        ...
        /// </summary>
        readonly Mpi PrivateKey = Helper.GenerateClientPrivateKey();
        ...
        string AuthId
        {
            get
            {
                return Properties.TryGetValue("AuthId", out var value) ? value as string : Username;
            }
        ...
        {
            var m = ServerMessage1.Deserialize(challenge);
            ...
            SharedKey = Helper.ComputeSharedKey(
                m.Salt,
                Username,
                Password,
                PublicKey,
            ...
            ClientProof = Helper.ComputeClientProof(
                m.SafePrimeModulus,
                m.Generator,
                Username,
                m.Salt,
            ...
            var response = new ClientMessage2(PublicKey, ClientProof);
        ...
        {
            var m = ServerMessage2.Deserialize(challenge);
            // Compute the proof and compare it with the one sent by the server.
            var proof = Helper.ComputeServerProof(
                PublicKey,
                ClientProof,
                SharedKey,
                AuthId,
            ...
            return proof.EqualsTo(m.Proof) ? Array.Empty<byte>() : Encoding.UTF8.GetBytes("*");
        }
        ...
        private static Tuple<string, Type> SelectHashAlgorithm(string list)
        {
            var supported = list.Split(',');

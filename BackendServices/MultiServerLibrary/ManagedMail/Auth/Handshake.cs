...
namespace MultiServerLibrary.ManagedMail.Auth
{
    ...
    internal class Handshake
    {
        ...
        /// </summary>
        public HandshakeType MessageId { get; set; }
        ...
        public static Handshake Deserialize(byte[] data)
        {
                ...
                using (var r = new BEBinaryReader(ms))
                {
                    ...
                    catch (Exception e)
                    {
                        throw new SerializationException(
                            "The specified byte array contains " + "invalid data.",
                            e
                        );
                    }
                }
            ...
            {
                throw new SerializationException(
                    "Unexpected handshake version: " + hs.MajorVersion + "." + hs.MinorVersion
                );
            }
            // When the handshake message has a MessageId of HandshakeError, the
            // AuthPayload field _must_ have a length of 8 bytes.
            return hs.MessageId == HandshakeType.HandshakeError && hs.PayloadSize != 8
                ? throw new SerializationException(
                    "Unexpected payload size. Expected " + "8, but was: " + hs.PayloadSize
                )
                : hs;
        }

...
namespace MultiServerLibrary.ManagedMail.Auth
{
    /// <summary>
    ...
    internal partial class FilterStream : Stream
    {
        ...
        void ReadServerResponse()
        {
            ...
            var base64 = ReadLine(innerStream);
            var type = HandshakeType.HandshakeInProgress;
            byte[] decoded;
            ...
            try
            {
                ...
                base64 = MyRegex().Replace(base64, String.Empty);
            ...
            var hs = new Handshake(type, (ushort)decoded.Length);
            receivedData = new ByteBuilder().Append(hs.Serialize()).Append(decoded).ToArray();
            receivedConsumed = 0;
        }
                ...
                case FilterStreamState.ReadingPayload:
                    if (ReadPayload(buffer, offset, count))
                    {
                        if (SendPayload())
                        {
                            ...
                            receivedData = new Handshake(
                                HandshakeType.HandshakeDone,
                                0
                            ).Serialize();
                            receivedConsumed = 0;
                        }
        ...
        bool ReadHandshake(byte[] buffer, int offset, int count)
        {
            ...
            var read = Math.Min(count, 5 - handshakeData.Length);
        ...
        {
            var read = Math.Min(count, handshake.PayloadSize - payloadData.Length);
        ...
        bool SendPayload()
        {
            ...
            // CRLF.
            var base64 = Base64.ToBase64String(payloadData.ToArray());
            payloadData.Clear();
                    ...
                    if (b == Newline)
                    {
                        ...
                        if (resolveLiterals)
                        {
                            s = Regex.Replace(
                                ...
                                {
                                    return "\""
                                        + ReadLiteral(stream, Convert.ToInt32(m.Groups[1].Value))
                                        + "\""
        ...
        static string ReadLiteral(Stream stream, int byteCount)
        {

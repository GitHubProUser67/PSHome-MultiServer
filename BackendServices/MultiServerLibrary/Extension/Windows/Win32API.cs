...
namespace MultiServerLibrary.Extension.Windows
{
    ...
    public partial class Win32API
    {
        ...
        public static partial int OpenProcessToken(
...
#pragma warning restore
        ...
        {
            var result = WTSGetActiveConsoleSessionId();
    ...
    public class ProcessImpersonation : IDisposable
    {
        ...
        {
            var prToken = IntPtr.Zero;
            try
            {
                using (var curprocess = Process.GetCurrentProcess())
                {
                    var prHandel = IntPtr.Zero;
                    ...
                    if (
                        ...
                        Win32API.DuplicateToken(
                            prToken,
                            (int)Win32API.SECURITY_IMPERSONATION_LEVEL.SecurityImpersonation,
                            ref expProcToken
                        );
                }
            }
        ...
        public void Impersonate(Invoker impersonatedMthd)
        {
            if (expProcToken != IntPtr.Zero)
            {
                using (var dupnewId = new WindowsIdentity(expProcToken))
                {
                    WindowsIdentity.RunImpersonated(
                        dupnewId.AccessToken,
                        () =>
                        {
                            impersonatedMthd();
                        }
    ...
    public class HighProcessImpersonation : ProcessImpersonation
    {
        protected override void Initialize()
        {
            ...
            try
            {
                var accSessionId = Win32API.WTSGetActiveConsoleSessionId();
                var prHandel = IntPtr.Zero;
                foreach (var proc in Process.GetProcessesByName("explorer"))
                {
                ...
                if (
                    ...
                    result = Win32API.DuplicateToken(
                        prToken,
                        (int)Win32API.SECURITY_IMPERSONATION_LEVEL.SecurityImpersonation,
                        ref expProcToken
                    );

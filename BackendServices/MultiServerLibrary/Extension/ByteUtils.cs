...
#pragma warning disable SYSLIB1054 // Not possible in partial class
...
#pragma warning restore SYSLIB1054
        ...
        extension(byte[] a)
        {
            ...
            public bool EqualsTo(byte[] b, CompareDirection direction = CompareDirection.Forward)
            {
            ...
            public byte[] SubArray(int sizeToSubstract, bool atStart = false)
            {
                var newSize = a.Length - sizeToSubstract;
                var output = new byte[newSize];
                Array.Copy(a, atStart ? sizeToSubstract : 0, output, 0, newSize);
                return output;
            }
            ...
            public byte[] Trim()
            {
                ...
                Array.Copy(a, data, newLength);
                return data;
            }

            public byte[] ShadowCopy()
            {
        ...
        private static int FindDataPositionInBinary(byte[] data1, byte[] data2)
        {
            ...
            for (var i = 0; i < data1.Length - data2.Length + 1; i++)
            {
                var found = true;
        ...
        {
            var isfirstNull = first == null;
            var issecondNull = second == null;

            if (isfirstNull && issecondNull)
                return null;
            else if (issecondNull || second.Length == 0)
            {
                var sizeOfArray = first.Length;
                var copy = new byte[sizeOfArray];
                Array.Copy(first, 0, copy, 0, sizeOfArray);
                return copy;
            }
            else if (isfirstNull || first.Length == 0)
            {
                var sizeOfArray = second.Length;
                var copy = new byte[sizeOfArray];
                    ...
                    else
                    {
                        if (Avx512F.IsSupported)
                        {
                            ...
                            for (; j <= len2 - 64; j += 64)
                                Avx512F.Store(
                                    dstPtr + len1 + j,
                                    Avx512F.LoadVector512(src2Ptr + j)
                                );
                        }
                        if (Avx.IsSupported)
                        {
        ...
        public static byte[] CombineByteArrays(byte[] first, byte[] second, byte[] third)
        {
            ...
            var totalLength = len1 + len2 + len3;
                ...
                )
                {
                    if (AdvSimd.IsSupported)
                    {
                        ...
                        for (; k <= len3 - 16; k += 16)
                            AdvSimd.Store(
                                dstPtr + len1 + len2 + k,
                                AdvSimd.LoadVector128(src3Ptr + k)
                            );
                    }
                    else
                    {
                        if (Avx512F.IsSupported)
                        {
                            ...
                            for (; k <= len3 - 64; k += 64)
                                Avx512F.Store(
                                    dstPtr + len1 + len2 + k,
                                    Avx512F.LoadVector512(src3Ptr + k)
                                );
                        }
                        if (Avx.IsSupported)
                        {
        ...
        public static byte[] CombineByteArrays(byte[] first, byte[][] second)
        {
            ...
            var totalLength = firstLength + second.Sum(arr => arr.Length);
            if (totalLength > Array.MaxLength || totalLength < 0)
            {
                // Return the first array if total length exceeds limits
                var sizeOfArray = first.Length;
            ...
            var offsets = new int[second.Length];
            var currentOffset = firstLength;
            ...
            Parallel.ForEach(
                Enumerable.Range(0, second.Length),
        ...
        /// <returns>A int (-1 if not found).</returns>
        public static int FindBytePattern(
            byte[] buffer,
            byte[] searchPattern,
            int offset = 0,
            int maxOffset = -1
        )
        {
            ...
            if (
                ...
                || offset < 0
                || offset > buffer.Length - searchPattern.Length
            )
                return -1;

            var searchPatternSize = searchPattern.Length;
            ...
            for (var i = offset; i <= maxOffset; i++)
            {
                if (buffer[i] == searchPattern[0])
                {
                    ...
                    for (var y = 1; y < searchPatternSize; y++)
                    {
        ...
        )
        {
            if (
                ...
                || buffer.Length < searchPattern.Length
                || offset < 0
                || offset > buffer.Length - searchPattern.Length
            )
                return -1;
            ...
            {
                if (
                    buffer[i] == searchPattern[0]
                    && buffer.Slice(i, searchPatternSize).SequenceEqual(searchPattern)
                )
                    return i;
            }

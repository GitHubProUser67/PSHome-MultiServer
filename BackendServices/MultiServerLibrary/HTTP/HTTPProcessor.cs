...
using System.Web;
using CastleLibrary.SharpCompress.ZStandard;
...
namespace MultiServerLibrary.HTTP
{
    public partial class HTTPProcessor
    {
        ...
        public static readonly Dictionary<string, byte[]> PathernDictionary = new()
        {
            ...
            {
                "video/mp4",
                new byte[] { 0x00, 0x00, 0x00, 0x20, 0x66, 0x74, 0x79, 0x70, 0x6D, 0x70 }
            },
        };

        public static readonly HashSet<string> SupportedPS3ContentTypes = new(
        ...
        public static string RequestURLGET(string url, bool timeOut = false, bool pipeline = false)
        {
        ...
        public static (byte[] data, Dictionary<string, string> headers) RequestFullURLGET(
            string url,
            bool timeOut = false,
            bool pipeline = false
        )
        {
            var (_, data, headers) = RequestURL(
                url,
                HttpMethod.Get,
                null,
                null,
        ...
        public static string RequestURLPOST(
            string url,
        ...
        )
        {
            var result = RequestURL(
                ...
                HttpMethod.Post,
                requestHeaders,
                !string.IsNullOrEmpty(postData) ? Encoding.UTF8.GetBytes(postData) : null,
                ContentType,
                timeOut,
                false,
                pipeline
        ...
        {
            var restrictedHeaders = new HashSet<string>(StringComparer.InvariantCultureIgnoreCase)
            {
                "accept-encoding",
                "keep-alive",
                "connection",
            ...
            {
                using (
                    FixedWebClient client = timeOut
                        ? new FixedWebClientWithTimeout()
                        : new FixedWebClient()
                )
                {
                    client.PipelineSupport = pipeline;
                    ...
                    if (requestHeaders != null)
                    {
                        ...
                        {
                            if (!restrictedHeaders.Contains(header.Key.ToLower()))
                    ...
                    if (keepAlive)
                    {
                        ...
                        client.Headers[HttpRequestHeader.KeepAlive] = client
                            .KeepAlive.ToString()
                            .ToLower();
                    }
                    ...
                    try
                    {
                        switch (method.Method.ToUpperInvariant())
                        {
                            ...
                            case "PATCH":
                            case "DELETE":
                                responseData = client.UploadData(
                                    url,
                                    method.Method,
                            ...
                            default:
                                CustomLogger.LoggerAccessor.LogError(
                                    $"[HTTPProcessor] - RequestURL: HTTP method {method} not supported."
                                );
                                break;
                        }
                        ...
                        if (client.ResponseHeaders != null)
                        {
                            foreach (var key in client.ResponseHeaders.AllKeys)
                                responseHeaders[key] = client.ResponseHeaders[key];
                        }

                        return (client.StatusCode, responseData, responseHeaders);
                    }
                    catch (WebException wex)
                    {
                        ...
                        if (wex.Response is HttpWebResponse response)
                        {
                            ...
                            using (var ms = new MemoryStream())
                            {
                                ...
                                {
                                    stream.CopyTo(ms);
                                    errorData = ms.ToArray();
                                }
                            }
                        }

                        return (statusCode, errorData, responseHeaders);
                    }
                }
            }
            catch (Exception ex)
...
#if DEBUG
                CustomLogger.LoggerAccessor.LogError(
                    $"[HTTPProcessor] - RequestURL: Error while requesting url:{url} (Method:{method}). Exception:{ex}"
                );
#endif
            }
        ...
        public static string DecodeUrl(string url, bool allowPathTraversal = true)
        {
            if (url == null) // String empty handled in the Uri func.
                return string.Empty;
            ...
            return allowPathTraversal ? newUrl.Replace("%2f", "/") : newUrl; // For some reasons, NET will not escape this specific sequence (security? We handle path traversal exploits anyway...).
        }
        ...
        public static string GetMimeType(string extension)
        {
            ...
            else
            {
                ...
                return MimeTypes.TryGetValue(extension, out var mime)
                    ? mime
        ...
        public static string GetMimeType(string extension, Dictionary<string, string> mimeTypesDic)
        {
            ...
            else
            {
                ...
                return mimeTypesDic.TryGetValue(extension, out var mime)
                    ? mime
                    : "application/octet-stream";
            }
        }
        ...
        public static string GetExtensionFromMime(string mimeType)
        {
            return string.IsNullOrEmpty(mimeType)
                ? ".unknown"
                : MimeTypes.FirstOrDefault(x => x.Value == mimeType).Key ?? ".unknown";
        }

        public static string GetExtensionFromMime(
            string mimeType,
            Dictionary<string, string> mimeTypesDic
        )
        {
            return string.IsNullOrEmpty(mimeType)
                ? ".unknown"
                : mimeTypesDic.FirstOrDefault(x => x.Value == mimeType).Key ?? ".unknown";
        }
        ...
        public static bool CheckHeaderMatch(byte[] byteArray, int startIndex, string header)
        {
            ...
            {
                if (
                    startIndex + i >= byteArray.Length
                    || byteArray[startIndex + i] != (byte)header[i]
                )
                    return false;
            }
            return true;
        ...
        public static bool CheckLastWriteTime(
            string filePath,
            string Since,
            bool reverseConditional = false
        )
        {
            if (string.IsNullOrWhiteSpace(Since) || string.IsNullOrEmpty(filePath))
                return false;

            try
            {
                var time = ToDateTimeOffset(Since);

                if (time.HasValue && File.Exists(filePath))
                    return reverseConditional
                        ? time.Value < new FileInfo(filePath).LastWriteTime
                        : time.Value >= new FileInfo(filePath).LastWriteTime;
            }
            catch { }
        ...
        public static string ExtractBoundary(string contentType)
        {
            if (!string.IsNullOrEmpty(contentType))
            {
                var boundaryIndex = contentType.IndexOf(
                    "boundary=",
        ...
        public static string ExtractDirtyProxyPath(string referer)
        {
            ...
            var match = DirtyProxyRegex().Match(referer);
        ...
        public static Dictionary<string, List<string>> ExtractAndSortUrlEncodedPOSTData(
            byte[] urlEncodedDataByte
        )
        {
            // Parse the URL-encoded data
            var formData = HttpUtility.ParseQueryString(
                Encoding.UTF8.GetString(urlEncodedDataByte)
            );

            // Use a dictionary with a list to handle multiple values for the same key
            var formDataDictionary = new Dictionary<string, List<string>>();
            ...
            foreach (var key in formData.AllKeys)
            {
                if (key != null)
                {
                    ...
                    formDataDictionary[key]
                        .AddRange(formData.GetValues(key) ?? Array.Empty<string>());
                }
            }
        ...
        {
            return string.IsNullOrEmpty(input) ? null
                : input.StartsWith("http", StringComparison.InvariantCultureIgnoreCase)
                && input.Contains("://")
                    ? HttpUtility.ParseQueryString(input)
                : HttpUtility.ParseQueryString(ParseUriFromAbsolutePath(input).Query);
        }
        ...
        public static string ProcessQueryString(string input, bool extractQuery = false)
        {
            ...
            return indexOfQuestionMark >= 0
                ? extractQuery
                    ? input.Substring(indexOfQuestionMark)
        ...
        public static Dictionary<string, string> GetQueryParameters(string fullurl)
        {
            if (!string.IsNullOrEmpty(fullurl))
            {
                ...
                {
                    var trimmedurl = fullurl.Substring(questionMarkIndex + 1);
                    foreach (var UrlArg in HttpUtility.ParseQueryString(trimmedurl).AllKeys) // Thank you WebOne.
                    {
                        if (!string.IsNullOrEmpty(UrlArg))
                            parameterDictionary[UrlArg] =
                                HttpUtility.ParseQueryString(trimmedurl)[UrlArg] ?? string.Empty;
                    }
                }
        ...
        public static string ETag(string filename, object parameters = null)
        {
            ...
            {
                CustomLogger.LoggerAccessor.LogError(
                    "[HTTPProcessor] - ETag - File not found",
                    filename
                );
                return null;
            }
            ...
            return "\""
                + Convert.ToHexStringLower(
                    CastleLibrary.NetHasher.DotNetHasher.ComputeMD5(
                        Encoding.UTF8.GetBytes(
                            $"{filename.GetHashCode()}-{fileInfo.LastWriteTimeUtc.Ticks}-{fileInfo.Length}"
                                + (
                                    parameters != null
        ...
        public static DateTimeOffset? ToDateTimeOffset(string input)
        {
            ...
            return input.Equals("now", StringComparison.CurrentCultureIgnoreCase)
                ? DateTimeOffset.Now
        ...
        public static byte[] CompressBrotli(byte[] input)
        {
            ...
            using (
                var brStream = new System.IO.Compression.BrotliStream(
                    output,
        ...
        public static Stream ZstdCompressStream(Stream input)
        {
            ...
            using (input)
            {
                try
                {
                    ...
                    using (var outZStream = new CompressionStream(outMemoryStream))
                    {
                        outZStream.SetParameter(
                            ZSTD_cParameter.ZSTD_c_nbWorkers,
                            Environment.ProcessorCount
                        );
            ...
            using (input)
            {
                try
                {
                    if (input.Length > Array.MaxLength)
                        ...
                        outMemoryStream = new MemoryStream();
                    using (
                        var outBStream = new System.IO.Compression.BrotliStream(
                            outMemoryStream,
                            System.IO.Compression.CompressionLevel.Fastest,
                            true
                        )
            ...
            using (input)
            {
                try
                {
                    if (input.Length > Array.MaxLength)
                        ...
                        outMemoryStream = new MemoryStream();
                    using (var outZStream = new ZOutputStreamLeaveOpen(outMemoryStream, 1, true))
                        StreamUtils.CopyStream(input, outZStream);
                    outMemoryStream.Seek(0, SeekOrigin.Begin);
                }
                catch

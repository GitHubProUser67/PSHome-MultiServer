namespace WatsonWebserver.Lite
{
    ...
    public class WebserverLite : WebserverBase, IDisposable
    {
        ...
        private async void ClientConnected(object sender, ClientConnectedEventArgs args)
        {
            ...
            try
            {
                TcpClient tcpClient = _TcpServer.GetRawTcpClient(args.Client.Guid);
                Stream clientStream = _TcpServer.GetStream(args.Client.Guid);
            ...
            catch (Exception e)
            {
                if (ctx != null)
                {
                    ...
                    Events.HandleExceptionEncountered(this, new Core.ExceptionEventArgs(ctx, e));
                }

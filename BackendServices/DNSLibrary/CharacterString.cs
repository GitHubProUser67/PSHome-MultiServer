...
namespace DNSLibrary
{
    ...
    public class CharacterString
    {
        ...
        public static IList<CharacterString> GetAllFromArray(
            byte[] message,
            int offset,
            out int endOffset
        )
        ...
        {
            if (message.Length < 1)
                throw new ArgumentException("[CharacterString] - Empty message");
        ...
        public static IList<CharacterString> FromString(string message, Encoding encoding)
        {
            ...
            var size = (int)Math.Ceiling((double)bytes.Length / MAX_SIZE);
            IList<CharacterString> characterStrings = [with(size)];
        ...
        public CharacterString(byte[] data)
        {
            if (data.Length > MAX_SIZE)
                Array.Resize(ref data, MAX_SIZE);
            this.data = data;
        }

        public CharacterString(string data, Encoding encoding)
            : this(encoding.GetBytes(data)) { }

        public CharacterString(string data)
            : this(data, Encoding.ASCII) { }

...
namespace SpaceWizards.HttpListener.WebSockets
{
    internal static partial class HttpWebSocket
    {
        ...
        {
            var acceptString = string.Concat(secWebSocketKey, HttpWebSocket.SecWebSocketKeyGuid);
            ...
            return Base64.ToBase64String(SHA1.HashData(toHash));
        }
        ...
        )
        {
            ...
            var requestProtocols = clientSecWebSocketProtocol.Split(
                ',',
            ...
            for (var i = 0; i < requestProtocols.Length; i++)
            {
                var currentRequestProtocol = requestProtocols[i];
                if (
                    string.Equals(
                        acceptProtocol,
        ...
        )
        {
            ...
            if (receiveBufferSize > MaxBufferSize)
            {
                throw new ArgumentOutOfRangeException(
                    ...
                    receiveBufferSize,
                    SR.Format(
                        SR.net_WebSockets_ArgumentOutOfRange_TooBig,
                        nameof(receiveBufferSize),
                        receiveBufferSize,
                        MaxBufferSize
                    )
        ...
        private static void ValidateWebSocketHeaders(HttpListenerContext context)
        {
            ...
            var secWebSocketVersion = context.Request.Headers[
            ...
            var secWebSocketKey = context.Request.Headers[HttpKnownHeaderNames.SecWebSocketKey];
            var isSecWebSocketKeyInvalid = string.IsNullOrWhiteSpace(secWebSocketKey);
            if (!isSecWebSocketKeyInvalid)
            {
                try
                {
                    ...
                    isSecWebSocketKeyInvalid =
                        secWebSocketKey!.IsBase64().DecodedBytes.Length != 16;
                }
                catch
                {

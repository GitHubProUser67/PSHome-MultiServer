...
namespace SpaceWizards.HttpListener
{
    internal sealed class ServiceNameStore
    {
        ...
        private static string NormalizeServiceName(string inputServiceName)
        {
            ...
            var shashIndex = inputServiceName.IndexOf('/');
            if (shashIndex < 0)
            {
                return inputServiceName;
            }
            var prefix = inputServiceName.Substring(0, shashIndex + 1); // Includes slash
            var hostPortAndDistinguisher = inputServiceName.Substring(shashIndex + 1); // Excludes slash
            ...
            var host = hostPortAndDistinguisher;
            var port = string.Empty;
            var distinguisher = string.Empty;

            // Check for the absence of a port or distinguisher.
            var hostType = Uri.CheckHostName(hostPortAndDistinguisher);
            if (hostType == UriHostNameType.Unknown)
            {
                var hostAndPort = hostPortAndDistinguisher;

                // Check for distinguisher
                var nextSlashIndex = hostPortAndDistinguisher.IndexOf('/');
                if (nextSlashIndex >= 0)
                {
                ...
                // Check for port
                var colonIndex = hostAndPort.LastIndexOf(':'); // Allow IPv6 addresses
                if (colonIndex >= 0)
                {
                    // host:port
            ...
            var normalizedHost = constructedUri.GetComponents(
                UriComponents.NormalizedHost,
                UriFormat.SafeUnescaped
            );

            var normalizedServiceName = prefix + normalizedHost + port + distinguisher;

            // Don't return the new one unless we absolutely have to.  It may have only changed casing.
            return inputServiceName.Equals(
        ...
        public bool Add(string uriPrefix)
        {
            ...
            var newServiceNames = BuildServiceNames(uriPrefix);

            var addedAny = false;
            foreach (var spn in newServiceNames)
            {
        ...
        public bool Remove(string uriPrefix)
        {
            ...
            var newServiceName = BuildSimpleServiceName(uriPrefix);
            newServiceName = NormalizeServiceName(newServiceName);
            var needToRemove = Contains(newServiceName);
        ...
        private static string ExtractHostname(string uriPrefix, bool allowInvalidUriStrings)
        {
            ...
            {
                var hostUri = new Uri(uriPrefix);
                return hostUri.Host;
            }
            else if (allowInvalidUriStrings)
            {
                var i = uriPrefix.IndexOf("://", StringComparison.Ordinal) + 3;

                var j = FindEndOfHostname(uriPrefix, i);
        ...
        {
            var hostname = ExtractHostname(uriPrefix, false);

            return hostname != null ? "HTTP/" + hostname : null;
        }

        public string[] BuildServiceNames(string uriPrefix)
        {
            var hostname = ExtractHostname(uriPrefix, true);
            ...
            else if (!hostname.Contains('.'))
            {
                ...
                try
                {
                    var fqdn = Dns.GetHostEntry(hostname).HostName;

...
namespace System.Net
{
    internal class CaseInsensitiveAscii : IEqualityComparer, IComparer
    {
        ...
        internal static readonly CaseInsensitiveAscii StaticInstance = new();
        ...
        public int GetHashCode(object myObject)
        {
            ...
            var myHashCode = myString.Length;
            if (myHashCode == 0)
            {
                return 0;
            }
            myHashCode ^=
                (AsciiToLower[(byte)myString[0]] << 24)
        ...
        {
            var firstString = firstObject as string;
            var secondString = secondObject as string;
            if (firstString == null)
            {
            ...
            var comparisons = result > 0 ? secondString.Length : firstString.Length;
            int difference,
                index = 0;
            while (index < comparisons)
            {
                difference = (int)(
                    AsciiToLower[firstString[index]] - AsciiToLower[secondString[index]]
                );
                if (difference != 0)
                {
                    result = difference;

...
public sealed class Histogram : Collector<Histogram.Child>, IHistogram
{
        ...
        : base(name, help, instanceLabelNames, staticLabels, suppressInitialValue, exemplarBehavior)
    {
        ...
        if (!double.IsPositiveInfinity(_buckets[^1]))
        {
        ...
        if (Avx.IsSupported)
        {
            _bucketsAlignmentBuffer = GC.AllocateUninitializedArray<double>(
                _buckets.Length + (AvxAlignBytes / sizeof(double)),
                pinned: true
            );
            ...
            unsafe
            {
                ...
                var bytesUntilNextAlignedPosition =
                    (AvxAlignBytes - pointerTooFarByBytes) % AvxAlignBytes;

                if (bytesUntilNextAlignedPosition % sizeof(double) != 0)
                    throw new Exception(
                        "Unreachable code reached - all double[] allocations are expected to be at least 8-aligned."
                    );

                _bucketsAlignmentBufferOffset = (int)(
                    bytesUntilNextAlignedPosition / sizeof(double)
                );
            }
    ...
    public sealed class Child : ChildBase, IHistogram
    {
        ...
        public void Observe(double val, Exemplar? exemplarLabels) =>
            ObserveInternal(val, 1, exemplarLabels);
        ...
        private unsafe int GetBucketIndexAvx(double val)
        {
            ...
            for (var i = 0; i < Parent._buckets.Length - remaining; i += Vector256<double>.Count)
            {
                // The buckets are permanently pinned, no need to re-pin them here.
                var boundPointer = (double*)
                    Unsafe.AsPointer(
                        ref Parent._bucketsAlignmentBuffer[Parent._bucketsAlignmentBufferOffset + i]
                    );
            ...
            for (var i = Parent._buckets.Length - remaining; i < Parent._buckets.Length; i++)
            {
                if (val <= Parent._buckets[i])
                    return i;
            }
    ...
    public static double[] ExponentialBuckets(double start, double factor, int count)
    {
        if (count <= 0)
            throw new ArgumentException(
                $"{nameof(ExponentialBuckets)} needs a positive {nameof(count)}"
            );
        if (start <= 0)
            throw new ArgumentException(
                $"{nameof(ExponentialBuckets)} needs a positive {nameof(start)}"
            );
        if (factor <= 1)
            throw new ArgumentException(
                $"{nameof(ExponentialBuckets)} needs a {nameof(factor)} greater than 1"
            );
    ...
    public static double[] LinearBuckets(double start, double width, int count)
    {
        if (count <= 0)
            throw new ArgumentException(
                $"{nameof(LinearBuckets)} needs a positive {nameof(count)}"
            );
    ...
    public static double[] PowersOfTenDividedBuckets(int startPower, int endPower, int divisions)
    {
        if (startPower >= endPower)
            throw new ArgumentException(
                $"{nameof(startPower)} must be less than {nameof(endPower)}.",
                nameof(startPower)
            );
        ...
        if (divisions <= 0)
            throw new ArgumentOutOfRangeException(
                nameof(divisions),
                $"{nameof(divisions)} must be a positive integer."
            );

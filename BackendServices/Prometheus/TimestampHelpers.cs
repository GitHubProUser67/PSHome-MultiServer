...
static class TimestampHelpers
{
    ...
    private const int DaysTo1970 =
        (DaysPer400Years * 4) + (DaysPer100Years * 3) + (DaysPer4Years * 17) + DaysPerYear; // 719,162
    ...
    public static double ToUnixTimeSecondsAsDouble(DateTimeOffset timestamp)
    {
        ...
        var ticksSinceUnixEpoch =
            timestamp.ToUniversalTime().Ticks - (UnixEpochSeconds * TimeSpan.TicksPerSecond);

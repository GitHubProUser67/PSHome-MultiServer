...
internal readonly struct LabelSequence : IEquatable<LabelSequence>
{
    ...
    private LabelSequence(StringSequence names, StringSequence values)
    {
        if (names.Length != values.Length)
            throw new ArgumentException(
                "The list of label values must have the same number of elements as the list of label names."
            );
    ...
    public byte[] Serialize()
    {
        ...
        for (var i = 0; i < Names.Length; i++)
        {
            ...
            index += PrometheusConstants.ExportEncoding.GetBytes(
                nameEnumerator.Current,
                0,
                nameEnumerator.Current.Length,
                bytes,
                index
            );
        ...
        return index != byteCount
            ? throw new Exception(
                "API contract violation - we counted the same bytes twice but got different numbers."
            )
            : bytes;
    }
    ...
    {
        return obj is LabelSequence ls && Equals(ls);
    }

...
public sealed class MetricFactory : IMetricFactory
{
    ...
    internal MetricFactory(CollectorRegistry registry)
        : this(registry, LabelSequence.Empty) { }
    ...
    {
        return _factoryLabels.Length != 0
            ? _factoryLabels.Concat(_registry.GetStaticLabels())
            : _registry.GetStaticLabels();
    }
    ...
    ) => CreateCounter(name, help, configuration?.LabelNames ?? [], configuration);

    /// <summary>
    /// Gauges can have any numeric value and change arbitrarily.
    /// </summary>
    public Gauge CreateGauge(string name, string help, GaugeConfiguration? configuration = null) =>
        CreateGauge(name, help, configuration?.LabelNames ?? [], configuration);
    ...
    ) => CreateSummary(name, help, configuration?.LabelNames ?? [], configuration);
    ...
    ) => CreateHistogram(name, help, configuration?.LabelNames ?? [], configuration);
    ...
    internal Counter CreateCounter(
        string name,
        string help,
    ...
    {
        var exemplarBehavior =
            configuration?.ExemplarBehavior ?? ExemplarBehavior ?? ExemplarBehavior.Default;

        return _registry.GetOrAdd(
            name,
            help,
            instanceLabelNames,
            _staticLabelsLazy.Value,
            configuration ?? CounterConfiguration.Default,
            exemplarBehavior,
            _createCounterInstanceFunc
        );
    }

    internal Gauge CreateGauge(
        string name,
        string help,
        StringSequence instanceLabelNames,
    ...
    )
    {
        ...
        return _registry.GetOrAdd(
            ...
            exemplarBehavior,
            _createGaugeInstanceFunc
        );
    }

    internal Histogram CreateHistogram(
        string name,
        string help,
        StringSequence instanceLabelNames,
    ...
    )
    {
        ...
        return _registry.GetOrAdd(
            ...
            instanceLabelNames,
            _staticLabelsLazy.Value,
            configuration ?? HistogramConfiguration.Default,
            exemplarBehavior,
            _createHistogramInstanceFunc
        );
    }
    ...
    )
    {
        ...
        return _registry.GetOrAdd(
            ...
            exemplarBehavior,
            _createSummaryInstanceFunc
        );
    }

    private static Counter CreateCounterInstance(
        string Name,
        string Help,
    ...
    private static Gauge CreateGaugeInstance(
        string Name,
        string Help,
        in StringSequence InstanceLabelNames,
        in LabelSequence StaticLabels,
    ...
    private static Histogram CreateHistogramInstance(
        string Name,
        string Help,
        in StringSequence InstanceLabelNames,
        in LabelSequence StaticLabels,
    ...
    private static Summary CreateSummaryInstance(
        string name,
        string help,
        in StringSequence instanceLabelNames,
        in LabelSequence staticLabels,
    ...
    private static readonly CollectorInitializer<
        Counter,
        CounterConfiguration
    > _createCounterInstanceFunc = CreateCounterInstance;
    private static readonly CollectorInitializer<
        Gauge,
        GaugeConfiguration
    > _createGaugeInstanceFunc = CreateGaugeInstance;
    private static readonly CollectorInitializer<
        Histogram,
        HistogramConfiguration
    > _createHistogramInstanceFunc = CreateHistogramInstance;
    private static readonly CollectorInitializer<
        Summary,
        SummaryConfiguration
    > _createSummaryInstanceFunc = CreateSummaryInstance;

    /// <summary>
    /// Counters only increase in value and reset to zero when the process restarts.
    /// </summary>
    public Counter CreateCounter(string name, string help, params string[] labelNames) =>
        CreateCounter(name, help, labelNames, null);

    /// <summary>
    /// Gauges can have any numeric value and change arbitrarily.
    /// </summary>
    public Gauge CreateGauge(string name, string help, params string[] labelNames) =>
        CreateGauge(name, help, labelNames, null);

    /// <summary>
    /// Summaries track the trends in events over time (10 minutes by default).
    /// </summary>
    public Summary CreateSummary(string name, string help, params string[] labelNames) =>
        CreateSummary(name, help, labelNames, null);

    /// <summary>
    /// Histograms track the size and number of events in buckets.
    /// </summary>
    public Histogram CreateHistogram(string name, string help, params string[] labelNames) =>
        CreateHistogram(name, help, labelNames, null);

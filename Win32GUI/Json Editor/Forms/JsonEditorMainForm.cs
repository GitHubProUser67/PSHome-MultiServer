    ...
    public sealed partial class JsonEditorMainForm : Form
    {
        ...
        string OpenedFileName
        {
            ...
            set
            {
                ...
                Text = (internalOpenedFileName ?? string.Empty) + @" - Json Editor by ZTn";
            }
        }
        ...
        public JsonEditorMainForm()
        {
            ...
            if (commandLineArgs.Skip(1).Any())
            {
                ...
                try
                {
                    using (var stream = File.OpenRead(commandLineArgs[1]))
                    {
        ...
        private void JsonValidationTimerHandler()
        {
            try
            {
                ...
                if (selectionStart > jsonValueTextBox.Text.Length)
                    selectionStart = jsonValueTextBox.Text.Length;

                jsonValueTextBox.SelectionStart = selectionStart;
                jsonValueTextBox.SelectionLength = 0;

...
namespace RemoteControl
{
    public partial class FormMain : Form
    {
        ...
        private readonly Dictionary<uint, ControlWriter> _writersList = [];

        private const string exeExtension = ".exe";
        private const string jsonExtension = ".json";

        private readonly uint httpCRC = CRC32.CreateCastagnoli(
            Encoding.ASCII.GetBytes("ApacheNet" + exeExtension)
        );
        private readonly uint dnsCRC = CRC32.CreateCastagnoli(
            Encoding.ASCII.GetBytes("MitmDNS" + exeExtension)
        );
        private readonly uint horizonCRC = CRC32.CreateCastagnoli(
            Encoding.ASCII.GetBytes("Horizon" + exeExtension)
        );
        private readonly uint multisocksCRC = CRC32.CreateCastagnoli(
            Encoding.ASCII.GetBytes("MultiSocks" + exeExtension)
        );
        private readonly uint multispyCRC = CRC32.CreateCastagnoli(
            Encoding.ASCII.GetBytes("MultiSpy" + exeExtension)
        );
        private readonly uint quazalCRC = CRC32.CreateCastagnoli(
            Encoding.ASCII.GetBytes("QuazalServer" + exeExtension)
        );
        private readonly uint ssfwCRC = CRC32.CreateCastagnoli(
            Encoding.ASCII.GetBytes("SSFWServer" + exeExtension)
        );
        private readonly uint svoCRC = CRC32.CreateCastagnoli(
            Encoding.ASCII.GetBytes("SVO" + exeExtension)
        );
        private readonly uint edenCRC = CRC32.CreateCastagnoli(
            Encoding.ASCII.GetBytes("EdenServer" + exeExtension)
        );
        ...
        public FormMain()
        {
            ...
            bool horizonConfsLoaded = false,
                multisocksConfsLoaded = false;
...
#endif
            var startupSoundPath = currentDir + "/startup.mp3";

            if (File.Exists(startupSoundPath))
            {
                using (var jingleSt = File.OpenRead(startupSoundPath))
            ...
            textBoxApacheNetJsonPath.Text =
                Program.currentDir + "/static/ApacheNet" + jsonExtension;
            textBoxDNSJsonPath.Text = Program.currentDir + "/static/MitmDNS" + jsonExtension;
            var horizonConfigPath = textBoxHorizonJsonPath.Text =
                Program.currentDir + "/static/Horizon" + jsonExtension;
            var multisocksConfigPath = textBoxMultisocksJsonPath.Text =
                Program.currentDir + "/static/MultiSocks" + jsonExtension;
            textBoxMultispyJsonPath.Text = Program.currentDir + "/static/MultiSpy" + jsonExtension;
            textBoxQuazalserverJsonPath.Text =
                Program.currentDir + "/static/QuazalServer" + jsonExtension;
            textBoxSSFWServerJsonPath.Text =
                Program.currentDir + "/static/SSFWServer" + jsonExtension;
            textBoxSVOJsonPath.Text = Program.currentDir + "/static/SVO" + jsonExtension;
            textBoxEdenserverJsonPath.Text =
                Program.currentDir + "/static/EdenServer" + jsonExtension;
            ...
            try
            {
                if (File.Exists(horizonConfigPath))
                {
                    ...
                    textBoxMediusJsonPath.Text =
                        configRoot?["medius"]?["config"]?.ToString() ?? string.Empty;
                    textBoxDMEJsonPath.Text =
                        configRoot?["dme"]?["config"]?.ToString() ?? string.Empty;
                    textBoxMUISJsonPath.Text =
                        configRoot?["muis"]?["config"]?.ToString() ?? string.Empty;
                    textBoxNatJsonPath.Text =
                        configRoot?["nat"]?["config"]?.ToString() ?? string.Empty;
                    textBoxBwpsJsonPath.Text =
                        configRoot?["bwps"]?["config"]?.ToString() ?? string.Empty;
                    textBoxEbootDefsJsonPath.Text =
                        configRoot?["eboot_defs_config"]?.ToString() ?? string.Empty;
                    textBoxHorizonDatabaseJsonPath.Text =
                        configRoot?["database"]?.ToString() ?? string.Empty;
            ...
            try
            {
                if (File.Exists(multisocksConfigPath))
                {
                    ...
                    textBoxAriesDatabaseJsonPath.Text =
                        configRoot?["dirtysocks_database_path"]?.ToString() ?? string.Empty;
            ...
            if (!horizonConfsLoaded)
            {
                ...
                textBoxEbootDefsJsonPath.Text =
                    Program.currentDir + "/static/ebootdefs" + jsonExtension;
                textBoxHorizonDatabaseJsonPath.Text =
                    Program.currentDir + "/static/db.config" + jsonExtension;
            }

            if (!multisocksConfsLoaded)
                textBoxAriesDatabaseJsonPath.Text =
                    Program.currentDir + "/static/dirtysocks.db" + jsonExtension;
            ...
            Console.SetOut(
                new MultiTextWriter(new ControlWriter(richTextBoxInformation), Console.Out)
            );
        ...
        private void MainForm_FormClosing(object sender, FormClosingEventArgs e)
        {
            var result = MessageBox.Show(
                "Do you really want to close the application?\nShutdown can take a little while if servers are running.",
                "Confirmation",
                ...
                Parallel.ForEach(
                    ...
                    guid =>
                    {
                        ...
                        catch (Exception ex)
                        {
                            MessageBox.Show(
                                ex.Message,
                                $"Application with guid:{guid} thrown an exception while being closed."
                            );
                        }
                    }
                );
        ...
        {
            Utils.UpdateConfigurationFile(
                textBoxApacheNetJsonPath,
                Utils.OpenExecutableFile(textBoxApacheNetPath)
            );
        }
        ...
        {
            var exePath = Utils.OpenExecutableFile(textBoxHorizonPath);
            if (!string.IsNullOrEmpty(exePath))
            {
                var configRoot = JsonNode.Parse(
                    File.ReadAllText(Utils.UpdateConfigurationFile(textBoxHorizonJsonPath, exePath))
                );
                textBoxMediusJsonPath.Text =
                    configRoot?["medius"]?["config"]?.ToString() ?? string.Empty;
        ...
        {
            var exePath = Utils.OpenExecutableFile(textBoxMultisocksPath);
            if (!string.IsNullOrEmpty(exePath))
            {
                var configRoot = JsonNode.Parse(
                    File.ReadAllText(
                        Utils.UpdateConfigurationFile(textBoxMultisocksJsonPath, exePath)
                    )
                );
                textBoxAriesDatabaseJsonPath.Text =
                    configRoot?["dirtysocks_database_path"]?.ToString() ?? string.Empty;
        ...
        private void buttonStartHTTP_Click(object sender, EventArgs e)
        {
            _ = Task.Run(() =>
            {
                ...
                try
                {
                    ...
                    ProcessManager.StartupProgram(
                        _writersList[httpCRC],
                        textBoxHTTP,
                        groupBoxHTTP,
                        prefix,
                        textBoxApacheNetPath.Text,
                        httpCRC
                    );
                    textBoxHTTP.Invoke(
                        new Action(() =>
                ...
                catch (Exception ex)
                {
                    LoggerAccessor.LogInfo(
                        $"[{prefix}] - An assertion was thrown while starting the server! (Exception: {ex})"
                    );
                    textBoxHTTP.Invoke(
                        new Action(() =>
                        {
        ...
        private void buttonStartHorizon_Click(object sender, EventArgs e)
        {
            _ = Task.Run(() =>
            {
                ...
                try
                {
                    ...
                    ProcessManager.StartupProgram(
                        _writersList[horizonCRC],
                        textBoxHorizon,
                        groupBoxHorizon,
                        prefix,
                        textBoxHorizonPath.Text,
                        horizonCRC
                    );
                    textBoxHorizon.Invoke(
                        new Action(() =>
                        {
                            textBoxHorizon.Text = "Running";
                        })
                    );
                    groupBoxHorizon.Invoke(
                        new Action(() =>
                        {
        ...
        private void buttonStartMultisocks_Click(object sender, EventArgs e)
        {
            _ = Task.Run(() =>
            {
                ...
                try
                {
                    ...
                    ProcessManager.StartupProgram(
                        _writersList[multisocksCRC],
                        textBoxMultisocks,
                        groupBoxMultisocks,
                        prefix,
                        textBoxMultisocksPath.Text,
                        multisocksCRC
                    );
                    textBoxMultisocks.Invoke(
                        new Action(() =>
                        {
                            textBoxMultisocks.Text = "Running";
                        })
                    );
                    groupBoxMultisocks.Invoke(
                        new Action(() =>
                        {
        ...
        private void buttonStartMultispy_Click(object sender, EventArgs e)
        {
            _ = Task.Run(() =>
            {
                ...
                try
                {
                    ...
                    ProcessManager.StartupProgram(
                        _writersList[multispyCRC],
                        textBoxMultispy,
                        groupBoxMultispy,
                        prefix,
                        textBoxMultispyPath.Text,
                        multispyCRC
                    );
                    textBoxMultispy.Invoke(
                        new Action(() =>
                        {
                            textBoxMultispy.Text = "Running";
                        })
                    );
                    groupBoxMultispy.Invoke(
                        new Action(() =>
                        {
        ...
        private void buttonStartQuazalserver_Click(object sender, EventArgs e)
        {
            _ = Task.Run(() =>
            {
                ...
                try
                {
                    ...
                    ProcessManager.StartupProgram(
                        _writersList[quazalCRC],
                        textBoxQuazalserver,
                        groupBoxQuazalserver,
                        prefix,
                        textBoxQuazalserverPath.Text,
                        quazalCRC
                    );
                    textBoxQuazalserver.Invoke(
                        new Action(() =>
                        {
                            textBoxQuazalserver.Text = "Running";
                        })
                    );
                    groupBoxQuazalserver.Invoke(
                        new Action(() =>
                        {
        ...
        private void buttonStartSSFWServer_Click(object sender, EventArgs e)
        {
            _ = Task.Run(() =>
            {
                ...
                try
                {
                    ...
                    ProcessManager.StartupProgram(
                        _writersList[ssfwCRC],
                        textBoxSSFWServer,
                        groupBoxSSFWServer,
                        prefix,
                        textBoxSSFWServerPath.Text,
                        ssfwCRC
                    );
                    textBoxSSFWServer.Invoke(
                        new Action(() =>
                        {
                            textBoxSSFWServer.Text = "Running";
                        })
                    );
                    groupBoxSSFWServer.Invoke(
                        new Action(() =>
                        {
        ...
        private void buttonStartEdenserver_Click(object sender, EventArgs e)
        {
            _ = Task.Run(() =>
            {
                ...
                try
                {
                    ...
                    ProcessManager.StartupProgram(
                        _writersList[edenCRC],
                        textBoxEdenserver,
                        groupBoxEdenserver,
                        prefix,
                        textBoxEdenserverPath.Text,
                        edenCRC
                    );
                    textBoxEdenserver.Invoke(
                        new Action(() =>
                        {
                            textBoxEdenserver.Text = "Running";
                        })
                    );
                    groupBoxEdenserver.Invoke(
                        new Action(() =>
                        {
        ...
        private void linkLabelGithub_LinkClicked(object sender, LinkLabelLinkClickedEventArgs e)
        {
            var url =
                e.Link.LinkData != null
                    ...
                    : linkLabelGithub.Text.Substring(e.Link.Start, e.Link.Length);

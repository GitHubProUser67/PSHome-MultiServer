...
public static partial class EdenServerConfiguration
{
    ...
    public static string ORBServerAddress { get; set; } =
        InternetProtocolUtils.TryGetServerIP(out var ip).Result ? ip : ip;
    public static ushort ORBServerPort { get; set; } = 9000;
    public static string STATSServerAddress { get; set; } =
        InternetProtocolUtils.TryGetServerIP(out var ip).Result ? ip : ip;
    public static ushort STATSServerPort { get; set; } = 9001;
    public static string CLANServerAddress { get; set; } =
        InternetProtocolUtils.TryGetServerIP(out var ip).Result ? ip : ip;
    public static ushort CLANServerPort { get; set; } = 9002;
    public static string WEATHERServerAddress { get; set; } =
        InternetProtocolUtils.TryGetServerIP(out var ip).Result ? ip : ip;
    public static ushort WEATHERServerPort { get; set; } = 9003;
    public static string PHOTOServerAddress { get; set; } =
        InternetProtocolUtils.TryGetServerIP(out var ip).Result ? ip : ip;
    public static ushort PHOTOServerPort { get; set; } = 9004;
    public static string GAMBLINGServerAddress { get; set; } =
        InternetProtocolUtils.TryGetServerIP(out var ip).Result ? ip : ip;
    public static ushort GAMBLINGServerPort { get; set; } = 9005;
    public static string MODERATIONServerAddress { get; set; } =
        InternetProtocolUtils.TryGetServerIP(out var ip).Result ? ip : ip;
    public static ushort MODERATIONServerPort { get; set; } = 9006;
    public static string CHECKServerAddress { get; set; } =
        InternetProtocolUtils.TryGetServerIP(out var ip).Result ? ip : ip;
    public static ushort CHECKServerPort { get; set; } = 9007;
    public static string CARDEALERServerAddress { get; set; } =
        InternetProtocolUtils.TryGetServerIP(out var ip).Result ? ip : ip;
    public static ushort CARDEALERServerPort { get; set; } = 9008;
    public static string RANKINGServerAddress { get; set; } =
        InternetProtocolUtils.TryGetServerIP(out var ip).Result ? ip : ip;
    public static ushort RANKINGServerPort { get; set; } = 9009;
    public static string SAVEGAMEServerAddress { get; set; } =
        InternetProtocolUtils.TryGetServerIP(out var ip).Result ? ip : ip;
    public static ushort SAVEGAMEServerPort { get; set; } = 9010;
    public static string LOGINServerAddress { get; set; } =
        InternetProtocolUtils.TryGetServerIP(out var ip).Result ? ip : ip;
    public static ushort LOGINServerPort { get; set; } = 9011;
    public static string AMHProxyServerAddress { get; set; } =
        InternetProtocolUtils.TryGetServerIP(out var ip).Result ? ip : ip;
    ...
    public static string LoginDatabasePath { get; set; } =
        $"{Directory.GetCurrentDirectory()}/static/Eden/Database/tdu_accounts.sqlite";
    ...
    {
        var match = ServerCrcRegex().Match(serviceName);
        if (!match.Success)
            return null;

        var serviceToken = match.Groups[1].Value;
        ...
        return addressProperty == null || portProperty == null
            ? null
            : (addressProperty.GetValue(null) as string, (ushort?)portProperty.GetValue(null));
    }
    ...
    public static void RefreshVariables(string configPath)
    {
        ...
        if (!File.Exists(configPath))
        {
            ...
            var configObject = new
            {
                ...
                telnet = new { enable = EnableTelnet },
                database = new { login = LoginDatabasePath },
                amh = new
                {
                ...
                encryption = new { enable = EnableEncryption, big_endian = BigEndianEncryption },
        ...
        try
        {
            ...
            using (var doc = JsonDocument.Parse(File.ReadAllText(configPath)))
            {
                ...
                {
                    if (
                        config.TryGetProperty("telnet", out var telnetElement)
                        && telnetElement.TryGetProperty("enable", out var enableElement)
                    )
                        EnableTelnet = enableElement.GetBoolean();
                    if (config.TryGetProperty("amh", out var amhElement))
                    {
                        AMHProxyServerAddress = GetValueOrDefault(
                            amhElement,
                            "proxy_server_address",
                            AMHProxyServerAddress
                        );
                        AMHProxyEncryptionKey = GetValueOrDefault(
                            amhElement,
                            "proxy_encryption_key",
                            AMHProxyEncryptionKey
                        );
                        AMHMasterServerPort = GetValueOrDefault(
                            amhElement,
                            "master_server_port",
                            AMHMasterServerPort
                        );
                    }
                    if (config.TryGetProperty("database", out var databaseElement))
                    {
                        LoginDatabasePath = GetValueOrDefault(
                            amhElement,
                            "login",
                            LoginDatabasePath
                        );
                    }
                    if (config.TryGetProperty("encryption", out var encryptionElement))
                    {
                        EnableEncryption = GetValueOrDefault(
                            encryptionElement,
                            "enable",
                            EnableEncryption
                        );
                        BigEndianEncryption = GetValueOrDefault(
                            encryptionElement,
                            "big_endian",
                            BigEndianEncryption
                        );
                    }
                    ProxyServerAddress = GetValueOrDefault(
                        config,
                        "proxy_server_address",
                        ProxyServerAddress
                    );
                    ProxyServerPort = GetValueOrDefault(
                        config,
                        "proxy_server_port",
                        ProxyServerPort
                    );
                    ORBServerAddress = GetValueOrDefault(
                        config,
                        "orb_server_address",
                        ORBServerAddress
                    );
                    ORBServerPort = GetValueOrDefault(config, "orb_server_port", ORBServerPort);
                    STATSServerAddress = GetValueOrDefault(
                        config,
                        "stats_server_address",
                        STATSServerAddress
                    );
                    STATSServerPort = GetValueOrDefault(
                        config,
                        "stats_server_port",
                        STATSServerPort
                    );
                    CLANServerAddress = GetValueOrDefault(
                        config,
                        "clan_server_address",
                        CLANServerAddress
                    );
                    CLANServerPort = GetValueOrDefault(config, "clan_server_port", CLANServerPort);
                    WEATHERServerAddress = GetValueOrDefault(
                        config,
                        "weather_server_address",
                        WEATHERServerAddress
                    );
                    WEATHERServerPort = GetValueOrDefault(
                        config,
                        "weather_server_port",
                        WEATHERServerPort
                    );
                    PHOTOServerAddress = GetValueOrDefault(
                        config,
                        "photo_server_address",
                        PHOTOServerAddress
                    );
                    PHOTOServerPort = GetValueOrDefault(
                        config,
                        "photo_server_port",
                        PHOTOServerPort
                    );
                    GAMBLINGServerAddress = GetValueOrDefault(
                        config,
                        "gambling_server_address",
                        GAMBLINGServerAddress
                    );
                    GAMBLINGServerPort = GetValueOrDefault(
                        config,
                        "gambling_server_port",
                        GAMBLINGServerPort
                    );
                    MODERATIONServerAddress = GetValueOrDefault(
                        config,
                        "moderation_server_address",
                        MODERATIONServerAddress
                    );
                    MODERATIONServerPort = GetValueOrDefault(
                        config,
                        "moderation_server_port",
                        MODERATIONServerPort
                    );
                    CHECKServerAddress = GetValueOrDefault(
                        config,
                        "check_server_address",
                        CHECKServerAddress
                    );
                    CHECKServerPort = GetValueOrDefault(
                        config,
                        "check_server_port",
                        CHECKServerPort
                    );
                    CARDEALERServerAddress = GetValueOrDefault(
                        config,
                        "cardealer_server_address",
                        CARDEALERServerAddress
                    );
                    CARDEALERServerPort = GetValueOrDefault(
                        config,
                        "cardealer_server_port",
                        CARDEALERServerPort
                    );
                    RANKINGServerAddress = GetValueOrDefault(
                        config,
                        "ranking_server_address",
                        RANKINGServerAddress
                    );
                    RANKINGServerPort = GetValueOrDefault(
                        config,
                        "ranking_server_port",
                        RANKINGServerPort
                    );
                    SAVEGAMEServerAddress = GetValueOrDefault(
                        config,
                        "savegame_server_address",
                        SAVEGAMEServerAddress
                    );
                    SAVEGAMEServerPort = GetValueOrDefault(
                        config,
                        "savegame_server_port",
                        SAVEGAMEServerPort
                    );
                    LOGINServerAddress = GetValueOrDefault(
                        config,
                        "login_server_address",
                        LOGINServerAddress
                    );
                    LOGINServerPort = GetValueOrDefault(
                        config,
                        "login_server_port",
                        LOGINServerPort
                    );
                    ClientLongTimeoutSeconds = GetValueOrDefault(
                        config,
                        "client_long_timeout_seconds",
...
class Program
{
    ...
    private static readonly string configPath = configDir + serverName + ".json";

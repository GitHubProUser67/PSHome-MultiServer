...
namespace SSFWServer.Helpers.FileHelper
{
    public class FileHelperCryptoClass
    {
        ...
        public static string GetEncryptionKey(string secretKey)
        {
            ...
            var hashedSecret = CastleLibrary.NetHasher.DotNetHasher.ComputeMD5(
            ...
            var hashedSecretLast12HexBytes = Encoding.UTF8.GetBytes(
                BitConverter.ToString(hashedSecretLast12Bytes).ToLower().Replace("-", string.Empty)
            );
            var secretFirst12Bytes = Encoding.UTF8.GetBytes(
                secretKey.Replace("FLWSECK-", string.Empty)[..12]
            );
            var combineKey = new byte[24];

            Array.Copy(secretFirst12Bytes, 0, combineKey, 0, secretFirst12Bytes.Length);
            Array.Copy(
                hashedSecretLast12HexBytes,
                hashedSecretLast12HexBytes.Length - 12,
                combineKey,
                12,
                12
            );
        ...
        public static byte[] DecryptData(byte[] InData, string encryptionKey)
        {
            ...
            try
            {
                ...
                var firstSixBytes = new byte[6];

                Array.Copy(InData, 0, firstSixBytes, 0, firstSixBytes.Length);

                if (ByteUtils.FindBytePattern(firstSixBytes, "XTNDV2"u8.ToArray()) != -1)
                {
                    var xteakey = new byte[16];
                    ...
                    var dst = new byte[InData.Length - 6];

                    Array.Copy(InData, 6, dst, 0, dst.Length);

                    InData = xtea.Decrypt(
                        dst,
                        InitiateCustomXTEACipheredKey(xteakey, ReverseByteArray(xteakey))
                    );
            ...
            catch (Exception ex)
            {
                LoggerAccessor.LogError(
                    $"[FileHelperCryptoClass] : Thrown an exception in DecryptData - {ex}"
                );
            }
        ...
        private static byte[] InitiateCustomXTEACipheredKey(
            byte[] KeyBytes,
            byte[] ReversedKeyBytes
        )
        {
            // Create the cipher
            ...
            var ciphertextBytes = new byte[cipher.GetOutputSize(KeyBytes.Length)];

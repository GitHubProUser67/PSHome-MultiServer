...
using System.Xml;
using CastleLibrary.S0ny.Home.ChannelID;
        ...
        public static void UpdateSceneDictionary(object? state)
        {
            try
            {
                if (File.Exists(SSFWServerConfiguration.ScenelistFile))
                {
                    CustomLogger.LoggerAccessor.LogInfo(
                        $"[ScenelistParser] - Dictionary refresh started at: {DateTime.Now}"
                    );
                    ...
                    lock (sceneDictionary)
                    {
                        ...
                        var sceneNodes = xmlDoc.SelectNodes(
                            "//REGION/SCENE[@Type='Home' or @Type='Clubhouse' or @Type='PrivateNoVideo'] | //SCENE[@Type='Home' or @Type='Clubhouse' or @Type='PrivateNoVideo']"
                        );
                        ...
                        if (sceneNodes != null)
                        {
                            ...
                            Parallel.ForEach(
                                sceneNodes.Cast<XmlNode>(),
                                sceneNode =>
                                {
                                    if (sceneNode.Attributes != null)
                                    {
                                        var ID = sceneNode.Attributes["ID"]?.Value;

                                        if (!string.IsNullOrEmpty(ID))
                                        {
                                            var channelID = sceneNode
                                            ...
                                            if (!string.IsNullOrEmpty(channelID))
                                            {
                                                try
                                                {
                                                    if (channelID.Contains('-'))
                                                    {
                                                        ...
                                                        var SceneID = SIDKeyGenerator
                                                            .ExtractSceneIDNewerType(sceneKey)
                                                            .ToString("X")
                                                            .PadLeft(4, '0'); // SSFW was introduced in modern Home only, safe to assume only newer type.
                                                        if (!sceneDictionary.ContainsKey(ID))
                                                        {
#if DEBUG
                                                            CustomLogger.LoggerAccessor.LogInfo(
                                                                $"[SSFW] - ScenelistParser: Added entry: ID:{ID}|SceneID:{SceneID}"
                                                            );
#endif
...
#if DEBUG
                                                            CustomLogger.LoggerAccessor.LogInfo(
                                                                $"[SSFW] - ScenelistParser: Added entry: ID:{ID}|SceneID:{channelID}"
                                                            );
#endif
            ...
            catch (Exception ex)
            {
                CustomLogger.LoggerAccessor.LogError(
                    $"[ScenelistParser] - Errored out while updating scene list Dictionary: {ex}"
                );
            }
        }
    }

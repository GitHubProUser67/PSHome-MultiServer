...
namespace SSFWServer.Helpers.FileHelper
{
    public class CustomXTEA
    {
        ...
        public byte[] Decrypt(byte[] data, byte[] key)
        {
            ...
            try
            {
                ...
                using (MemoryStream stream = new(buffer))
                {
                    ...
                    {
                        blockBuffer[0] = BitConverter.ToUInt32(
                            !LittleEndian ? EndianUtils.ReverseArray(buffer) : buffer,
                            i
                        );
                        blockBuffer[1] = BitConverter.ToUInt32(
                            !LittleEndian ? EndianUtils.ReverseArray(buffer) : buffer,
                            i + 4
                        );
                ...
                // verify valid length
                var length = BitConverter.ToUInt32(
                    !LittleEndian ? EndianUtils.ReverseArray(buffer) : buffer,
                    0
                );
            ...
            catch (Exception ex)
            {
                LoggerAccessor.LogError(
                    $"[CUSTOMXTEA] : Thrown an exception in CustomXTEA Decrypt - {ex}"
                );
            }
        ...
        /// <returns></returns>
        public static uint[] CreateKey(byte[] key, bool Endianess)
        {
            ...
            for (var i = key.Length; i < hash.Length; i++)
            {
            ...
            return
            [
                BitConverter.ToUInt32(!Endianess ? EndianUtils.ReverseArray(hash) : hash, 0),
                BitConverter.ToUInt32(!Endianess ? EndianUtils.ReverseArray(hash) : hash, 4),
                BitConverter.ToUInt32(!Endianess ? EndianUtils.ReverseArray(hash) : hash, 8),
                BitConverter.ToUInt32(!Endianess ? EndianUtils.ReverseArray(hash) : hash, 12),
            ];
        }
        ...
        private static void Decrypt(uint rounds, uint[] v, uint[] key)
        {
            ...
            for (var j = 0; j <= 3; j++)
            {
                v0 = v[0] ^ 0x9E3779B9 ^ 0x9E3779B9 ^ 0x9E3779B9 ^ 0x9E3779B9 ^ 0x9E3779B9;
                v1 = v[1] ^ 0x9E3779B9 ^ 0x9E3779B9 ^ 0x9E3779B9 ^ 0x9E3779B9 ^ 0x9E3779B9;
                delta = 0x9E3779B9;
                sum = delta * rounds;
                for (uint i = 0; i < rounds; i++)
                {

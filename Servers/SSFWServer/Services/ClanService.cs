...
{
    public class ClanService(string sessionid)
    {
        private readonly string? _sessionid = sessionid;

        // Handles GET,POST,DELETE on this function
        public HttpResponse HandleClanDetailsService(
            HttpRequest req,
            HttpResponse res,
            string absolutepath
        )
        {
            var filePath =
                $"{SSFWServerConfiguration.SSFWStaticFolder}{HTTPProcessor.ParseUriFromAbsolutePath(absolutepath).AbsolutePath}.json";
            ...
            if (req.Method == HttpMethod.Post.ToString())
            {
                try
                {
                    if (
                        JsonDocument
                            .Parse(req.Body)
                            .RootElement.TryGetProperty("sceneObjectId", out var idElement)
                    )
                    {
                        var psnClanId = absolutepath.Split("/").LastOrDefault();
...
""clanId"": ""{DotNetHasher.ComputeMD5String(Encoding.UTF8.GetBytes(psnClanId))}""
            ...
            else if (req.Method == HttpMethod.Get.ToString()) // GET ONLY
            {
                ...
                return File.Exists(filePath)
                    ? res.MakeGetResponse(File.ReadAllText(filePath), "application/json")
                ...
                if (File.Exists(filePath))
                {
                    var sceneObjectId =
                        JsonDocument
                            ...
                            .RootElement.GetProperty("sceneObjectId")
                            .GetString()
                        ?? string.Empty;
                    File.Delete(filePath);
                    return res.MakeGetResponse(

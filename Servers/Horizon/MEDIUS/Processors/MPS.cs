...
{
    public class MPS : BaseMediusProcessor
    {
        public override ushort TCPPort
        {
            get => HorizonServerConfiguration.MEDIUSMPSPort;
            set
            {
                throw new ArgumentOutOfRangeException(
                    nameof(value),
                    "[MPS] - TCP Port can't be assigned."
                );
            }
        }
        ...
        public override ushort UDPPort
        {
            ...
            set
            {
                throw new ArgumentOutOfRangeException(
                    nameof(value),
                    "[MPS] - UDP Port can't be assigned."
                );
            }
        }
        ...
        private readonly char[] _trimChars = new char[] { ':', 'f', '{', '}' };
        ...
        )
        {
            ...
            switch (message)
            {
                case RT_MSG_CLIENT_HELLO clientHello:
                {
                    if (data.State > ServerClientState.HELLO)
                    {
                        LoggerAccessor.LogError(
                            $"[MPS] - Unexpected RT_MSG_CLIENT_HELLO from {clientChannel.RemoteAddress}: {clientHello}"
                        );
                        break;
                    }
                    ...
                    Queue(
                        new RT_MSG_SERVER_HELLO()
                        {
                            RsaPublicKey = enableEncryption
                                ? ScertClientAttribute.DefaultRsaAuthKey.N
                ...
                case RT_MSG_CLIENT_CRYPTKEY_PUBLIC clientCryptKeyPublic:
                {
                    ...
                    {
                        LoggerAccessor.LogError(
                            $"[MPS] - Unexpected RT_MSG_CLIENT_CRYPTKEY_PUBLIC from {clientChannel.RemoteAddress}: {clientCryptKeyPublic}"
                        );
                        break;
                    }
                ...
                case RT_MSG_CLIENT_CONNECT_TCP clientConnectTcp:
                {
                    ...
                    {
                        LoggerAccessor.LogError(
                            $"[MPS] - Unexpected RT_MSG_CLIENT_CONNECT_TCP from {clientChannel.RemoteAddress}: {clientConnectTcp}"
                        );
                        break;
                    }
                    ...
                    {
                        LoggerAccessor.LogError(
                            $"[MPS] - Client Connected {clientChannel.RemoteAddress} with an invalid connect payload!"
                        );
                        break;
                    }
                    #endregion
                    ...
                    {
                        LoggerAccessor.LogError(
                            $"[MPS] - Client Connected {clientChannel.RemoteAddress} with an empty key!"
                        );
                        break;
                    }
                    ...
                    if (targetChannel == null)
                    {
                        ...
                        {
                            LoggerAccessor.LogError(
                                $"[MPS] - Client: {clientConnectTcp.AccessToken} tried to join, but targetted WorldId:{clientConnectTcp.TargetWorldId} doesn't exist!"
                            );
                            await clientChannel
                                .CloseAsync()
                    ...
                    )
                    {
                        ...
                        if (data.ClientObject != null)
                            LoggerAccessor.LogInfo(
                                $"[MPS] - Client Connected {clientChannel.RemoteAddress}!"
                            );
                        else
                        {
                            data.Ignore = true;
                            LoggerAccessor.LogError(
                                $"[MPS] - ClientObject could not be granted for {clientChannel.RemoteAddress}: {clientConnectTcp}"
                            );
                            break;
                        }
                    ...
                    {
                        LoggerAccessor.LogInfo(
                            $"[MPS] - Client Connected {clientChannel.RemoteAddress} with new ClientObject!"
                        );
                ...
                case RT_MSG_CLIENT_APP_TOSERVER clientAppToServer:
                {
                    ...
                    {
                        LoggerAccessor.LogError(
                            $"[MPS] - Unexpected RT_MSG_CLIENT_APP_TOSERVER from {clientChannel.RemoteAddress}: {clientAppToServer}"
                        );
                        break;
                    }
                ...
                case RT_MSG_CLIENT_DISCONNECT _:
                {
                    ...
                    LoggerAccessor.LogInfo(
                        $"[MPS] - Client disconnected by request with no specific reason\n"
                    );
                    break;
                }
                case RT_MSG_CLIENT_DISCONNECT_WITH_REASON clientDisconnectWithReason:
                {
                    if (
                        ...
                        LoggerAccessor.LogInfo(
                            $"[MPS] - Disconnected by request with reason of {clientDisconnectWithReason.Reason}\n"
                        );
                    else
                        LoggerAccessor.LogInfo(
                            $"[MPS] - Disconnected by request with (application specified) reason of {clientDisconnectWithReason.Reason}\n"
                        );
        ...
        )
        {
            ...
            switch (message)
            {
                ...
                {
                    if (
                        !string.IsNullOrEmpty(createGameWithAttrResponse.MessageID.Value)
                        && createGameWithAttrResponse.MessageID.Value.Contains('-')
                    ...
                    )
                    {
                        ...
                        var messageParts = createGameWithAttrResponse.MessageID.Value.Split('-');
                        ...
                        {
                            LoggerAccessor.LogError(
                                $"[MPS] - An assertion was thrown while parsing createGameWithAttrResponse MessageID: {ex}"
                            );
                        }
                        ...
                        if (!handled)
                        {
                            LoggerAccessor.LogWarn(
                                "[MPS] - createGameWithAttrResponse received an invalid MessageID, ignoring request..."
                            );
                            break;
                        }

                        var game = Program.MUMManager.GetGameByGameId(gameOrPartyId);
                        var party = Program.MUMManager.GetPartyByPartyId(gameOrPartyId);
                        var rClient = Program.MUMManager.GetClientByAccountId(
                            accountId,
                        ...
                        {
                            LoggerAccessor.LogError(
                                $"[MPS] - MediusServerCreateGameWithAttributesResponse - Tried to grab a ClientObject, but AccountId:{accountId} on AppId:{data.ClientObject.ApplicationId} returned no result!"
                            );
                            break;
                        }
                        ...
                        if (partyType == 1 && party != null)
                        {
                            ...
                            await party.PartyCreated().ConfigureAwait(false);
                            rClient.Queue(
                        ...
                        else if (data.ApplicationId == 22920 || data.ApplicationId == 21834)
                        {
                            ...
                            if (!createGameWithAttrResponse.IsSuccess)
                            {
                                ...
                                if (game != null)
                                    await game.EndGame(rClient.ApplicationId).ConfigureAwait(false);
                            }
                            else if (game != null)
                            {
                                game.MediusWorldId = game.GameChannel!.Id =
                                    createGameWithAttrResponse.MediusWorldId;
                                await game.GameCreated().ConfigureAwait(false);
                                ...
                                await Program
                                    .MediusManager.Plugins.OnEvent(
                                        PluginEvent.MEDIUS_GAME_ON_CREATED,
                                        new OnPlayerGameArgs() { Player = rClient, Game = game }
                                    )
                                    .ConfigureAwait(false);
                            }
                        }
                        ...
                        data.ClientObject?.Queue(
                            new MediusCreateGameResponse()
                            {
                                MessageID = new MessageId(
                                    createGameWithAttrResponse.MessageID.Value
                                ),
                                StatusCode = MediusCallbackStatus.MediusSuccess,
                ...
                case MediusServerJoinGameResponse joinGameResponse:
                {
                    if (data.ClientObject != null)
                    {
                        ...
                        var messageParts = joinGameResponse.MessageID?.Value.Split('-');
                        ...
                        if (
                            ...
                            && data.ClientObject.ClientHomeData.VersionAsDouble >= 01.21
                        )
                        ...
                        {
                            LoggerAccessor.LogWarn(
                                "[MPS] - joinGameResponse received an invalid MessageID, ignoring request..."
                            );
                            break;
                        }
                        ...
                        if (!joinGameResponse.IsSuccess)
                        {
                            rClient?.Queue(
                                ...
                                {
                                    SetMaxPlayers = approvedMaxPlayersAppIds.Contains(
                                        data.ClientObject.ApplicationId
                                    ),
                                    MessageID = new MessageId(msgId),
                                    StatusCode = MediusCallbackStatus.MediusFail,
                        ...
                        else if (partyType == 1 && party != null)
                        {
                            if (rClient != null)
                            {
                                // Join game DME
                                await rClient
                                    .JoinParty(party, party.MediusWorldId)
                                    .ConfigureAwait(false);
                                        ...
                                        ConnectionInfo = new NetConnectionInfo()
                                        {
                                            ...
                                            AddressList = new NetAddressList()
                                            {
                                                ...
                                                ]
                                                {
                                                    ...
                                                    {
                                                        Address = data
                                                            .ClientObject.IP.MapToIPv4()
                                                            .ToString(),
                                                        Port = data.ClientObject.Port,
                                                        AddressType =
                        ...
                        else if (gameOrPartyId != 0 && game != null)
                        {
                            if (rClient != null)
                            {
                                            ...
                                            ConnectInfo = new NetConnectionInfo()
                                            {
                                                ...
                                                AddressList = new NetAddressList()
                                                {
                                                    ...
                                                    ]
                                                    {
                                                        new()
                                                        {
                                                            Address = game.netAddressList
                                                                .AddressList[0]
                                                                .Address,
                                                            Port = game.netAddressList
                                                                .AddressList[0]
                                                                .Port,
                                                            AddressType =
                                                                NetAddressType.NetAddressTypeSignalAddress,
                                                        ...
                                                        {
                                                            AddressType =
                                                                NetAddressType.NetAddressNone,
                                                        },
                                                    },
                                                },
                                                Type =
                                                    NetConnectionType.NetConnectionTypePeerToPeerUDP,
                                            },
                                ...
                                )
                                {
                                                ...
                                                AddressList = new NetAddressList()
                                                {
                                                    ...
                                                    ]
                                                    {
                                                        new()
                                                        {
                                                            AddressType =
                                                                NetAddressType.NetAddressTypeBinaryExternalVport,
                                                            IPBinaryBitOne = game.netAddressList
                                                                .AddressList[0]
                                                                .IPBinaryBitOne,
                                                            IPBinaryBitTwo = game.netAddressList
                                                                .AddressList[0]
                                                                .IPBinaryBitTwo,
                                                            IPBinaryBitThree = game.netAddressList
                                                                .AddressList[0]
                                                                .IPBinaryBitThree,
                                                            IPBinaryBitFour = game.netAddressList
                                                                .AddressList[0]
                                                                .IPBinaryBitFour,
                                                            BinaryPort = game.netAddressList
                                                                .AddressList[0]
                                                                .BinaryPort,
                                                        },
                                                        new()
                                    ...
                                    (rClient.MediusVersion > 108 && rClient.ApplicationId != 10994)
                                    || rClient.ApplicationId == 10680
                                    || rClient.ApplicationId == 10681
                                    || rClient.ApplicationId == 10683
                                    || rClient.ApplicationId == 10684
                                )
                                {
                                    // Join game DME
                                    await rClient
                                        .JoinGame(game, joinGameResponse.DmeClientIndex)
                                        .ConfigureAwait(false);
                            ...
                            if (
                                data.ClientObject.MediusVersion < 109
                                && data.ClientObject.ApplicationId != 10394
                            )
                        ...
                        LoggerAccessor.LogError(
                            "[MPS] - joinGameResponse was requested but client is null, aborting!"
                        );
                    break;
                }
                #endregion
                ...
                case MediusServerCreateGameOnSelfRequest serverCreateGameOnSelfRequest:
                {
                    ...
                    {
                        LoggerAccessor.LogError(
                            $"[MPS] - INVALID OPERATION: {clientChannel} is trying to MediusServerCreateGameOnSelfRequest without a Client Object"
                        );
                        break;
                    }

                    if (
                        serverCreateGameOnSelfRequest.AddressList.AddressList[0].AddressType
                        == NetAddressType.NetAddressTypeBinaryExternal
                    )
                        data.ClientObject?.SetIp(
                            InternetProtocolUtils
                                .GetIPAddressFromUInt(
                                    serverCreateGameOnSelfRequest
                                        .AddressList
                                        .AddressList[0]
                                        .BinaryAddress
                                )
                    ...
                    {
                        data.ClientObject?.SetIp(
                            serverCreateGameOnSelfRequest.AddressList.AddressList[0].IPBinaryBitOne
                                + "."
                                + serverCreateGameOnSelfRequest
                                    .AddressList
                                    .AddressList[0]
                        ...
                        serverCreateGameOnSelfRequest.AddressList.AddressList[0].AddressType
                            == NetAddressType.NetAddressNone
                        || serverCreateGameOnSelfRequest.AddressList.AddressList[0].AddressType
                            == NetAddressType.NetAddressTypeSignalAddress /* TODO: Signal Address not supported yet! */
                    )
                        data.ClientObject?.SetIp(
                        ...
                        data.ClientObject?.SetIp(
                            serverCreateGameOnSelfRequest.AddressList.AddressList[0].Address
                                ?? "0.0.0.0"
                        );
                    ...
                    if (
                        !MediusManager.PassTextFilter(
                            ...
                            Convert.ToString(serverCreateGameOnSelfRequest.GameName)
                        )
                    )
                    {
                        data.ClientObject?.Queue(
                    ...
                    await Program
                        .MediusManager.Plugins.OnEvent(
                            ...
                            {
                                Player = data.ClientObject,
                                Request = serverCreateGameOnSelfRequest,
                            }
                        )
                        .ConfigureAwait(false);

                    data.MeClientObject = await Program
                        .MUMManager.CreateGameP2P(
                            data.ClientObject!,
                            serverCreateGameOnSelfRequest,
                            clientChannel
                        )
                ...
                case MediusServerCreateGameOnSelfRequest0 serverCreateGameOnSelfRequest0:
                {
                    ...
                    {
                        LoggerAccessor.LogError(
                            $"[MPS] - INVALID OPERATION: {clientChannel} is trying to MediusServerCreateGameOnSelfRequest0 without a Client Object"
                        );
                        break;
                    }

                    if (
                        serverCreateGameOnSelfRequest0.AddressList.AddressList[0].AddressType
                        == NetAddressType.NetAddressTypeBinaryExternal
                    )
                        data.ClientObject?.SetIp(
                            InternetProtocolUtils
                                .GetIPAddressFromUInt(
                                    serverCreateGameOnSelfRequest0
                                        .AddressList
                                        .AddressList[0]
                                        .BinaryAddress
                                )
                    ...
                    {
                        data.ClientObject?.SetIp(
                            serverCreateGameOnSelfRequest0.AddressList.AddressList[0].IPBinaryBitOne
                                + "."
                                + serverCreateGameOnSelfRequest0
                                    .AddressList
                                    .AddressList[0]
                        ...
                        data.ClientObject?.SetIp(
                            serverCreateGameOnSelfRequest0.AddressList.AddressList[0].Address
                                ?? "0.0.0.0"
                        );
                    ...
                    if (
                        !MediusManager.PassTextFilter(
                            data.ApplicationId,
                            TextFilterContext.GAME_NAME,
                            Convert.ToString(serverCreateGameOnSelfRequest0.GameName)
                        )
                    )
                    {
                        data.ClientObject?.Queue(
                    ...
                    await Program
                        .MediusManager.Plugins.OnEvent(
                            ...
                            {
                                Player = data.ClientObject,
                                Request = serverCreateGameOnSelfRequest0,
                            }
                        )
                        .ConfigureAwait(false);
                ...
                case MediusServerCreateGameOnMeRequest serverCreateGameOnMeRequest:
                {
                    ...
                    {
                        LoggerAccessor.LogError(
                            $"[MPS] - INVALID OPERATION: {clientChannel} is trying to MediusServerCreateGameOnMeRequest without a Client Object"
                        );
                        break;
                    }

                    if (
                        serverCreateGameOnMeRequest.AddressList.AddressList[0].AddressType
                        == NetAddressType.NetAddressTypeBinaryExternal
                    )
                        data.ClientObject?.SetIp(
                            InternetProtocolUtils
                                .GetIPAddressFromUInt(
                                    serverCreateGameOnMeRequest
                                        .AddressList
                                        .AddressList[0]
                                        .BinaryAddress
                                )
                    ...
                    {
                        data.ClientObject?.SetIp(
                            serverCreateGameOnMeRequest.AddressList.AddressList[0].IPBinaryBitOne
                                + "."
                                + serverCreateGameOnMeRequest
                                    .AddressList
                                    .AddressList[0]
                        ...
                        data.ClientObject?.SetIp(
                            serverCreateGameOnMeRequest.AddressList.AddressList[0].Address
                                ?? "0.0.0.0"
                        );
                    ...
                    if (
                        !MediusManager.PassTextFilter(
                            data.ApplicationId,
                            TextFilterContext.GAME_NAME,
                            Convert.ToString(serverCreateGameOnMeRequest.GameName)
                        )
                    )
                    {
                        data.ClientObject?.Queue(
                    ...
                    await Program
                        .MediusManager.Plugins.OnEvent(
                            ...
                            {
                                Player = data.ClientObject,
                                Request = serverCreateGameOnMeRequest,
                            }
                        )
                        .ConfigureAwait(false);
                ...
                case MediusServerMoveGameWorldOnMeRequest serverMoveGameWorldOnMeRequest:
                {
                    ...
                    var game = Program.MUMManager.GetGameByGameId(
                        serverMoveGameWorldOnMeRequest.CurrentMediusWorldID
                    );
                    var party = Program.MUMManager.GetPartyByPartyId(
                        serverMoveGameWorldOnMeRequest.CurrentMediusWorldID
                    );

                    if (game != null)
                    {
                        var result = game.ReassignGameMediusWorldID(serverMoveGameWorldOnMeRequest);

                        if (result == 0)
                        {
                            data.MeClientObject?.Queue(
                        ...
                        {
                            game.netAddressList.AddressList[0] = serverMoveGameWorldOnMeRequest
                                .AddressList
                                .AddressList[0];
                            game.netAddressList.AddressList[1] = serverMoveGameWorldOnMeRequest
                                .AddressList
                                .AddressList[1];
                            ...
                            data.MeClientObject?.Queue(
                                new MediusServerMoveGameWorldOnMeResponse()
                                {
                                    ...
                                    MediusWorldID =
                                        serverMoveGameWorldOnMeRequest.NewGameMediusWorldID,
                                }
                            );
                        }
                    }
                    ...
                    {
                        var result = party.ReassignGameMediusWorldID(
                            serverMoveGameWorldOnMeRequest
                        );
                        ...
                        {
                            party.netAddressList.AddressList[0] = serverMoveGameWorldOnMeRequest
                                .AddressList
                                .AddressList[0];
                            party.netAddressList.AddressList[1] = serverMoveGameWorldOnMeRequest
                                .AddressList
                                .AddressList[1];
                ...
                case MediusServerWorldReportOnMe serverWorldReportOnMe:
                {
                    if (data.MeClientObject?.CurrentGame != null)
                        await data
                            .MeClientObject.CurrentGame.OnWorldReportOnMe(serverWorldReportOnMe)
                            .ConfigureAwait(false);
                    else if (data.MeClientObject?.CurrentParty != null)
                        await data
                            .MeClientObject.CurrentParty.OnWorldReportOnMe(serverWorldReportOnMe)
                            .ConfigureAwait(false);
                    break;
                }
                ...
                case MediusServerEndGameOnMeRequest serverEndGameOnMeRequest:
                {
                    ...
                    {
                        LoggerAccessor.LogWarn(
                            "[MPS] - Client Object: {data.ClientObject} called MediusServerEndGameOnMeRequest without a Me Client Object, ignoring."
                        );
                        break;
                    }

                    var game = Program.MUMManager.GetGameByGameId(
                        serverEndGameOnMeRequest.MediusWorldID
                    );
                    var party = Program.MUMManager.GetPartyByPartyId(
                        serverEndGameOnMeRequest.MediusWorldID
                    );
                    ...
                    if (game != null)
                    {
                        await game.EndGame(data.MeClientObject.ApplicationId).ConfigureAwait(false);

                        data.MeClientObject.Queue(
                            new MediusServerEndGameOnMeResponse()
                            {
                    ...
                    {
                        await party
                            .EndParty(data.MeClientObject.ApplicationId)
                            .ConfigureAwait(false);
                ...
                case MediusServerConnectNotification connectNotification:
                {
                    if (data.ClientObject != null)
                    {
                        var gameConn = Program.MUMManager.GetGameByMediusWorldId(
                            data.ClientObject.SessionKey ?? string.Empty,
                            connectNotification.MediusWorldUID
                        );
                        var partyConn = Program.MUMManager.GetPartyByMediusWorldId(
                            data.ClientObject.SessionKey ?? string.Empty,
                            connectNotification.MediusWorldUID
                        );
                        ...
                        if (gameConn != null)
                            await gameConn
                                .OnMediusServerConnectNotification(connectNotification)
                                .ConfigureAwait(false);
                ...
                case MediusServerEndGameRequest endGameRequest:
                {
                    ...
                    {
                        LoggerAccessor.LogError(
                            $"[MPS] - INVALID OPERATION: {clientChannel} is trying to MediusServerEndGameRequest without a Client Object"
                        );
                        break;
                    }

                    var game = Program.MUMManager.GetGameByGameId(endGameRequest.MediusWorldID);
                    var party = Program.MUMManager.GetPartyByPartyId(endGameRequest.MediusWorldID);
                    ...
                    if (game != null)
                    {
                        if (endGameRequest.BrutalFlag)
                            ...
                            await data.ClientObject.LeaveGame(game).ConfigureAwait(false);

                        data.ClientObject.Queue(
                            new MediusServerEndGameResponse()
                            {
                    ...
                    else if (party != null)
                    {
                        if (endGameRequest.BrutalFlag)
                            await party
                                .EndParty(data.ClientObject.ApplicationId)
                                .ConfigureAwait(false);
                        else
                            await data.ClientObject.LeaveParty(party).ConfigureAwait(false);
                ...
                case MediusServerSessionEndRequest sessionEndRequest:
                {
                    ...
                    {
                        LoggerAccessor.LogError(
                            $"[MPS] - INVALID OPERATION: {clientChannel} is trying to end server session without a Client Object"
                        );
                        break;
                    }
                    ...
                    data?.ClientObject.Queue(
                        new MediusServerSessionEndResponse()
                        {
        ...
        {
            var channels = _MediusServer.ScertHandler?.Channels;
            ...
            try
            {
                ...
                return appId == 10680 || appId == 10681
                    ? (
                        channels
                            ?.Select(x => _channelDatas[x.Id.AsLongText()]?.ClientObject)
                            .Where(x =>
                                x is not null
                                && x.LocationId == preferredLocation
                                && (x.ApplicationId == appId || x.ApplicationId == 0)
                                && x.IsActiveServer
                                && x.IP.Equals(
                                    InternetProtocolUtils
            ...
            {
                LoggerAccessor.LogError(
                    $"[MPS] - No DME Game Server assigned to this AppId. (Exception:{e})"
                );
            }
        ...
        public void SendServerCreateGameOrPartyWithAttributesRequestP2P(
            string msgId,
            int acctId,
            int gameId,
            object gameOrParty,
        ...
        )
        {
            ...
            if (gameOrParty == null)
                throw new ArgumentNullException(nameof(gameOrParty));

            var (attributes, maxPlayers, worldId, isParty) = gameOrParty switch
            {
                Party p => (p.Attributes, p.MaxPlayers, p.MediusWorldId, true),
                Game g => (g.Attributes, g.MaxPlayers, g.MediusWorldId, false),
                _ => throw new ArgumentException(
                    "[MPS] - gameOrParty must be Game or Party",
                    nameof(gameOrParty)
                ),
            };

            var ipBytes = client.IP.GetAddressBytes();

            Queue(
                new RT_MSG_SERVER_APP
                {
                    Message = new MediusServerCreateGameWithAttributesRequest2 // TODO, party has it's own packet as seen in Twisted Metal 2012.
                    {
                        MessageID = new MessageId($"{gameId}-{acctId}-{msgId}-{isParty}"),
                        ...
                        ConnectInfo = new NetConnectionInfo
                        {
                            ...
                            AddressList = new NetAddressList
                            {
                                AddressList = new NetAddress[Constants.NET_ADDRESS_LIST_COUNT]
                                {
                                    new NetAddress
                                    {
                                        BinaryAddress = BitConverter.ToUInt32(
                                            EndianTools.EndianAwareConverter.isLittleEndianSystem
                                                ? ipBytes
                                                : EndianUtils.ReverseArray(ipBytes)
                                        ),
                                        BinaryPort = 0,

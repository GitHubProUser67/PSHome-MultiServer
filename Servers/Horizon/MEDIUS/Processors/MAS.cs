...
{
    public partial class MAS : BaseMediusProcessor
    {
        ushort _tcpPort = 0;
            ...
            set
            {
                if (value < 0 || value > 65535)
                    throw new ArgumentOutOfRangeException(
                        nameof(value),
                        "[MAS] - Port must be between 0 and 65535."
                    );
                _tcpPort = value;
            }
        }
        ...
        public override ushort UDPPort
        {
            ...
            set
            {
                throw new ArgumentOutOfRangeException(
                    nameof(value),
                    "[MAS] - UDP Port can't be assigned."
                );
            }
        }

        private static readonly char[] trimChars = new char[] { ':', 'f', '{', '}' };
        ...
        )
        {
            ...
            switch (message)
            {
                ...
                case RT_MSG_CLIENT_CONNECT_TCP clientConnectTcp:
                {
                    ...
                    if (appid < 0)
                    {
                        LoggerAccessor.LogError(
                            $"[MAS] - Client Connected {clientChannel.RemoteAddress} with an invalid connect payload!"
                        );
                        break;
                    }
                    ...
                    {
                        LoggerAccessor.LogError(
                            $"[MAS] - Client {clientChannel.RemoteAddress} attempting to authenticate with incompatible app id {appid}"
                        );
                        await clientChannel
                            .CloseAsync()
                    ...
                    {
                        LoggerAccessor.LogError(
                            $"[MAS] - Client Connected {clientChannel.RemoteAddress} with an empty key!"
                        );
                        break;
                    }
                    ...
                    if (targetChannel == null)
                    {
                        ...
                        {
                            LoggerAccessor.LogError(
                                $"[MAS] - Client: {clientConnectTcp.AccessToken} tried to join, but targetted WorldId:{clientConnectTcp.TargetWorldId} doesn't exist!"
                            );
                            await clientChannel
                                .CloseAsync()
                                .TryAwait(TimeSpan.FromMilliseconds(2000))
                    ...
                    )
                    {
                        ...
                        if (data.ClientObject != null)
                            LoggerAccessor.LogInfo(
                                $"[MAS] - Client Connected {clientChannel.RemoteAddress}!"
                            );
                        else
                        {
                            data.Ignore = true;
                            LoggerAccessor.LogError(
                                $"[MAS] - ClientObject could not be granted for {clientChannel.RemoteAddress}: {clientConnectTcp}"
                            );
                            break;
                        }
                    ...
                    {
                        LoggerAccessor.LogInfo(
                            $"[MAS] - Client Connected {clientChannel.RemoteAddress} with new ClientObject!"
                        );
                    ...
                    if (HorizonServerConfiguration.MEDIUSHttpsSVOCheckPatcher)
                    {
                ...
                case RT_MSG_SERVER_CHEAT_QUERY clientCheatQuery:
                {
                    ...
                    if (QueryData != null)
                    {
                        LoggerAccessor.LogDebug(
                            $"[MAS] - QUERY CHECK - Client:{data.ClientObject?.IP} Has Data:{QueryData.BytesToHexStr()} in offset: {clientCheatQuery.StartAddress}"
                        );

                        if (
                            HorizonServerConfiguration.MEDIUSHttpsSVOCheckPatcher
                ...
                case RT_MSG_CLIENT_DISCONNECT _:
                {
                    ...
                    LoggerAccessor.LogInfo(
                        $"[MAS] - Client disconnected by request with no specific reason"
                    );
                    break;
                }
                case RT_MSG_CLIENT_DISCONNECT_WITH_REASON clientDisconnectWithReason:
                {
                    if (
                        ...
                        LoggerAccessor.LogInfo(
                            $"[MAS] - Disconnected by request with reason of {clientDisconnectWithReason.Reason}"
                        );
                    else
                        LoggerAccessor.LogInfo(
                            $"[MAS] - Disconnected by request with (application specified) reason of {clientDisconnectWithReason.Reason}"
                        );
            ...
            switch (message)
            {
                    ...
                    if (data.ClientObject != null)
                    {
                        ...
                        if (!data.ClientObject.IsConnected)
                        {
                            data.ClientObject.Queue(
                                new MediusServerSessionBeginResponse()
                                {
                        ...
                        {
                            var host = InternetProtocolUtils.GetFirstActiveIPAddress(
                                HorizonServerConfiguration.MEDIUSNATIp
                                    ?? "natservice.pdonline.scea.com",
                                "0.0.0.0"
                            );

                            await InternetProtocolUtils
                                .TryGetServerIP(out var IPAddress)
                                .ConfigureAwait(false);
                            ...
                            data.ClientObject.ServerVersion =
                                serverSessionBeginRequest.ServerVersion;
                            data.ClientObject.Port = serverSessionBeginRequest.Port;
                            data.ClientObject.SetIp(
                                ((IPEndPoint)clientChannel.RemoteAddress)
                                    .Address.ToString()
                                    .Trim(trimChars)
                            );
                                ...
                                data.ClientObject.Queue(
                                    new MediusServerSessionBeginResponse()
                                    {
                                        ...
                                        ConnectInfo = new NetConnectionInfo()
                                        {
                                            ...
                                            AddressList = new NetAddressList()
                                            {
                                                ...
                                                ]
                                                {
                                                    new()
                                                    {
                                                        Address = host,
                                                        Port =
                                                            HorizonServerConfiguration.MEDIUSNATPort,
                                                        AddressType =
                                                            NetAddressType.NetAddressTypeNATService,
                                                    },
                                                    new()
                                                    {
                                                        Address = IPAddress.ToString(),
                                                        Port =
                                                            HorizonServerConfiguration.MEDIUSMPSPort,
                                                        AddressType =
                                            ...
                                            Type =
                                                NetConnectionType.NetConnectionTypeClientServerUDP,
                                        },
                                    }
                                );
                            }
                ...
                case MediusServerSessionBeginRequest1 serverSessionBeginRequest1:
                {
                    if (data.ClientObject != null)
                    {
                        var host = Dns.GetHostEntry(
                            HorizonServerConfiguration.MEDIUSNATIp ?? "natservice.pdonline.scea.com"
                        );
                        ...
                        LoggerAccessor.LogInfo(
                            $"[MAS] - Registered MGCL client for appid {data.ClientObject.ApplicationId} with access token {data.ClientObject.AccessToken}"
                        );
                                ...
                                ConnectInfo = new NetConnectionInfo()
                                {
                                    ...
                                    AddressList = new NetAddressList()
                                    {
                                        ...
                                        ]
                                        {
                                            new()
                                            {
                                                Address = host.AddressList.First().ToString(),
                                                Port = HorizonServerConfiguration.MEDIUSNATPort,
                                                AddressType =
                                                    NetAddressType.NetAddressTypeNATService,
                                            },
                                            new()
                                            {
                                                Address = IPAddress.ToString(),
                                                Port = HorizonServerConfiguration.MEDIUSMPSPort,
                ...
                case MediusServerAuthenticationRequest mgclAuthRequest:
                {
                    ...
                    if (data.ClientObject != null)
                    {
                        ...
                        data.ClientObject.NetConnectionType =
                            NetConnectionType.NetConnectionTypeClientServerTCP;

                        if (
                            mgclAuthRequest.AddressList.AddressList[0].AddressType
                            == NetAddressType.NetAddressTypeBinaryExternal
                        )
                            data.ClientObject.SetIp(
                                InternetProtocolUtils
                                    .GetIPAddressFromUInt(
                                        mgclAuthRequest.AddressList.AddressList[0].BinaryAddress
                                    )
                                    .ToString()
                            );
                        else if (
                        ...
                        {
                            data.ClientObject.SetIp(
                                mgclAuthRequest.AddressList.AddressList[0].IPBinaryBitOne
                                    + "."
                            ...
                            data.ClientObject.SetIp(
                                mgclAuthRequest.AddressList.AddressList[0].Address ?? "0.0.0.0"
                            );

                        if (nonSecure.Contains(data.ClientObject.ApplicationId))
                        {
                            data.ClientObject.Queue(
                                new MediusServerAuthenticationResponse()
                                {
                ...
                {
                    var dmeObject = data.ClientObject;
                    if (dmeObject == null)
                    {
                    ...
                    // Reply with success
                    dmeObject.Queue(
                        new MediusServerSetAttributesResponse()
                        {
                ...
                case MediusServerSessionEndRequest sessionEndRequest:
                {
                    ...
                    {
                        LoggerAccessor.LogError(
                            $"[MAS] - INVALID OPERATION: {clientChannel} is trying to end server session without an Client Object"
                        );
                        break;
                    }
                ...
                case MediusExtendedSessionBeginRequest extendedSessionBeginRequest:
                {
                    if (data.ClientObject != null)
                    {
                        data.ClientObject.MediusConnectionType =
                            extendedSessionBeginRequest.ConnectionClass;
                                ...
                                (r) =>
                                {
                                    ...
                                    )
                                    {
                                        ...
                                        else
                                        {
                                            // Reply
                                            data.ClientObject.Queue(
                                                new MediusSessionBeginResponse()
                                                {
                                                    MessageID =
                                                        extendedSessionBeginRequest.MessageID,
                ...
                case MediusSessionBeginRequest sessionBeginRequest:
                {
                    ...
                    {
                        data.ClientObject.MediusConnectionType =
                            sessionBeginRequest.ConnectionClass;

                        LoggerAccessor.LogInfo(
                            $"[MAS] - Retrieved ApplicationID {data.ClientObject.ApplicationId} from client connection"
                        );
                ...
                case MediusSessionBeginRequest1 sessionBeginRequest1:
                {
                    ...
                    {
                        data.ClientObject.MediusConnectionType =
                            sessionBeginRequest1.ConnectionClass;
                        ...
                        if (HorizonServerConfiguration.MEDIUSSystemMessageSingleTest)
                        {
                            await QueueBanMessage(
                                    data,
                                    "MAS.Notification Test:\nYou have been banned from this server."
                                )
                                .ConfigureAwait(false);
                ...
                case MediusSessionEndRequest sessionEndRequest:
                {
                    ...
                    {
                        LoggerAccessor.LogError(
                            $"[MAS] - INVALID OPERATION: {clientChannel} is trying to end session of a non-existing Client Object with SessionKey: {sessionEndRequest.SessionKey}"
                        );
                        break;
                    }
                ...
                case MediusSetLocalizationParamsRequest setLocalizationParamsRequest:
                {
                    ...
                    {
                        LoggerAccessor.LogError(
                            $"[MAS] - INVALID OPERATION: {clientChannel} sent {setLocalizationParamsRequest} without a session."
                        );
                        break;
                    }

                    data.ClientObject.CharacterEncoding =
                        setLocalizationParamsRequest.CharacterEncoding;
                ...
                case MediusSetLocalizationParamsRequest1 setLocalizationParamsRequest1:
                {
                    ...
                    {
                        LoggerAccessor.LogError(
                            $"[MAS] - INVALID OPERATION: {clientChannel} sent {setLocalizationParamsRequest1} without a session."
                        );
                        break;
                    }

                    data.ClientObject.CharacterEncoding =
                        setLocalizationParamsRequest1.CharacterEncoding;
                ...
                case MediusSetLocalizationParamsRequest2 setLocalizationParamsRequest2:
                {
                    ...
                    {
                        LoggerAccessor.LogError(
                            $"[MAS] - INVALID OPERATION: {clientChannel} sent {setLocalizationParamsRequest2} without a session."
                        );
                        break;
                    }

                    data.ClientObject.CharacterEncoding =
                        setLocalizationParamsRequest2.CharacterEncoding;
                ...
                case MediusGetTotalGamesRequest getTotalGamesRequest:
                {
                    ...
                    {
                        LoggerAccessor.LogError(
                            $"[MAS] - INVALID OPERATION: {clientChannel} sent {getTotalGamesRequest} without a session."
                        );
                        break;
                    }
                    ...
                    {
                        LoggerAccessor.LogError(
                            $"[MAS] - INVALID OPERATION: {clientChannel} sent {getTotalGamesRequest} without being logged in."
                        );
                        break;
                    }
                ...
                case MediusGetTotalChannelsRequest getTotalChannelsRequest:
                {
                    ...
                    {
                        LoggerAccessor.LogError(
                            $"[MAS] - INVALID OPERATION: {clientChannel} sent {getTotalChannelsRequest} without a session."
                        );
                        break;
                    }
                    ...
                    {
                        LoggerAccessor.LogError(
                            $"[MAS] - INVALID OPERATION: {clientChannel} sent {getTotalChannelsRequest} without being logged in."
                        );
                        break;
                    }
                ...
                case MediusMachineSignaturePost machineSignaturePost:
                {
                    if (HorizonServerConfiguration.MEDIUSDnasEnablePost)
                    {
                        ...
                        data.MachineId = BitConverter.ToString(
                            machineSignaturePost.MachineSignature
                        );

                        LoggerAccessor.LogDebug(
                            $"[MAS] - Session Key {machineSignaturePost.SessionKey} | Posting Machine signatures"
                        );
                ...
                case MediusDnasSignaturePost dnasSignaturePost:
                {
                    if (HorizonServerConfiguration.MEDIUSDnasEnablePost)
                    {
                        ...
                        if (dnasSignaturePost.DnasSignatureType == MediusDnasCategory.DnasConsoleID)
                        {
                            ...
                            LoggerAccessor.LogDebug(
                                $"[MAS] - Posting ConsoleID - ConsoleSigSize={dnasSignaturePost.DnasSignatureLength}"
                            );
                            ...
                            dnasSignaturePost.DnasSignatureType == MediusDnasCategory.DnasTitleID
                        )
                            LoggerAccessor.LogError(
                                $"[MAS] - DnasSignaturePost Error - Invalid SignatureType"
                            );
                        else if (
                            dnasSignaturePost.DnasSignatureType == MediusDnasCategory.DnasDiskID
                        )
                            LoggerAccessor.LogDebug(
                                $"[MAS] - Posting DiskID - DiskSigSize={dnasSignaturePost.DnasSignatureLength}"
                            );
                    }
                ...
                case MediusGetAccessLevelInfoRequest getAccessLevelInfoRequest:
                {
                    ...
                    {
                        LoggerAccessor.LogError(
                            $"[MAS] - INVALID OPERATION: {clientChannel} sent {getAccessLevelInfoRequest} without a session."
                        );
                        break;
                    }
                    ...
                    {
                        LoggerAccessor.LogError(
                            $"[MAS] - INVALID OPERATION: {clientChannel} sent {getAccessLevelInfoRequest} without being logged in."
                        );
                        break;
                    }
                ...
                case MediusVersionServerRequest mediusVersionServerRequest:
                {
                    ...
                    if (
                        ...
                        && !appIdBeforeSession.Contains(data.ApplicationId)
                    ) // KILLZONE PS2 GET VERSION SERVER INFO BEFORE SESSION
                    {
                        LoggerAccessor.LogError(
                            $"[MAS] - INVALID OPERATION: {clientChannel} sent {mediusVersionServerRequest} without a session."
                        );
                        break;
                    }

                    #region F1 2005 PAL
                    var F12005AppIds = new List<int> { 10952, 10954 };
                        ...
                        data.SendQueue.Enqueue(
                            new RT_MSG_SERVER_APP()
                            {
                                Message = new MediusVersionServerResponse()
                                {
                                    MessageID = mediusVersionServerRequest.MessageID,
                                    VersionServer =
                                        "Medius Authentication Server Version 1.50.0009",
                                    StatusCode = MediusCallbackStatus.MediusSuccess,
                                },
                            }
                        );
                    ...
                    //Default
                    {
                        data.ClientObject?.Queue(
                            ...
                            {
                                MessageID = mediusVersionServerRequest.MessageID,
                                VersionServer = HorizonServerConfiguration.MEDIUSMASVersion,
                                StatusCode = MediusCallbackStatus.MediusSuccess,
                            }
                        );
                    }
                ...
                case MediusGetLocationsRequest getLocationsRequest:
                {
                    ...
                    {
                        LoggerAccessor.LogError(
                            $"[MAS] - INVALID OPERATION: {clientChannel} sent {getLocationsRequest} without a session."
                        );
                        break;
                    }
                    ...
                    await HorizonServerConfiguration
                        ...
                        .ContinueWith(r =>
                        {
                            ...
                            {
                                LoggerAccessor.LogError(
                                    $"[MAS] - GetLocationsRequest failed [{r.Exception}]"
                                );
                ...
                case MediusPickLocationRequest pickLocationRequest:
                {
                    ...
                    {
                        LoggerAccessor.LogError(
                            $"[MAS] - INVALID OPERATION: {clientChannel} sent {pickLocationRequest} without a session."
                        );
                        break;
                    }
                    ...
                    data.ClientObject.Queue(
                        new MediusPickLocationResponse()
                        {
                ...
                case MediusAccountRegistrationRequest accountRegRequest:
                {
                    ...
                    {
                        LoggerAccessor.LogError(
                            $"[MAS] - INVALID OPERATION: {clientChannel} sent {accountRegRequest} without a session."
                        );
                        break;
                    }
                    ...
                    if (appSettings.DisableAccountCreation)
                    {
                        // Reply error
                        data.ClientObject.Queue(
                            new MediusAccountRegistrationResponse()
                            {
                    ...
                    if (
                        !MediusManager.PassTextFilter(
                            data.ApplicationId,
                            TextFilterContext.ACCOUNT_NAME,
                            accountRegRequest.AccountName
                        )
                    )
                    {
                        data.ClientObject.Queue(
                    ...
                    await HorizonServerConfiguration
                        .Database.CreateAccount(
                            new CreateAccountDTO()
                            {
                                ...
                                MediusStats = Base64.ToBase64String(
                                    new byte[Constants.ACCOUNTSTATS_MAXLEN]
                                ),
                                ...
                                else
                                {
                                    ...
                                    data.ClientObject.Queue(
                                        new MediusAccountRegistrationResponse()
                                        {
                                            MessageID = accountRegRequest.MessageID,
                                            StatusCode =
                                                MediusCallbackStatus.MediusAccountAlreadyExists,
                                        }
                                    );
                                }
                            }
                ...
                case MediusAccountGetIDRequest accountGetIdRequest:
                {
                    ...
                    {
                        LoggerAccessor.LogError(
                            $"[MAS] - INVALID OPERATION: {clientChannel} sent {accountGetIdRequest} without a session."
                        );
                        break;
                    }
                ...
                case MediusAccountDeleteRequest accountDeleteRequest:
                {
                    ...
                    {
                        LoggerAccessor.LogError(
                            $"[MAS] - INVALID OPERATION: {clientChannel} sent {accountDeleteRequest} without a session."
                        );
                        break;
                    }
                    ...
                    {
                        LoggerAccessor.LogError(
                            $"[MAS] - INVALID OPERATION: {clientChannel} sent {accountDeleteRequest} without being logged in."
                        );
                        break;
                    }
                        ...
                        await HorizonServerConfiguration
                            ...
                            .ContinueWith(
                                (r) =>
                                {
                                    ...
                                    {
                                        LoggerAccessor.LogInfo(
                                            $"[MAS] - Logging out {data?.ClientObject?.AccountName}'s account - Deleting from Medius Server"
                                        );

                                        data?.ClientObject?.Logout();

                                        data?.ClientObject?.Queue(
                                            new MediusAccountDeleteResponse()
                                            {
                                    ...
                                    else
                                    {
                                        LoggerAccessor.LogWarn(
                                            $"[MAS] - Logout FAILED for {data?.ClientObject?.AccountName}'s account - Data still persistent on Medius Server"
                                        );
                ...
                case MediusAnonymousLoginRequest anonymousLoginRequest:
                {
                    ...
                    {
                        LoggerAccessor.LogError(
                            $"[MAS] - INVALID OPERATION: {clientChannel} sent {anonymousLoginRequest} without a session."
                        );
                        break;
                    }

                    // get client ip
                    var clientIp = MyRegex()
                        .Match(clientChannel.RemoteAddress.ToString())
                        .Groups["ip"]
                        .Value;
                    ...
                    if (
                        await HorizonServerConfiguration
                            .Database.GetIsIpBanned(IPAddress.Parse(clientIp))
                            .ConfigureAwait(false)
                        || (
                    ...
                    {
                        LoggerAccessor.LogWarn(
                            $"[MAS] - MediusAnonymousLoginRequest: Anonymous User tried to login with banned IP {clientIp}"
                        );

                        data?.ClientObject?.Queue(
                            new MediusAnonymousLoginResponse()
                            {
                    ...
                    // cid is banned
                        ...
                        await HorizonServerConfiguration
                            .Database.GetIsMacBanned(data.MachineId ?? string.Empty)
                            .ConfigureAwait(false)
                    )
                    {
                        LoggerAccessor.LogWarn(
                            $"[MAS] - MediusAnonymousLoginRequest: Anonymous User tried to login with banned CID {data.MachineId}"
                        );
                ...
                case MediusAccountLoginRequest accountLoginRequest:
                {
                    ...
                    {
                        LoggerAccessor.LogError(
                            $"[MAS] - INVALID OPERATION: {clientChannel} sent {accountLoginRequest} without a session."
                        );
                        break;
                    }
                    ...
                    await Program
                        .MediusManager.Plugins.OnEvent(
                            PluginEvent.MEDIUS_ACCOUNT_LOGIN_REQUEST,
                    ...
                    if (
                        Program
                            .MUMManager.GetClientByAccountName(
                    ...
                    {
                        data.ClientObject.Queue(
                            new MediusAccountLoginResponse()
                            {
                    ...
                    else
                    {
                        ...
                        {
                            LoggerAccessor.LogWarn(
                                $"[MAS] - MediusAccountLoginRequest: User {accountLoginRequest.Username} tried to login with banned IP {clientIp}"
                            );
                        ...
                        {
                            LoggerAccessor.LogWarn(
                                $"[MAS] - MediusAccountLoginRequest: User {accountLoginRequest.Username} tried to login with banned CID {data.MachineId}"
                            );
                                ...
                                .ContinueWith(
                                    async (r) =>
                                    {
                                            ...
                                            if (r.Result.IsBanned)
                                            {
                                                ...
                                                data?.ClientObject?.Queue(
                                                    new MediusAccountLoginResponse()
                                                    {
                                                        MessageID = accountLoginRequest.MessageID,
                                                        StatusCode =
                                                            MediusCallbackStatus.MediusAccountBanned,
                                                    }
                                                );
                                            }
                                            else if (
                                                appSettings.EnableAccountWhitelist
                                                && !appSettings.AccountIdWhitelist.Contains(
                                                    r.Result.AccountId
                                                )
                                            )
                                            {
                                            ...
                                            )
                                            {
                                                data.ClientObject.Queue(
                                                    new MediusAccountLoginResponse()
                                                    {
                                                        MessageID = accountLoginRequest.MessageID,
                                                        StatusCode =
                                                            MediusCallbackStatus.MediusAccountLoggedIn,
                                                    }
                                                );
                                            }
                                            else if (
                                                ComputeSHA256(accountLoginRequest.Password)
                                                == r.Result.AccountPassword
                                            )
                                                await Login(
                                                        accountLoginRequest.MessageID,
                                                        clientChannel,
                                                        data,
                                                        r.Result,
                                                        false
                                            ...
                                            else
                                            {
                                                ...
                                                data?.ClientObject?.Queue(
                                                    new MediusAccountLoginResponse()
                                                    {
                                                        MessageID = accountLoginRequest.MessageID,
                                                        StatusCode =
                                                            MediusCallbackStatus.MediusInvalidPassword,
                                                    }
                                                );
                                            }
                                        }
                                        else if (appSettings.CreateAccountOnNotFound)
                                        {
                                            ...
                                            )
                                            {
                                                data.ClientObject?.Queue(
                                                    new MediusAccountLoginResponse()
                                                    {
                                                        MessageID = accountLoginRequest.MessageID,
                                                        StatusCode =
                                                            MediusCallbackStatus.MediusVulgarityFound,
                                                    }
                                                );
                                                return;
                                            }
                                            ...
                                            await Program
                                                .MediusManager.Plugins.OnEvent(
                                                    PluginEvent.MEDIUS_PRE_ACCOUNT_CREATE_ON_NOT_FOUND,
                                                    new OnAccountLoginRequestArgs()
                                                    {
                                                        Player = data?.ClientObject,
                                                        Request = accountLoginRequest,
                                                    ...
                                                    .ContinueWith(
                                                        async (r) =>
                                                        {
                                                            ...
                                                            )
                                                            {
                                                                await Program
                                                                    .MediusManager.Plugins.OnEvent(
                                                                        PluginEvent.MEDIUS_POST_ACCOUNT_CREATE_ON_NOT_FOUND,
                                                                        new OnAccountLoginRequestArgs()
                                                                        {
                                                                            Player =
                                                                                data.ClientObject,
                                        ...
                                        else
                                        {
                                            ...
                                            data?.ClientObject?.Queue(
                                                new MediusAccountLoginResponse()
                                                {
                                                    MessageID = accountLoginRequest.MessageID,
                                                    StatusCode =
                                                        MediusCallbackStatus.MediusAccountNotFound,
                                                }
                                            );
                                        }
                                    }
                ...
                case MediusAccountUpdatePasswordRequest accountUpdatePasswordRequest:
                {
                    ...
                    {
                        LoggerAccessor.LogError(
                            $"[MAS] - INVALID OPERATION: {clientChannel} sent {accountUpdatePasswordRequest} without a session."
                        );
                        break;
                    }
                    ...
                    {
                        LoggerAccessor.LogError(
                            $"[MAS] - INVALID OPERATION: {clientChannel} sent {accountUpdatePasswordRequest} without being logged in."
                        );
                        break;
                    }
                ...
                case MediusAccountLogoutRequest accountLogoutRequest:
                {
                    ...
                    {
                        LoggerAccessor.LogError(
                            $"[MAS] - INVALID OPERATION: {clientChannel} sent {accountLogoutRequest} without a session."
                        );
                        break;
                    }

                    var result = MediusCallbackStatus.MediusFail;

                    // Check token
                    if (
                        data.ClientObject.IsLoggedIn
                        && accountLogoutRequest.SessionKey == data.ClientObject.SessionKey
                    )
                    {
                ...
                case MediusAccountUpdateStatsRequest accountUpdateStatsRequest:
                {
                    ...
                    {
                        LoggerAccessor.LogError(
                            $"[MAS] - INVALID OPERATION: {clientChannel} sent {accountUpdateStatsRequest} without a session."
                        );
                        break;
                    }
                    ...
                    {
                        LoggerAccessor.LogError(
                            $"[MAS] - INVALID OPERATION: {clientChannel} sent {accountUpdateStatsRequest} without being logged in."
                        );
                        break;
                    }
                ...
                case MediusTicketLoginRequest ticketLoginRequest:
                {
                    ...
                    {
                        LoggerAccessor.LogError(
                            $"[MAS] - INVALID OPERATION: {clientChannel} sent {ticketLoginRequest} without a session or XI5 Ticket."
                        );
                        break;
                    }
                    ...
                    var ticket = XI5Ticket.ReadFromBytes(
                        ByteUtils.CombineByteArrays(
                            BitConverter.GetBytes(
                                EndianTools.EndianAwareConverter.isLittleEndianSystem
                                    ? EndianUtils.ReverseUint(ticketLoginRequest.Version)
                                    : ticketLoginRequest.Version
                            ),
                            BitConverter.GetBytes(
                                EndianTools.EndianAwareConverter.isLittleEndianSystem
                                    ? EndianUtils.ReverseUint(ticketLoginRequest.Size)
                                    : ticketLoginRequest.Size
                            ),
                            ticketLoginRequest.TicketData
                        )
                    );
                    ...
                    if (!ticket.Valid)
                    {
                        // log to console
                        LoggerAccessor.LogWarn(
                            $"[MAS] - MediusTicketLoginRequest: User {username} tried to alter their ticket data"
                        );

                        data.ClientObject.Queue(
                            new MediusTicketLoginResponse()
                            {
                    ...
                    {
                        LoggerAccessor.LogInfo(
                            $"[MAS] - MediusTicketLoginRequest : User {username} connected at: {DateTime.Now} and is on RPCN"
                        );
                    ...
                    {
                        LoggerAccessor.LogError(
                            $"[MAS] - MediusTicketLoginRequest : User {username} was caught using a RPCN suffix while not on it!"
                        );
                        ...
                        LoggerAccessor.LogInfo(
                            $"[MAS] - MediusTicketLoginRequest : User {username} connected at: {DateTime.Now} and is on PSN"
                        );

                    // get existing account
                    var existingClient = Program.MUMManager.GetClientByAccountName(
                        username,
                        data.ClientObject.ApplicationId
                    );
                    ...
                    {
                        LoggerAccessor.LogWarn(
                            $"[MAS] - MediusTicketLoginRequest: User {username} tried to login with banned IP {clientIp}"
                        );
                    ...
                    {
                        LoggerAccessor.LogWarn(
                            $"[MAS] - MediusTicketLoginRequest: User {username} tried to login with banned CID {data.MachineId}"
                        );
                    ...
                    // get account
                    var account = await HorizonServerConfiguration
                        .Database.GetAccountByName(username, data.ClientObject.ApplicationId, true)
                        .ConfigureAwait(false);
                    ...
                    if (account == null)
                    {
                        ...
                        {
                            LoggerAccessor.LogError(
                                $"[MAS] - MediusTicketLoginRequest: AppId {data?.ClientObject?.ApplicationId} has account creation disabled"
                            );
                        ...
                        // create new account
                        account = await HorizonServerConfiguration
                            .Database.CreateAccount(
                                new CreateAccountDTO()
                                {
                    ...
                    {
                        LoggerAccessor.LogError(
                            $"[MAS] - MediusTicketLoginRequest: {username} tried to login without being whitelisted"
                        );
                    ...
                    {
                        LoggerAccessor.LogWarn(
                            $"[MAS] - MediusTicketLoginRequest: User {username} tried to login with banned account"
                        );
                    ...
                    {
                        LoggerAccessor.LogWarn(
                            $"[MAS] - MediusTicketLoginRequest: User {username} tried to login while using an anti Poke/Query eboot"
                        );
                                    ...
                                    .ContinueWith(
                                        (r) =>
                                        {
                                            ...
                                            if (r.IsCompletedSuccessfully && r.Result != null)
                                            {
                                                ...
                                                txt = !string.IsNullOrEmpty(r.Result.EulaTitle)
                                                    ? r.Result.EulaTitle + "\n" + txt
                                                    : string.Empty;
                                                LoggerAccessor.LogDebug(
                                                    $"[MAS] - GetPolicy Succeeded:{getPolicyRequest.MessageID}"
                                                );
                                                data.ClientObject.Queue(
                                            ...
                                            {
                                                LoggerAccessor.LogError(
                                                    $"[MAS] - GetPolicy Failed = [{r.Exception}]"
                                                );
                                                data.ClientObject.Queue(
                                                    new MediusGetPolicyResponse()
                                                    {
                ...
                case MediusGetLadderStatsRequest getLadderStatsRequest:
                {
                    ...
                    {
                        LoggerAccessor.LogError(
                            $"[MAS] - INVALID OPERATION: {clientChannel} sent {getLadderStatsRequest} without a session."
                        );
                        break;
                    }
                    ...
                    {
                        LoggerAccessor.LogError(
                            $"[MAS] - INVALID OPERATION: {clientChannel} sent {getLadderStatsRequest} without being logged in."
                        );
                        break;
                    }
                    ...
                    switch (getLadderStatsRequest.LadderType)
                    {
                        ...
                        {
                            LoggerAccessor.LogWarn(
                                $"[MAS] - Unhandled MediusGetLadderStatsRequest {getLadderStatsRequest}"
                            );
                            break;
                        }
                    }
                ...
                case MediusGetLadderStatsWideRequest getLadderStatsWideRequest:
                {
                    ...
                    {
                        LoggerAccessor.LogError(
                            $"[MAS] - INVALID OPERATION: {clientChannel} sent {getLadderStatsWideRequest} without a session."
                        );
                        break;
                    }
                    ...
                    {
                        LoggerAccessor.LogError(
                            $"[MAS] - INVALID OPERATION: {clientChannel} sent {getLadderStatsWideRequest} without being logged in."
                        );
                        break;
                    }
                    ...
                    switch (getLadderStatsWideRequest.LadderType)
                    {
                                ...
                                .ContinueWith(
                                    (r) =>
                                    {
                                        ...
                                        if (
                                            r.IsCompletedSuccessfully
                                            && r.Result != null
                                            && r.Result.AccountWideStats != null
                                        )
                                        {
                                            data.ClientObject.Queue(
                        ...
                        {
                            LoggerAccessor.LogWarn(
                                $"[MAS] - Unhandled MediusGetLadderStatsWideRequest {getLadderStatsWideRequest}"
                            );
                            break;
                        }
                    }
                ...
                case MediusChannelListRequest channelListRequest:
                {
                    ...
                    {
                        LoggerAccessor.LogError(
                            $"[MAS] - INVALID OPERATION: {clientChannel} sent {channelListRequest} without a session."
                        );
                        break;
                    }
                    ...
                    {
                        LoggerAccessor.LogError(
                            $"[MAS] - INVALID OPERATION: {clientChannel} sent {channelListRequest} without being logged in."
                        );
                        break;
                    }

                    var channelResponses = new List<MediusChannelListResponse>();

                    var lobbyChannels = Program.MUMManager.GetChannelList(
                        data.ClientObject.ApplicationId,
                ...
                case MediusTextFilterRequest textFilterRequest:
                {
                    ...
                    {
                        LoggerAccessor.LogError(
                            $"[MAS] - INVALID OPERATION: {clientChannel} sent {textFilterRequest} without a session."
                        );
                        break;
                    }
                ...
                case MediusGetServerTimeRequest getServerTimeRequest:
                {
                    ...
                    {
                        LoggerAccessor.LogError(
                            $"[MAS] - INVALID OPERATION: {clientChannel} sent {getServerTimeRequest} without a session."
                        );
                        break;
                    }
                    ...
                    await GetTimeZone(DateTime.Now)
                        .ContinueWith(
                            (r) =>
                            {
                                if (r.IsCompletedSuccessfully)
                ...
                case MediusGetMyIPRequest getMyIpRequest:
                {
                    ...
                    {
                        LoggerAccessor.LogError(
                            $"[MAS] - INVALID OPERATION: {clientChannel} sent {getMyIpRequest} without a session."
                        );
                        break;
                    }
                    ...
                    {
                        LoggerAccessor.LogInfo(
                            $"[MAS] - Error: Retrieving Client IP address {clientChannel.RemoteAddress}."
                        );
                ...
                case MediusUpdateUserState updateUserState:
                {
                    ...
                    {
                        LoggerAccessor.LogError(
                            $"[MAS] - INVALID OPERATION: {clientChannel} sent {updateUserState} without a session."
                        );
                        break;
                    }
                    ...
                    switch (updateUserState.UserAction)
                    {
                        ...
                        {
                            LoggerAccessor.LogInfo(
                                $"[MAS] - Successfully Joined ChatWorld [{data.ClientObject.CurrentChannel?.Id}] {data.ClientObject.AccountId}:{data.ClientObject.AccountName}"
                            );
                            break;
                        }
                        ...
                        {
                            LoggerAccessor.LogInfo(
                                $"[MAS] - Successfully Left GameWorld {data.ClientObject.AccountId}:{data.ClientObject.AccountName}"
                            );
                            break;
                        }
                        ...
                        {
                            LoggerAccessor.LogInfo(
                                $"[MAS] - Successfully Left PartyWorld {data.ClientObject.AccountId}:{data.ClientObject.AccountName}"
                            );
                            break;
                        }
                        default:
                        {
                            LoggerAccessor.LogWarn(
                                $"[MAS] - Requested a non-existant UserState {data.ClientObject.AccountId}:{data.ClientObject.AccountName}, please report to GITHUB."
                            );
                            break;
                        }
                    }
        ...
        )
        {
            ...
            await InternetProtocolUtils.TryGetServerIP(out var IPAddress).ConfigureAwait(false);

            List<int> pre108Secure = new()
            {
                10010,
                10031,
                10190,
            ...
            if (data.ClientObject!.IP == System.Net.IPAddress.Any)
                data.ClientObject!.SetIp(
                    ((IPEndPoint)clientChannel.RemoteAddress)
                        .Address.ToString()
                        .Trim(new char[] { ':', 'f', '{', '}' })
            ...
            )
            {
                if (
                    ...
                    && data.ClientObject.ClientHomeData.VersionAsDouble >= 01.82
                )
                {
                    _ =
                        !string.IsNullOrEmpty(data.ClientObject.ClientHomeData.Type)
                        && (
                            data.ClientObject.ClientHomeData.Type.Contains("HDK")
                            || data.ClientObject.ClientHomeData.Type.Contains("Online Debug")
                        )
                            ? Task.Delay(4000)
                                .ContinueWith(r =>
                ...
                switch (data.ClientObject.ClientHomeData.Type)
                {
                    case "HDK With Offline":
                        ...
                        {
                            case "01.86.09":
                                if (HorizonServerConfiguration.MEDIUSPokePatchOn)
                                {
            ...
            await data.ClientObject.Login(accountDto, false).ConfigureAwait(false);

            #region Update DB IP and CID
            await HorizonServerConfiguration
                .Database.PostAccountIp(
                    accountDto.AccountId,
                    ((IPEndPoint)clientChannel.RemoteAddress).Address.MapToIPv4().ToString()
                )
                .ConfigureAwait(false);
            ...
            Program.MUMManager.AddOrUpdateLoggedInClient(data.ClientObject);

            LoggerAccessor.LogInfo(
                $"[MAS] - LOGGING IN AS {data.ClientObject.AccountName} with access token {data.ClientObject.AccessToken}"
            );
                                ...
                                AddressList = new NetAddress[Constants.NET_ADDRESS_LIST_COUNT]
                                {
                                    new()
                                    {
                                        Address = !string.IsNullOrEmpty(
                                            HorizonServerConfiguration.MEDIUSNpMLSIpOverride
                                        )
                                            ? HorizonServerConfiguration.MEDIUSNpMLSIpOverride
                                            : IPAddress.ToString(),
                                        Port =
                                            (
                                                HorizonServerConfiguration.MEDIUSNpMLSPortOverride
                                                != -1
                                            )
                                                ? HorizonServerConfiguration.MEDIUSNpMLSPortOverride
                                                : HorizonServerConfiguration.MEDIUSMLSPort,
                                        AddressType = NetAddressType.NetAddressTypeExternal,
                                    },
                                    new()
                                    {
                            ...
                            ConnectInfo = new NetConnectionInfo()
                            {
                                ...
                                ServerKey = new RSA_KEY(
                                    LIBRARY
                                        ...
                                        .Reverse()
                                        .ToArray()
                                ), // Some Older Medius games don't set a RSA Key
                                AddressList = new NetAddressList()
                                {
                                    AddressList = new NetAddress[Constants.NET_ADDRESS_LIST_COUNT]
                                    {
        ...
        private static async Task LoginAnonymous(
        ...
        )
        {
            ...
            var iAccountID = MUM.MumManager.AnonymousAccountIDGenerator(
                HorizonServerConfiguration.MEDIUSAnonymousIDRangeSeed
            );
#if DEBUG
            LoggerAccessor.LogInfo(
                $"[MAS] - AnonymousIDRangeSeedGenerator AccountID returned {iAccountID}"
            );
#endif
            if (data.ClientObject != null)
            {
            ...
            if (data.ClientObject != null)
            {
                ...
                // Login
                await data
                    .ClientObject.LoginAnonymous(anonymousLoginRequest, iAccountID)
                    .ConfigureAwait(false);
                ...
                LoggerAccessor.LogInfo(
                    $"[MAS] - LOGGING IN ANONYMOUSLY AS {data.ClientObject.AccountDisplayName} with access token {data.ClientObject.AccessToken}"
                );
                ...
                if (File.Exists(Directory.GetCurrentDirectory() + $"/static/poke_config.json"))
                {
                    try
                    {
                        var jsonObject = JObject.Parse(
                            File.ReadAllText(
                                Directory.GetCurrentDirectory() + $"/static/poke_config.json"
                            )
                        );
                        ...
                        foreach (var appProperty in jsonObject.Properties())
                        {
                            ...
                            )
                            {
                                if (appProperty.Value is JObject innerObject)
                                {
                                    ...
                                    {
                                        var offset = offsetProperty.Name;
                                        var valuestr = offsetProperty.Value.ToString();

                                        if (
                                            !string.IsNullOrEmpty(offset)
                                            && !string.IsNullOrEmpty(valuestr)
                                            && uint.TryParse(
                                                offset.Replace("0x", string.Empty),
                                        ...
                                        {
                                            LoggerAccessor.LogInfo(
                                                $"[MAS] - MemoryPoke sent to appid {appId} with infos : offset:{offset} - value:{valuestr}"
                                            );
                                            Queue(
                                                new RT_MSG_SERVER_MEMORY_POKE()
                                                {
                                                    start_Address = offsetValue,
                                                    Payload = BitConverter.GetBytes(
                                                        !EndianTools
                                                            .EndianAwareConverter
                                                            .isLittleEndianSystem
                                                            ? EndianUtils.ReverseUint(hexValue)
                                                            : hexValue
                                                    ),
                                                    SkipEncryption = true,
                                                },
                                            ...
                                            LoggerAccessor.LogWarn(
                                                $"[MAS] - MemoryPoke failed to convert json properties! Check your Json syntax."
                                            );
                                    }
                                }
                            }
        ...
        private static string ComputeSHA256(string input)
        {
            return CastleLibrary
                .NetHasher.DotNetHasher.ComputeSHA256String(Encoding.UTF8.GetBytes(input))
                .ToLower();
        }

        [GeneratedRegex(@"\[(?:.*?:)?(?<ip>\d+\.\d+\.\d+\.\d+)\]")]

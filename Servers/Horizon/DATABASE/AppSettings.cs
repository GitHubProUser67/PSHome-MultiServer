...
{
    public class AppSettings(int appId)
    {
        /// <summary>
        ...
        /// <summary>
        /// When true, dme relay will encrypt all messages.
        /// </summary>
        public bool EnableDmeEncryption { get; private set; } = true;
        ...
        public void SetSettings(Dictionary<string, string> settings)
        {
            ...
            if (
                settings.TryGetValue("CreateAccountOnNotFound", out value)
                && bool.TryParse(value, out var createAccountOnNotFound)
            )
                CreateAccountOnNotFound = createAccountOnNotFound;
            // DisableAccountCreation
            if (
                settings.TryGetValue("DisableAccountCreation", out value)
                && bool.TryParse(value, out var disableAccountCreation)
            )
                DisableAccountCreation = disableAccountCreation;
            // EnableAccountWhitelist
            if (
                settings.TryGetValue("EnableAccountWhitelist", out value)
                && bool.TryParse(value, out var enableAccountWhitelist)
            )
                EnableAccountWhitelist = enableAccountWhitelist;
            // AccountIdWhitelist
            if (settings.TryGetValue("AccountIdWhitelist", out value))
                AccountIdWhitelist = [.. value
            ...
            // EnableEncryption
            if (
                settings.TryGetValue("EnableEncryption", out value)
                && bool.TryParse(value, out var enableEncryption)
            )
                EnableEncryption = enableEncryption;
            // EnableDmeEncryption
            if (
                settings.TryGetValue("EnableDmeEncryption", out value)
                && bool.TryParse(value, out var enableDmeEncryption)
            )
                EnableDmeEncryption = enableDmeEncryption;
            // EnableMediusFileServices
            if (
                settings.TryGetValue("EnableMediusFileServices", out value)
                && bool.TryParse(value, out var enableMediusFileServices)
            )
                EnableMediusFileServices = enableMediusFileServices;
            // BanSystemMessageSeverity
            if (
                settings.TryGetValue("BanSystemMessageSeverity", out value)
                && int.TryParse(value, out var banSystemMessageSeverity)
            )
                BanSystemMessageSeverity = banSystemMessageSeverity;
            // ServerEchoIntervalSeconds
            if (
                settings.TryGetValue("ServerEchoIntervalSeconds", out value)
                && int.TryParse(value, out var serverEchoIntervalSeconds)
            )
                ServerEchoIntervalSeconds = serverEchoIntervalSeconds;
            // KeepAliveGracePeriodSeconds
            if (
                settings.TryGetValue("KeepAliveGracePeriodSeconds", out value)
                && int.TryParse(value, out var keepAliveGracePeriodSeconds)
            )
                KeepAliveGracePeriodSeconds = keepAliveGracePeriodSeconds;
            // ClientTimeoutSeconds
            if (
                settings.TryGetValue("ClientTimeoutSeconds", out value)
                && int.TryParse(value, out var clientTimeoutSeconds)
            )
                ClientTimeoutSeconds = clientTimeoutSeconds;
            // GameTimeoutSeconds
            if (
                settings.TryGetValue("GameTimeoutSeconds", out value)
                && int.TryParse(value, out var gameTimeoutSeconds)
            )
                GameTimeoutSeconds = gameTimeoutSeconds;
            // DmeTimeoutSeconds
            if (
                settings.TryGetValue("DmeTimeoutSeconds", out value)
                && int.TryParse(value, out var dmeTimeoutSeconds)
            )
                DmeTimeoutSeconds = dmeTimeoutSeconds;
            // BanEchoCheckCadenceSeconds
            if (
                settings.TryGetValue("BanEchoCheckCadenceSeconds", out value)
                && int.TryParse(value, out var banEchoCheckCadenceSeconds)
            )
                BanEchoCheckCadenceSeconds = banEchoCheckCadenceSeconds;

            // DefaultClientWorldAggTime
            if (
                settings.TryGetValue("DefaultClientWorldAggTime", out value)
                && int.TryParse(value, out var defaultClientWorldAggTime)
            )

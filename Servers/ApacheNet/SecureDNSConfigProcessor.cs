    ...
    public static partial class SecureDNSConfigProcessor
    {
        ...
        public static void InitDNSSubsystem()
        {
            ...
            LoggerAccessor.LogWarn(
                "[HTTPS_DNS] - DNS system configuration is initialising, endpoints will be available when initialized..."
            );
            ...
            if (!string.IsNullOrEmpty(ApacheNetServerConfiguration.DNSOnlineConfig))
            {
                ...
                var onlineConfig = HTTPProcessor.RequestURLGET(
                    ApacheNetServerConfiguration.DNSOnlineConfig
                );
                        ...
                        else
                        {
                            ...
                            if (split.Length == 3)
                            {
                                ...
                                switch (split[1].Trim().ToLower())
                                {
                                    ...
                                    default:
                                        LoggerAccessor.LogWarn(
                                            $"[HTTPS_DNS] - Rule : {s} is not a formated properly, skipping..."
                                        );
                                        break;
                                }
            ...
            switch (Uri.CheckHostName(ip))
            {
                ...
                default:
                {
                    ...
                    LoggerAccessor.LogError(
                        $"Unhandled UriHostNameType {Uri.CheckHostName(ip)} from {ip} in SecureDNSConfigProcessor.GetIp()"
                    );
                    break;
                }
            }

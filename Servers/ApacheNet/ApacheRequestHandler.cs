    ...
    public static class ApacheRequestHandler
    {
        public static Task<bool> HandleHEAD(ApacheContext ctx)
        {
            ...
            if (fileInfo != null && fileInfo.Exists)
            {
                var ContentType = HTTPProcessor.GetMimeType(
                    Path.GetExtension(ctx.FilePath),
                    ApacheNetServerConfiguration.MimeTypes ?? HTTPProcessor.MimeTypes
                );
                if (ContentType == "application/octet-stream")
                {
                    var matched = false;
                    var VerificationChunck = FileSystemUtils.TryReadFileChunck(
                        ctx.FilePath,
                        10,
                        FileShare.ReadWrite,
                ...
                ctx.Response.Headers.Set(
                    "Last-Modified",
                    File.GetLastWriteTime(ctx.FilePath!).ToString("r")
                );
        ...
        public static Task<bool> HandlePROPFIND(ApacheContext ctx)
        {
            if (File.Exists(ctx.FilePath))
            {
                ...
                var httpVer = ApacheNetServerConfiguration.HttpVersion;

...
{
    public static string ServerBindAddress { get; set; } =
        InternetProtocolUtils.TryGetServerIP(out var extractedIp).Result
            ? extractedIp
            : extractedIp;
    public static bool RPCS3Workarounds { get; set; } = true;
    public static bool EnableBlazeEncryption { get; set; } = false;
    public static string DirtySocksDatabasePath { get; set; } =
        $"{Directory.GetCurrentDirectory()}/static/dirtysocks.db.sqlite";
    ...
    public static void RefreshVariables(string configPath)
    {
        var defaultServers = new JArray(
            ...
            new JObject(
                new JProperty("type", "Aries"),
                new JProperty("subtype", "Redirector"),
                new JProperty("port", 20151),
                new JProperty("target_ip", ServerBindAddress),
                new JProperty("target_port", 20152),
                new JProperty("project", "NBA08"),
                new JProperty("sku", "PS3"),
                new JProperty("secure", "true"),
                new JProperty("cn", "ps3nba08.ea.com")
            ),
            new JObject(
                new JProperty("type", "Aries"),
                new JProperty("subtype", "Redirector"),
                new JProperty("port", 20150),
                new JProperty("target_ip", ServerBindAddress),
                new JProperty("target_port", 20152),
                new JProperty("project", "NBA08"),
                new JProperty("sku", "PS3")
            ...
            #endregion
            #region NASCAR 08 PS3
            new JObject(
                new JProperty("type", "Aries"),
                new JProperty("subtype", "Redirector"),
                new JProperty("port", 20671),
                new JProperty("target_ip", ServerBindAddress),
                new JProperty("target_port", 20672),
                new JProperty("project", "NASCAR08"),
                new JProperty("sku", "PS3"),
                new JProperty("secure", "true"),
                new JProperty("cn", "ps3nascar08.ea.com")
            ),
            new JObject(
                new JProperty("type", "Aries"),
                new JProperty("subtype", "Redirector"),
                new JProperty("port", 20670),
                new JProperty("target_ip", ServerBindAddress),
                new JProperty("target_port", 20672),
            ...
            #endregion
            #region NBA 08 PS3 Matchmaker
            new JObject(
                new JProperty("type", "Aries"),
                new JProperty("subtype", "Matchmaker"),
                new JProperty("port", 20152),
                new JProperty("listen_ip", "0.0.0.0"),
                new JProperty("project", "NBA08"),
                new JProperty("sku", "PS3")
            ),
            #endregion
            #region NASCAR 08 PS3 Matchmaker
            new JObject(
                new JProperty("type", "Aries"),
                new JProperty("subtype", "Matchmaker"),
                new JProperty("port", 20672),
                new JProperty("listen_ip", "0.0.0.0"),
                new JProperty("project", "NASCAR08"),
                new JProperty("sku", "PS3")
            ),
            ...
            new JObject(
                ...
                new JProperty(
                    ...
                    new JArray(
                        new JObject(
                            new JProperty("name", "Earth"),
                            new JProperty("is_global", true)
                        )
                    )
                )
            ),
            ...
            new JObject(
                ...
                new JProperty(
                    ...
                    new JArray(
                        new JObject(
                            new JProperty("name", "Veronaville"),
                            new JProperty("is_global", true)
                        ),
                        new JObject(
                            new JProperty("name", "Strangetown"),
                            new JProperty("is_global", true)
                        ),
                        new JObject(
                            new JProperty("name", "Pleasantview"),
                            new JProperty("is_global", true)
                        ),
                        new JObject(
                            new JProperty("name", "Belladonna Cove"),
                            new JProperty("is_global", true)
                        ),
                        new JObject(
                            new JProperty("name", "Riverblossom Hills"),
                            new JProperty("is_global", true)
                        )
                    )
                )
            ...
            // ────────────────────────────────
            new JObject(
                new JProperty("type", "Blaze2"),
                new JProperty("subtype", "Redirector"),
                new JProperty("game", "Blaze2 Redirector"),
                new JProperty("port", 42100)
            ),
            new JObject(
                new JProperty("type", "Blaze2"),
                new JProperty("subtype", "Main"),
                new JProperty("game", "FIFA2010_WORLDCUP (PS3)"),
                new JProperty("port", 32000),
                new JProperty(
                    "components",
                    new JArray("Components.PS3.FIFA2010WorldCup.Auth.AuthComponent")
                )
            ),
            // ────────────────────────────────
            // Blaze3 Servers
            ...
            new JObject(
                ...
                new JProperty(
                    "components",
                    new JArray(
                        "Components.PS3.MassEffect3.Auth.AuthComponent",
                        "Components.PS3.MassEffect3.Util.UtilComponent"
                    )
                )
            )
        );
        ...
        try
        {
            ...
            {
                ServerBindAddress = GetValueOrDefault(
                    config,
                    "server_bind_address",
                    ServerBindAddress
                );
                RPCS3Workarounds = GetValueOrDefault(config, "rpcs3_workarounds", RPCS3Workarounds);
                EnableBlazeEncryption = GetValueOrDefault(
                    config,
                    "enable_blaze_encryption",
                    EnableBlazeEncryption
                );
                DirtySocksDatabasePath = GetValueOrDefault(
                    config,
                    "dirtysocks_database_path",
                    DirtySocksDatabasePath
                );
...
class Program
{
    ...
    public static void InitializeServers()
    {
        ...
        {
            var srvType = server.Type.ToLower();
            ...
            catch (Exception ex)
            {
                LoggerAccessor.LogError(
                    $"Failed to initialize {server.Type} {server.Subtype}: {ex}"
                );
            }
        }
    }
    ...
    {
        DirtySocksDatabase = MultiSocksServerConfiguration.DirtySocksDatabasePath.EndsWith(
            ".json",
            StringComparison.InvariantCultureIgnoreCase
        )
            ? new DirtySocksJSONDatabase()
            : new DirtySocksSQLiteDatabase();
        ...
        switch (config.Subtype.ToLower())
        {
            ...
            case "matchmaker":
                ...
                {
                    roomTuples = config
                        .RoomsToAdd.Select(r => Tuple.Create(r.Name, r.IsGlobal))
                        .ToList();
                }
    ...
    private static void InitializeBlaze(ServerConfig config)
    {
        ...
        var blazeVersion = config.Type.ToLower();
        var sslDomain = config.CN ?? defaultSslDomain;

        // Factory selector for Blaze2 (defaults to Blaze3)
        Func<string, IPEndPoint, X509Certificate2?, bool, dynamic> createBlazeServer =
            blazeVersion switch
            {
                "blaze2" => (name, endpoint, cert, secure) =>
                    Blaze2.CreateBlazeServer(name, endpoint, cert, secure),
                _ => (name, endpoint, cert, secure) =>
                    Blaze3.CreateBlazeServer(name, endpoint, cert, secure),
            };
        ...
        switch (config.Subtype.ToLower())
        {
            case "redirector":
            {
                var redirector = createBlazeServer(
                    ...
                    BlazeSSLCache
                        .GetVulnerableCustomEaCert(sslDomain, "Global Online Studio", true)
                        .Item3,
                    config.Secure
                );
                ...
                LoggerAccessor.LogInfo(
                    $"[{blazeVersion.ToUpper()}] Redirector running on port {config.Port}"
                );
                break;
            }
            ...
            case "mitm":
            {
                ...
                {
                    LoggerAccessor.LogWarn(
                        $"[{blazeVersion.ToUpper()}] Missing 'game' field for Blaze mitm server on port {config.Port}"
                    );
                    return;
                }
                ...
                Func<
                    ...
                    bool,
                    dynamic
                > createBlazeMitmServer = blazeVersion switch
                {
                    _ => (
                        name,
                        target_ip,
                    ...
                    foreach (var compName in config.Components)
                    {
                        try
                        {
                            ...
                            if (type != null)
                            {
                                ...
                                LoggerAccessor.LogInfo(
                                    $"[{blazeVersion.ToUpper()}] Added component: {compName}"
                                );
                            }
                            else
                                LoggerAccessor.LogWarn(
                                    $"[{blazeVersion.ToUpper()}] Component not found: {compName}"
                                );
                        }
                        catch (Exception ex)
                        {
                            LoggerAccessor.LogError(
                                $"[{blazeVersion.ToUpper()}] Failed to add component '{compName}': {ex}"
                            );
                        }
                    }
                }
...
#pragma warning disable
                    SSL3SupportCheck.PerformSSL3Checks();
#pragma warning restore
                ...
                LoggerAccessor.LogInfo(
                    $"[{blazeVersion.ToUpper()}] Mitm server '{config.Game}' running on port {config.Port}"
                );
                break;
            }
            ...
            case "main":
            {
                ...
                {
                    LoggerAccessor.LogWarn(
                        $"[{blazeVersion.ToUpper()}] Missing 'game' field for Blaze main server on port {config.Port}"
                    );
                    return;
                }
                ...
                LoggerAccessor.LogInfo(
                    $"[{blazeVersion.ToUpper()}] Main server '{config.Game}' running on port {config.Port}"
                );
                break;
            }

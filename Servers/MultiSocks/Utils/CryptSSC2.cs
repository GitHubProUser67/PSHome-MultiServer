...
namespace MultiSocks.Utils
{
    ...
    {
        private static readonly CryptSSC2T _SSC2_Random = new();
        private static bool _SSC2_Init = false;

        private static readonly uint[] _Cipher_Crc32 = new uint[]
        {
            0x00000000,
            0x77073096,
            0xee0e612c,
     ...
     */
            ...
            if (iKey > 0)
            {
                ...
                for (iIdx = 0; iIdx < iIter * 256; ++iIdx)
                {
                    ...
                    uCrc =
                        (
                            (uCrc >> 8)
                            ^ _Cipher_Crc32[(int)(uCrc & 0xFF) ^ (pKey[iIdx % iKey] & 0xFF)]
                        ) & 0xFFFFFFFFL;
                    // put sbox data into crc
                    uCrc =
                        (
                            (uCrc >> 8)
                            ^ _Cipher_Crc32[(int)(uCrc & 0xFF) ^ (pState.sbox[iIdx % 256] & 0xFF)]
                        ) & 0xFFFFFFFFL;
        ...
        public static void cryptSSC2Apply(CryptSSC2T pState, byte[] pData, int iData)
        {
            ...
            for (; iData > 0; --iData)
            {
                ...
                uCrc =
                    (
                        (uCrc >> 8)
                        ^ _Cipher_Crc32[(int)(uCrc & 0xFF) ^ (pState.sbox[iIdx++ & 0xFF] & 0xFF)]
                    ) & 0xFFFFFFFFL;
         ...
         * @return success
         */
        public static int cryptSSC2StringEncrypt(
            byte[] pDst,
            int iLen,
            byte[] pSrc,
            byte[] pKey,
         ...
         * @return success
         */
        public static int cryptSSC2StringDecrypt(
            byte[] pDst,
            int iLen,
            byte[] pSrc,
            byte[] pKey,

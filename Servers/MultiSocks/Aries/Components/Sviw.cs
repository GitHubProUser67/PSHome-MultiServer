using MultiSocks.Aries.Components.Burnout3Plugin.MultiSocks.Aries.Messages;
...
namespace MultiSocks.Aries.Components
{
    public class Sviw : AbstractMessage
    {
        ...
        {
            var VIEW = GetInputCacheValue("VIEW");
            ...
            else if (client.VERS == "FLM/A1") //Burnout 3 Takedown
            {
                ...
                client.SendMessage(new PlusRom() { I = "420", N = user.PersonaName });
            }
            else
            {
                ...
                if (user.SelectedPersona == -1)
                    return; //failed?
                client.SendMessage(new Pers() { NAME = user.Username, PERS = user.PersonaName });
            }

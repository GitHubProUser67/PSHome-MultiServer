...
namespace MultiSocks.Aries.Model
{
    public class AriesUser
    {
        ...
        {
            get => SelectedPersona == -1 ? null : Personas[SelectedPersona];
        }
        ...
        public string GetParametersString(
            Func<int, string, (bool, string)>? CustomParameterProcess = null
        )
        {
            var abortCustomProcessProcessing = false;
            var i = 0;
            ...
            foreach (var param in Parameters)
            {
                if (st.Length != 0)
                {
                    ...
                    {
                        var customProcessResult = CustomParameterProcess(i, param);
        ...
        public PlusUser GetInfo()
        {
            ...
            {
                I = ID.ToString(),
                M = (GetIsGameHost() ? "@" : string.Empty) + Username,
                N = (GetIsGameHost() ? "@" : string.Empty) + PersonaName ?? string.Empty,
                A = ADDR,
                X = Auxiliary,
        ...
        public void SendPlusWho(AriesUser user, string? VERS = null)
        {
            ...
            if (!string.IsNullOrEmpty(VERS))
            {
                ...
                else if (VERS.Contains("FLM/A1"))
                {
                    ...
                    VERS.Contains("BURNOUT5")
                    || VERS.Contains("DPR-09")
                    || (
                        (
                            VERS.Contains("NASCAR08")
                            || VERS.Contains("NASCAR09")
                            || VERS.Contains("NBA08")
                        ) && "PS3".Equals(user.Connection?.SKU)
                    )
                )
                {
                    who = new PlusWho()

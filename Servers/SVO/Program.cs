...
public static class SVOServerConfiguration
{
    ...
    public static string HTTPSCertificateFile { get; set; } =
        $"{Directory.GetCurrentDirectory()}/static/SSL/SVO.pfx";
    ...
    public static string SVOStaticFolder { get; set; } =
        $"{Directory.GetCurrentDirectory()}/static/wwwsvoroot";
        ...
        else
        {
            try
            {
                ...
                if (config_version >= 2)
                {
                    ...
                    PSHomeRPCS3Workaround = GetValueOrDefault(
                        config,
                        "pshome_rpcs3workaround",
                        PSHomeRPCS3Workaround
                    );
...
class Program
{
    ...
    private static readonly string configPath = configDir + "svo.json";
    ...
    private static void StartOrUpdateServer()
    {
        ...
        MultiServerLibrary.SSL.CertificateHelper.InitializeSSLChainSignedCertificates(
            SVOServerConfiguration.HTTPSCertificateFile,
            SVOServerConfiguration.HTTPSCertificatePassword,
            SVOServerConfiguration.HTTPSDNSList,
            SVOServerConfiguration.HTTPSCertificateHashingAlgorithm
        );
    ...
    static void Main()
    {
        ...
        CastleLibrary.FixedSsl.SslSocket.BypassRemoteCertificateChecks = true; // Needed so the tunnel accept our own self-signed certificates (not ideal, but can be revisited later).

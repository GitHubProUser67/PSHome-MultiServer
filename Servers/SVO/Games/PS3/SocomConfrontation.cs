...
namespace SVO.Games.PS3
{
    ...
    {
        public static async Task SocomConfrontation_SVO(
        ...
        )
        {
                        ...
                        case "/CONFRONTATION_XML/index.jsp":
                            ...
                            switch (request.HttpMethod)
                            {
                                case "GET":
                                    ...
                                    {
                                        var language = request.Headers.Get("Accept-Language");

                                        var languageID = 1;

                                        try
                                        {
                                            var parsed = HttpUtility
                                        ...
                                        var domain = "killzoneps3.svo.online.scee.com";
                                        ...
                                        var index = SVOServerConfiguration.SVOHTTPSBypass
                                    ...
                                    else
                                    {
                                        ...
                                        using (MemoryStream ms = new())
                                        {
                                            ...
                                            if (!ticket.Valid)
                                            {
                                                ...
                                                LoggerAccessor.LogWarn(
                                                    $"[SVO] - User {psnname} tried to alter their ticket data"
                                                );
                                            ...
                                            if (ticket.IsSignedByRPCN)
                                            {
                                                LoggerAccessor.LogInfo(
                                                    $"[SVO] - User {psnname} connected at: {DateTime.Now} and is on RPCN"
                                                );
                                            ...
                                            else if (psnname.EndsWith($"@{XI5Ticket.RPCNSigner}"))
                                            {
                                                LoggerAccessor.LogError(
                                                    $"[SVO] - User {psnname} was caught using a RPCN suffix while not on it!"
                                                );
                                                ...
                                                LoggerAccessor.LogInfo(
                                                    $"[SVO] - User {psnname} connected at: {DateTime.Now} and is on PSN"
                                                );
                                        ...
                                        try
                                        {
                                            await SVOServerConfiguration
                                                .Database.GetAccountByName(psnname, 21784)
                                                .ContinueWith(
                                                    (r) =>
                                                    {
                            ...
                            switch (request.HttpMethod)
                            {
                                case "GET":
                                    ...
                                    else
                                    {
                                        ...
                                        var homesvml = Encoding.UTF8.GetBytes(
                                            "<SVML> "
                            ...
                            switch (request.HttpMethod)
                            {
                                case "GET":
                                    ...
                                    else
                                    {
                                        ...
                                        var menusvml = Encoding.UTF8.GetBytes(
                                            "<SVML> <PAGEID name=\"kin-preparsed_menu_online_menu\" /> <_c name=\"onlinemenuAssets\" location=\"SVO/SVOMainMenu\" /> "
            ...
            {
                LoggerAccessor.LogError(
                    $"[SVO] - SocomConfrontation_SVO thrown an assertion - {ex}"
                );

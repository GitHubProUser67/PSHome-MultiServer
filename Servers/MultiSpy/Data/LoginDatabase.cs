    ...
    public class LoginDatabase : IDisposable
    {
                    ...
                    if (!read)
                    {
                        ...
                        using (
                            var createTables = new SQLiteCommand(
                                "CREATE TABLE users ( id INTEGER PRIMARY KEY, name TEXT NOT NULL, password TEXT NOT NULL, email TEXT NOT NULL, country TEXT NOT NULL, lastip TEXT NOT NULL, lasttime INTEGER NULL DEFAULT '0', session INTEGER NULL DEFAULT '0' )",
                                _instance._db
                            )
                        )
                        {
        ...
        private void PrepareStatements()
        {
            _getUsersByName = new SQLiteCommand(
                "SELECT id, password, email, country, session FROM users WHERE name=@name COLLATE NOCASE",
                _db
            );
            _getUsersByName.Parameters.Add("@name", DbType.String);

            _getUsersByEmail = new SQLiteCommand(
                "SELECT id, name, country, session FROM users WHERE email=@email AND password=@password",
                _db
            );
            ...
            _updateUser = new SQLiteCommand(
                "UPDATE users SET password=@pass, email=@email, country=@country, session=@session WHERE name=@name COLLATE NOCASE",
                _db
            );
            ...
            _createUser = new SQLiteCommand(
                "INSERT INTO users (name, password, email, country, lastip) VALUES ( @name, @pass, @email, @country, @ip )",
                _db
            );
            ...
            _logUser = new SQLiteCommand(
                "UPDATE users SET lastip=@ip, lasttime=@time WHERE name=@name COLLATE NOCASE",
                _db
            );
            ...
            _logUserUpdateCountry = new SQLiteCommand(
                "UPDATE users SET country=@country, lastip=@ip, lasttime=@time WHERE name=@name COLLATE NOCASE",
                _db
            );
        ...
        public List<Dictionary<string, object>> GetData(string email, string passwordEncrypted)
        {
            ...
            lock (_dbLock)
            {
                ...
                using (var reader = _getUsersByEmail.ExecuteReader())
                {
                    while (reader.Read())
                    {
        ...
        public void SetData(string name, Dictionary<string, object> data)
        {
            ...
            lock (_dbLock)
            {
                _updateUser.Parameters["@pass"].Value = data.TryGetValue(
                    "passwordenc",
                    out var value
                )
                    ? value
                    : oldValues["passwordenc"];
                _updateUser.Parameters["@email"].Value = data.TryGetValue("email", out value)
                    ? ((string)value).ToLowerInvariant()
                    : oldValues["email"];
                _updateUser.Parameters["@country"].Value = data.TryGetValue("country", out value)
                    ? value.ToString().ToUpperInvariant()
                    : oldValues["country"];
                _updateUser.Parameters["@session"].Value = data.TryGetValue("session", out value)
                    ? value
                    : oldValues["session"];
        ...
        public void LogLogin(string name, IPAddress address)
        {
            ...
            if (
                ...
                && !data["country"]
                    .ToString()
                    .Equals(country, StringComparison.InvariantCultureIgnoreCase)
            )
            {
                lock (_dbLock)
                {
                    ...
                    _logUserUpdateCountry.Parameters["@time"].Value = (
                        DateTime.UtcNow.ToUniversalTime() - new DateTime(1970, 1, 1)
                    ).TotalSeconds;
            ...
            else
            {
                lock (_dbLock)
                {
                    ...
                    _logUser.Parameters["@time"].Value = (
                        DateTime.UtcNow.ToUniversalTime() - new DateTime(1970, 1, 1)
                    ).TotalSeconds;

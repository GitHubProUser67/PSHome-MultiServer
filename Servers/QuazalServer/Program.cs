...
public static class QuazalServerConfiguration
{
    ...
    public static string ServerPublicBindAddress { get; set; } =
        InternetProtocolUtils.GetPublicIPAddress();
    public static string EdNetBindAddressOverride { get; set; } = string.Empty;
    public static string QuazalStaticFolder { get; set; } =
        $"{Directory.GetCurrentDirectory()}/static/Quazal";
    public static bool UsePublicIP { get; set; } =
        InternetProtocolUtils.TryGetServerIP(out _).Result;
    public static List<Tuple<int, string, string>>? BackendServersList { get; set; } =
    ...
    public static List<Tuple<int, int, string, string>>? RendezVousServersList { get; set; } =
    ...
    public static void RefreshVariables(string configPath)
    {
        ...
        try
        {
            ...
            if (config_version >= 2)
            {
                ...
                ServerPublicBindAddress = GetValueOrDefault(
                    config,
                    "server_public_bind_address",
                    ServerPublicBindAddress
                );
                EdNetBindAddressOverride = GetValueOrDefault(
                    config,
                    "ednet_bind_address_override",
                    EdNetBindAddressOverride
                );
                QuazalStaticFolder = GetValueOrDefault(
                    config,
                    "quazal_static_folder",
                    QuazalStaticFolder
                );
                ...
                try
                {
                    ...
                    if (BackendServersListArray != null)
                        BackendServersList = BackendServersListArray.ToObject<
                ...
                try
                {
                    ...
                    if (RendezVousServersListArray != null)
                        RendezVousServersList = RendezVousServersListArray.ToObject<
...
class Program
{
    ...
    private static readonly string configPath = configDir + "QuazalServer.json";
    ...
    private static readonly ConcurrentDictionary<ushort, QPacketHandlerPRUDP> _backendHandlers =
        new();
    private static readonly ConcurrentDictionary<ushort, QPacketHandlerPRUDP> _rendezvousHandlers =
        new();
    ...
    private static void StartOrUpdateServer()
    {
        ...
        if (QuazalServerConfiguration.BackendServersList != null)
        {
            ...
            _ = BackendServer.StartAsync(
                ...
                (serverPort) =>
                {
                    QuazalServer.RDVServices.ServiceFactoryRDV.TryInsertFactory(
                        QuazalServerConfiguration
                            .BackendServersList.First(entry => entry.Item1 == serverPort)
                            .Item3
                    );
                },
                (serverPort, listener) =>
                {
                    ...
                    var entry = QuazalServerConfiguration.BackendServersList.First(entry =>
                        entry.Item1 == serverPort
                    );
                    QPacketHandlerPRUDP packetHandler = new(
                        listener,
                        _serverPID,
                        serverPort,
                        serverPort,
        ...
        if (QuazalServerConfiguration.RendezVousServersList != null)
        {
            ...
            _ = RendezVousServer.StartAsync(
                ...
                (serverPort) =>
                {
                    QuazalServer.RDVServices.ServiceFactoryRDV.TryInsertFactory(
                        QuazalServerConfiguration
                            .RendezVousServersList.First(entry => entry.Item1 == serverPort)
                            .Item4
                    );
                },
                (serverPort, listener) =>
                {
                    ...
                    var entry = QuazalServerConfiguration.RendezVousServersList.First(entry =>
                        entry.Item1 == serverPort
                    );
                    QPacketHandlerPRUDP packetHandler = new(
                        listener,
    ...
    static void Main()
    {
        ...
        var oldConfigPath = Path.GetDirectoryName(configPath) + $"/quazal.json";
        if (File.Exists(oldConfigPath))
        {
            if (!File.Exists(configPath))
            {
                LoggerAccessor.LogWarn(
                    "[Main] - Detected older incorrect Quazal configuration file path, performing file renaming..."
                );

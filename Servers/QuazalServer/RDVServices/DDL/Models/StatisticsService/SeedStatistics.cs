...
namespace QuazalServer.RDVServices.DDL.Models
{
    ...
    public static class SeedStatistics
    {
        public static PlayerStatisticsBoard GeneratePlayerBoard(int boardId, uint playerId)
        {
            ...
            foreach (
                var statPropertyDesc in AllStatisticDescriptions.Where(x => x.statBoard == boardId)
            )
            {
                var value = new StatisticsBoardValue(statPropertyDesc);
                playerBoard.Values.Add(
                    new PlayerStatisticsBoardValue()
                    {
        ...
        public static StatisticsBoardValue GetStatisticBoardValueByPropertyId(
            int boardId,
            int propertyId
        )
        {
            var statPropertyDesc = AllStatisticDescriptions.FirstOrDefault(x =>
                x.statBoard == boardId && x.statInBoardId == propertyId
            );

    ...
    public class PlayerStatisticsService : RMCServiceBase
    {
        ...
        public RMCResult WritePlayerStats(IEnumerable<StatisticWriteWithBoard> playerStats)
        {
            ...
            using (var db = DBHelper.GetDbContext(Context.Handler.Factory.Item1))
            {
                ...
                {
                    var playerBoard = db.PlayerStatisticBoards.FirstOrDefault(x =>
                        x.PlayerId == playerId && x.BoardId == writeBoard.boardId
                    );

                    if (playerBoard == null)
                    {
                        playerBoard = SeedStatistics.GeneratePlayerBoard(
                            writeBoard.boardId,
                            playerId
                        );
                        db.Add(playerBoard);
                    ...
                    var propertyIdsOnly = writeBoard
                        .statisticList.Select(ws => (int)ws.propertyId)
                        .ToList(); // ToList matters in EF8

                    var existingBoardValues = db
                        .PlayerStatisticBoardValues.Where(x =>
                            propertyIdsOnly.Contains(x.PropertyId)
                            && x.PlayerBoardId == playerBoard.Id
                        )
                        .ToArray();

                    var newBoardScore = 0.0f;

                    foreach (var writeStat in writeBoard.statisticList)
                    {
                        var existingVariantJSON = existingBoardValues.FirstOrDefault(x =>
                            x.PropertyId == writeStat.propertyId
                        );
                        ...
                        if (existingVariantJSON != null)
                        {
                            ...
                            {
                                value = JsonConvert.DeserializeObject<StatisticValueVariant>(
                                    existingVariantJSON.ValueJSON
                                ),
                                rankingCriterionIndex = (byte)
                                    existingVariantJSON.RankingCriterionIndex,
                                scoreLostForNextSlice =
                                    JsonConvert.DeserializeObject<StatisticValueVariant>(
                                        existingVariantJSON.ScoreLostForNextSliceJSON
                                    ),
                                sliceScore = JsonConvert.DeserializeObject<StatisticValueVariant>(
                                    existingVariantJSON.SliceScoreJSON
                                ),
                            };

                            // Update variant value
                            if (
                                valueToUpdate.UpdateValueWithPolicy(
                                    writeStat.value,
                                    (StatisticPolicy)writeStat.writePolicy
                                )
                            )
                            {
                                var statDesc =
                                    SeedStatistics.AllStatisticDescriptions.FirstOrDefault(x =>
                                        x.statBoard == writeBoard.boardId
                                        && x.statInBoardId == writeStat.propertyId
                                    );
                                LoggerAccessor.LogInfo(
                                    $"[PlayerStatisticsService] - player {Context.Client.PlayerInfo.Name} {statDesc.statName} ({writeBoard.boardId}:{writeStat.propertyId}) changed to '{writeStat.value.ToString()}' with policy {(StatisticPolicy)writeStat.writePolicy}"
                                );
                            }
                            ...
                            // put back the values
                            existingVariantJSON.RankingCriterionIndex =
                                valueToUpdate.rankingCriterionIndex;
                            existingVariantJSON.ValueJSON = JsonConvert.SerializeObject(
                                valueToUpdate.value
                            );
                            existingVariantJSON.ScoreLostForNextSliceJSON =
                                JsonConvert.SerializeObject(valueToUpdate.scoreLostForNextSlice);
                            existingVariantJSON.SliceScoreJSON = JsonConvert.SerializeObject(
                                valueToUpdate.sliceScore
                            );
                        ...
                        LoggerAccessor.LogWarn(
                            $"[PlayerStatisticsService] - stats property id ({writeBoard.boardId}:{writeStat.propertyId}) was not found, creating new one..."
                        );

                        var newBoardValue = SeedStatistics.GetStatisticBoardValueByPropertyId(
                            writeBoard.boardId,
                            writeStat.propertyId
                        );
                        if (newBoardValue != null)
                        {
                            ...
                            var newVariantJSON = new PlayerStatisticsBoardValue()
                            {
                                ...
                                ScoreLostForNextSliceJSON = JsonConvert.SerializeObject(
                                    newBoardValue.scoreLostForNextSlice
                                ),
                                SliceScoreJSON = JsonConvert.SerializeObject(
                                    newBoardValue.sliceScore
                                ),
                            };
                        ...
                        {
                            LoggerAccessor.LogWarn(
                                $"[PlayerStatisticsService] - UNKNOWN stats property id ({writeBoard.boardId}:{writeStat.propertyId})"
                            );
                        }
                    }
        ...
        [RMCMethod(3)]
        public void WritePlayerStatsWithFriendsComparison(
            IEnumerable<StatisticWriteWithBoard> playerStats,
            List<uint> playerPIDs
        )
        {
            UNIMPLEMENTED();
        }
        ...
        public RMCResult ReadPlayerStats(IEnumerable<StatisticData> data, List<uint> playerIds)
        {
            ...
            foreach (var playerId in playerIds)
            {
                ...
                var scoreListRead = new ScoreListRead() { pid = playerId, pname = player.Name };
                ...
                using (var db = DBHelper.GetDbContext(Context.Handler.Factory.Item1))
                {
                    ...
                    {
                        var playerBoard = db
                            .PlayerStatisticBoards.Include(x => x.Values)
                            .FirstOrDefault(x =>
                                x.PlayerId == playerId && x.BoardId == statReqData.boardId
                            );
                        ...
                        readBoardValue.scores = playerBoard
                            .Values.Where(x => statReqData.propertyIds.Contains(x.PropertyId))
                            .Select(x => new StatisticReadValue()
                            {
        ...
        [RMCMethod(5)]
        public RMCResult ReadStatsLeaderboardByRange(
            int boardId,
            int columnId,
            int rankStart,
            int numRows,
        ...
        )
        {
            ...
            {
                var playerBoards = db
                    .PlayerStatisticBoards.Where(x => x.BoardId == boardId)
                    .Take(numRows);
                ...
                foreach (var board in playerBoards)
                {
                    ...
                    var boardValue = db
                        .PlayerStatisticBoardValues.Where(x =>
                            x.PlayerBoardId == board.Id && x.PropertyId == columnId
                        )
                    ...
                    if (boardValue != null)
                    {
                        var rv = new StatisticReadValue()
                        {
                            ...
                            value = JsonConvert.DeserializeObject<StatisticValueVariant>(
                                boardValue.ValueJSON
                            ),
                            rankingCriterionIndex = (byte)boardValue.RankingCriterionIndex,
                            scoreLostForNextSlice =
                                JsonConvert.DeserializeObject<StatisticValueVariant>(
                                    boardValue.ScoreLostForNextSliceJSON
                                ),
                            sliceScore = JsonConvert.DeserializeObject<StatisticValueVariant>(
                                boardValue.SliceScoreJSON
                            ),
                        };
                ...
                var leaderBoardStat = SeedStatistics.AllStatisticDescriptions.FirstOrDefault(x =>
                    x.statBoard == boardId && x.statInBoardId == columnId
                );
                if (leaderBoardStat.statRankingOrder == RankingOrder.Ascending)
                    playerStats.Sort(
                        (a, b) =>
                            (int)(a.scoresByBoard.First().score - b.scoresByBoard.First().score)
                    );
                else
                    playerStats.Sort(
                        (a, b) =>
                            (int)(b.scoresByBoard.First().score - a.scoresByBoard.First().score)
                    );
        ...
        [RMCMethod(8)]
        public RMCResult ReadStatsLeaderboardByPIDs(
            IEnumerable<LeaderboardData> dataList,
            List<uint> playerPIDs
        )
        {
            var playerStats = new List<ScoreListRead>();
            ...
            using (var db = DBHelper.GetDbContext(Context.Handler.Factory.Item1))
            {
                ...
                {
                    var playerBoards = db
                        .PlayerStatisticBoards.Where(x => x.BoardId == data.boardId)

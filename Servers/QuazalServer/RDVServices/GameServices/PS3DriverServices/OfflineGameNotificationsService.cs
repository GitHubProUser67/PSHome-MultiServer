    ...
    public class OfflineGameNotificationsService : RMCServiceBase
    {
        ...
        public RMCResult PollNotifications()
        {
            return Result(
                new
                {
                    list_notifications = new List<NotificationEvent>(),
                    nb_remaining_notifs = (uint)0,
                }
            );
        }
        ...
        {
            return Result(
                new { list_timed_notification = new List<TimedNotification>(), ret = (uint)0 }
            );
        }
        ...
        public RMCResult PollAnyOfflineNotifications()
        {
            return Result(
                new
                {
                    list_timed_notification = new List<TimedNotification>(),
                    nb_remaining_notifs = (uint)0,
                }
            );
        }

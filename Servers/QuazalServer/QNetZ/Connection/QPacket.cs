...
namespace QuazalServer.QNetZ
{
    public class QPacket
    {
        ...
        public uint Port;
        private readonly bool NewerCheckum;
        ...
        public QPacket(string AccessKey, Stream stream)
        {
            ...
            payloadSize = flags.Contains(PACKETFLAG.FLAG_HAS_SIZE)
                ? Helper.ReadU16(stream)
                : (ushort)(stream.Length - stream.Position - (NewerCheckum ? 4 : 1));
            ...
            if (
                ...
                && payload.Length > 0
                && type != PACKETTYPE.SYN
                && m_oSourceVPort.type != STREAMTYPE.NAT
            )
            {
            ...
            checkSum = NewerCheckum
                ? new byte[4]
                {
                    Helper.ReadU8(stream),
                    Helper.ReadU8(stream),
                    Helper.ReadU8(stream),
                    Helper.ReadU8(stream),
                }
                : new byte[1] { Helper.ReadU8(stream) };
        ...
        public byte[]? getProcessedPayload(string AccessKey)
        {
            ...
            )
            {
                if (
                    ...
                    && AccessKey != "uG9Kv3p"
                    && AccessKey != "1WguH+y"
                ) // Old LZO based clients not need compressed response.
                {
                    var sizeBefore = (uint)tmpPayload.Length;
                    var buff = Helper.Compress(tmpPayload);
                    var count = (byte)(sizeBefore / buff.Length);
        ...
        public byte[] toBuffer(string AccessKey)
        {
            ...
            using (MemoryStream m = new())
            {
                ...
                // compress
                var processedPayload = getProcessedPayload(AccessKey);
        ...
        public static byte[] AddCheckSum(
            byte[] buff,
            string AccessKey,
            bool NewerCheckum,
            ref byte[]? checkSum
        )
        {
            try
            {
                var result = new byte[buff.Length + (NewerCheckum ? 4 : 1)];
            ...
            catch (Exception ex)
            {
                CustomLogger.LoggerAccessor.LogError(
                    $"[QPacket] - AddCheckSum failed with exception:{ex}"
                );
            }
        ...
        private static byte GetProtocolSetting(byte proto, string AccessKey)
        {
            switch (proto)
            {
                case 3:
                    return
                        AccessKey.Length == 4
                        && AccessKey.Contains("0x")
                        && byte.TryParse(AccessKey, out var Protocol)
                        ? Protocol
                        : (byte)Encoding.ASCII.GetBytes(AccessKey).Sum(b => b);
                case 1:
                case 5:
                default:
                    return 0;
        ...
        private static byte MakeChecksum(byte[] data, string AccessKey, byte setting = 0xFF)
        {
            ...
            var isBigEndian = !EndianTools.EndianAwareConverter.isLittleEndianSystem;
            uint tmp = 0;
            for (var i = 0; i < data.Length / 4; i++)
                tmp += BitConverter.ToUInt32(
                    isBigEndian ? EndianUtils.ReverseArray(data) : data,
                    i * 4
                );

            var leftOver = (uint)data.Length & 3;
            uint processed = 0;
            byte tmp2 = 0,
                tmp3 = 0,
                tmp4 = 0;
            var pos = (uint)data.Length - leftOver;
            ...
            tmp4 = processed >= leftOver ? setting : (byte)(setting + data[pos]);
        ...
        private static uint Key4(string AccessKey)
        {
            ...
            for (var i = 0; i < AccessKey.Length; i += 4)
            {
                for (var j = 0; j < 4; j++)
                {
        ...
        {
            var LittleEndian = EndianTools.EndianAwareConverter.isLittleEndianSystem;
            var len = data.Length;
            len -= len % 4;
            ...
            var Checksum = dataSum + Key4(AccessKey) + BitConverter.ToUInt32(trailerBytes, 0);

            return BitConverter.GetBytes(
                !LittleEndian ? EndianUtils.ReverseUint(Checksum) : Checksum
            );
        }

        private void ExtractFlags()
        {
            var v = (byte)(m_byPacketTypeFlags >> 3);
            var values = (int[])Enum.GetValues(typeof(PACKETFLAG));

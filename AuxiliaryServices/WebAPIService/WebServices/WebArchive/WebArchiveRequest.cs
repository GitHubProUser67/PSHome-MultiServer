namespace WebAPIService.WebServices.WebArchive
{
    /// <summary>
    ...
    public class WebArchiveRequest
    {
        ...
        public WebArchiveRequest(string URL)
        {
            ...
            using (
                var CdxResponse = new HttpClient()
                    .SendAsync(
            ...
            )
            {
                if (!CdxResponse.IsSuccessStatusCode)
                    throw new Exception(
                        "Unsuccessful Web Archive request: " + CdxResponse.ReasonPhrase
                            ?? " without reason"
                    );
                using (var st = new StreamReader(CdxResponse.Content.ReadAsStreamAsync().Result))
                {
                    var CdxBody = st.ReadToEnd().TrimEnd().Split('\n');
                    st.Close();
                    ...
                    var LastCdxEntry = string.Empty;
                    foreach (var CdxEntry in CdxBody)
                    {
                        var Fields = CdxEntry.Split(' ');
                        if (Fields.Length != CdxFieldsCount)
                            continue;
                        if (ArchiveDateLimit > 0)
                        {
                            long.TryParse(Fields[0], out var Timestamp);
                            if (Timestamp > ArchiveDateLimit * Math.Pow(10, 6))
                                continue;
                        }
                        if (Fields[2] == "200")
                            LastCdxEntry = CdxEntry;
                    }
                    if (LastCdxEntry == string.Empty)
                        LastCdxEntry = CdxBody[^1];

                    var ResultFields = LastCdxEntry.Split(' ');
                    ...
                    Archived = true;
                    ArchivedURL = string.Format(
                        "http://web.archive.org/web/{0}/{1}",
                        ResultFields[0],
                        ResultFields[1]
                    );
                }

...
namespace WebAPIService.GameServices.PSHOME.LOOT
{
    ...
    {
        public static string ProcessDatabaseRequest(
            byte[] PostData,
            string ContentType,
            string WorkPath
        )
        {
            ...
            if (!string.IsNullOrEmpty(boundary))
            {
                ...
                using (var ms = new MemoryStream(PostData))
                {
                    ...
                    var action = data.GetParameterValue("action");
                    var psn_acct_name = data.GetParameterValue("psn_acct_name");
                    var guid = data.GetParameterValue("guid");
                    var serialized_data = data.GetParameterValue("serialized_data");
                    var protocol_version = data.GetParameterValue("protocol_version");

                    var LOOTDBPlayerSavePath = $"{WorkPath}/LOOT/SS3/{guid}";
                    ...
                    if (ticketData != null)
                    {
                        ...
                        var psnname = Encoding
                            .ASCII.GetString(extractedData)
                            .Replace(" ", string.Empty);

#if DEBUG
                        LoggerAccessor.LogInfo(
                            $"[LOOT] ProtocolVersion: {protocol_version} - Player {psn_acct_name} Action detected as: {action} GUID: {guid} Serialized_Data {serialized_data} "
                        );
#endif
                        ...
                        switch (action)
                        {
                            ...
                            case "501":
                                {
                                    switch (guid)
                                    {
                                        ...
                                        case "1BDF949A-0A804B07-9C4C7BAE-46C15AAE":
                                        {
                                            ...
                                            var profilePath =
                                                $"{LOOTDBPlayerSavePath}/{psn_acct_name}.cache";
                                            ...
                                            try
                                            {
                                                ...
                                                {
                                                    LoggerAccessor.LogInfo(
                                                        $"[LOOT] - Detected existing player data for {guid}, overwriting!"
                                                    );
                                                ...
                                                {
                                                    LoggerAccessor.LogInfo(
                                                        $"[LOOT] - New player with no saved for {guid}! Saving for FIRST TIME!"
                                                    );
                                                    File.WriteAllText(profilePath, serialized_data);
                                            ...
                                            catch (Exception ex)
                                            {
                                                LoggerAccessor.LogError(
                                                    $"[LOOT] - Encountered a fatal exception trying to save! Here is the details:\n{ex}"
                                                );
                                                return "<response>\r\n"
                                                    + "    <code>500</code>\r\n"
                                        ...
                                        default:
                                            {
                                                LoggerAccessor.LogWarn(
                                                    $"[LOOT] - Unhandled GET DB Status with object {guid} please report to GITHUB"
                                                );
                                            }
                                            break;
                                    }
                                    ...
                                    switch (guid)
                                    {
                                        ...
                                        case "1BDF949A-0A804B07-9C4C7BAE-46C15AAE":
                                        {
                                            ...
                                            try
                                            {
                                                ...
                                                {
                                                    LoggerAccessor.LogInfo(
                                                        $"[LOOT] - Detected existing player data for {guid}, sending!"
                                                    );
                                                    var playerData = File.ReadAllText(profilePath);
                                                ...
                                                {
                                                    LoggerAccessor.LogInfo(
                                                        $"[LOOT] - New player with no saved for {guid}! Using Default!"
                                                    );
                                        ...
                                        default:
                                            {
                                                LoggerAccessor.LogWarn(
                                                    $"[LOOT] - Unhandled UPLOAD DB Status with object {guid} please report to GITHUB"
                                                );
                                            }
                                            break;
                                    }
                            ...
                            default:
                                {
                                    LoggerAccessor.LogWarn(
                                        $"[LOOT] - Unhandled Action {action} please report to GITHUB"
                                    );
                                }
                                break;
                        }

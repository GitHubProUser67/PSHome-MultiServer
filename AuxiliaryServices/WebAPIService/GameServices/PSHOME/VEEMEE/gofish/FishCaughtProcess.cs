    ...
    public class FishCaughtProcess
    {
        public static string SetPOST(byte[] PostData, string ContentType, string apiPath)
        {
            if (ContentType == "application/x-www-form-urlencoded" && PostData != null)
            {
                ...
                var fish_mask_upper = data["fish_mask_upper"].First();
                var fish_mask_lower = data["fish_mask_lower"].First();
                ...
                if (File.Exists($"{apiPath}/VEEMEE/gofish/Fish_User_Data/{psnid}.xml"))
                {
                    ...
                    xmlDoc.LoadXml(
                        $"<xml>{File.ReadAllText($"{apiPath}/VEEMEE/gofish/Fish_User_Data/{psnid}.xml")}</xml>"
                    );

                    // Find the <fish_mask_upper> element

                    if (
                        xmlDoc.SelectSingleNode("/xml/fish_mask_upper")
                        is XmlElement fish_mask_upperElement
                    )
                    {
                ...
                {
                    var XmlData =
                        $"<fish_mask_upper>{fish_mask_upper}</fish_mask_upper><fish_mask_lower>{fish_mask_lower}</fish_mask_lower>";
                    File.WriteAllText(
                        $"{apiPath}/VEEMEE/gofish/Fish_User_Data/{psnid}.xml",
                        XmlData
                    );

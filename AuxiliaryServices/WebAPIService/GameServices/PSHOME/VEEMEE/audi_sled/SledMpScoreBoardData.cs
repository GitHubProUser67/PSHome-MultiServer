...
namespace WebAPIService.GameServices.PSHOME.VEEMEE.audi_sled
{
    internal class SledMpScoreBoardData(
        ...
        object obj = null
    ) : ScoreboardService<SledMpScoreboardEntry>(options)
    {
        public override async Task<List<SledMpScoreboardEntry>> GetTopScoresAsync(int max = 10)
        {
            ...
            {
                db.Database.Migrate();
                return await db.Set<SledMpScoreboardEntry>()
                    .OrderBy(e => e.Score)
                    .Take(max)
                    .ToListAsync()
        ...
        )
        {
            ...
            var numOfRaces = (int)extraData[0];
            var time = (float)extraData[1];

            using (var db = new LeaderboardDbContext(_dboptions))
            {
                db.Database.Migrate();
                var set = db.Set<SledMpScoreboardEntry>();
        ...
        public int GetNumOfRacesForUser(string playerId)
        {
            using (var db = new LeaderboardDbContext(_dboptions))
            {
                db.Database.Migrate();
                return db.Set<SledMpScoreboardEntry>()
                        .Where(x => x.PlayerId.ToLower() == playerId.ToLower())
        ...
        public float GetScoreForUser(string playerId)
        {
            using (var db = new LeaderboardDbContext(_dboptions))
            {
                db.Database.Migrate();
        ...
        public float GetTimeForUser(string playerId)
        {
            using (var db = new LeaderboardDbContext(_dboptions))
            {
                db.Database.Migrate();
        ...
        {
            var iY = 142; // Initial Y position
            var data = new StringBuilder(
                $"<XML><PAGE><TEXT X=\"100\" Y=\"70\" col=\"#FFFFFF\" size=\"4\">{gameName}</TEXT>"
            );
            ...
            {
                data.AppendFormat(
                    "<TEXT X=\"100\" Y=\"{0}\" col=\"#FFFFFF\" size=\"3\">{1}</TEXT>",
                    iY + 7,
                    i + 1
                );
                data.AppendFormat(
                    "<TEXT X=\"190\" Y=\"{0}\" col=\"#FFFFFF\" size=\"3\">{1}</TEXT>",
                    iY + 5,
                    entries[i].PlayerId
                );
                data.AppendFormat(
                    "<TEXT X=\"800\" Y=\"{0}\" col=\"#FFFFFF\" size=\"3\">{1}</TEXT>",
                    iY + 5,
                    entries[i].numOfRaces
                );
                data.AppendFormat(
                    "<TEXT X=\"1060\" Y=\"{0}\" col=\"#FFFFFF\" size=\"3\">{1}</TEXT>",
                    iY + 5,
                    AudiSledSecondsAsString(entries[i].Score)
                );
        ...
        private static string AudiSledSecondsAsString(float time)
        {
            ...
            var hundreds = (int)Math.Floor(((time - seconds) * 100) + 0.5);
            if (hundreds < 0)
                hundreds = 0;

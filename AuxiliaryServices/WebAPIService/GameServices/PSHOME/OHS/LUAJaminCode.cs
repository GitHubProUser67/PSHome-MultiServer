...
local _wordMT = {
    __index =
        function ( tbl, val )
            ...
            local max = math.floor(size / 2) - ((size+1) % 2)
...
local function encodeTable( value )
    ...
    for k, v in pairs(value) do
        ...
        local kstr = Jamin.encode(k)
        local vstr = Jamin.encode(v)
...
function Jamin.encodeIntegerArray( value, wordlen )
    ...
    -- assert(table.equal(value, decodeIntegerArray(result)))
...
local function encodeError( value )
    error(string.format(""can't encode %s of type %s"", tostring(value), typeEx(value)))
end
...
local function decodeString( str, pos )
    ...
    if length then
        ...
        if str:sub(finish+1, finish+1) == "";"" then
            ...
            error(string.format(""couldn't find terminating ';' for string at pos %d"", pos))
        end
    else
...
function decodeIntegerArray( str, pos )
    pos = pos or 1

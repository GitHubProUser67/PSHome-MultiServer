...
namespace WebAPIService.GameServices.PSHOME.OHS
{
    public class LuaUtils
    {
            ...
            if (nested && token.Type == JTokenType.Object)
            {
                ...
                foreach (var property in token.Children<JProperty>())
                {
                    if (property.Value.Type == JTokenType.Array)
                    {
                        ...
                        foreach (var arrayItem in property.Value)
                        {
                        ...
                        resultBuilder.Append(
                            $"[\"{property.Name}\"] = {ConvertJTokenToLuaTable(property.Value, true)}, "
                        );
                    else if (property.Value.Type == JTokenType.String)
                        resultBuilder.Append($"[\"{property.Name}\"] = \"{property.Value}\", ");
                    else if (token.Type == JTokenType.Boolean)
                        resultBuilder.Append(
                            $"[\"{property.Name}\"] = {property.Value.ToString().ToLower()}, "
                        );
        ...
        public static object ConvertLuaTableToDictionary(LuaTable table)
        {
            ...
            {
                var value = table[key];
                dict[key] = value is LuaTable nestedTable
                    ? ConvertLuaTableToDictionary(nestedTable)
                    : value;
            }
            return dict;
        }
        ...
        public static object[] ExecuteLuaScript(
            string luaScript,
            ...
            using (var lua = new Lua())
            {
                try
                {
                    if (queryParams != null)
                    {
                        ...
                        lua.DoString("query = {}");
                        var queryTable = (LuaTable)lua["query"];
                ...
                catch (Exception ex)
                {
                    ...
                    LoggerAccessor.LogError(
                        "[ExecuteLuaScriptString] - Error executing Lua script: " + ex
                    );
                    returnValues = [];
                }
            }
        ...
        public static object[] ExecuteLuaScriptFile(
            string luaPath,
            NameValueCollection queryParams = null,
            string postData = null
        )
        {
            ...
            using (var lua = new Lua())
            {
                ...
                catch (Exception ex)
                {
                    // Handle any exceptions that might occur during script execution
                    LoggerAccessor.LogError(
                        "[ExecuteLuaScriptFile] - Error executing Lua script: " + ex
                    );
                    returnValues = [];
                }
            }
        ...
        public static string HotfixBooleanValuesForLUA(string luaScript)
        {
            ...
            for (var i = 0; i < luaScript.Length; i++)
            {
                var currentChar = luaScript[i];
                ...
                if (
                    i + 3 < luaScript.Length
                    && luaScript.Substring(i, 4).Equals("True", StringComparison.OrdinalIgnoreCase)
                )
                {
                    result.Append("true");
                    ...
                    i + 4 < luaScript.Length
                    && luaScript.Substring(i, 5).Equals("False", StringComparison.OrdinalIgnoreCase)
                )
                {
                    result.Append("false");

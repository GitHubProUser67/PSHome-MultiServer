...
namespace WebAPIService.GameServices.PSHOME.CODEGLUE
{
    internal class WipeoutShooterScoreBoardData(
        ...
        object obj = null
    ) : ScoreboardService<WipeoutShooterScoreboardEntry>(options)
    {
        private readonly string _gametype = (string)obj;

        public override async Task<List<WipeoutShooterScoreboardEntry>> GetTopScoresAsync(
            int max = 10
        )
        {
        ...
        )
        {
            ...
            using (var db = new LeaderboardDbContext(_dboptions))
            {
                ...
                var existing = await set.Where(x => x.ExtraData1 == _gametype)
                ...
                {
                    await set.AddAsync(
                            new WipeoutShooterScoreboardEntry
                            {

            ...
            if (ticketData != null && ticketData.Length > 188)
            {
                ...
                if (!ticket.Valid)
                {
                    ...
                    LoggerAccessor.LogWarn(
                        $"[HFGames] - Poker : User {username} tried to alter their ticket data"
                    );
                ...
                if (ticket.IsSignedByRPCN)
                {
                    ...
                    var hash = DotNetHasher.ComputeMD5String(
                        Encoding.ASCII.GetBytes(resultString + "Pok3r!")
                    );
                    ...
                    LoggerAccessor.LogInfo(
                        $"[HFGames] - Poker : User {username} connected at: {DateTime.Now} and is on RPCN"
                    );
                }
                else if (username.EndsWith($"@{XI5Ticket.RPCNSigner}"))
                {
                    LoggerAccessor.LogError(
                        $"[HFGames] - Poker : User {username} was caught using a RPCN suffix while not on it!"
                    );
                ...
                else
                {
                    ...
                    // Calculate the MD5 hash of the result
                    var hash = DotNetHasher.ComputeMD5String(
                        Encoding.ASCII.GetBytes(resultString + "Il0vep0K3r!")
                    );
                    ...
                    LoggerAccessor.LogInfo(
                        $"[HFGames] - Poker : User {username} connected at: {DateTime.Now} and is on PSN"
                    );
                }

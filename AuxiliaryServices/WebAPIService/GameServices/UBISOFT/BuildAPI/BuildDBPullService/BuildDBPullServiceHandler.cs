...
namespace WebAPIService.GameServices.UBISOFT.BuildAPI.BuildDBPullService
{
    public class BuildDBPullServiceHandler
    {
        public static string buildDBRequestParser(byte[] PostData, string ContentType)
        {
            ...
            var xdoc = XDocument.Parse(Encoding.UTF8.GetString(PostData));
            ...
            var innerRequestName = resultElement.Name.ToString();

            List<string> buildDbRequests =
            [
                "GetCurrentLauncherVersion",
                "GetConsoleOwner",
                "GetProjectDetailWithMacValidation",
                "GetFilteredBuildVersionsWithMacValidation",
                "GetProjectsWithMacValidation",
                "GetFilteredBuildVersionsWithMacValidation",
            ];
                ...
                switch (innerRequestName)
                {
                    case "GetCurrentLauncherVersion":
                    {
                        LoggerAccessor.LogInfo(
                            $"[BuildDBPullService] - GetCurrentLauncherVersion: TRIGGERED!"
                        );
    ...
    <LauncherVersion
      xmlns=""http://builddatabasepullapi/"">
        ...
        <PlatformId>4</PlatformId>
                    ...
                    {
                        LoggerAccessor.LogInfo(
                            $"[BuildDBPullService] - GetCurrentLauncherVersion: FAILED!"
                        );
                        return @"<?xml version=""1.0"" encoding=""utf-8""?>
<soap:Envelope
  xmlns:soap=""http://www.w3.org/2003/05/soap-envelope""

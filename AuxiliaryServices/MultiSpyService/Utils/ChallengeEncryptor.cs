namespace MultiSpyService.Utils
{
    ...
    public class ChallengeEncryptor
    {
        private static byte[] enctype1_data
        {
            get
            {
                if (_enctype1_data != null)
                    return _enctype1_data;
                _enctype1_data =
                [
                    .. enctype1_data_string
                        .Split(' ')
                        .Select(
                            (b) =>
                            {
                                return (byte)Convert.ToInt32(b, 16);
                            }
                        ),
                ];
                return _enctype1_data;
        ...
        private static readonly string enctype1_data_string =
        ...
        private static byte[] gsseckey(byte[] src, byte[] key, int enctype)
        {
            ...
            int i,
                size,
                keysz;
            var enctmp = new byte[256];
            var tmp = new byte[66];

            size = src.Length;
            var dst = new byte[(size * 4 / 3) + 3];
            if ((size < 1) || (size > 65))
            {
                dst[0] = 0;
                return dst;

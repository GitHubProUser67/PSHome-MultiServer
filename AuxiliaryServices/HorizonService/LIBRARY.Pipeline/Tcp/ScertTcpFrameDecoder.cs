...
namespace Horizon.LIBRARY.Pipeline.Tcp
{
    public class ScertTcpFrameDecoder : ByteToMessageDecoder
    {
        ...
        public ScertTcpFrameDecoder(
            ...
            int maxFrameLengthLocal,
            int lengthFieldOffsetLocal,
            int lengthFieldLengthLocal,
            int lengthAdjustmentLocal,
            int initialBytesToStripLocal,
            bool failFastLocal
        )
        {
            if (maxFrameLengthLocal <= 0)
                throw new InvalidOperationException(
                    nameof(maxFrameLengthLocal)
                        + " maxFrameLength must be a positive integer: "
                        + maxFrameLengthLocal
                );
            if (lengthFieldOffsetLocal < 0)
                throw new InvalidOperationException(
                    nameof(lengthFieldOffsetLocal)
                        + " lengthFieldOffset must be a non-negative integer: "
                        + lengthFieldOffsetLocal
                );
            if (initialBytesToStripLocal < 0)
                throw new InvalidOperationException(
                    nameof(initialBytesToStripLocal)
                        + " initialBytesToStrip must be a non-negative integer: "
                        + initialBytesToStripLocal
                );
        ...
        public override void ExceptionCaught(IChannelHandlerContext context, Exception exception)
        {
            LoggerAccessor.LogError(
                $"[ScertTcpFrameDecoder] - An assertion was caught. (Exception:{exception})"
            );
        ...
        protected virtual object Decode(IChannelHandlerContext context, IByteBuffer input)
        {
            ...
            {
                var bytesToDiscardLocal = bytesToDiscard;
                var localBytesToDiscard = (int)Math.Min(bytesToDiscardLocal, input.ReadableBytes);
            ...
            var frameLength = GetUnadjustedFrameLength(
                input,
            ...
            var signed = input.GetByte(input.ReaderIndex) >= 0x80;
            frameLength +=
                lengthAdjustment + lengthFieldEndOffset + ((signed && frameLength > 0) ? 4 : 0);
            ...
            {
                var startOff = (int)Math.Min(20, input.ArrayOffset);
                throw new InvalidOperationException(
        ...
        protected virtual long GetUnadjustedFrameLength(
            IByteBuffer buffer,
            int offset,
            int length,
            ByteOrder order
                ...
                case 8:
                    frameLength =
                        order == ByteOrder.BigEndian
                            ...
                            : buffer.GetLongLE(offset);
                    break;
                default:
                    LoggerAccessor.LogError(
                        "[ScertTcpFrameDecoder] - unsupported lengthFieldLength: "
                            + lengthFieldLength
        ...
        protected virtual IByteBuffer ExtractFrame(
            IChannelHandlerContext context,
            IByteBuffer buffer,
            int index,
            int length
        ...
        void FailIfNecessary(bool firstDetectionOfTooLongFrame)
        {
            if (bytesToDiscard == 0)
            {
                ...
                var tooLongFrameLengthLocal = tooLongFrameLength;
